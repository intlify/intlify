...
Phase 1 ships a parser core that parses one caller-owned `SourceStore` entry identified by `SourceId` into an owned syntax-only `ParseResult` (CST tables + diagnostics). Dictionary-shaped message-local tooling — i18n checkers, LSPs, batch linters, and adapters that repeatedly inspect individual dictionary messages — works at a different granularity: many `(locale, message_id, source_text)` entries, frequently revisited as files change. Platform export and generated-artifact preparation under [014](./014-ox-mf2-message-linker-design.md) are a separate payload-oriented workflow rather than dictionary codegen covered by this cache.
...
2. **Cache-backed message-local syntax consumers reuse the cached pair.** Within a dictionary, editor, or future cache-backed lint workflow that has selected `ParseCache`, variable extraction, syntax diagnostics, formatting, semantic construction, and LSP requests reuse the `SourceStore` / `ParseResult` pair retained by `CachedParse` rather than calling `parse_source` again for that entry. A semantic consumer calls parser-owned `build_semantic_model(cached.sources.as_ref(), &cached.result)` after confirming parser diagnostics are empty; the resulting `SemanticModel` is a separate artifact and is never inserted into `ParseResult`. The formatter receives the same pair through its workspace-internal parsed-artifact API; it neither depends on the `CachedParse` wrapper nor converts the entry to a Binary AST snapshot. This invariant does not require every source-backed product API to install or consult this cache: the initial Phase 3C `lintMessage(source, options)` remains source-backed, and 014 `intlify_export::prepare_export` performs its independently specified complete validation without reading, wrapping, or extending `ParseCache`. A future cross-call export-validation cache remains the separate `intlify_export` consumer cache defined by 014.
