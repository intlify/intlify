...
- v0.1 adds parser diagnostic code `InvalidInputDeclaration = 16` for `.input` declarations whose placeholder is not a variable-expression. The v0.1 decoder accepts `16` as a known diagnostic code value; older v0.1 decoders reject snapshots that carry it as `DecodeErrorCode::UnknownDiagnosticCode`, matching the draft exact-match compatibility policy. The parser also starts emitting the already-defined `InvalidMatcherSyntax = 6` for selector-less or variant-less matchers; that code's numeric surface is unchanged. Both changes back the zero-diagnostic guarantee in `design/002` that the Phase 3B formatter strict diagnostics policy relies on.
