...
- While the repository README marks the project as WIP, v0.1 is a mutable draft and compatibility between different v0.1 repository/package revisions is not guaranteed. Wire-visible v0.1 changes update the writer, decoder, accessors, fixtures, tests, and this changelog together.
- The current v0.1 decoder uses exact header matching; this distinguishes `0.1` from other header versions but not different WIP revisions that share the `0.1` header.
...
- v0.1 adds parser diagnostic code `InvalidInputDeclaration = 16` for `.input` declarations whose placeholder is not a variable-expression. The current v0.1 decoder accepts `16` as a known diagnostic code value; older v0.1 decoders reject snapshots that carry it as `DecodeErrorCode::UnknownDiagnosticCode`, which is allowed because mutable WIP v0.1 revisions do not promise cross-revision compatibility. The parser also starts emitting the already-defined `InvalidMatcherSyntax = 6` for selector-less or variant-less matchers; that code's numeric surface is unchanged. Both changes back the zero-diagnostic guarantee in `design/002` that the Phase 3B formatter strict diagnostics policy relies on.
...
1. Decide whether the change stays in the mutable WIP v0.1 format or needs a version bump. While the README WIP marker remains, a wire-visible change may keep the v0.1 header, but the writer, decoder, accessors, fixtures, tests, and changelog must be updated together and cross-revision compatibility is not promised. After the first snapshot compatibility point is declared, a wire-visible change while `major_version = 0` uses the next `0.N` draft version and exact version matching. This includes emitting a newly assigned core `SyntaxKind`; assigned kind numbers are still never reordered or reused. After v1.0, backward-compatible optional additions may use a minor bump, while a new core kind or other change an existing decoder cannot interpret requires a major bump.
2. While v0.1 remains a mutable WIP draft, amend the v0.1 section above with one bullet per intentional change. After the first compatibility point, open a new `v0.N` / `vM.0` heading for a versioned wire change.
