...
- Translation candidates, source-locale approval, reviewer roles, Store publication, Release publication, deployment, or Runtime credentials.
- Completing 015's Trust Policy, Source Admission Policy, or Resource Limit Policy artifact bodies, or its bootstrap Resource Limit Policy Verification Input representation.
...
These permissions do not adopt an unsupported authoring form or change a 016 decision still marked `Proposed`. An implementation admits only its explicitly adopted 016 semantic subset, even when the caller holds every grant in this table.
...
| Phase 4–5 and broader 028 | Reuse the separation of content checks, authority, exact operations, and completed publication | Separately adopt library/import, source-locale approval, candidate/governance, and Release security; this local registry subset does not authorize them |
...
- Add library/publisher trust, source-locale approval, localization-supply and governance actions, Store/Release publication, deployment/execution admission, and their exact provenance under the owning integration designs.
