...
017 must specify the exact canonical projections and record schemas before persisted/shared artifacts are emitted. The semantic projection used for IntentRevision is not automatically the canonical localized-content projection. Arbitrary JSON hashing, a parser's in-memory layout, or an authoring inventory digest cannot fill this gap.
...
The initial operation set separates Store reads, explicit new-lineage initialization, candidate publication, source/localized review, selection/replacement, revocation, and Store publication. Review/selection/revocation authority permits the exact decision; making it current additionally requires Store-publication authority. A publisher may commit the exact checked decision but cannot author or alter it implicitly. 018 must define and test the corresponding scoped authorization extension; its current source/registry grant table does not authorize these operations.
...
For local persistence, 029's [conditional publication and interruption handling](./029-intlify-product-workflow-and-packaging-design.md) provide reusable host techniques, not an already admitted Store codec or authorization domain. The Store adapter needs its own exact subject/result bindings and real reopen, concurrency, and fault tests. A mutex-only in-memory model proves no disk durability.
...
- 017: materialize complete source/localized message, validation/governance/provenance, Approval/Selection Policy, Store snapshot/transition, and publication representations with exact identity, canonical ordering, and non-circular digest rules. The current authoring subset does not encode them.
- 018: adopt the exact Trust/Source Admission/Resource Limit inputs, governance/Store read and write powers, confirmations, permitted provenance, local establishment, and historical versus current authorization checks needed here. No existing registry grant is extended by implication.
