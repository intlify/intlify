...
Require a present valid `messages` section for any enabled linker-backed rule, resolve the 008 static input-capability union before work, skip reference production when none requires it, and otherwise reuse one shared producer/cache/linker orchestration and one `LinkOutcome`.
...
| --- | --- |
| One interval per workflow iteration | Every `message_project_input_io` pair; `message_definition_project` / `definition_projection`; every `message_link_core` pair; `message_project_link_e2e` / `complete_workflow`; and `message_link_peak_memory` / `link_core_peak_live_memory` |
...
| --- | --- | --- | --- |
| `message_project_input_io` / `inventory_metadata_io` | `project_inventory_metadata_io` | Resolved project root, checked messages/resources configuration, compiled include/exclude matchers, and an empty inventory | Start immediately before the first directory-enumeration or metadata operation. Include bounded directory traversal, exact Unicode representability checks needed to retain an entry, supported-source classification, and physical-identity metadata reads. Stop when the complete bounded unordered discovery/metadata fact set is retained, before canonical sorting, alias grouping, source reads, or content processing. |
| `message_project_input_io` / `definition_snapshot_read` | `project_definition_snapshot_read` | Canonically ordered admitted definition physical groups and trusted project-root-relative source identities | Start before the first safe source open. Include no-follow capability checks attached to that open, metadata validation, and complete exact byte reads for every selected primary source in canonical group order. Stop when all source-byte snapshots are retained, before host-byte preflight, UTF-8 decoding, selected-source checksum construction, extraction, or projection. |
| `message_project_input_io` / `reference_snapshot_read` | `project_reference_snapshot_read` | Canonically ordered admitted built-in producer physical groups and their checked selected source profiles | Start before the first safe source open. Include no-follow capability checks attached to that open, metadata validation, and complete exact byte reads in canonical group order. Stop when all source-byte snapshots are retained, before UTF-8 decoding, parsing, recognition, fingerprinting, or cache lookup. |
...
| `message_reference_produce_js_cache` / `cache_miss_production` | `js_cache_miss_production` | Exact source snapshot, selected profile, resolved producer configuration, cache key components, and an initialized cache known not to contain the key | Start before lookup. Include miss validation, the complete ordinary producer pipeline, checked cache-value construction, and atomic successful publication. Stop when the same complete checked artifact is returned and visible to a later matching lookup. File read and precomputed key-component hashing remain outside. |
| `message_reference_produce_js_cache` / `cache_hit_validation_and_access` | `js_cache_hit_access` | The same prepared inputs and one matching complete published cache entry | Start before lookup. Include key comparison, freshness/component validation, bounded value validation, and checked artifact access. Stop when the existing complete artifact is returned, without parsing, recognition, reconstruction, or publication. |
...
| `message_definition_project` / `pre_extraction_admission` | `definition_pre_extraction_admission` | One already formed canonical physical-source group, resolved catalog assignments, exact selected source snapshot, and effective limits | Include group binding/format/path/alias admission and construction of the checked host-owned extraction request. Stop when that request is returned. Exclude inventory, source read, host parsing, resource extraction, project-global domain aggregation, and contract projection. |
...
| `message_link_core` / `request_validation_and_scope_mapping` | `link_request_validation_scope_mapping` | Complete ordinary `LinkRequest` | Include aggregate request preflights, artifact and policy admission, scope mapping, completeness/policy consistency checks, and construction of the checked mapped request state. Stop at the first semantic-index stage marker. |
...
| `message_definition_project` / `pre_extraction_admission` | Canonical physical-group paths, resolved host format, scope, locale, portable path/alias admission facts, selected-source checksum, and the complete checked host-owned extraction request returned before extraction. Project-global domain observations are unavailable at this stage and are excluded. |
...
- Lint-gating fixtures resolve rules and severities before producer work; perform no linker invocation when no linker-backed rule is enabled; reject enabled linker-backed rules before inventory when `messages` is absent; and skip reference inventory, artifact load, source scan, and reference-cache lookup when enabled rules require only definition/policy inputs. That fast path retains producer declarations only as metadata, selects no reference participant, derives `references: Closed` over the empty selected set, derives neither `ProducerOmitted` nor `OpenEditorWorld`, and filters every unrequested reference-dependent finding at the adapter. A transition to a reference-requiring capability constructs a new full request and inventory. Reference-requiring rules invoke exactly one shared linker orchestration, adding the shared producer/cache path only once when one or several enabled rules require references.
