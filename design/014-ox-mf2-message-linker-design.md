...
`tools/messages-bench/benchmark-phases.mjs` owns the closed phase/cost table and non-serialized measurement-boundary registry for every active message-linker milestone, `tools/messages-bench/fixture-selection.json` owns the versioned fixture/profile inputs and declared scales, `tools/messages-bench/benchmark-profile.mjs` owns the active required-case matrix and E2E-to-resource companion mappings, `tools/messages-bench/checksum-observations.mjs` owns the checked messages-specific duration observation registry and codecs, and `tools/messages-bench/result-schema.mjs` owns the tool's versioned result shape. The phase table emitted in a result must equal the active table exactly. Boundary and observation registries are checked code and are not duplicated in a result document; their meaning is identified by the result `schemaVersion`. A new phase, cost, fixture meaning, measurement boundary, checksum observation, companion mapping, or required case becomes valid only when this design activates it and the owning files, fixtures, revisions, and schema tests are updated together.
...
`benchmark-phases.mjs` exports one canonically ordered boundary descriptor for every active phase/cost pair. A descriptor contains the exact phase, cost, unique stable ASCII `boundaryId`, metric tag, first timed stage marker, final timed stage marker, and the ordered included and excluded stage-marker sets. The runner and tests import the registry, but results continue to serialize only the familiar phase/cost table and metric records.

An injected monotonic clock and stage-marker recorder verify duration boundaries without changing the production operation. Boundary conformance fixtures require exactly one start and stop around the declared markers, reject a missing, duplicate, nested, reordered, or unexpected timed marker, and prove that excluded setup and observation markers occur outside the interval. The production benchmark uses the same boundary path with the real monotonic clock. A benchmark-only internal stage hook may retain opaque checked stage facts for the runner, but it cannot become a default-feature product API, change an ordinary result, or admit a caller-authored intermediate state.
...
- Benchmark fixtures exercise every milestone-required tuple from the exact active `benchmark-profile.mjs`, including the exact `all_definitions_baseline` / `linked_output` artifact-size variants, through `tools/messages-bench`; build, smoke execution, schema validity, the exact active non-serialized measurement-boundary and checksum-observation registries, within-invocation duration-checksum equality, valid product payload fingerprints, same-invocation exporter/comparison observation equality, and bucket reconciliation are gated. They prove each required tuple appears exactly once, every active phase/cost has exactly one unique boundary ID and metric-compatible descriptor, every active messages-owned duration phase/cost has exactly one unique codec ID and checked observation codec, metric kinds that forbid a generic checksum have none, additional observational cases cannot satisfy a required tuple, and the validator never loads a checked-in or previous result as an expected checksum, fingerprint, timing, memory, or size baseline; cross-invocation differences and elapsed-time, memory, and size values remain observational. Result fixtures reject any serialized boundary registry, observation registry, or codec descriptor.
  - Boundary-conformance fixtures use an injected monotonic clock, stage-marker recorder, and counting allocator to require the exact prepared input, first/final timed markers, included/excluded marker order, non-overlapping `message_link_core` stage intervals, intentional E2E overlap, and M0 input-I/O separation. They reject a missing, duplicate, nested, reordered, leaked-setup, prefix-success, or wrong-metric observation and require a result-schema/profile revision change whenever an accepted boundary meaning changes without a phase/cost rename.
