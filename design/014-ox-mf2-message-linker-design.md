...
The linker owns one presentation-neutral location-role decision for every closed finding variant. It selects only typed `SourceOrigin` and `DefinitionLocation` evidence already present in, or addressable from, the checked `LinkOutcome`; it does not derive a host URI, line/column range, lint diagnostic, or LSP diagnostic.
...
Reference-origin lookup uses the finding's `ReferenceRecordIdentity` and the admitted reference artifact. The finding codec does not duplicate an origin merely to support presentation.

013 maps each `DefinitionLocation` to its exact host entry span. Producer provenance and the current admitted source snapshot map each `SourceOrigin`. 008 consumes the roles for lint presentation, while 009 alone projects the mapped evidence to current document URIs, LSP ranges, and `relatedInformation`. Neither adapter may choose a different primary location, reorder related locations, drop an ambiguity collider, or fabricate source evidence from a scope, locale, fallback chain, reason, participant, configuration path, or other non-location evidence.
...
- Benchmark fixtures exercise every milestone-required tuple from the exact active `benchmark-profile.mjs`, including the exact `all_definitions_baseline` / `linked_output` artifact-size variants, through `tools/messages-bench`; build, smoke execution, schema validity, the exact active non-serialized interval-boundary, allowed-overlap, and checksum-observation registries, within-invocation duration-checksum equality, valid product payload fingerprints, same-invocation exporter/comparison observation equality, and bucket reconciliation are gated. They prove each required tuple appears exactly once; every active messages-owned phase/cost uses exactly its milestone-admitted `duration`, `peak_live_memory`, or `artifact_payload_size` token; every active interval phase/cost has exactly one unique boundary ID whose metric equals the profile and result; every active messages-owned interval pair uses a locally owned descriptor; every admitted foreign interval pair imports and re-exports its exact owner-provided descriptor unchanged and matches its metric; and every artifact-size pair has no interval descriptor or boundary ID. They reject every unknown, inactive, case-varied, hyphenated, phase-specific, generic, or payload-incompatible metric spelling, including `peak_rss` before its complete activation contract lands. Every active messages-owned duration phase/cost has exactly one unique codec ID and checked observation codec; metric kinds that forbid a generic checksum have none. Additional observational cases cannot satisfy a required tuple, and the validator never loads a checked-in or previous result as an expected checksum, fingerprint, timing, memory, or size baseline; cross-invocation differences and elapsed-time, memory, and size values remain observational. Result fixtures reject any serialized boundary registry, overlap topology, observation registry, or codec descriptor.
  ...
  - Messages result-schema fixtures require one exact 013-owned `resource_extract` / `host_parse_and_entry_extraction` record for every profile-declared qualifying E2E tuple; pair them through the checked one-to-one companion mapping and equal fixture, variant, scale, runtime, and execution-model fields; reject a companion tuple reused by two E2E tuples, mismatched fields, phase, cost, operation, status, count, or checksum, a missing/duplicate/nested extraction metric, and measured E2E status after extraction failure; and preserve 013 ownership of aggregation semantics without a cross-product envelope or serialized relation field.
...
| [008-ox-mf2-phase-3c-linter-design.md](./008-ox-mf2-phase-3c-linter-design.md) | later presentation adapter: linker findings ship as rules through its contracts and the catalog-level addendum, with the missing-`messages` activation error and static rule capability table owned there, without gating linker M0 |
