...
Language-specific producers own source, object, or binary analysis and emit that contract. When an editor feature requires language-specific source recognition or spelling conversion, the applicable producer also owns the checked language frontend query or projection; E0 therefore reuses `intlify_producer_js` rather than teaching the editor or linker JavaScript grammar.

The future `intlify_linker` core consumes only reference artifacts, definition artifacts, checked scope mapping and completeness inputs, a resolved link policy, and the delivery graph; it owns execution of the contract-defined selector matching, resolution, reachability, placement, findings, bundle plans, the M1 key-only typed-key model, and linker-result identity composites such as `DefinitionLocation`. The M1 model carries resolved scope and domain-qualified message-key identity only; the linker never parses `MessagePayload` or derives an MF2 argument signature.
...
`intlify_resource` and `intlify_contract` own distinct boundary-specific Rust newtypes and neither crate depends on the other. Matching catalog-key, scope, locale, structural-path, and entry-reference semantics do not imply shared representations. The host-owned post-extraction projection performs the checked bridge and must surface any value, variant, or limit incompatibility before constructing an artifact.

The workspace-internal `intlify_export` crate owns shared MF2 export preparation, mapped export diagnostics, parser-backed MF2 argument-signature derivation for typed platform output, `ValidatedExportBatch`, the object-safe exporter trait, common exporter output/error contracts, and built-in exporters. Its preparation gate validates the identity-deduplicated union of plan-selected delivery definitions and the coverage-baseline definitions required by admitted M1 models. It depends on `intlify_linker`, `intlify_contract`, and `ox_mf2_parser`; `intlify_linker` never gains a parser or exporter dependency.
...
The linker never parses JS/TS source, Rust crates, or C/C++ objects directly. Each language producer emits a common-format `MessageReferenceArtifact`; on the catalog side, `intlify_cli` forms and admits physical-source groups, and `intlify_resource` extracts each admitted group once. The host aggregates resource-owned domains from the complete successful extraction set and runs one project-global domain admission stage. Its catalog-domain gate runs before its recognizer/root-domain gate. Only after both ordered gates succeed does host-owned post-extraction projection construct one common-format `MessageDefinitionArtifact` per successful group.
...
The M0 built-in path is `intlify_producer_js`, covering JavaScript and TypeScript syntax. Vue SFC reference production is the next producer phase tracked in Deferred Follow-Up Notes. Rust, C/C++, and WASM reference production belongs to the later native track and is not presented as an M0 implementation.
...
Begins in the `intlify_cli` project inventory. It enumerates configured logical catalog targets, resolves each assignment, inspects physical identity, forms one group per selected physical source, and fixes the canonical primary-plus-alias order.

The CLI alone owns this grouping. Physical identity remains local execution evidence and never enters the artifact contract.

#### Host-owned definition production

Uses two per-group stages separated by one project-global domain admission stage. That stage contains two ordered gates: the catalog-domain gate followed by the recognizer/root-domain gate.
...
3. After the complete definition extraction attempt, the host aggregates resource-owned domain observations. The stage's catalog-domain gate first rejects a catalog scope containing multiple observed domains. Its recognizer/root-domain gate then compares each single observed domain with the configured recognizer/root declarations. Either failure is `config_validation_failed`, constructs no `MessageDefinitionArtifact`, and starts no reference-source scan.
4. Only after both ordered gates in the project-global stage succeed does the post-extraction stage consume each successful extraction and its admitted group, perform checked resource-to-contract conversion, and construct one `MessageDefinitionArtifact`.

The resource boundary owns host parsing, canonical catalog keys, message text, source spans, and validated write-back. It does not import the linker contract, inspect application code, aggregate files, or decide reachability.

The initial orchestration is a shared module in the `intlify_cli` project-inventory layer rather than a new crate. It owns per-group admission and projection plus project-global domain aggregation, but no host parser, catalog-key semantics, selector matching, reachability, or linker analysis. A later non-CLI host must reuse this exact boundary or extract it into a shared crate before implementing another definition-production path; it must not duplicate the rules independently or move them into `intlify_resource`.

#### Message Definition Artifact

Represents exactly one admitted physical-source group after one complete extraction and successful project-global domain admission. Its primary portable source identity and ordered aliases describe the group, while its ordered language-neutral definitions carry scope, key domain, canonical key, locale, message payload, and source-entry evidence.
...
When many-to-one resolution collapses scope-keyed policy, equal configured-root reasons or coverage-baseline locales merge canonically. Unequal values make the resolved request invalid; no source scope wins by order.
...
Feed `intlify lint` through the rule, severity, counting, result, and reporter contracts owned by 008, with concrete catalog-entry evidence mapped through 013. They feed editor integrations through the projection, publication, generation, and lifecycle contracts owned by 009, again using 013 for concrete catalog-entry and host-span mapping. Consumers present the findings but do not rerun link analysis or move editor behavior into the linker.
...
Records one deterministic language-neutral key surface for an explicitly configured resolved scope after the coverage-baseline gate. It carries the resolved scope and canonical domain-qualified message keys only.

It contains no parsed MF2 state, argument signature, platform source, logical output path, destination, or runtime binding. M3 export preparation owns parser-backed argument-signature derivation when a platform representation requires it.

For that M3 handoff, the checked `LinkOutcome` privately retains one exact unambiguous coverage-baseline definition snapshot for every admitted model key. This baseline-source relation is not part of the public M1 model and is not exposed as parser state. It lets preparation consume the already checked payload and `DefinitionLocation` without reloading an artifact or choosing among competing definitions.

#### Platform build integrations

Orchestrate the platform build around the neutral contracts.

A Vite/Rolldown plugin, Cargo task or `build.rs`, CMake integration, or another toolchain adapter runs producers or scanners, supplies the checked host-owned scope mapping table and execution-derived completeness table, supplies the delivery graph, invokes the linker, and asks `intlify_export` to create one opaque `ValidatedExportBatch` over the admitted plans, M1 key models, and M3 validation state before invoking exactly one selected exporter for the export transaction. Built-in M0 integrations use the empty mapping table; a custom in-process integration may construct a non-empty table through the same checked API.
...
Convert the message-validated plans, their selected definition snapshots, and any admitted M1 key models plus M3-derived validated argument signatures exposed read-only through `ValidatedExportBatch` into an ordered list of generic `ExportArtifact` records representing ESM, binary blobs, baked Rust, generated C/C++, or outputs for other languages and platforms.
...
Eighth, one catalog-key structural parse/token phase visits every `Exact` and `Prefix` payload in record-ordinal order and each decoded domain token from left to right. It performs the selected domain's separator, escape, typed-segment, complete-key or non-empty-prefix grammar together with `catalog_key_tokens` admission. `Pattern`, `AllInScope`, and `UnboundedDynamic` contribute nothing to this phase.

As in pattern parsing, a structural error or token overrun is selected at the first failing segment in canonical record-and-segment order. Validation does not fabricate a token count for malformed input or continue to a later record merely to find a token overrun.

The reason and path passes use the established `ReferenceArtifactGroup(identity)` subject. `ReasonBytes`, `PathSegments`, and `PathSegmentBytes` return exactly `Exact(effective_limit + 1)`; `PathBytes` returns the exact attempted per-path running total. The structural-token phase uses the same subject and returns `CatalogKeyTokens` with exactly `Exact(effective_limit + 1)`.

An earlier complete pass wins over every later failure regardless of record ordinal. Only after all eight admission phases succeed does validation continue with remaining selector semantics, reason grammar, origin path grammar and span validation, and other record fields.

Sequential, cached, partitioned, and parallel implementations must expose the same phase and canonical-order result without retaining a key, prefix, token, reason, path, segment, or ordinal in limit evidence.
...
`CatalogKeyTokens` requires exactly `DefinitionArtifactGroup(source)` for a definition key and exactly `ReferenceArtifactGroup(identity)` for an `Exact` or `Prefix` selector path. It never accepts `Request`, a raw key or prefix, a decoded token, a definition index, a record ordinal, or a complete submitted token count.
...
`CatalogKeyTokens` also always returns exactly `Exact(effective_limit + 1)`. Domain parsing checks the incremented structural-token count before retaining the next token, and direct construction or cache revalidation never substitutes a complete submitted count. Its protocol ceiling is `256`, so `ArithmeticOverflow` is unconstructible.
...
A message key is **not restricted to an object path** and has no universal dot-separated format. 013 defines the canonical catalog-key semantics and resource-owned value. `intlify_contract` owns the distinct checked linker-facing `CatalogKey` and `CatalogKeyDomain`, and the host-owned post-extraction projection reconstructs those contract values from the resource values. The linker interprets a `CatalogKey` only together with its `CatalogKeyDomain`.
...
##### Catalog-key structural-token limit

Every admitted complete `CatalogKey` and every admitted structural `Prefix` contains at most `256` decoded domain tokens. The domain-valid empty root key contains zero tokens; `Prefix` remains independently non-empty and therefore contains at least one.

One token is one path segment after the selected domain's separator, escape, and typed-segment grammar have been validated. JSON Pointer segments, YAML typed segments, and XLIFF typed hierarchy segments each count once independently of their decoded or encoded byte length. No layer counts slash separators, JSON escaping, the XLIFF concrete source/target side omitted from its logical catalog key, a display key, or a host path component.

`CatalogKeyTokens` is the forty-first common `LinkLimitCounter` variant, with exact structured spelling `catalog_key_tokens` and an inclusive protocol ceiling of `256`. It applies independently to every `MessageDefinition.key`, `Exact.key`, and `Prefix.prefix`. A caller may select a lower immutable value through `LinkLimits`, including zero, but cannot raise the protocol ceiling.

Domain parsing walks tokens from left to right and checks the incremented count before retaining the next token. The first token above the effective limit rejects the complete artifact with `CatalogKeyTokens`, exactly `Exact(effective_limit + 1)`, and the established owning `DefinitionArtifactGroup(source)` or `ReferenceArtifactGroup(identity)`. Evidence never retains the key, prefix, token, definition index, record ordinal, or complete submitted token count.

The 013 source contract independently rejects a resource-produced catalog key above the same fixed protocol maximum before publishing an extraction artifact. Definition projection rechecks the contract-owned value and applies any lower linker budget. Serialized decode, direct checked construction, cache revalidation, and defensive linker admission perform the same structural check, so an artifact that did not pass through 013 cannot bypass the candidate ceiling.

This token ceiling is a public catalog-key contract, not an inference from one parser's recursion or host nesting setting. The current built-in 013 adapters prove the same value from their format-specific nesting and key-derivation rules, but a future producer or adapter must satisfy the explicit token contract directly.
...
The value is derived from the coordinated `CatalogKeyTokens` protocol ceiling of `256`, not from an implementation parser's recursion or host-nesting setting. A literal or `*` consumes exactly one candidate token, so a pattern that can match an admitted candidate has at most 256 consuming tokens.
...
Any larger admitted spelling would necessarily require at least 257 consuming tokens and could never match a protocol-admitted candidate. `*/*` and the alternating maximum remain expressible; the token ceiling does not count decoded token bytes or NFA state pairs.

This derivation is part of the coordinated 013/014 catalog-key contract rather than a universal glob constant. Changing the `CatalogKeyTokens` protocol ceiling requires re-evaluating the formula `2 × candidate_token_ceiling + 1`, its protocol value, artifact compatibility, work ceilings, and fixtures in both documents.
...
A caller that needs less per-evaluation work may lower `selector_pattern_tokens`, `catalog_key_tokens`, or both. A lower candidate budget shrinks admitted definition and `Exact` / `Prefix` paths but does not rewrite the fixed 513-token protocol maximum for `Pattern`; the actual state rectangle is bounded by the effective values for that invocation. Aggregate logical work uses the inclusive request-level `LinkLimits` counter `pattern_match_states_total`.
...
These boundaries keep source syntax in producers, canonical catalog identity in `intlify_resource`, portable semantics in `intlify_contract`, filesystem inventory and physical grouping in the host integration, and definition-key enumeration plus selector matching in `intlify_linker`.
...
One `MessageDefinitionArtifact` represents exactly one admitted physical-source group after one complete `intlify_resource` extraction and checked scope/locale binding. Its conceptual envelope and entry shape are:
...
The types shown here are `intlify_contract` types, not aliases or re-exports of resource-owned types with similar names. The host-owned post-extraction projection exhaustively maps the four comparable built-in resource `CatalogKeyDomain` variants, rejects resource `StandaloneMf2`, reconstructs checked contract `CatalogKey`, `CatalogScopeName`, `Locale`, and `EntryReference` values, and propagates every constructor or limit failure. Neither crate imports the other, and no blanket infallible conversion may turn an added resource variant into an accidental linker domain. Coordinated conformance fixtures require the bridge to remain exhaustive.
...
The definition-side `MessageDefinition.key` carries the exact canonical key value projected from 013's resource-owned `CatalogKey` into the distinct contract-owned `CatalogKey`. It is interpreted only with the record's contract-owned `CatalogKeyDomain`.
...
Every definition key also passes the independent `CatalogKeyTokens` contract fixed in [Catalog-key structural-token limit](#catalog-key-structural-token-limit). Its inclusive protocol ceiling is `256` decoded domain tokens, matching 013's fixed source-side ceiling. A domain-valid empty root key contributes zero.

The host-owned projection rechecks the 013-produced key while reconstructing the contract-owned `CatalogKey`; serialized decode, direct checked construction, cache revalidation, and defensive linker admission perform the same check. A caller-selected lower linker limit may reject an otherwise valid 013 artifact without changing the source-side maximum or key identity.
...
- `ProducerId` uses the canonical lowercase-ASCII grammar `<reverse-dns>/<producer-slug>` and is at most 255 decoded bytes in total.
  ...
  - The built-in M0 JS/TS reference producer uses `dev.intlify/js-reference`. JS, JSX, TS, and TSX are frontend profiles of that one implementation family rather than separate producer IDs.
...
The `intlify_cli` project inventory resolves catalog membership and binding independently for every selected logical target, inspects physical identity, then forms the targets into physical-source groups. This grouping and canonical alias ordering occur before the host-owned definition-production boundary and do not add physical identity to the artifact contract.

The project-backed editor session in 009 reuses this same grouping boundary before applying open-buffer substitution. URI-local parsing, diagnostics, and formatting remain independent, but linker definition production still receives one source snapshot per already formed physical group. One open alias, or several aliases with byte-identical current UTF-8 contents, may provide that snapshot when every contributing URI/version is captured for freshness. Unequal buffers or unresolved group ownership yield `Partial(OpenEditorWorld)`, no group artifact, and no disk fallback; no URI wins by open order, alias order, or primary-path status.

A newly observed catalog path that appears to match project policy before refreshed inventory has formed its complete physical group is not yet an admitted definition participant. The editor may process it through document-local resource workflows, but it emits no provisional `MessageDefinitionArtifact` and marks the affected definition side `Partial(OpenEditorWorld)`. Only refreshed grouping followed by complete assignment and binding, then the project-global domain admission stage's catalog-domain gate and recognizer/root-domain gate, may admit it to ordinary post-extraction projection.

The host-owned pre-extraction stage receives one already formed group. Every participating alias must resolve to the same binding tuple: `ArtifactNamespace`, canonical host-format id, `CatalogScopeId`, and exact locale. M0 fixes the namespace to `Project`, and canonical host-format disagreement has already failed through the 013-owned `catalog_format_conflict`. A remaining scope or exact-locale mismatch is the 013-owned `config_validation_failed` with `details.reason: "catalog_binding_conflict"`.

Definition production emits no artifact for it and does not pick one binding by config order, discovery order, or path order. The contradiction constructs no `LinkRequest`, starts no reference-source scan, and never records `SourceFailed`; 013 owns its canonical path-plus-definition selection order and bounded evidence. Excluded or otherwise non-participating logical targets are absent from the group and from alias evidence.

The CLI orders a valid group's paths segment-by-segment by exact UTF-8 bytes, with a shorter equal-prefix path first. The first path becomes `SourceDocumentIdentity.path`; the remaining paths become `logical_aliases` in that same order. Pre-extraction admission validates this complete ordered set and its path/alias limits without regrouping or reordering it.

After every group passes admission, the CLI performs one complete extraction per group and retains each successful resource artifact. It constructs no definition artifact until the complete extraction attempt has supplied the project-global domain observations and the project-global domain admission stage has passed its catalog-domain gate followed by its recognizer/root-domain gate. Post-extraction projection then emits one definition artifact per successful group whose entries use the primary source identity. Input enumeration order, filesystem enumeration order, symlink traversal order, command operand order, and future worker completion cannot affect admission or artifact construction.
...
The contract-owned `Locale` is a checked, opaque identity used by every `MessageDefinition` and resolved policy or output field that names a locale. Its M0 scalar grammar is exactly one or more Unicode scalar values. The 013 resolved binding follows the same value grammar but remains a distinct resource-owned value; the post-extraction projection reconstructs this contract newtype through its checked constructor.
...
The contract-owned `CatalogScopeName` has the same non-empty Unicode-scalar grammar as the distinct 013 validated resource value. The post-extraction projection reconstructs it through the contract constructor. Its decoded UTF-8 bytes are retained and compared case-sensitively without trimming, Unicode normalization, case folding, separator rewriting, percent decoding, or locale-sensitive comparison.
...
After uniform resolution and resolved-domain consistency, the linker canonicalizes scope-keyed policy before building semantic indexes:

1. M0 configured roots are grouped by resolved `(scope, domain, selector)`.
2. Beginning with M1, coverage-baseline entries are grouped by resolved scope.

Configured roots in one group collapse to one canonical root only when their optional reasons are equal. Equal baseline entries collapse only when their locales are equal. These merges express the mapping table's intentional many-to-one equivalence; they do not create another policy occurrence or duplicate reachability/model work.

A root group with unequal reasons or a baseline group with unequal locales is an invalid resolved request. The linker returns one fail-complete `LinkOperationalError` mapped to `message_link_failed` with `details.kind: "invalid_request"` before findings, plans, models, export preparation, or cache admission. It never chooses by declared-scope order, configuration order, mapping order, or first/last occurrence and never invents a multi-reason root or multi-baseline scope.

When both collision classes are present, the M0 configured-root pass precedes the appended M1 coverage-baseline pass. Within either class, the smallest canonical resolved policy identity selects the failure. Sequential, partitioned, cached, and parallel implementations expose the same result.

This post-mapping canonicalization does not relax pre-mapping policy validation. Submitted roots, baseline entries, mapping endpoints, and their decoded bytes remain independently charged and validated before resolution; a semantic merge grants no limit deduction. Link-request and incremental-cache identity continue to include the complete canonical pre-mapping policy plus mapping table, even when two entries later collapse to one semantic policy subject.
...
`Closed` has a positive meaning. On the definition side it means every catalog source selected for that exact scope completed extraction, binding, source-snapshot freshness checks, and artifact projection.

On the reference side it means every enabled producer selected for that scope completed its closed-world scan, including the valid case where all of them produced zero references. Exact M0 `producers.artifacts` declarations first collapse by selected regular-file physical identity: one canonical physical-source group is one project-global producer participant even when several declared file-symlink or hard-link aliases identify it. Successful bounded decoding of the group's one complete authoritative selected snapshot completes that participant for every target scope, while its individual records still retain their explicit scopes. The CLI selects the grouped file bytes; the editor may instead select one unambiguous current open buffer, or a byte-identical set of alias buffers, for that group under 009. `Closed` is never copied from an artifact field; for either snapshot it follows from the explicit inventory group plus successful processing of the complete selected bytes.

For a capability-gated lint or editor request that requires no references, “selected” is evaluated after checked capability selection. Configured producer declarations remain metadata, but no producer or external artifact is a participant in that request. Its reference side is therefore `Closed` over the empty selected participant set. This request-relative value does not prove that the complete configured producer world was enumerated or scanned and derives neither `ProducerOmitted` nor `OpenEditorWorld`.

The lint or editor adapter exposes only finding kinds requested by its active capability union, so it filters any unrequested reference-dependent finding produced from that empty selected set. It also ignores every bundle plan from a definition-only result. `messages emit` and `messages prune` are separate generation consumers: they always select and process their complete configured reference participants and cannot reuse capability-relative closure as closed-world evidence.

A **source-snapshot freshness failure** means that the integration cannot establish one authoritative complete selected snapshot for a known filesystem inventory participant because its file contents, physical identity, or checked read changed during acquisition or validation. Definition physical-source groups, selected built-in producer source groups, and configured external reference-artifact physical-source groups all reuse `input_read_failed` with exact `details.reason: "source_changed"` under the 005/007 project-inventory contract. The error carries the canonical primary logical path and no `ioKind`, `rawOsError`, byte sample, digest, or prior/current metadata. It is a host-acquisition failure and is never relabeled as `message_artifact_failed` merely because the participant belongs to the reference side.

The failed participant emits no artifact or cache value. A definition group derives `SourceFailed`. A selected built-in producer source group derives `ProducerFailed` for every scope bound by the enabled producer recognizers. A configured external reference-artifact group derives `ProducerFailed` for every target reference scope because acquisition failed before its embedded scopes could be established.

One scheduled host-integration attempt performs exactly one checked snapshot acquisition per participant. It never automatically reopens or rereads the participant after `source_changed`. A CLI retry requires a new command invocation. An editor retry requires a relevant source, inventory, or watched-file event or an explicit project reanalysis request; that explicit request may retry the same installed generation.

This failure is distinct from 009's **publication-freshness mismatch**, in which a completed job's captured document version or generation no longer matches installed editor state. That stale job is discarded without an operational error or completeness change; it never becomes `SourceFailed`.

An integration derives it only after comparing the resolved inventory with complete successful execution results.

Every user-actionable original-extraction failure follows the same current-generation boundary: `resource_format_unsupported`, `resource_parse_failed`, `resource_entry_unsupported`, `resource_document_unsupported`, and `resource_limit_exceeded` with `phase: "extract"`. Such a failure derives `SourceFailed` only when it belongs to the authoritative selected snapshot of a known definition physical group for the current execution generation, every applicable configuration gate completed without contradiction, and the candidate configuration—when one exists—was atomically installed. A candidate failure remains isolated execution evidence until that installation. A configuration contradiction constructs no `LinkRequest` or partial state, and a final publication-freshness mismatch discards the stale result without changing completeness. A `validate_write_back` failure concerns only an uninstalled formatting candidate and never contributes completeness evidence.

`Partial` records why the integration cannot make that proof. `SourceOmitted` and `SourceFailed` are valid only for `definitions`; `ProducerOmitted` and `ProducerFailed` are valid only for `references`; `OpenEditorWorld` and `ExternalArtifactUnverified` are valid for either side. Configuration-derived catalog assignment, locale, scope, alias-binding, or domain contradictions are not completeness reasons: they fail through their owning `config_validation_failed` contract before a `LinkRequest` exists. A successfully decoded exact `producers.artifacts` physical-group disk snapshot, one mapped configured primary/alias buffer, or a byte-identical set of mapped buffers selected under 009 is authoritative for that invocation and never receives `ExternalArtifactUnverified`. Every contributing buffer version belongs to the editor freshness snapshot. That reason is reserved for a cache, ad hoc editor overlay, package artifact, or another integration-supplied external value that is not tied to an authoritative current-invocation declaration. Invalid selected configured bytes are `ProducerFailed`; unequal claiming buffers or unresolved live-source ownership are `OpenEditorWorld`, and neither case falls back to another snapshot.
...
The sole non-product exception is the internal artifact-size support behind the exact Cargo feature `benchmark` defined below. It may construct the paired full-retention outcome only through `intlify_linker`'s private checked plan machinery after the same request admission and result-limit validation. Only the dedicated workspace benchmark runner enables that documentation-hidden, non-stable feature in the supported configuration. Default product builds, product CLI/platform packages, and shipped artifacts do not enable it, and it cannot supply an arbitrary caller-authored plan.
...
The definition-production boundary inherits these inclusive source-local ceilings directly from 013:

| Counter | Initial protocol ceiling |
| --- | --: |
| `host_bytes` | 64 MiB (`67,108,864` bytes) |
| `entries` / emitted `definitions` | `100,000` |
| `catalog_key_tokens` for one definition's decoded domain path | `256` |
...
A caller-selected lower value may intentionally reject it. The built-in post-extraction projection consumes the accepted extraction's counters and exact messages without reparsing the host document or maintaining a second interpretation of entry identity.
...
The open limits work therefore covers reference artifacts, serialized/decoded definition expansion, aliases and paths, aggregate request counts, the delivery graph, and bounded output construction — not replacement values for these six source-local ceilings.
...
Only after all five byte/message passes succeed does validation apply locale and scope-name grammar in preserved raw-entry order.

It then runs one complete catalog-key structural parse/token phase over definitions in that same order. For each key, the selected domain's separator, escape, and typed-segment grammar are validated while decoded tokens are admitted from left to right under `catalog_key_tokens`. The first structural failure or token overrun in canonical record-and-segment order wins; validation never invents a token count for malformed input or scans later records merely to find a token overrun.

Only after that phase succeeds does validation apply entry-reference continuity and the remaining definition-record semantics.
...
A structural-token overrun returns `CatalogKeyTokens` with the same subject and exactly `Exact(effective_limit + 1)`. It retains no raw key, decoded token, definition index, occurrence index, or complete submitted token count.
...
The host-owned pre-extraction stage applies all five counters to the complete participating physical-source group supplied by the CLI inventory. If any ceiling or lower caller budget is exceeded, it rejects the group without extracting, truncating aliases, selecting a smaller subset, changing the primary path, or regrouping aliases.
...
A configured root's declared-policy identity before scope mapping is exactly its checked `(CatalogScopeId, CatalogKeyDomain, MessageSelector)` tuple. `reason` is optional declaration evidence and does not participate in identity or reachability.

Two declared roots with the same tuple reject the complete policy even when their reasons differ; the policy constructor never runs both, chooses one reason, or merges reasons into a new multi-reason representation. Both submitted roots remain charged to the collection limit before duplicate detection. The separate post-mapping rule may collapse roots that were distinct only because their declared scopes differed, but only when their reasons are equal.
...
Shared export preparation runs only when `bundle_plans` is `Some`. It consumes the concrete plans together with any admitted M1 models and their private baseline-source relation, then exposes validated plans and typed output to exporters only through `ValidatedExportBatch`; exporters never inspect findings or attempt to repair a blocked outcome.
...
It has no effect on resolution. The `coverageBaseline` mapping is optional. In the initial M1/M3/E0 contract, the presence of at least one explicit declared baseline entry resolving to a scope is itself the only selection mechanism for coverage-baseline reporting and typed-key model construction. There is no separate typed-key scope list, request flag, recognizer-derived requirement, delivery-target-derived requirement, or CLI leaf.

An admitted resolved scope without such an entry is not selected for M1 model construction. It produces no typed-key model and no additional configuration, operational, or linker finding merely because its baseline is absent. E0 exposes no completion for that scope, and M3 emits no typed accessor module for it while ordinary reference resolution and message bundle emission remain available. A consumer cannot replace that absent model with the union of definitions, a fallback locale, the first discovered locale, or an open-document key set.

The integration never infers a baseline from the largest catalog, the first discovered locale, fallback order, or definition order. The pre-discovery configuration validator requires only that the selected scope exists and the selected locale belongs to `messages.locales`; it does not inspect concrete catalog matches or claim inventory completeness.

After optional scope mapping, post-mapping policy canonicalization guarantees at most one locale for each resolved scope. Equal mapped locale values merge; unequal values fail the resolved request before this coverage phase begins.

M1 model construction begins for each canonical resolved scope with a baseline only after the definition-side completeness state is known. `Partial` uses the existing blocking `degraded-analysis` finding and produces no typed-key model for that scope; M3 export is already withheld by the same checked outcome. It does not add another config or operational error for the same incomplete world.

When the definition side is `Closed`, the baseline key set is the exact set of admitted definitions for that locale and may be empty. A closed empty baseline is not an incomplete inventory. If another production locale defines any key, the ordinary baseline-versus-union gate fails model construction; if every production locale is empty, the resulting empty typed-key model is valid.
...
If another locale contains a key absent from the baseline, M1 model construction for that scope fails complete and returns no stale or partial typed-key model; it never silently omits the runtime-resolvable key or widens the generated surface from the union. Beginning with M3, that failure also prevents the ESM exporter from rendering or registering an accessor module for the scope.
...
Configured roots reach this phase only after post-mapping policy canonicalization has produced at most one root for each resolved `(scope, domain, selector)`. Equal reasons merge; conflicting reasons fail the resolved request rather than changing reachability or retaining arbitrary evidence.
...
`kind()`, `blocking()`, and `resolved_scope()` are derived from `record`; no independent kind, blocking, or scope field exists. Every closed record variant already carries exactly one post-mapping resolved scope in its typed subject or evidence. The scope accessor gives presentation adapters one uniform filter key without asking them to inspect source locations or duplicate per-variant semantic knowledge. The linker-owned semantic phases are the only constructors and can construct a record only after its complete subject/evidence invariants and result budgets have passed.
...
| `missing-translation` | reference-artifact identity, ordinal, requested locale, key | delivery unit, resolved scope, domain, probed locales, selected locale, definition location, optional origin |
...
| `missing-translation` | reference-record identity, requested locale, and resolved canonical key | delivery unit, resolved scope, domain, probed locales, selected definition locale and location, plus optional reference origin |
...
The exact evidence order is `deliveryUnit`, `scope`, `domain`, `probedLocales`, `selectedLocale`, `definition`, then optional `origin`:
...
`origin`, when present on the selected reference, is copied into the finding as the exact checked `SourceOrigin`; absence omits the member rather than emitting `null`. Retaining it makes the fully owned `LinkOutcome` sufficient for later lint and editor presentation without borrowing or looking up the admitted reference artifact.

The finding contains no selector, reason, payload, or complete fallback suffix after the selected locale. Those are irrelevant after first-success resolution. `blocking` is always `false` in M2 and M3.
...
It maps each category one-to-one to a lint rule under the same ID. Configuration, reporting, counting, and exit status remain governed by the 008 contracts and the project-level finding addendum already required by 013: project-scope execution, typed subject identities, and cross-file related evidence.

Every such rule has 008 required analysis input kind `project-link-outcome`. Only the project-capable CLI/editor adapter admits and evaluates it from this section's one checked `LinkOutcome`. The message-only Rust `lint_message` API and N-API/WASM `lintMessage` bindings reject an explicitly configured linker-backed rule with `invalid_options` / `rule_requires_project_context`; L0/L1 do not add a public `lintLinkOutcome` API or imply resource/linker bindings.
...
### Finding presentation location roles

The linker owns one presentation-neutral location-role decision for every closed finding variant. It selects only typed `SourceOrigin` and `DefinitionLocation` evidence already present in the checked `LinkOutcome`; it does not derive a host URI, line/column range, lint diagnostic, or LSP diagnostic.

| Finding variant | Primary location role | Related location roles |
| --- | --- | --- |
| `ambiguous-message-definition` | the first `DefinitionLocation` in the already canonical complete collider vector | every remaining collider `DefinitionLocation`, in that same canonical order |
| `unresolved-message` | the subject reference's `SourceOrigin`, when present | none |
| `missing-translation` | the selected fallback definition's `DefinitionLocation` | the subject reference's `SourceOrigin`, when present |
| `orphaned-translation` | the orphaned definition's `DefinitionLocation` | none |
| `unused-message` | the unused definition's `DefinitionLocation` | none |
| `unbounded-dynamic-reference` | the subject reference's `SourceOrigin`, when present | none |
| `degraded-analysis` / `wide-selector` | the subject reference's `SourceOrigin`, when present | none |
| `degraded-analysis` / `partial-completeness` | none | none |

Every reference-backed finding that can expose a location role retains its optional checked `SourceOrigin` directly in its typed evidence, including `missing-translation`. A presentation adapter reads that evidence from the one checked `LinkOutcome`; it does not retain, borrow, or look up an admitted reference artifact.

013 maps each `DefinitionLocation` to its exact host entry span. Producer provenance and the current admitted source snapshot map each retained `SourceOrigin`. The 008 project-level finding collection consumes the roles for lint presentation, while 009 alone projects the mapped evidence to current document URIs, LSP ranges, and `relatedInformation`. Neither adapter may choose a different primary location, reorder related locations, drop an ambiguity collider, or fabricate source evidence from a scope, locale, fallback chain, reason, participant, configuration path, or other non-location evidence.

When the selected primary role is absent, the adapter retains the finding as a project/workspace finding without a synthetic document anchor. Related evidence never promotes itself to primary. One linker finding counts exactly once under 008 warning/error accounting and becomes at most one 009 diagnostic or workspace finding, regardless of the number of related locations.

### Lint presentation scope selection

For a filesystem L0/L1 invocation, 008 derives one canonical presentation set of `ResolvedCatalogScopeId` values from ordinary selected logical targets that are also linker-participating configured definitions. After configuration assignment and the same checked scope mapping used by the prospective `LinkRequest` succeed, each such target contributes its resolved scope before metadata, physical grouping, reads, extraction, or message-level processing. A later target-local failure does not retract it. A configuration-derived assignment or mapping contradiction instead fails complete before a set or request exists.

Each selected logical alias contributes, so one selected alias is sufficient even when the physical group's canonical primary or another alias was not selected or was ignored. Equal binding admission makes those aliases agree; canonical sorting and de-duplication retain each resolved scope once. Standalone `.mf2`, config-free direct catalog inputs, and configured resource definitions without the coordinated linker `scope` and locale pair contribute no value.

This set is presentation state only. Operands and ignore rules never remove configured definition, producer-source, or external-artifact participants from the link request, alter completeness, or change `LinkOutcome`, artifact, and cache identity. The lint adapter tests `LinkFinding::resolved_scope()` against the set after linking and after capability filtering. It does not test the primary location path or require that a JS/TS reference source be an ordinary lint target.

When the final set is empty, the adapter stops before project inventory, producer or external-artifact work, cache lookup, and `intlify_linker::link`. It returns only ordinary message-level lint results. Independent config validation and the missing-`messages` activation check still occur first. This is a vacuous application over no selected linker scopes, not `Partial` evidence, implicit whole-project presentation, or silent stdin behavior.

A matching finding retains the complete linker-owned location roles. Its primary may identify a selected catalog, another catalog in the same scope, or a producer source outside the ordinary target set, and it may be absent. An absent primary remains one project-level finding with no synthetic file or zero-length span. A nonmatching finding is omitted from lint presentation regardless of location availability, but remains unchanged in the checked outcome used by other consumers. The editor session does not use this operand-derived filter; 009 applies its separately owned workspace capability and publication rules.

The editor presents both reference and definition directions incrementally from whatever truthful source evidence is available in the open development world.
...
The lint integration first resolves rule configuration, preset membership, and severity. If at least one linker-backed rule is enabled, a present valid `messages` section is required by the 006/008 post-section activation contract before any definition inventory or linker work begins.

It then unions the static capabilities of every enabled linker-backed rule according to the normative [008 capability table](./008-ox-mf2-phase-3c-linter-design.md#linker-backed-rule-activation-and-input-capabilities). `ambiguous-message-definition` and `orphaned-translation` are definition-only. `unresolved-message`, `missing-translation`, `unused-message`, `unbounded-dynamic-reference`, and `degraded-analysis` require application-reference input. `degraded-analysis` requires it because the one rule includes the reference-record `wide-selector` variant as well as partial-completeness evidence.

These capabilities are implementation-owned rule metadata, not configuration, severity, reporter state, or cache identity. If no linker-backed rule is enabled, the integration performs no linker orchestration.

For a filesystem lint invocation with at least one enabled linker-backed rule, capability-driven project orchestration proceeds only when the presentation set above is non-empty. If that set is non-empty but no enabled rule requires application-reference analysis, the integration invokes the shared linker orchestration once with the required definition/policy inputs, but neither constructs a reference inventory nor loads or produces reference artifacts and pays no source-scan cost. Configured producer declarations remain checked metadata, but the capability-relative request selects no reference participant. Every target scope therefore receives `references: Closed` over the empty selected set, without claiming a scan of the configured producer world or deriving `ProducerOmitted` or `OpenEditorWorld`.

For that non-empty presentation set, if reference input is required, the same orchestration additionally uses the shared producer and artifact cache. In both enabled cases all adapters consume the resulting single `LinkOutcome`; a rule is never allowed to scan or link independently.
...
An exact configured external-artifact physical-group snapshot uses a separate consumer-side decode-cache identity: its canonical host-primary normalized project-relative path, exact byte length, BLAKE3-256 digest of the complete selected file bytes, decoder contract revision, and compatible effective limits. The ordered host-alias set is deliberately absent because aliases do not change bounded artifact decoding. Adding, removing, or reordering aliases while the primary path and every other component remain equal therefore permits a hit; changing which path is primary causes a conservative miss even when the bytes are equal. This digest is cache metadata, not a new `MessageReferenceArtifact` wire member or a claim about upstream source freshness. Equal cached bytes may reuse bounded decoding only when every included component matches; changed bytes naturally miss.
...
For a filesystem `intlify lint` invocation, ordinary operands and the lint ignore stack constrain message-level targets and later linker-finding presentation only. They never narrow the configuration-authoritative definition inventory or the capability-selected reference inventory supplied to the shared orchestration. A narrow operand, ignored reporting target, or zero ordinary target set therefore does not itself derive `SourceOmitted`, `ProducerOmitted`, or another `Partial` reason. Explicit resource and producer include/exclude policy owns project participation; execution evidence for that complete selected inventory owns completeness.

Initial L0/L1 stdin is message-level only and cannot activate a project-link rule. After independent config validation and the earlier missing-`messages` activation check, any enabled linker-backed rule makes stdin mode fail through `invalid_cli_argument` with `details.reason: "linker_rule_requires_filesystem_inventory"`, `details.option: "--stdin-filepath"`, and the ASCII-smallest enabled linker-backed `details.ruleId`. It stops before stdin read or project work rather than skipping that rule, scanning an unrelated disk project, or treating the virtual path as a temporary catalog overlay.
...
A later project-level finding addendum may promote individual rules only after measuring scan cost, cache behavior, operand-scoped presentation behavior, completeness UX, and false positives.
...
`useMessageSet` remains a producer-side bounded-selector API. It may consume generated key or pattern types in a later JS integration, but M1 does not replace it with, or make it an implicit entry point into, the M3-generated accessor module.

- **Determinism and freshness**: linker plans and M1 typed-key models are deterministic for identical inputs. Every filesystem-writing CLI generation surface, beginning with M3 `messages emit`, has a `--check` mode that re-runs and diffs the complete registered artifact set instead of writing. A pure Rust/in-process model-construction API has no command-line flag and does not pretend to implement `--check`.
...
The built-in JS/TS producer emits exactly one `MessageReferenceArtifact` for each selected physical source group, including a successfully scanned group with zero references. It uses the shared host inspection and grouping rules for equal regular-file identity, so repeated glob matches, symlink aliases, and hard-link aliases do not cause the same physical bytes to be scanned or emitted more than once. Under 009, an active project-backed editor session applies its checked single-or-byte-identical open-buffer override only after this group is formed; unequal claims produce no artifact and weaken the affected reference side to `OpenEditorWorld`.

While a request requiring references is active, a newly observed application path that appears to match the producer include before refreshed inventory has formed its complete physical group emits no provisional reference artifact or cache entry. The editor marks every producer-bound scope `Partial(OpenEditorWorld)`. Only the refreshed ordinary invocation may admit the group after canonical primary/alias formation and the complete group-count, source-byte, aggregate-byte, profile, and alias-compatibility preflights.

While the effective capability union is definition-only, no reference participant or reference inventory exists for that request. Opening or changing an application path therefore starts no refresh or producer work and derives no `OpenEditorWorld`; a later reference-requiring capability constructs a new full inventory and request.
...
Every participating logical alias in one physical source group derives its profile independently before the group is scanned. All aliases must resolve to the same complete grammar/source-goal profile. An unsupported alias or disagreement such as one physical source selected through both `.ts` and `.cts` names fails the group as `ProducerFailed`; primary-path order never chooses one interpretation and the producer emits neither a partial nor duplicate artifact.
...
The built-in producer analyzes one complete exact UTF-8 source snapshot. A disk source uses the bytes selected by the shared input-snapshot contract. An authoritative editor override uses the exact Unicode scalar sequence shared by its one buffer or byte-identical buffer set, encoded as UTF-8 under 009, and captures every contributing protocol version; neither route falls back to another snapshot after analysis begins.
...
Built-in JS/TS source admission has three independent inclusive fixed ceilings:
...
For an editor override, the key depends on its exact current UTF-8 bytes but not its one or several protocol versions. Equal bytes may reuse equal production safely; 009's complete captured URI/version set remains outside the cache as the mandatory publication-freshness gate. A cache hit never authorizes publication when any contributing buffer version is superseded and never claims that disk bytes, an unequal open buffer, or an upstream external producer are current.
...
For an ordinary JS/TS file, that range addresses the exact selected file bytes. An endpoint beyond `u32` or a non-scalar boundary fails the complete source's producer operation and derives `ProducerFailed`; this built-in profile does not recover by omitting origin.
...
For one M3 CLI command, export preparation is project-shared rather than repeated per selected target. After one successful `LinkOutcome` with plans, orchestration invokes `prepare_export` exactly once over the identity-deduplicated union of the outcome's plan-selected delivery definitions and the baseline definitions privately related to every admitted M1 model key. A message-validation failure blocks every selected target before any exporter instance, destination mapping, lock, or registration work; one successful borrowed `ValidatedExportBatch` is reused by every independent target transaction.

Target options such as eager locales change exporter representation but do not change the linker's selected delivery records, private model-to-baseline relation, or the MF2 syntax/semantic validity of their exact payloads. A future target type that genuinely selects a different message set requires an explicit preparation-partition contract rather than silently rerunning and duplicating the M3 diagnostic result.
...
Any integration that turns plans or M1 models into deployable output must pass the complete `LinkOutcome` through one shared export-preparation pipeline before invoking the transaction's selected exporter. The pipeline reads the exact decoded `MessagePayload` retained by either a selected `ResolvedMessage` or the checked private baseline-source relation; it never reloads or re-resolves a definition artifact. For each unique definition in their union, that pipeline:
...
An unused baseline definition still belongs to the gate when an admitted M1 model exposes its key, because the generated accessor module consumes the signature derived from that definition. A definition outside both the delivery set and every admitted model's baseline-source set remains outside the gate; its parser and semantic diagnostics stay visible through ordinary lint/editor diagnostics but do not block deployment of a disjoint valid output set. The gate does not format, normalize, repair, re-escape, or run configurable lint rules over a payload.

#### Definition selection and deduplication

The preparation boundary forms two exact sets from the complete checked outcome:

- the **delivery validation set** contains every definition selected by the complete `Some(bundle_plans)` value; and
- the **typed-signature validation set** contains the one private coverage-baseline definition related to every key in every admitted M1 model.

The **export validation set** is their union, deduplicated by `DefinitionLocation` — the record's `SourceDocumentIdentity` plus its entry-level `EntryReference` under the canonical codec above. The same definition is parsed exactly once when it appears in both roles or through repeated placement, locales, delivery units, or model keys. Message text, logical message identity, plan position, validation role, and exporter identity are not dedupe dimensions.

Every delivery or baseline-source occurrence with an equal `DefinitionLocation` is linker-guaranteed to carry equal resolved scope, domain, key, definition locale, and exact payload. A disagreement is an internal `LinkOutcome` invariant violation, not a second definition, first/last-wins choice, or request to inspect an external artifact.

Within one `prepare_export` call, implementations may reuse parse and semantic computation for byte-identical payloads only if each distinct `DefinitionLocation` retains the same diagnostic mapping and observable result as its own complete validation. The initial contract admits no reuse from an earlier call.
...
`Some(Vec::new())` has an empty delivery validation set, but it does not necessarily have an empty export validation set. If admitted M1 models exist, preparation still validates their baseline definitions and may produce typed output; only an outcome with an empty plan slice and no admitted models produces a truly empty valid `ValidatedExportBatch`. The integration invokes the transaction's one selected exporter exactly once in every successful `Some` case.
...
For a non-empty export validation set, one parser or semantic diagnostic in either role fails the complete export transaction: no exporter is invoked, no asset, generated source, blob, loader map, or manifest is registered, and `--check` also reports failure.
...
The gate validates every unique export-validation definition in stable `DefinitionLocation` order even after the diagnostic reporting limit is reached. Delivery-versus-typed-signature role does not affect ordering, and a definition present in both roles contributes diagnostics only once.
...
Zero deliberately retains no diagnostic records while still validating the complete export validation set and reporting the exact nonzero total and truncation state. A value above `10,000` is rejected before parsing rather than clamped.

This retention budget is separate from linker-core `LinkLimits`: it neither changes `LinkOutcome` nor limits how many export-validation definitions, parser diagnostics, or semantic diagnostics are examined.
...
Construction is mapper-owned and accepts one existing `DefinitionLocation` from the checked export validation set, the exact decoded payload, and either an `ox_mf2_parser` parser diagnostic or parser-owned `SemanticDiagnostic` rather than independently supplied field values. The internal validation record clones that complete linker-owned identity from either `ResolvedMessage::definition()` or the private model baseline-source relation and never reconstructs a source/entry pair through another public path.
...
Neither `SourceId`, `location`, nor host coordinates are added. `category` is required because one failure may contain parser diagnostics from one export-validation definition and semantic diagnostics from another.
...
All platform integrations use the same parser, SemanticModel, and semantic-validation contracts rather than implementing target-specific MF2 validity rules. They map each ordinary diagnostic through the exact `DefinitionLocation` retained by its checked export-validation record, regardless of whether that record entered through delivery, typed-signature preparation, or both.
...
Preparation unions the outcome's plan-selected delivery definitions with its private model baseline-source definitions, groups every occurrence by exact `DefinitionLocation`, verifies the linker-owned equal-snapshot invariant across both roles, and validates each unique payload exactly once.
...
An empty batch follows that same rule. An empty plan slice paired with non-empty typed output is not an empty batch. `Ok(Some(batch))` always permits and requires the selected transaction to invoke its exporter once; only `Ok(None)` means that no exporter transaction exists.
...
The shared export-preparation gate has already parsed and semantically validated every definition in the delivery-plus-baseline export validation set before the ESM exporter runs. That proof makes the raw payload deployable source data but does not turn the ESM module into an MF2 runtime: it exports no formatter function, locale negotiation, function registry, ICU data provider, retry policy, cache, suspense primitive, or application provider.
...
intlify_cli project inventory
  └─ enumerates configured definition/reference inputs, inspects physical identity,
     forms canonical physical-source groups, records execution outcomes,
     and derives scope completeness

host-owned definition production (initially an intlify_cli project-inventory module)
  ├─ pre-extraction: validates an already formed group's bindings, profile,
  │  portable primary/aliases, and limits
  ├─ extraction: runs intlify_resource once for every admitted physical group
  ├─ project-global domain admission: aggregates resource-owned domains,
  │  then runs the catalog-domain gate followed by the recognizer/root-domain gate
  └─ post-extraction: only after both gates, checks resource-to-contract conversions
     and constructs one MessageDefinitionArtifact per successful extraction

language reference producers
  ├─ intlify_producer_js   (oxc-based JS/TS; Vue SFC frontend deferred)
...
intlify_lint integration (later; not an M0 dependency)
  └─ maps linker findings to rules (008 + project-level finding addendum)
...
013's deferred catalog-native structural checks and the cross-locale, unused-message, and application-source reference analyses owned by this design all layer above the one shared catalog extraction path; no second resource extractor is introduced.
...
The 014 section validator accepts an absent `messages` section as absent. After all independent sections succeed, 006 performs the 008-owned consumer-activation check. Any enabled linker-backed lint rule with no `messages` section produces one `config_validation_failed` with `details.reason: "linker_messages_required"` at the selected activating rule and stops before discovery or linker orchestration. This cross-section reason is not a member of the closed 014 section-local reason table below, and it never causes 014 to synthesize locales, policy, producers, or roots.
...
7. M1 appends complete static `coverageBaseline` validation when present.
...
| M1 | `invalid_message_coverage_baseline` | Optional non-empty `coverageBaseline` mapping and its static scope/locale references. |
...
The duplicate rule above applies to declared roots before scope mapping. A custom in-process mapping may make roots from different declared scopes share one resolved `(scope, domain, selector)`. The linker collapses them only when their optional reasons are equal; unequal reasons make the resolved request invalid before reachability analysis. The built-in CLI supplies an empty mapping table, so this post-mapping rule does not normalize duplicate configuration entries or change `invalid_message_roots`.
...
An explicit entry whose scope is unknown or whose locale is outside `messages.locales` is rejected by the pre-discovery configuration validator. Concrete catalog matches, extraction outcomes, and completeness do not participate in `invalid_message_coverage_baseline`.

A custom in-process mapping may make entries from different declared scopes target one resolved scope. Equal locale values collapse to one semantic baseline. Unequal locales make the resolved link request invalid before coverage or model construction; neither declared-scope order nor mapping order selects a baseline. The built-in CLI's empty mapping table means this rule never overwrites a duplicate configuration member or changes `invalid_message_coverage_baseline`.
...
`ProducerOmitted` is reserved for a producer selected into the resolved inventory of a request that requires references but not executed or included in that request. It never represents absence of producer configuration or capability-based non-selection by a definition-only request.

`producers.js` configures the built-in `intlify_producer_js` implementation invoked by `intlify_cli` and is itself optional. In M0, `intlify_producer_js` owns JS/TS parsing, recognizer matching, static evaluation, source mapping, producer-specific cache identity, and `MessageReferenceArtifact` construction. `intlify_cli` owns project inventory, physical grouping, source-snapshot selection, invocation, execution-outcome accounting, completeness construction, and `LinkRequest` orchestration; it does not parse JS/TS or construct reference records itself. The deferred Vue SFC frontend remains producer-owned when promoted and does not move SFC parsing into the CLI.

When `producers.js` is present, it requires both a non-empty `include` array and a non-empty `recognizers` object. An empty array, an empty object, or omission of either member is a configuration error; a project that does not use the built-in producer omits `js` itself.
...
The L0/L1 filesystem lint adapter reuses the same configuration-authoritative inventory for linker analysis. Its ordinary operands and ignore stack form a separate message-level target set and derive the resolved-scope finding-presentation set defined above; neither set subtracts configured linker participants. Initial L0/L1 stdin does not enter this workflow.

The definition inventory contains every concrete logical file selected by a resource catalog definition with the coordinated M0 `scope` and locale binding. Resource definitions without that pair remain entry-level `fmt` / `lint` inputs and are not linker sources, so the ordinary project-wide linker inventory does not select them. An integration that nevertheless explicitly attempts to project one as a linker definition fails before linking rather than inventing or omitting identity metadata. `PolicyAbsent`, `PolicyEmpty`, and the config-free direct-file exception never trigger an implicit project scan for a linker command.
...
After that structural validation, project inventory inspects every declared selected file's physical identity through the 005 boundary. File-symlink and hard-link declarations that identify one regular file form one physical-source group. Exact project-relative logical paths are ordered segment-by-segment by UTF-8 bytes with the shorter equal prefix first; the first path is the host primary and the remaining paths are host aliases. A group is one project-global producer participant and is ordered among external-artifact groups by its primary path. The host primary and aliases are retained as inventory, error-selection, benchmark-observation, and editor-freshness evidence. Neither is inserted into, compared with, or used to rewrite the decoded artifact's embedded identity, producer, records, or fingerprint. Separately, the canonical primary alone supplies the path component of the consumer-side decode-cache identity fixed above; aliases never enter that key.

Each physical group makes one complete selected file snapshot the authoritative external reference input for that invocation. The CLI reads the complete bounded input once, fingerprints those exact bytes for decode caching, and validates version, structure, identities, records, and limits once through the ordinary published decoder. Successful decode accepts the producer's semantic claim that this file is its complete current output; the CLI cannot and does not infer or re-scan the producer's upstream language sources to prove freshness. Build integrations that require source-level freshness regenerate or verify the artifact before invoking the linker. A future signed or source-manifest protocol may strengthen that build boundary without changing M0 acceptance.
...
For project-backed editor diagnostics, 009 applies the open-document override after this physical group is fixed and before the read/decode boundary. One mapped current buffer for any primary or alias path, or several mapped buffers with byte-identical current UTF-8 contents, replaces the complete group's disk bytes and, after one complete successful bounded decode, is authoritative for that editor invocation. The editor captures every contributing URI/version for freshness. Invalid selected bytes are `ProducerFailed` without disk fallback; unequal claims or unresolved source ownership are `Partial(OpenEditorWorld)`; closing every override selects the then-current grouped disk snapshot. The editor does not label a successfully decoded configured override `ExternalArtifactUnverified` or infer upstream-source freshness.

The physical group is project-global rather than scope-qualified. A successfully decoded artifact containing zero references is still one completed external producer participant for every target scope. Metadata, I/O, or contract failure conservatively makes every target reference side partial because the failed complete snapshot cannot reveal a trustworthy narrower scope set. A declaration whose metadata failure prevents physical grouping never falls back to path-text identity or a second alias snapshot.
...
Inventory and execution order are deterministic. Representable definition and built-in producer paths use their exact slash-normalized project-relative logical identities and are ordered by those identities, independently within each side. External artifact declarations are structurally admitted in canonical resolved-path order after duplicate rejection, then their successfully identified physical groups execute in canonical primary-path order. Native directory-entry order is used only to select and order filesystem-discovery failures for names that cannot enter the Unicode logical-path order. Worker completion, filesystem iteration, config member order, glob order, and physical alias discovery cannot change artifacts, completeness, findings, plans, errors, or output order.

The complete resolved inventory is fixed before constructing a `LinkRequest`. Configuration-derived checks remain fail-complete even when concrete paths, physical groups, or successfully extracted entries are required to evaluate them.

The total order is fixed. Path-independent structural and fixed-locale production-membership checks complete before discovery. Concrete definition targets then run in canonical logical-path order; within one target, format assignment and `catalog_format_conflict` precede path capture and `locale_capture_mismatch`, which precede captured `catalog_locale_not_production`. Only after every target succeeds are physical-source groups visited by canonical primary-path order for `catalog_binding_conflict`. Only after every admitted group has attempted extraction does `catalog_scope_key_domain_conflict` run over successful resource-owned observations. The final gate is `scope_key_domain_mismatch` under the canonical recognizer-then-root order.

The first failure in that order is the only returned configuration error. The integration does not aggregate several reasons or let filesystem traversal, config member order, alias discovery, cache state, or future worker completion choose one. Each failure returns its owning `config_validation_failed`, constructs no `MessageDefinitionArtifact`, starts no reference-source scan, and never derives `Partial`. Only after the project-global domain admission stage passes its catalog-domain gate followed by its recognizer/root-domain gate may successful extractions enter post-extraction projection.

After those configuration gates succeed, a known definition-source enumeration, metadata, read, extraction, source-snapshot freshness, or projection failure records `SourceFailed`; a known configured definition deliberately not executed records `SourceOmitted`. A built-in source-scan failure records `ProducerFailed` for every scope bound by that producer's enabled recognizers, while a deliberately unexecuted matched source records `ProducerOmitted`. An exact configured external-artifact physical group becomes one complete authoritative participant after successful metadata grouping, bounded read, and contract decoding; it needs no embedded source fingerprint, sidecar, or independent upstream-source proof. Its metadata, I/O, or contract-decoding failure is `ProducerFailed` for every target scope. `ExternalArtifactUnverified` applies only when another integration submits an external artifact or cached value without the authoritative configured-snapshot evidence required by that integration.

If project-root setup, configuration setup, or a later configuration-derived assignment/binding check fails, the command returns the ordinary configuration or operational error and does not invoke the linker, regardless of whether an inventory or successful extraction prefix already exists. Once the inventory exists and every applicable configuration gate succeeds, source-attributable discovery and execution failures remain operational evidence and also derive the affected partial side; the linker may still return safe present-world findings, but no command with an operational failure or partial target scope generates, registers, diffs, or mutates output.
...
After a successful shared prefix, selected targets are independent transactions. The initial M3 CLI visits them sequentially in exact checked target-name order; it does not introduce a target worker pool ahead of the deferred shared CLI scheduler. A later extension of the shared [CLI parallel-execution boundary](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary) may reuse its command-scoped bounded pool to overlap independent target transactions only after the shared prefix completes and only if it preserves the same per-target transaction, result, failure, and side-effect semantics.
...
The stable artifact/link code, kind, and evidence mapping is also consumed by 009's project-link editor operational channel. The top-level CLI `errors[]`, summary, exit-code, and target-result placement rules in this section remain CLI-only; 009 owns editor notification episodes, retry, retained publication, and recovery and never turns these operational values into document diagnostics.
...
| `profile` | `unsupported_source_suffix`, `conflicting_alias_profile` |
...
The artifact, producer, linker, and output-registration variants are reason-only contracts. Their exact stable `details` object contains only the applicable `reason`, which is serialized first. They do not expose an `invariant`, stage, producer id, source or entry identity, path, span, target, transaction phase, nested error, or implementation-local context. Bounded human-facing logs or tracing may retain such context without changing structured output. A future need for another machine-readable field requires an explicit compatibility review and owning-design addendum; an implementation cannot add one ad hoc.
...
M0 implements the built-in source producer in `crates/intlify_producer_js`. It separately implements reusable `crates/intlify_cli` orchestration for project inventory, physical grouping and snapshot selection, complete definition extraction, project-global domain admission, checked definition projection, producer invocation, the single-unit graph, execution-derived completeness, `LinkRequest` construction, and the in-process integration-test path.

The M0 orchestration publishes no new executable CLI leaf and activates no editor link session. E0 enables 009-owned JS/TS catalog-key completion after M1, M3 exposes `intlify messages emit`, M5 exposes `intlify messages prune`, and L0 enables linker-backed editor/lint diagnostics; each reuses this M0-owned orchestration rather than defining another inventory or link path.
...
M0 includes a JavaScript/TypeScript source-scan producer without bundler integration and has no dependency on `intlify_lint`. Vue SFC reference production is the next producer phase in Deferred Follow-Up Notes and does not gate M0. M0 depends on 013 Tier 1 extraction, the coordinated promotion of explicit 013 `scope` plus `path` / `fixed` locale binding, and explicit producer-to-scope/domain bindings.
...
After the Phase 3C contracts and the dedicated project-level finding addendum are available, map the five M0 linker finding categories into initially default-off `intlify lint` rules without changing linker analysis.

Require a present valid `messages` section for any enabled linker-backed rule, resolve the 008 static input-capability union before work, and derive the lint presentation set. An empty set stops before project orchestration. For a non-empty set, skip reference production when no enabled rule requires it and otherwise reuse one shared producer/cache/linker orchestration and one `LinkOutcome`.
...
`tools/messages-bench/benchmark-phases.mjs` owns the closed phase/cost table, the messages-owned portion of the non-serialized interval-boundary registry, and the allowed interval-overlap topology for every active message-linker milestone. `tools/messages-bench/fixture-selection.json` owns the versioned fixture/profile inputs and declared scales, `tools/messages-bench/benchmark-profile.mjs` owns the active required-case matrix and E2E-to-resource companion mappings, `tools/messages-bench/checksum-observations.mjs` owns the checked messages-specific duration observation registry and codecs, and `tools/messages-bench/result-schema.mjs` owns the tool's versioned result shape. The active interval registry also imports owner-provided descriptors for admitted foreign phase/cost pairs. The phase table emitted in a result must equal the active table exactly. Boundary, overlap, and observation registries are checked code and are not duplicated in a result document; their exact active meaning is identified by the result's (`schemaVersion`, `benchmarkProfileRevision`) pair. `schemaVersion` versions the messages result shape and messages-owned registry/codec semantics. `benchmarkProfileRevision` selects the active required-case, fixture-profile, and foreign-import composition. For an imported descriptor, this pair identifies its admitted messages-side composition while the foreign owner's result-schema revision remains the primary authority for the descriptor itself. A new phase, cost, fixture meaning, measurement boundary, allowed overlap, checksum observation, companion mapping, or required case becomes valid only when this design activates it and the owning files, fixtures, revisions, and schema tests are updated together.

Initial M0 benchmark phases:

- `message_project_input_io`: exact costs `inventory_metadata_io`, `definition_snapshot_read`, `reference_snapshot_read`, and `external_artifact_read`
- `message_reference_produce_js`: JS/TS reference production with exact costs `source_parse`, `recognizer_match_and_static_evaluation`, `key_and_provenance_construction`, and `reference_artifact_construction`
- `message_reference_produce_js_cache`: exact costs `cache_miss_production` and `cache_hit_validation_and_access`, with cache miss and hit retained as separate variants
- `message_artifact_codec`: exact costs `reference_artifact_encode`, `reference_artifact_decode`, `definition_artifact_encode`, and `definition_artifact_decode`
- `message_definition_project`: exact costs `pre_extraction_admission` and `definition_projection`; both exclude host parsing and resource extraction
- `message_link_core`: exact costs `request_validation_and_scope_mapping`, `semantic_index_construction`, `selector_expansion_and_reference_resolution`, `reachability_and_placement`, and `finding_and_plan_materialization`
- `message_link_peak_memory`: exact cost `link_core_peak_live_memory`, with reference-dense, definition-dense, selector-expansion, and finding-dense requests retained as separate variants
...
- `message_typed_key_model` at M1: exact costs `coverage_baseline_selection` and `typed_key_model_construction`
- `message_link_fallback` at M2: exact costs `fallback_chain_construction`, `locale_aware_resolution`, and `locale_finding_materialization`
- `message_export_prepare` at M3: exact costs `selected_message_parse`, `message_semantic_validation`, `portable_diagnostic_mapping`, `argument_signature_derivation`, and `validated_export_batch_construction`
- `message_export_esm` at M3: exact costs `locale_asset_rendering`, `loader_map_rendering`, `typed_key_accessor_rendering`, and `export_artifact_set_construction`
- `message_export_artifact_size` at M3: exact cost `payload_size_comparison` across complete-set, initial/eager-load, per-locale, per-delivery-unit, and per-artifact-kind totals
- `message_output_register` at M3: exact costs `capability_preflight`, `path_mapping_and_ownership_inspection`, `staging`, `commit`, and `check_comparison`, using isolated output roots
- `messages_emit_e2e` at M3: exact cost `complete_workflow`
- `messages_emit_check_e2e` at M3: exact cost `complete_workflow`
- `messages_emit_json` at M3: exact cost `json_reporter`
- `message_delivery_graph_link` at M4: exact costs `reachability` and `duplicate_placement`
- `messages_prune_e2e` at M5: exact costs `dry_run_planning` and `structural_mutation`, after the coordinated 013 deletion boundary exists

A project-workflow E2E record qualifies for a companion extraction record exactly when its measured workflow invokes Tier 1 catalog extraction. The qualifying phase/cost set is `message_project_link_e2e` / `complete_workflow`, `messages_emit_e2e` / `complete_workflow`, `messages_emit_check_e2e` / `complete_workflow`, and either `messages_prune_e2e` cost when that concrete M5 case performs extraction. A case that starts from checked prebuilt definition artifacts does not qualify and must declare a distinct fixture or variant that makes that boundary explicit.

Every qualifying record requires exactly one ordinary result record using the exact 013-owned `resource_extract` phase and `host_parse_and_entry_extraction` cost. `benchmark-profile.mjs` owns one checked companion mapping from the complete required E2E tuple to the complete imported extraction tuple. The two records have equal fixture, variant, scale, runtime, and execution-model fields while retaining their own exact phase, cost, and operation. No imported extraction tuple may satisfy or be associated with two E2E tuples.

The result document serializes neither a relation field nor a nested metric. The messages result-schema validator loads the checked profile mapping, requires both mapped tuples exactly once, and rejects a missing, duplicate, shared, mismatched, skipped, or failed extraction companion. 013 exclusively owns the extraction record's physical-group order, interval boundary, counters, successful aggregation, failure behavior, and checksum inputs. Each imported record covers the aggregate of its related case's sequential JSON physical-source-group extraction calls, while the E2E record retains the complete workflow wall time.

Cross-record equality is limited exactly to fixture, variant, scale, runtime, and execution model. The validator checks the E2E phase, cost, operation, status, counts, and checksum against the 014 profile and result contract, and independently checks the extraction phase, cost, operation, status, counts, and checksum against the imported 013 contract. It recomputes the resource repetition checksum from the current invocation's complete resource per-iteration sequence and compares it only with the imported resource record. Neither count family nor checksum is compared across the two records.

The harness does not rename the imported phase to `message_resource_extract`, fold it into `message_definition_project`, or introduce a duplicate 014-owned extraction vocabulary. `tools/messages-bench/benchmark-phases.mjs` admits the one imported phase/cost pair with owner 013 and imports then re-exports the exact resource-owned `resource_host_parse_and_entry_extraction` descriptor unchanged. It cannot allocate another boundary ID or copy, restate, replace, or extend the owner-provided marker sets. A later mixed-format or concurrent case must first extend the 013 measurement contract.
...
`fixture-selection.json` gives every fixture a unique name, positive fixture revision, exact checked source or generator declaration, and its allowed variants and scales. `benchmark-profile.mjs` has one positive benchmark-profile revision and lists every milestone-required case as the exact tuple:
...
The matrix may require several cases for one phase/cost and need not form the Cartesian product of all fixtures, variants, and scales. Each required tuple must occur exactly once as measured output in a successful smoke invocation. An additional observational case is permitted only when it uses the active phase/cost vocabulary and declared fixture dimensions, but it cannot replace, merge with, or satisfy a required tuple. Activating a milestone appends its cases and updates the profile revision; it does not silently reinterpret an existing tuple.

The top-level messages benchmark result follows the existing formatter/resource pattern. It declares `schemaVersion`, `benchmarkProfileRevision`, tool and package version, `generatedAt`, default iterations and warmup iterations, the exact active phase/cost table, fixture metadata, and the ordered result records. The emitted fixture metadata and `benchmarkProfileRevision` must equal the active checked files. `generatedAt` is report context only and never participates in result matching or comparison.

Each result record declares phase, cost, fixture, variant, scale, runtime, execution model, operation, applicable input and output counts, metric, and execution status. `executionModel` is the exact closed token `in_process` or `fresh_process` and is orthogonal to the selected runtime. In-process core, codec, linker, export, and artifact-size records use `in_process`; a newly launched CLI command uses `fresh_process`. An imported resource-extraction record uses the same execution model as its related E2E record.

The messages-owned validator uses those ordinary fields and the closed phase/cost and required-case tables directly; it does not introduce a cross-product `BenchmarkRecordEnvelope`, `BenchmarkCaseIdentity`, relation field, or shared schema revision.

The messages-owned `metric` vocabulary is closed and milestone-gated:

| First active | Exact `metric` token | Record meaning |
| --- | --- | --- |
| M0 | `duration` | Aggregate monotonic-clock duration over the phase/cost's complete measured occurrence set. |
| M0 | `peak_live_memory` | Allocator-observed peak and retained live bytes plus successful allocation-event count for the exact measured interval. |
| M3 | `artifact_payload_size` | Exact `ExportArtifactPayload::len()` observations and the self-contained baseline-versus-linked bucket comparison. |

No case-folded, hyphenated, phase-specific, generic `memory`, generic `size`, or implementation-defined alias is accepted. Each active messages-owned phase/cost maps to exactly one token in `benchmark-profile.mjs`. An interval descriptor carries the same exact token, and the result record must equal both declarations. An admitted foreign interval instead retains its owner-provided metric tag unchanged; the messages profile and result must equal that imported descriptor rather than redeclaring its vocabulary locally.

A `duration` record includes positive `iterations`, aggregate non-negative `elapsedMs`, and the required deterministic-result `checksum`; it forbids allocator-memory, RSS, and artifact-size payload fields. A `peak_live_memory` record includes `peakLiveBytes`, `retainedLiveBytes`, and `allocationCount` under the documented allocator setup; it forbids `elapsedMs`, a generic checksum, RSS payload, and artifact-size comparison fields. An `artifact_payload_size` record uses only the product payload fingerprints, associations, relationships, exact byte observations, and bucket reconciliation defined below; it has no measured interval, `boundaryId`, elapsed-time payload, allocator payload, RSS payload, or generic checksum.

`peak_rss` is the planned exact token for a future separately supported fresh-process metric, but it is inactive and rejected by the phase table, profile, boundary registry, and result schema until its owner defines the sampling interval, exact record payload, platform support and skip rules, fixtures, and coordinated schema/profile revision. It is never substituted silently for `peak_live_memory`.

The metric tag fixes checksum presence. A measured duration record requires one non-negative safe-integer `checksum`. Peak-live-memory and peak-RSS records forbid it. An artifact-size record uses the existing product payload fingerprints and bucket reconciliation defined below instead of a generic checksum. A skipped record contains its bounded reason and no measured metric payload or checksum; a milestone-required skip makes the smoke result unacceptable. An execution failure aborts the benchmark invocation and cannot be serialized as a measured result.

For an ordinary duration record, all warmup iterations finish before timing and contribute to no reported count, elapsed value, or checksum. `iterations` is the positive measured repetition count and `elapsedMs` is the total elapsed time across those repetitions. Applicable input/output and structural counts describe one workflow iteration and must remain equal across every measured repetition. Each measured repetition produces one checksum for its deterministic semantic result. The harness requires all of those per-iteration checksums to be equal, fails on a mismatch rather than averaging it, and serializes the checksum produced by combining every measured value through the messages benchmark's versioned repetition-aggregation rule. Thus the output retains the existing formatter/resource aggregate-result shape without weakening within-invocation determinism checks.

Every fixture variant whose workflow reads or changes persistent project or output state declares one canonical pre-iteration snapshot. Before each warmup and each measured repetition, the harness establishes and verifies that exact snapshot independently. Snapshot construction, copying or reflinking, output-root initialization, restoration, verification, and teardown occur outside the phase timer and checksum input. Failure to establish the declared state aborts the case; it cannot become a skip or a measured prefix.

The declared variants keep different state paths separate. Initial emit write starts from an absent declared output root; exact-equality write skip starts from the complete expected owned root; emit check has separate equal and different roots; and prune structural mutation starts from the same complete source snapshot before every repetition. Dry-run cases likewise retain the declared immutable source snapshot. The contract fixes these logical states, not whether the harness implements them through copies, reflinks, fresh temporary directories, or another behaviorally equal private mechanism.

Ordinary `in_process` and `fresh_process` records retain no raw sample vector, sample-set identity, or sample ordinal. They report only their aggregate unadjusted timing. Raw product/startup samples and ordinal pairing are added only when startup adjustment is explicitly requested for a `fresh_process` record, in which case the 006 paired-startup contract applies without changing the ordinary aggregate result. An `in_process` record cannot request or carry startup adjustment.

### Interval Measurement Boundary Registry

`benchmark-phases.mjs` exports one canonically ordered boundary descriptor for every active interval phase/cost pair. The active M0 interval metric tags are exactly `duration` and `peak_live_memory`; the future `peak_rss` token remains inactive under the gate above. `artifact_payload_size` phases have no measured interval, must have no boundary descriptor or `boundaryId`, and remain governed by their product-payload fingerprint and bucket-reconciliation contract below.

Each active registry entry contains one exact owner and one boundary descriptor. The descriptor contains the exact phase, cost, unique stable ASCII `boundaryId`, metric tag, occurrence policy, first and final measurement-stage markers, and the ordered included and excluded stage-marker sets. A boundary ID is a semantic identifier and carries no `_vN` suffix. Duration markers delimit the monotonic-clock interval; memory markers delimit the allocator or process-sampling interval.

For a messages-owned descriptor, the messages result-schema and benchmark-profile revisions are the only version authorities for its meaning. Messages-owned descriptors are defined in this module. An admitted foreign pair instead uses the exact descriptor imported from its owning module; only the surrounding registry entry adds its owner. The foreign owner's result-schema revision is the primary authority for that descriptor. Importing a changed foreign descriptor into an active messages benchmark requires the coordinated messages result-schema and benchmark-profile revisions to advance as well. One active registry never contains two revisions of the same phase/cost boundary.

The runner and tests import the registry, but results continue to serialize only the familiar phase/cost table and metric records.

The same module owns a separate canonically ordered allowed-overlap topology. Each edge is one exact `parentBoundaryId` / `childBoundaryId` pair over active interval descriptors. The topology is acyclic, contains no self-edge or duplicate edge, and expresses only permission to nest; it does not require either interval to occur in a case. Required case and companion-record presence remain owned by `benchmark-profile.mjs`. An E2E boundary or explicitly composite component boundary may be a parent. A foreign descriptor such as the 013 extraction interval may be referenced as a child without changing or wrapping that descriptor.

The initial M0 topology has exactly these direct edges:
...
These rules reject an undeclared nesting, overlapping siblings, crossing intervals, an unmatched event, a duplicate start or stop within one occurrence, an extra occurrence, and a reordered stop while allowing only declared parent-child overlap.

#### M0 occurrence and interval aggregation

Every messages-owned M0 interval descriptor has exactly one of these occurrence policies:

| Occurrence policy | Active M0 phase/cost pairs |
| --- | --- |
| One interval per workflow iteration | `message_project_input_io` / `definition_snapshot_read`, `reference_snapshot_read`, and `external_artifact_read`; `message_definition_project` / `definition_projection`; every `message_link_core` pair; `message_project_link_e2e` / `complete_workflow`; and `message_link_peak_memory` / `link_core_peak_live_memory` |
| One interval per planned project inventory class | `message_project_input_io` / `inventory_metadata_io` |
| One interval per planned definition physical-source group | `message_definition_project` / `pre_extraction_admission` |
...
Before a measured repetition starts, the harness constructs the complete expected occurrence-identity sequence for every active pair from the checked fixture input. The stage recorder retains each occurrence's zero-based ordinal and checked item identity internally. A one-interval policy requires exactly ordinal `0`. A repeated policy requires every planned identity exactly once in canonical order. A missing, duplicate, extra, unknown, or reordered occurrence aborts the case and emits no measured prefix.

For one duration result record, `elapsedMs` is the sum of that pair's non-overlapping occurrence intervals across all measured repetitions. The applicable structural counts describe one workflow iteration and remain equal across repetitions; they do not multiply by `iterations`. For a repeated policy, the per-iteration checksum observation is one counted sequence of the occurrence DTOs in the same canonical order. Each occurrence DTO retains the phase-owned fields in the table below. The existing repetition rule first requires the complete per-iteration checksum and structural counts to be equal, then wrapping-adds the equal checksum once per measured repetition.

The occurrence policy, expected identity sequence, ordinals, stage events, and per-occurrence elapsed values are conformance state only. They are not serialized as a generic occurrence envelope, raw sample vector, relation field, or nested metric. A later concurrent implementation must introduce a coordinated boundary and aggregation update before allowing same-pair occurrences to overlap or changing this canonical aggregation.

Boundary conformance fixtures also prove that excluded setup and observation markers occur outside the applicable interval. The production benchmark uses the same boundary path with the real monotonic clock. A benchmark-only internal stage hook may retain opaque checked stage facts for the runner, but it cannot become a default-feature product API, change an ordinary result, or admit a caller-authored intermediate state.

The common M0 rules are:

- Fixture generation, resolved-input construction, pre-iteration snapshot establishment and verification, timer and allocator setup, checksum observation, `black_box`, result-record construction, result validation, reporting, and teardown are outside every measured interval unless one exact E2E row below says otherwise.
- A core duration interval starts only after its prepared checked input exists. It includes exactly the named operation and construction of that operation's ordinary typed output. It excludes file I/O and every earlier or later pipeline stage.
- The five `message_link_core` costs are non-overlapping stage intervals from one ordinary `link` invocation. Their ordered intervals supplement the complete invocation result; they do not call public partial-link APIs or construct five independent requests.
- An E2E interval intentionally overlaps the separately reported component intervals. It records the complete declared workflow without subtracting parser, extraction, linker, or I/O measurements.
- A measured operation failure aborts the case. The harness does not stop a prefix timer and serialize the successful prefix as a measured record.

The initial M0 duration boundaries are:

| Phase / cost | Boundary ID | Prepared input outside the timer | Exact timed work and stopping point |
| --- | --- | --- | --- |
| `message_project_input_io` / `inventory_metadata_io` | `project_inventory_metadata_io` | Resolved project root, checked messages/resources configuration, compiled include matchers, and one empty owning inventory class | Run one sequential occurrence for each planned inventory class. Include its directory traversal or configured-path enumeration, exact Unicode representability checks, source classification and assignment, physical-identity metadata reads, canonical physical grouping, and checked logical source identities. Stop when that class's complete canonical admitted logical facts, physical groups, and source-local metadata failures are retained, before source reads or content processing. |
| `message_project_input_io` / `definition_snapshot_read` | `project_definition_snapshot_read` | Canonically ordered pre-extraction-admitted definition physical groups and trusted project-root-relative source identities | Start before the first safe source open. Include no-follow capability checks attached to that open, metadata validation, complete exact byte reads, host-byte preflight, and exact UTF-8 snapshot construction for every selected primary source in canonical group order. Stop when all admitted immutable UTF-8 snapshots are retained, before input-fingerprint construction, host parsing, extraction, or projection. |
| `message_project_input_io` / `reference_snapshot_read` | `project_reference_snapshot_read` | Canonically ordered admitted built-in producer physical groups and checked logical source identities | Start before the first safe source open. Include no-follow capability checks attached to that open, metadata validation, complete exact byte reads in canonical group order, and immutable producer-group construction. Stop when all byte snapshots are retained, before UTF-8 decoding, parser-profile admission, parsing, recognition, fingerprinting, or cache lookup. |
| `message_project_input_io` / `external_artifact_read` | `project_external_artifact_read` | Canonically ordered checked external-artifact physical groups with primary and alias paths | Start before the first safe artifact open. Include no-follow capability checks attached to that open, metadata validation, and one complete exact byte read per group in canonical primary-path order. Stop when all artifact-byte snapshots are retained, before BLAKE3 cache-digest construction, contract decoding, or linking. |
| `message_reference_produce_js` / `source_parse` | `js_source_parse` | Exact admitted UTF-8 source, checked selected JS/TS source profile, and producer limits | Start immediately before invoking the selected parser frontend. Include parser work and checked conversion into the producer-owned ordered syntax/event view consumed by recognizers. Stop when that complete view or the checked parser-failure category is retained, before recognizer traversal. |
| `message_reference_produce_js` / `recognizer_match_and_static_evaluation` | `js_recognizer_static_evaluation` | Checked parsed source/event view and canonically ordered enabled recognizers | Include recognizer traversal, call-kind matching, bounded static key/set evaluation, and construction of canonical ordered match facts. Stop when those facts are retained, before key-syntax conversion or contract-value construction. |
| `message_reference_produce_js` / `key_and_provenance_construction` | `js_key_provenance_construction` | Canonical checked recognizer-match facts | Include source-facing key-syntax conversion, selector construction, checked scope/domain/key and provenance construction, dynamic-bound classification, and canonical reference-record ordering. Stop when all pre-artifact records are retained. |
| `message_reference_produce_js` / `reference_artifact_construction` | `js_reference_artifact_construction` | Complete checked pre-artifact reference records and producer/source identity metadata | Include ordinary `MessageReferenceArtifact` checked construction, all artifact-local limit and canonical-order validation, and production of the immutable artifact. Stop when the complete artifact is returned, before cache publication or wire encoding. |
| `message_reference_produce_js_cache` / `cache_miss_production` | `js_cache_miss_production` | Exact source snapshot, selected profile, resolved producer configuration, and an initialized cache known not to contain a matching key | Start before canonical cache-key construction and lookup. Include key construction, miss validation, the complete ordinary producer pipeline, checked cache-value construction, and atomic successful publication. Stop when the same complete checked artifact is returned and visible to a later matching lookup. File read remains outside. |
| `message_reference_produce_js_cache` / `cache_hit_validation_and_access` | `js_cache_hit_access` | The same prepared source/configuration inputs and one matching complete published cache entry | Start before canonical cache-key construction and lookup. Include key construction/comparison, freshness/component validation, bounded value validation, and checked artifact access. Stop when the existing complete artifact is returned, without parsing, recognition, reconstruction, or publication. |
| `message_artifact_codec` / `reference_artifact_encode` | `reference_artifact_encode` | Complete checked reference artifact and an empty bounded output sink | Include only the canonical reference-artifact writer and checked output completion. Stop when all exact wire bytes are retained. |
| `message_artifact_codec` / `reference_artifact_decode` | `reference_artifact_decode` | Complete input wire bytes already resident in memory and effective decoder limits | Include complete syntax, duplicate/member-order-independent admission, limit validation, checked value construction, and canonical in-memory artifact construction. Stop when the decoded artifact is returned; canonical re-encoding for checksum observation is outside. |
| `message_artifact_codec` / `definition_artifact_encode` | `definition_artifact_encode` | Complete checked definition artifact and an empty bounded output sink | Include only the canonical definition-artifact writer and checked output completion. Stop when all exact wire bytes are retained. |
| `message_artifact_codec` / `definition_artifact_decode` | `definition_artifact_decode` | Complete input wire bytes already resident in memory and effective decoder limits | Include complete syntax, duplicate/member-order-independent admission, limit validation, checked value construction, and canonical in-memory artifact construction. Stop when the decoded artifact is returned; canonical re-encoding for checksum observation is outside. |
| `message_definition_project` / `pre_extraction_admission` | `definition_pre_extraction_admission` | One already formed canonical physical-source group, resolved catalog assignments, the host-format registry, and effective limits | Include group binding/format/path/alias admission, scope/locale budget checks, and checked definition-artifact envelope preflight. Stop when the extraction-ready prepared group is returned. Exclude inventory, source read, host parsing, resource extraction, project-global domain aggregation, and contract projection. |
| `message_definition_project` / `definition_projection` | `definition_projection` | Complete successful extraction artifacts after the project-global domain admission stage passes its catalog-domain and recognizer/root-domain gates | Include checked resource-to-contract value conversion, definition-record construction, fingerprint/identity attachment from admitted inputs, and ordinary `MessageDefinitionArtifact` construction. Stop when every projected artifact is retained in canonical primary-source order. |
| `message_link_core` / `request_validation_and_scope_mapping` | `link_request_validation_scope_mapping` | Complete admitted reference/definition inventories, resolved policy, execution failures, target-scope inventory, effective limits, and the fixed M0 graph/mapping policy | Include empty scope-mapping construction, execution-derived completeness construction, fixed `["main"]` graph construction, aggregate request preflights, artifact/policy admission, scope mapping, and completeness/policy consistency checks. Stop when the complete ordinary checked `LinkRequest` is retained, before the first semantic-index stage marker. |
| `message_link_core` / `semantic_index_construction` | `link_semantic_index_construction` | Checked mapped request state from the same invocation | Include canonical definition/reference index construction, occurrence accounting, collision/ambiguity fact construction, and required bounded lookup structures. Stop before selector expansion or reference resolution. |
| `message_link_core` / `selector_expansion_and_reference_resolution` | `link_selector_resolution` | Complete semantic indices from the same invocation | Include bounded selector expansion, exact/prefix/pattern matching, dynamic/degraded classification, locale-independent reference resolution, and canonical resolution facts. Stop before root reachability or placement. |
| `message_link_core` / `reachability_and_placement` | `link_reachability_placement` | Complete canonical resolution facts, configured roots, and admitted delivery graph | Include root expansion, reachability traversal, selected-definition placement, and canonical reachable/placement facts. Stop before finding or bundle-plan materialization. |
...
`message_link_peak_memory` / `link_core_peak_live_memory` uses boundary ID `link_core_peak_live_memory` and the release-profile Rust benchmark runner with a counting global-allocator wrapper around the runner's selected platform allocator. M0 runs this case synchronously on the caller thread with no worker pool, background benchmark task, concurrent case, or allocator activity outside the measured operation.

The runner constructs and validates the complete `LinkRequest`, completes warmup calls, reaches allocator quiescence, records the current live-byte baseline, and resets peak and successful-allocation-event counters before the measured call. The measurement then starts immediately before ordinary `link(&request)` and ends with the complete successful `LinkOutcome` still alive.

`peakLiveBytes` is the greatest checked increase in live allocated bytes above the baseline during that interval. `retainedLiveBytes` is the checked increase above the same baseline at the end of the interval while the outcome remains alive. `allocationCount` counts each successful nonzero allocation or reallocation event during the interval exactly once; deallocation events affect live-byte accounting but not that count. Counter overflow, a live-byte value below the isolated baseline, allocator instrumentation failure, link failure, or concurrent allocator activity invalidates the case rather than producing a partial measurement.

After measurement, the runner passes the still-live outcome to `black_box`; it performs no checksum observation and has no observation codec. Dropping the outcome/request, allocator-counter teardown, fixture teardown, RSS sampling, and result serialization are outside. Peak RSS is a distinct optional fresh-process metric and cannot replace this allocator-level record.

### Duration Checksum Observations

The messages benchmark uses a versioned non-cryptographic `u32` checksum for deterministic observation and dead-code prevention, not artifact identity, authenticity, or historical regression gating. Its initial numeric routine is FNV-1a with offset basis `2,166,136,261`, multiplier `16,777,619`, XOR-before-multiply, and wrapping modulo `2^32`.

Typed values use the same initial framing shape as the 013 resource benchmark: `tag:u8 || payload_len:u64be || payload`, with counted length-prefixed sequences, exact UTF-8 or byte payloads, fixed-width unsigned big-endian integers, one-byte booleans, and explicit absent/present option tags. This is a shared primitive shape, not shared product ownership. A later change in either product does not alter the other without its own result-schema revision.

`checksum-observations.mjs` exports one canonically ordered closed registry entry for every active messages-owned duration phase/cost pair. Each entry contains the exact phase, cost, and one unique stable ASCII `codecId`, and resolves that ID to the checked codec implementation and its conformance vectors. The registry is imported by the runner, result-schema validator, and tests but is not serialized into benchmark results.

For every codec, that module owns the complete observation DTO, field order, closed token vocabulary, ascending field tags, typed framing, sequence order, and any nested checksum's exact ASCII domain plus byte input. A phrase in the semantic table below such as source profile, syntax kind, static-evaluation outcome, cache outcome, semantic-model fact, typed command outcome, or source-byte checksum cannot enter an active codec as an open string, implementation enum spelling, generic object, or undocumented digest. JS and Rust implementations that participate in the same codec must pass the same checked vectors before the phase/cost becomes active.

One measured operation first returns or retains its ordinary typed result. After the exact phase timer stops, the harness derives the phase's observation through the table below and feeds it to `black_box`. The observation codec never becomes a product API, changes a product artifact, or extends the measured interval. Every measured repetition must produce an equal per-iteration checksum. The serialized checksum starts at zero and wrapping-adds those equal `u32` values in measurement order; warmups contribute nothing.

Public artifacts use their canonical writers or existing product payload fingerprints. Internal stage observations use only the explicitly named typed facts in canonical product order. They never use Rust `Debug`, generic `serde` object order, pointer or arena identity, hash-map order, worker order, allocation capacity, timing, cache address, absolute path, process ID, timestamp, `generatedAt`, or human-rendered text.

`checksum-observations.mjs` maps every active messages-owned duration phase/cost pair as follows:

| Phase / cost | Per-iteration observation |
| --- | --- |
| `message_project_input_io` / `inventory_metadata_io` | Canonical admitted logical source facts followed by canonical physical-equivalence classes represented only through their primary-plus-alias logical paths and source kind; raw device, inode, volume, file ID, directory-enumeration order, and absolute path are excluded. |
| `message_project_input_io` / `definition_snapshot_read` | Canonical physical-group path sequence, exact source-byte length, and the codec-owned domain-separated checksum of the complete selected definition-source bytes for every group in canonical order. |
| `message_project_input_io` / `reference_snapshot_read` | Canonical physical-group path sequence, selected source profile, exact source-byte length, and the codec-owned domain-separated checksum of the complete selected reference-source bytes for every group in canonical order. |
| `message_project_input_io` / `external_artifact_read` | Canonical primary-plus-alias configured path sequence, exact selected byte length, and the codec-owned domain-separated checksum of the complete artifact-file bytes for every physical group in canonical primary-path order. |
| `message_reference_produce_js` / `source_parse` | Source profile and exact source-byte checksum, followed by the producer-owned ordered syntax events consumed by enabled recognizers. Each event contains its closed normalized syntax-kind tag and exact UTF-8 span. Parser failure uses its closed category and ordered spans instead of an AST/debug dump. |
| `message_reference_produce_js` / `recognizer_match_and_static_evaluation` | Canonical ordered recognizer matches, including recognizer identity, call kind, configured scope/domain, static-evaluation outcome, selector form/value when present, and source span. |
| `message_reference_produce_js` / `key_and_provenance_construction` | Canonical reference records before artifact framing: scope, domain, selector, source provenance, dynamic-bound state, and record order. |
| `message_reference_produce_js` / `reference_artifact_construction` | Canonical `MessageReferenceArtifact` bytes. |
| `message_reference_produce_js_cache` / `cache_miss_production`, `cache_hit_validation_and_access` | Exact cache outcome tag plus canonical returned `MessageReferenceArtifact` bytes and the checked cache-key/freshness components already owned by the producer contract. |
| `message_artifact_codec` / `reference_artifact_encode`, `definition_artifact_encode` | Complete exact encoded artifact bytes. |
| `message_artifact_codec` / `reference_artifact_decode`, `definition_artifact_decode` | Canonical re-encoding of the complete checked decoded artifact. Input spelling differences that decode to the same artifact therefore do not create a second semantic observation. |
| `message_definition_project` / `pre_extraction_admission` | Canonical physical-group paths, resolved host format, scope, locale, portable path/alias admission facts, and the selected admission outcome returned before source acquisition and extraction. Source bytes, selected-source checksums, and project-global domain observations are unavailable at this stage and are excluded. |
| `message_definition_project` / `definition_projection` | Canonical `MessageDefinitionArtifact` bytes for every successfully projected group in canonical primary-source order. |
| `message_link_core` / `request_validation_and_scope_mapping` | Checked mapped scope table, policy/scope-completeness identities, canonical admitted reference/definition identities, and selected validation outcome. |
| `message_link_core` / `semantic_index_construction` | Canonical domain-qualified reference and definition index keys, occurrence counts, and ambiguity/collision facts; private table layout and bucket capacity are excluded. |
| `message_link_core` / `selector_expansion_and_reference_resolution` | Canonical selector expansions and their ordered matched, unresolved, dynamic, and degraded-analysis facts. |
| `message_link_core` / `reachability_and_placement` | Canonical reachable key set and ordered `(delivery unit, requested locale, selected definition)` placements. |
...
| `message_typed_key_model` / `typed_key_model_construction` | Canonical typed-key model fields and the fixed private model-to-baseline relation required for export preparation. |
| `message_link_fallback` / `fallback_chain_construction` | Canonical production locale set and checked ordered fallback chains. |
| `message_link_fallback` / `locale_aware_resolution` | Canonical requested-locale/key resolutions and selected definition locale/source identities. |
| `message_link_fallback` / `locale_finding_materialization` | Canonical locale-aware findings and affected identities. |
...
| `messages_emit_json` / `json_reporter` | Typed command-result fields in their product order before JSON rendering, excluding `generatedAt`, benchmark metadata, insignificant JSON spelling, and human text. |
| `message_delivery_graph_link` / `reachability`, `duplicate_placement` | Canonical graph roots/edges followed by reachable placements or duplicate-placement facts in product order. |
| `messages_prune_e2e` / `dry_run_planning` | Canonical selected deletions, excluded/blocked evidence, and planned physical-group mutations. |
| `messages_prune_e2e` / `structural_mutation` | Canonical validated mutation records plus exact resulting source-byte checksums in canonical physical-group order. |

`message_link_peak_memory` and `message_export_artifact_size` are intentionally absent because their metric tags forbid a generic duration checksum. The imported `resource_extract` pair is likewise absent because 013 owns and versions its complete checksum observation and repetition aggregation.

A milestone cannot activate a duration phase/cost until its registry entry, unique codec ID, canonical typed observation, complete codec implementation, JS/Rust shared vectors where both implementations participate, and result-schema fixtures land together. Changing an observation's included fields, order, normalized tags, framing, domain, numeric routine, or repetition aggregation requires the messages result-schema revision and, when existing comparisons change meaning, the benchmark-profile revision to advance. The result never embeds the registry or codec descriptors as a second version authority.

### Export artifact-size comparison

`message_export_artifact_size` compares exactly two variants generated from one declared fixture in one benchmark invocation. Its fixture metadata contains:

- fixture name and fixture revision;
- benchmark-profile revision and active message-linker milestone/contract revision;
- selected exporter ID and exporter benchmark revision;
- exact checked target options and accepted artifact kind/version contracts;
- the profile's entry roots, grouping mode, and declared scales.

The result does not duplicate complete reference/definition artifacts, resolved linker policy, scope mapping, completeness, or delivery graph as a second comparison identity. The checked benchmark operation constructs baseline and linked output from the same admitted request and fails before producing a comparison when that common-input invariant cannot be established.

Fixture, profile, milestone/contract, and exporter benchmark revisions are positive `u32` values with initial value `1`; they are not package versions, git commits, timestamps, or arbitrary strings. The fixture owner increments the fixture revision whenever fixture-owned source or resolved configuration meaning changes. The profile owner increments its revision when entry roots, grouping, scale construction, or another measurement rule changes. The exporter owner increments its benchmark revision when canonical generation or benchmark artifact classification changes without an artifact kind/version change.

Two results from different invocations are comparable for trend/reporting only when their result `schemaVersion`, benchmark-profile revision, complete fixture metadata above, phase, cost, variant, scale, runtime, execution model, and operation are equal. Package version is reported but may differ because comparing implementations across revisions is the benchmark's purpose. Cross-invocation comparison never participates in the current CI gate. There is no serialized `BenchmarkComparisonIdentity` or comparison digest.
...
The non-published `tools/messages-bench/rs` runner is the only supported caller that enables the exact Cargo feature `benchmark`. Both `intlify_linker` and `intlify_export` define that non-default feature. `intlify_export/benchmark` forwards `intlify_linker/benchmark`, and the runner enables the former so Cargo feature unification activates the complete checked benchmark path. The runner and both crates are built with the Cargo release profile so the measurement uses optimized code. Under its feature, `intlify_linker` exposes one documentation-hidden checked operation that accepts the ordinary `LinkRequest` and returns an opaque pair containing the exact ordinary `link` outcome and its corresponding full-retention benchmark outcome. It runs ordinary request validation and linking for the first member, requires generation to be unblocked, and constructs the second member from that same admitted request, resolved policy, mapping, completeness, delivery graph, definition snapshots, and M1 model/baseline relation.
...
Both opaque outcomes then pass independently through the ordinary `prepare_export`, built-in exporter generation, checked `ExportArtifactSet`, and canonical encoding paths. Under `intlify_export/benchmark`, the export crate additionally exposes only the documentation-hidden checked observation path defined below; it neither changes `PlatformExporter` nor adds benchmark fields to product artifacts. Neither benchmark feature is enabled by `intlify_cli`, platform integrations, default crate features, product release builds, packages, or shipped artifacts. A release-profile build of the dedicated benchmark runner is explicitly allowed and is not a product release build. No normal product caller can construct the baseline outcome, and the harness never serializes it as a product plan, registers it, or publishes it.
...
The `intlify_export/benchmark` feature adds the following observation-only semantic values:
...
For one comparison case, `message_export_esm` emits separate records with exact variants `all_definitions_baseline` and `linked_output`. `message_export_artifact_size` emits one `payload_size_comparison` record with exact variant `all_definitions_baseline_vs_linked_output`; it contains both tagged artifact observations and all derived bucket comparisons. A reporter never presents two independent size records and asks a downstream consumer to join them heuristically.

The messages result-schema validator selects the two exporter records through equal fixture, scale, runtime, and execution-model fields plus their exact phase, final `export_artifact_set_construction` cost, and variant labels. It selects the comparison through the same ordinary fields and its exact phase/cost/variant. All three initial artifact-size records use `in_process`. No common case-identity or relation record is required; the comparison result remains self-contained even when exporter timing records are omitted from a human summary.
...
Each successful `message_export_esm` variant's `export_artifact_set_construction` record carries one canonical artifact observation vector. Every item contains the artifact's exact logical path, `ExportArtifactKind`, payload byte length, existing product `PayloadFingerprint`, and complete canonical common relationship vector. Every relationship reuses the product `ExportArtifactRelationshipKind` and exact target logical path; no benchmark-only relationship spelling or inferred edge is admitted. The artifact vector follows the checked set's canonical artifact order, and each relationship vector follows the order established by checked `ExportArtifactSet` construction. The payload field reuses the product fingerprint contract over exact payload bytes and introduces no benchmark-specific hash or byte framing.

The comparison record carries `baselineArtifacts` and `linkedArtifacts`, which must equal the complete observation vectors on the two exporter records selected by the exact fields above. It also carries both canonical association vectors, exact benchmark-profile entry roots, and every canonical `ArtifactSizeBucketRecord`. The result-schema validator resolves every relationship target against its variant's complete artifact vector, follows only canonical `EagerLoad` edges from the exact entry roots, and recomputes complete-set, initial/eager-load, locale, delivery-unit, and kind totals from the payload lengths, relationships, and associations. It rejects a missing, extra, duplicate, reordered, unresolved, or inconsistent observation or relationship.

There is no `artifactChecksum`, `baselineArtifactChecksum`, `linkedArtifactChecksum`, or `measurementChecksum`. A malformed product fingerprint, exporter/comparison observation mismatch, unresolved entry root, invalid association, or bucket reconciliation failure makes the result invalid. Timing, RSS, allocator state, registration data, absolute paths, and capacity enter neither product payload fingerprints nor artifact-size grouping.
...
The CI gate consists only of benchmark-target build success, smoke execution of every milestone-required case from the active benchmark profile, result-schema validation, and deterministic-result integrity within that one benchmark invocation. Schema validation requires the complete phase/cost vocabulary, exact active fixture/profile metadata, every required tuple exactly once, declared fixture, variant, scale, runtime, operation, execution-model, and metric fields, applicable counts and metrics, equal per-iteration duration checksums where required, valid product payload fingerprints, exact same-invocation exporter/comparison observation equality, and bucket reconciliation. A missing or duplicate required case, undeclared case dimension, malformed record, unknown vocabulary value, execution failure, within-run duration-checksum mismatch, artifact-observation mismatch, or reconciliation failure fails CI.

The validator never loads a checked-in or previous benchmark result as an expected checksum, fingerprint, timing, memory, or size baseline. A result from another invocation may be selected later for compatible trend reporting, but a difference from it is not a failure under this policy.

The separately invoked Phase 3A startup benchmark is not a milestone-required fixture or variant for this gate and remains outside normal CI. A fresh-process record always reports its aggregate unadjusted timing; raw samples are present only in an explicitly requested paired-startup observation.
...
A missing or failed sample, unequal count, different session, incompatible route, build, OS, architecture, libc, Node.js, or npm value, or any other compatibility mismatch omits the complete adjusted observation. It never changes the ordinary aggregate E2E record, reuses a startup result from another run, pairs by temporal proximity, or fails the 014 CI gate.

Elapsed time, allocator memory, RSS, artifact payload bytes, derived byte differences or ratios, and cross-invocation checksum or payload-fingerprint changes are recorded and compared as observations only. CI does not apply a historical-equality or numeric pass/fail threshold to any of them while the implementation, workloads, and measurement environment are still evolving. A regression threshold may be introduced only by a later explicit benchmark-policy update after a representative workload, stable measurement environment, accepted baseline, noise treatment, threshold, owning milestone, and failure behavior have all been fixed.
...
- Consumer-activation fixtures keep an omitted `messages` section valid when no linker consumer is enabled, then reject every explicitly enabled linker-backed lint rule against that absence with the 008-owned `linker_messages_required` reason, ASCII-smallest enabled rule id, exact `/lint/rules/<rule-id>` pointer, and no inferred value or downstream work. A malformed present `messages` section retains its earlier 014 failure, and future implicit preset activation uses `/lint` only under the separately gated 008 branch.
  - Stdin activation fixtures first preserve every independent section error and the missing-`messages` result above, then reject a valid configured stdin invocation with top-level `invalid_cli_argument`, `kind: "input"`, `details.reason: "linker_rule_requires_filesystem_inventory"`, `details.option: "--stdin-filepath"`, the ASCII-smallest enabled linker-backed `details.ruleId`, `results: []`, `summary.operation: "stdin"`, and exit `2`. They reject `pointer`, `argument`, `value`, path evidence, stdin reads, ignore setup, inventory, artifacts, caches, and an implicit overlay.
...
- Inventory-selection fixtures enumerate only linker-participating resource definitions, built-in producer include matches, and exact external artifact declarations; accept a valid zero-match pattern as a complete empty set; reject config-free direct-extension, content-sniffing, or standalone `.mf2` fallback; and group selected external file-symlink and hard-link declarations by physical identity without crossing definition, built-in source, or external-artifact inventory classes.
...
- Producer-include fixtures use the exact 013 glob grammar, accept the example's separate `.ts` and `.tsx` patterns, accept a valid non-empty pattern set that matches zero files as one completed closed scan, and reject brace expansion or a glob-shaped external artifact path rather than delegating syntax to a filesystem dependency.
  - Source-profile fixtures admit only exact lowercase `.js`, `.jsx`, `.mjs`, `.cjs`, `.ts`, `.tsx`, `.mts`, `.cts`, `.d.ts`, `.d.mts`, and `.d.cts` suffixes with longest-suffix matching; parse declaration suffixes through the declaration profile; and reject every unsupported, extensionless, or case-varied selected source — including `.vue` in M0 — as `ProducerFailed` with exact `profile` / `unsupported_source_suffix`, without metadata, editor-language, or project-config inference.
  - Source-goal fixtures use fixed module for `.mjs`/`.mts`/`.d.mts` and fixed CommonJS for `.cjs`/`.cts`/`.d.cts`. Neutral suffixes parse module first and retry the complete source exactly once as script only after an ordinary grammar rejection; they keep only the successful authoritative AST, prefer module when it succeeds, and never retry a limit, UTF-8, cancellation, invariant, internal, or post-scan failure.
...
- JS-call argument fixtures consume only an ordinary non-spread first argument, ignore every later argument for selector semantics, emit `UnboundedDynamic` for a dynamic lookup first argument, and fail the complete source as `ProducerFailed` for a matched zero-argument call, a spread first argument, or a dynamic set first argument.
  ...
  - Built-in-origin fixtures require every emitted JS/TS record to span exactly the complete first argument expression, including its own parentheses and admitted TypeScript wrappers but excluding call delimiters and later arguments. They apply the same rule to static and dynamic lookup records and fail the complete source rather than omit origin when the checked `u32` UTF-8 range cannot be constructed.
- Closed-world fixtures prove that `.gitignore`, `--ignore-path`, formatter/linter ignore patterns, hidden-name filtering, and default VCS/dependency/output exclusions cannot remove configured inputs; directory symlinks are not traversed, selected file symlinks and hard links use physical grouping, and a file selected by both definition and reference inventories participates once on each applicable side.
- Completeness fixtures map every admitted source and producer execution outcome to the fixed partial reason; treat one successfully decoded exact configured external-artifact physical group, including an empty artifact selected from CLI disk or one unambiguous primary/alias editor buffer set, as one authoritative project-global participant; conservatively weaken every target reference scope on its metadata, I/O, or contract failure; reserve `ExternalArtifactUnverified` for non-authoritative integration/cache inputs; and prove that setup failure creates no `LinkRequest` while post-inventory execution failure produces no export, check diff, registration, or mutation. Separate configuration-gate fixtures require every assignment, locale, scope, alias-binding, and domain contradiction to retain its owning `config_validation_failed`, produce no definition artifact, partial reason, or `LinkOutcome`, and start no reference-source scan even when discovery, physical grouping, or successful extraction was needed to observe it. Cross-kind fixtures enforce path-independent validation first; canonical target and per-target format/capture/production-locale order; canonical physical-group binding; complete extraction followed by the project-global domain admission stage's catalog-domain gate; and its final recognizer/root-domain gate, with exactly one result independent of traversal, alias, cache, and worker order.
  - Project-inventory freshness fixtures map bytes, physical identity, and checked-read changes during current snapshot acquisition for definition, selected built-in producer-source, and configured external reference-artifact groups to exact `input_read_failed` / `source_changed` with the canonical primary logical path. They require no `ioKind`, `rawOsError`, byte sample, digest, or prior/current metadata; emit no artifact or cache value; and perform no automatic reopen or reread. Definition groups derive `SourceFailed`; built-in source groups derive `ProducerFailed` for every recognizer-bound scope; and configured external-artifact groups derive `ProducerFailed` for every target reference scope. They keep formatter `source_snapshot_mismatch` and operand-driven no-stale-check behavior separate and permit a new attempt only through the documented CLI invocation or editor event/reanalysis boundaries.
  - Resource-extraction admission fixtures exercise `resource_format_unsupported`, `resource_parse_failed`, `resource_entry_unsupported`, `resource_document_unsupported`, and `resource_limit_exceeded` with `phase: "extract"`. They make each derive `SourceFailed` only for the authoritative selected snapshot of a known current-generation definition group after every applicable configuration gate completes without contradiction and any candidate configuration installs. They retain an isolated candidate failure only as candidate execution evidence, give a winning configuration contradiction no `LinkRequest` or partial state, discard a publication-freshness mismatch without an operational error or completeness change, and prove that `validate_write_back` failures never change completeness.
- Editor open-buffer override fixtures form physical groups before substitution; admit one mapped buffer or a set of mapped buffers only when their exact current UTF-8 bytes are equal; capture every contributing URI/version; and produce exactly one definition or reference artifact for an admitted group. They accept complete successful external-artifact decode as closed configured participation without `ExternalArtifactUnverified`, map invalid selected bytes to `ProducerFailed` without disk fallback, map unequal claims or unresolved ownership to `OpenEditorWorld` with no artifact or disk fallback, reject first/last/primary-alias winner selection, and return to the current disk snapshot only after every override closes. Newly observed matching catalog paths, and application paths while references are required, produce no provisional contract artifact or cache entry before refreshed grouping; they remain `OpenEditorWorld` until the ordinary definition gates or producer-wide preflights succeed. Under a definition-only capability union, application paths are not selected participants and neither start producer work nor change completeness.
- External-artifact cache fixtures hash the exact complete selected file bytes with BLAKE3-256; include only the canonical host-primary normalized project-relative path, byte length, decoder revision, and effective-limit compatibility; and exclude every host alias. They permit a hit after an alias-only set change, require a miss when the canonical primary changes or any included component differs, and prove that this consumer-side fingerprint neither enters the artifact wire contract nor claims upstream source freshness.
...
- Operational-mapping fixtures cover every typed artifact, linker, export-preparation, exporter, and registration branch; require the exact boundary-level code plus `details.kind` and canonical evidence; keep transport I/O on `input_read_failed`; require exact `source_changed` for every project-inventory filesystem-participant snapshot-freshness branch without relabeling reference participants as artifact or producer errors; and map every internal variant to its registered unique `internal_error` reason.
  - Reason-only invariant fixtures require the exact one-field `details` object for `message_artifact_invariant_failed`, `message_producer_invariant_failed`, `message_linker_invariant_failed`, and `message_output_registration_invariant_failed`. They reject `invariant`, stage, producer, source, entry, path, span, target, transaction, nested-error, and arbitrary debug fields while allowing bounded implementation-local tracing outside structured output.
  ...
  - Built-in producer-failure fixtures require exact `message_artifact_failed` / `producer_failed` evidence with `producer`, `stage`, `reason`, and primary `source` followed only by permitted optional `span` or paired `limit`/`observed`; accept exactly the current M0 closed stage/reason pairings; reject the deferred Vue-only `unsupported_vue_script_lang`, `unsupported_vue_template_lang`, `source_map`, and `host_span_*` vocabulary before that frontend is promoted; and reject null optionals, arbitrary parser text, source excerpts, selector spellings, dependency codes, and cross-stage evidence fields.
  ...
  - Producer-invariant fixtures keep impossible AST duplication, invalid parser spans over valid UTF-8, validated-state contradictions, and equivalent impossible output rejection on `internal_error` / `message_producer_invariant_failed`; they emit the reason-only details contract and never relabel an invariant with an input-facing producer stage.
...
- Placement fixtures keep artifact/link errors at the project-transaction top level and exporter/registration errors on the selected target result and require exit `2` for every operational code. The M0 and M3 invariant evidence contracts are complete here; a first-emission guard remains only for later M5 prune error variants whose bounded evidence or transaction fields are still explicitly deferred.
...
- Lint-gating fixtures resolve rules and severities before producer work; perform no linker invocation when no linker-backed rule is enabled or when the filesystem invocation's presentation set is empty; reject enabled linker-backed rules before inventory when `messages` is absent; and, for a non-empty presentation set, skip reference inventory, artifact load, source scan, and reference-cache lookup when enabled rules require only definition/policy inputs. That definition-only fast path retains producer declarations only as metadata, selects no reference participant, derives `references: Closed` over the empty selected set, derives neither `ProducerOmitted` nor `OpenEditorWorld`, and filters every unrequested reference-dependent finding at the adapter. A transition to a reference-requiring capability constructs a new full request and inventory. For a non-empty presentation set, reference-requiring rules invoke exactly one shared linker orchestration, adding the shared producer/cache path only once when one or several enabled rules require references.
  - Capability fixtures exercise every 008 table row alone and in unions: `ambiguous-message-definition` and `orphaned-translation` remain definition-only, while each of `unresolved-message`, `missing-translation`, `unused-message`, `unbounded-dynamic-reference`, and `degraded-analysis` activates the one shared reference path. The `degraded-analysis` case must retain its wide-selector variant rather than being narrowed from current completeness evidence.
  - They prove rule order, severity, preset, reporter, operands, ignore selection, and disabled findings do not alter artifact/cache identity or linker output; `emit` and `prune` remain independent full-reference consumers and never reuse lint-only request-relative closure. A producer selected by a request that otherwise requires references but intentionally left unexecuted derives typed `Partial` completeness and adapts `degraded-analysis` instead of fabricating absence-dependent results. Capability-based non-selection is instead the empty selected `Closed` set above and never derives `ProducerOmitted`.
  - Filesystem lint fixtures run the same enabled linker capability over non-empty full-project, single-file, directory, glob, and ignored-target selections and require the same configuration-authoritative participants, completeness table, artifacts, cache identities, and `LinkOutcome`; only ordinary target results and the resolved-scope finding-presentation set may differ. Scope-filter fixtures admit selected logical aliases before physical-primary selection, retain an admitted scope after target-local metadata/read/extraction failure, map through the request's admitted scope mapping, sort and de-duplicate the resulting scopes, admit reference-primary and primary-less findings for a selected scope, reject every finding from an unselected scope regardless of location, and prove filtering never mutates the outcome.
  - Empty-scope fixtures cover standalone `.mf2`, config-free catalogs, resource definitions without linker identity, and fully ignored selections. After independent validation and the missing-`messages` check, they require ordinary message-level results with no project inventory, producer, external artifact, cache, `LinkRequest`, `LinkOutcome`, `Partial`, or project finding. Stdin fixtures separately follow the exact activation error above and never construct an overlay participant.
- Lint-rollout fixtures make every L0/L1 linker-backed rule `off` and absent from `recommended` initially, require explicit configuration to enable it, apply ordinary 008 severity/quiet/max-warning/exit behavior, and count one linker finding once regardless of related-entry count.
  - They reject implicit enablement from resource config, cache presence, or another linker command and require a deliberate per-rule contract change for later preset promotion. Promotion fixtures additionally cover missing-`messages` selection by ASCII rule id and `/lint` evidence when no explicit rule node exists.
...
- **Definition subjects:** permit `EntryStructuralPathBytes`, `CatalogKeyBytes`, `CatalogKeyTokens`, `MessageBytes`, and `TotalMessageBytes` only with `DefinitionArtifactGroup`.
...
- **Reference subjects:** permit `SelectorPathBytes`, `SelectorPatternBytes`, `SelectorPatternTokens`, `CatalogKeyTokens`, and `ReasonBytes` only with `ReferenceArtifactGroup`.
...
- **Per-value observations:** require the eleven per-value field, selector, catalog-key-token, and path-segment counters, plus `ScopeMappingEntries`, `PathSegments`, and `LogicalAliases`, to carry exactly `Exact(effective_limit + 1)` and never `ArithmeticOverflow`.
...
- Export-gate fixtures construct the stable-identity union of definitions selected across all valid plans and the exact baseline definitions privately related to all admitted M1 model keys. They run the shared parse, `build_semantic_model`, and `validate_semantics` pipeline once per unique `DefinitionLocation` over exact decoded payloads before any exporter call.
  ...
  - They cover repeated placement/locale/unit references, one definition appearing in both validation roles, several model keys related to one definition, equal text at distinct identities, and optional byte-identical parse-and-semantic-result reuse with record-specific evidence.
  - `None` plans skip the gate. `Some(Vec::new())` with no models passes an empty gate, while the same empty plan slice with admitted models validates their baseline sources and can produce typed output.
  - An invalid unused baseline definition blocks when its key is exposed by an admitted model. An invalid unused definition outside both validation sets does not block a disjoint output.
  - They preserve stable `DefinitionLocation` order independently of validation role, parser order or semantic order within a record, category, and exact diagnostic mapping; prove reused and non-reused computation within one call observationally equivalent; and exercise invalid built-in and third-party artifacts, exactly one selected exporter per transaction, an exporter returning multiple artifact kinds, direct/custom integrations, and `--check`.
  ...
  - Retention fixtures continue parsing every export-validation record after the retention limit is full, retain only the bounded deterministic prefix, mapping-validate and count all later diagnostics with checked arithmetic, and expose an exact total plus truncation state.
  - Initial M3 CLI retention fixtures always supply exactly `1,000`; expose no command option, config field, environment variable, reporter override, target override, or worker-derived override; and still parse every export-validation record to compute the exact total.
  ...
  - Any ordinary parser or semantic diagnostic from either role in the export validation set yields one integration-owned fail-complete result with zero exporter invocations and no asset, generated source, blob, loader map, manifest, or partial registration.
...
- Scope-mapping fixtures accept an empty table and exact sorted one-hop mappings; accept distinct structural sources mapping many-to-one to one declared validated target; and leave every unmapped scope unchanged.
  ...
  - Post-mapping policy fixtures group roots by resolved `(scope, domain, selector)` and M1 baselines by resolved scope. Equal root reasons and equal baseline locales collapse to one canonical semantic entry; unequal root reasons or baseline locales return `message_link_failed` / `invalid_request` with no outcome.
    - Resolved-domain inconsistency precedes these collision passes. The M0 root pass precedes the appended M1 baseline pass, and each pass selects its smallest canonical resolved identity independently of declared-scope, mapping, configuration, hash-map, or worker order.
    - Merge fixtures retain every pre-mapping policy and mapping occurrence in limit accounting and link/cache identity, perform one reachability or model operation for the merged semantic subject, and prove that the built-in empty mapping table changes no raw configuration duplicate rule.
...
- **Artifact production:** produce one complete artifact for every selected physical-source group and retain stable raw-entry ordering.
  - Accept a valid source with zero entries.
  - Produce no artifact for a group after its source-level extraction or projection failure.
  - Construct no definition artifact for any candidate group after a configuration-derived assignment or binding failure, or after either ordered gate in the project-global domain admission stage fails.
  - A physical-source edit invalidates only that source group's artifact.
...
- Rust finding-union fixtures admit exactly the seven `LinkFindingRecord` variants and the two closed `DegradedAnalysisFinding` variants, derive one matching `LinkFindingKind`, blocking value, and `ResolvedCatalogScopeId` from every record, and expose only read-only record, resolved-scope, and per-kind subject/evidence access.
...
- Linker finding goldens exercise M0 directly without linking `intlify_lint`; later L0/L1 adapter fixtures prove one-to-one kind, subject, evidence, ordering, presentation location roles, and operational-error mapping into the 008/013 surfaces. They cover every finding variant, both present and absent optional reference origins, the canonical ambiguity primary/related split without collider loss, the selected-definition-plus-reference roles for `missing-translation`, and the always-unanchored `partial-completeness` variant. The `missing-translation` codec fixtures require its retained optional origin to equal the selected reference origin exactly and prove that presentation performs no reference-artifact lookup. They reject a promoted related location, adapter-specific primary selection, reordered related locations, a fabricated source anchor, and more than one counted diagnostic/workspace finding for one `LinkFinding`.
...
- Benchmark fixtures exercise every milestone-required tuple from the exact active `benchmark-profile.mjs`, including the exact `all_definitions_baseline` / `linked_output` artifact-size variants, through `tools/messages-bench`; build, smoke execution, schema validity, the exact active non-serialized interval-boundary, allowed-overlap, and checksum-observation registries, within-invocation duration-checksum equality, valid product payload fingerprints, same-invocation exporter/comparison observation equality, and bucket reconciliation are gated. They prove each required tuple appears exactly once; every active messages-owned phase/cost uses exactly its milestone-admitted `duration`, `peak_live_memory`, or `artifact_payload_size` token; every active interval phase/cost has exactly one unique boundary ID whose metric equals the profile and result; every active messages-owned interval pair uses a locally owned descriptor; every admitted foreign interval pair imports and re-exports its exact owner-provided descriptor unchanged and matches its metric; and every artifact-size pair has no interval descriptor or boundary ID. They reject every unknown, inactive, case-varied, hyphenated, phase-specific, generic, or payload-incompatible metric spelling, including `peak_rss` before its complete activation contract lands. Every active messages-owned duration phase/cost has exactly one unique codec ID and checked observation codec; metric kinds that forbid a generic checksum have none. Additional observational cases cannot satisfy a required tuple, and the validator never loads a checked-in or previous result as an expected checksum, fingerprint, timing, memory, or size baseline; cross-invocation differences and elapsed-time, memory, and size values remain observational. Result fixtures reject any serialized boundary registry, overlap topology, observation registry, or codec descriptor.
  - Boundary-conformance fixtures use an injected monotonic clock, stage-marker recorder, and counting allocator to require the exact prepared input, occurrence policy, complete planned occurrence-identity sequence, first/final measurement markers, included/excluded marker order, non-overlapping `message_link_core` sibling intervals, topology-declared E2E/component overlap, and M0 input-I/O separation. They require the overlap topology to contain only active IDs in canonical acyclic order with no self-edge or duplicate edge. They reject a missing, duplicate, extra, unknown, or reordered occurrence identity or ordinal; a missing or duplicate marker; same-ID reentry; undeclared nesting; overlapping siblings; crossing intervals; a reordered or unmatched stop; leaked setup; prefix success; or wrong-metric observation. Repeated-duration fixtures sum only the complete non-overlapping occurrence intervals, keep per-iteration structural counts unmultiplied, construct the checksum observation from the canonical counted occurrence sequence, and reject any difference across measured repetitions. The imported 013 extraction boundary is accepted as a child only through its declared 014 workflow edge, follows its owner-provided group occurrence and aggregation policy, and remains unchanged. A result-schema/profile revision change is required whenever an accepted boundary, occurrence, aggregation, or overlap meaning changes without a phase/cost rename.
  - Default-feature product API and shipped-artifact fixtures prove that no benchmark outcome constructor, raw-plan constructor, or benchmark association provider is reachable. Dedicated-runner fixtures build non-published `tools/messages-bench/rs`, `intlify_linker`, and `intlify_export` with their exact `benchmark` features under the Cargo release profile; prove that `intlify_export/benchmark` forwards `intlify_linker/benchmark`; require the paired operation's linked member to equal an independent ordinary `link` result; require both members to use the same admitted request and effective limits; and reject enablement by product CLI/platform packages, blocked generation, baseline limit excess, arbitrary caller-authored plans, partial pairs, registration, and publication.
  ...
  - Artifact-size fixture-metadata tests require unique fixture names and positive fixture, profile, milestone/contract, and exporter revisions; retain the exact exporter ID/options, accepted kind/version contracts, entry roots, grouping mode, and scales; reject a result that references undeclared or mismatched metadata; and permit cross-invocation trend comparison only when `schemaVersion`, benchmark-profile revision, the complete declared fixture metadata, phase, cost, variant, scale, runtime, execution model, and operation are equal. Such a comparison never changes CI status. Fixture-owned source/configuration, measurement-rule, and exporter-classification changes update their respective revisions rather than introducing a second artifact/policy identity or digest.
  - Artifact-size result fixtures require the two exact exporter variants and one exact comparison variant; cover every difference direction, equal and unequal zero, undefined zero-baseline ratio, exact unreduced ratio integers, union bucket insertion with a zero on the absent side, canonical axis/bucket order, and independent locale/unit/kind reconciliation. They require canonical path/kind/payload-length/product-fingerprint/relationship vectors on both exporter records, require exact equality with the two tagged vectors in the comparison record, resolve every target and recompute eager reachability plus every bucket from those vectors and associations, and reject heuristic joins, floating-point semantic ratios, clamping, unsigned underflow, malformed product fingerprints, unresolved entry roots or relationship targets, noncanonical relationships, and every missing, extra, duplicate, reordered, or inconsistent artifact, relationship, association, or bucket.
  - Messages result-schema fixtures require one exact 013-owned `resource_extract` / `host_parse_and_entry_extraction` record for every profile-declared qualifying E2E tuple; pair them through the checked one-to-one companion mapping; and require equality only for fixture, variant, scale, runtime, and execution-model fields. They reject a companion tuple reused by two E2E tuples or any mismatch in those five pairing fields. Separately, they validate each record's own phase, cost, operation, status, count payload, and checksum under its 014- or 013-owned profile and result contract; recompute the resource repetition checksum from the complete current-invocation resource sequence; compare it only with the imported resource record; never compare resource counts or checksum with the distinct E2E values; reject a missing/duplicate/nested extraction metric or measured E2E status after extraction failure; and preserve 013 ownership of aggregation semantics without a cross-product envelope or serialized relation field.
  - Ordinary duration fixtures complete warmups before timing, retain the positive measured iteration count and aggregate `elapsedMs`, require equal per-iteration structural counts and `u32` checksums, validate the serialized wrapping repetition aggregate against those measured values, and emit no raw sample vector or sample-set identity. Stateful fixtures establish and verify the same declared source/output snapshot before every warmup and measured repetition, keep reset and teardown outside timing and checksum input, distinguish initial write, equality skip, check equality/difference, and prune mutation variants, and fail without a measured prefix when reset cannot be established. Shared vectors cover the initial FNV-1a constants, typed framing, every active observation codec and nested checksum domain, canonical ordering, closed tokens, volatile-field exclusion, and wrapping overflow across JS and Rust where both participate. Peak-memory and peak-RSS fixtures reject a generic checksum; artifact-size fixtures use only product payload fingerprints and reconciliation; skipped fixtures retain only their reason; and an execution failure cannot become a measured record. Fresh-process fixtures always retain aggregate unadjusted timing and map native, source-wrapper, and installed-package routes to their exact 006 phases; explicit paired mode alone retains raw product/startup samples, alternates order, preserves signed observations, and omits the whole adjustment on every compatibility or sample-set mismatch without changing the ordinary aggregate result, case completeness, or CI threshold behavior.

### Field-level limit evidence

- Definition-identity field-limit fixtures accept an empty root `EntryStructuralPath` and every domain-valid empty root `CatalogKey`, accept each exact 64 MiB decoded boundary and the exact 256-token catalog-key boundary, and reject the first decoded byte or attempted token `257` independently under protocol and caller-selected lower limits.
  - They require the closed `EntryStructuralPathBytes`, `CatalogKeyBytes`, and `CatalogKeyTokens` variants with exact structured spellings, only `DefinitionArtifactGroup(source)`, and exactly `Exact(effective_limit + 1)` without a raw field or key, decoded token, `EntryReference`, definition/occurrence index, complete-length or token-count scan, or `ArithmeticOverflow`.
  - Precedence fixtures run complete non-interleaved raw-entry byte passes for `locale_bytes`, `catalog_scope_name_bytes`, `entry_structural_path_bytes`, and `catalog_key_bytes`, followed by the complete message pass, locale/scope grammar, and then the catalog-key structural parse/token phase. They require every earlier phase to win over an earlier-occurrence failure in a later phase and prove identical selection under JSON-member, input, duplicate-source, partition, and worker permutations.
  - Token fixtures cover every built-in domain, a zero-token valid root, lower limit zero, exact 256-token acceptance, first attempted token 257 rejection, malformed-segment precedence within the same canonical phase, and exhaustive checked projection from each comparable 013 domain.
...
- Definition-message limit fixtures accept empty messages and the exact 1 MiB per-message and 64 MiB per-source-total boundaries; reject the first per-message byte above and the first raw-entry addition whose exact checked sum exceeds the total; and apply independent zero and lower values.
  ...
  - Admission fixtures run this one raw-entry message pass only after the four fixed locale/catalog-scope-name/structural-path/catalog-key byte passes and before locale/scope grammar and the catalog-key structural-token phase. They check the per-message counter before adding that record to the total, let an earlier total overrun beat a later per-message overrun, and preserve the same result under decoder, cache, duplicate-source, partition, and worker permutations.
...
- Selector-limit evidence fixtures require the closed `SelectorPathBytes`, `SelectorPatternBytes`, and `SelectorPatternTokens` variants unchanged at ordinals 22 through 24, plus `CatalogKeyTokens` at ordinal 41, with the exact `selector_path_bytes`, `selector_pattern_bytes`, `selector_pattern_tokens`, and `catalog_key_tokens` spellings.
  ...
  - They reject `Request`, raw selector/key/prefix/pattern/token payloads, record ordinals, complete submitted lengths or token counts, and `ArithmeticOverflow`.
  - Decoder, direct-construction, producer, cache, partitioned, and parallel fixtures select identical evidence; `Exact` and `Prefix` independently pass both `SelectorPathBytes` and `CatalogKeyTokens`, while `Pattern` passes `SelectorPatternBytes` before parsing and `SelectorPatternTokens` while admitting parsed tokens.
  ...
  - Cross-record precedence fixtures require the complete scope-name pass, then the eight fixed admission phases: selector-path bytes, pattern bytes, pattern parse/tokens, reason bytes, origin path count, origin segment bytes, origin path bytes, and finally `Exact` / `Prefix` structural parse/tokens, each in its documented canonical order.
  - They prove that the first structural parse error or token overrun in either parsing phase wins within that phase without fabricating a token count or deferring the parse error to search later records, and that worker completion cannot change the selected phase or failure.

## Relationship to Other Documents

| Document | Relationship |
| --- | --- |
| [013-ox-mf2-resource-catalog-adapter-design.md](./013-ox-mf2-resource-catalog-adapter-design.md) | definition side: complete per-document extraction artifacts; CLI-owned physical-source grouping; host-owned pre-extraction group admission; one project-global domain admission stage whose catalog-domain gate precedes its recognizer/root-domain gate; checked post-extraction projection into one linker artifact per successful physical-source group; distinct resource/contract types without a cross-crate dependency; entry identity/spans; canonical catalog-key/domain production without selector matching; explicit project-local scope and locale binding; formatter value write-back; and the separate M5 structural-mutation boundary used by `prune` |
| [005-ox-mf2-phase-3-tooling-transport-design.md](./005-ox-mf2-phase-3-tooling-transport-design.md) | CLI project inventory, physical-file grouping and alias ordering, initial sequential execution, deterministic aggregation, and the deferred common worker-scheduler boundary reused by later linker/export orchestration |
...
| [008-ox-mf2-phase-3c-linter-design.md](./008-ox-mf2-phase-3c-linter-design.md) | later presentation adapter: linker findings ship as rules through its contracts and the project-level finding addendum, with the missing-`messages` activation error and static rule capability table owned there, without gating linker M0 |
| [003-ox-mf2-phase-2-binary-ast-snapshot-design.md](./003-ox-mf2-phase-2-binary-ast-snapshot-design.md) | versioned binary syntax-transport and format-evolution precedent; neither the M0 `MessagePayload` nor the M3 ESM representation uses a Binary AST snapshot |
| [009-ox-mf2-phase-3d-lsp-editor-design.md](./009-ox-mf2-phase-3d-lsp-editor-design.md) | editor surfacing; incremental open-world findings; E0 JS/TS catalog-key completion activation, cursor query, publication, and source-edit behavior |
| [012-ox-mf2-parser-semantic-validation-design.md](./012-ox-mf2-parser-semantic-validation-design.md) | parser-owned `SemanticModel` construction and `validate_semantics` contract reused by shared export preparation, including the retained distinction between `semantic_api_misuse` and staged `semantic_invariant_failed` |
| [ox-mf2-parse-artifact-cache.md](./ox-mf2-parse-artifact-cache.md) | syntax-only, source-identity-bound parse-cache precedent; a future export-validation cache may reuse its concurrency and bounded-residency principles but remains a separate `intlify_export`-owned cache with different keys, values, and lifetime |
| [appendix-ox-mf2-error-code.md](./appendix-ox-mf2-error-code.md) | registry and namespace index for public linker/export operational codes and their detail-schema ownership; 014 remains authoritative for the typed mappings and bounded evidence |
...
### Vue SFC Reference Producer Frontend

Vue SFC application-reference production is not part of M0. It is the first follow-up producer phase after the initial JS/TS producer.

Until that phase is promoted, `.vue` is an unsupported `producers.js` source profile. If an explicit include selects a `.vue` logical source, the complete physical source group derives `ProducerFailed`; the producer does not silently skip it, scan familiar-looking JavaScript text, infer support from an editor language ID, or borrow support from 013's independent Vue SFC resource-catalog adapter.

The follow-up extends the existing `intlify_producer_js` implementation family and retains producer ID `dev.intlify/js-reference`. Adding the Vue frontend is output-affecting and therefore requires a new immutable `ProducerRevision`. Its implementation, contract text, example configuration, schema/help presentation, cache provenance, and conformance fixtures must be promoted together before `.vue` can appear in an advertised supported include set.

The promotion also extends the closed producer-failure vocabulary atomically. The planned `profile` additions are `unsupported_vue_script_lang` and `unsupported_vue_template_lang`. The planned `source_map` stage admits only `host_span_unavailable`, `host_span_ambiguous`, and `host_span_unrepresentable`, with `span` permitted for that stage. Before promotion, none of those reasons and no `source_map` stage are valid M0 evidence; a selected `.vue` source fails earlier as `profile` / `unsupported_source_suffix`.

The promoted contract must define the exact boundary between the three mapping reasons, their safe-span availability, and their deterministic precedence before admitting them. An invalid direct JS/TS parser span, a broken checked map, or another impossible validated state remains `internal_error` / `message_producer_invariant_failed`; the frontend cannot use a new input-facing reason to hide an invariant.

#### SFC participant and source profile

One included `.vue` source becomes one producer participant. The frontend parses the complete SFC, then scans both inline `<script>` / `<script setup>` blocks and every JavaScript expression exposed by the standard `<template>` AST, including interpolations, directive values and dynamic arguments, event handlers, and slot expressions. Both script blocks participate when the SFC validly contains both; reference-record order follows exact host-source start position with the ordinary record tie-breakers.

An absent script `lang` means JavaScript. The admitted script tokens are exactly `js`, `jsx`, `ts`, and `tsx`. Every admitted inline script block uses module goal without an unambiguous/script retry. The template profile accepts only standard template syntax with no custom `lang`; Pug and every other preprocessor-owned template language remain unsupported.

An SFC syntax failure, unsupported script or template language, embedded JS/TS parse failure, or inability to construct the required exact host-span mapping fails the complete producer operation and derives `ProducerFailed`. The zero-syntax-error success rule applies independently to the SFC parse, every admitted script block, and every standard-template embedded expression. A recovery AST or partially decoded template AST is discarded, and no previously scanned block or expression contributes records or a cache value after any component fails.
...
#### Host-source mapping and editor behavior

Vue SFC parsing and every embedded-expression mapping use the complete exact UTF-8 host snapshot fixed by the M0 snapshot contract. A BOM consumed by the SFC frontend, entity decoding, parser-local UTF-16 coordinates, or any temporary embedded buffer must be mapped through one checked monotonic map to the original `.vue` byte coordinates before a record can be emitted.

Every locally produced `SourceOrigin` addresses the first argument expression in the original host source. The frontend never reports block-local offsets, splits a source scalar or decoded template escape, guesses a location after preprocessing, or drops origin merely to retain a reference.

Disk and authoritative open-buffer paths use the same complete SFC profile. The editor does not scan only the currently selected block or tolerate an invalid unsaved block by retaining references from a preceding version; a producer failure follows 009's ordinary generation and publication gates without stale-disk or stale-artifact fallback.

#### Reference semantics and catalog independence

The promoted frontend applies the existing configured callee, call-kind, scope, domain, key-syntax, static-evaluation, selector, reason, ordering, artifact identity, and source-limit contracts to embedded call expressions. Declarative template usage that is not a call expression — for example an `<i18n-t keypath="...">` prop — remains outside this frontend and follows the separate framework-declarative follow-up below. Until that later contract is promoted, projects represent such references with intentional configured roots or a complete external artifact.

013's Vue SFC `<i18n>` adapter extracts definitions and does not imply application-reference support. Once this frontend ships, one physical Vue source may independently participate once in the definition inventory and once in the reference inventory. Neither side suppresses the other; immutable snapshot reuse is only an orchestration optimization when both sides prove that they selected the same bytes.

#### Promotion validation

The follow-up must add conformance fixtures that:

- admit exact lowercase `.vue` only after promotion and keep unsupported, extensionless, and case-varied selected sources on the complete-source `ProducerFailed` path;
- scan both valid inline script forms and every standard-template embedded expression in canonical host-source order;
- accept only absent, `js`, `jsx`, `ts`, and `tsx` script language tokens, use fixed module goal for every script block, and reject every custom template language;
- map parser-local and embedded coordinates through BOM and template decoding to exact original `.vue` UTF-8 spans, with no normalized, block-local, UTF-16, or decoded-expression coordinate escaping as `SourceOrigin`;
- fail the complete source for SFC, language, embedded-parse, or mapping failure without a partial artifact or cache value, while admitting the two planned Vue profile reasons and the three planned `source_map` reasons only after their exact classification and precedence contract lands;
- never follow `<script src>` or scan style/custom blocks, while allowing the referenced external script to participate through its own independent include match and identity;
- preserve producer ID `dev.intlify/js-reference`, require a changed producer revision, and apply the ordinary source partition, alias grouping, cache, limit, and origin invariants; and
- exercise equal disk/open-buffer semantics, dual definition/reference participation, and the continued exclusion of declarative non-call APIs.
...
The initial export-preparation implementation retains no process-global, command-external, persistent, or cross-call parser/semantic validation cache. Every `prepare_export` call validates its complete delivery-plus-baseline export validation set.
...
A promoted cross-call cache is a separate consumer cache owned by `intlify_export`. It does not reuse, wrap, or extend the `ParseCache` described in [ox-mf2-parse-artifact-cache.md](./ox-mf2-parse-artifact-cache.md).

That existing syntax cache keys an entry by dictionary/source namespace, message id, parser revision and options, and exact source bytes. Its value owns the `SourceStore` that gives its `SourceId` values meaning. Export validation instead needs location-independent parse and semantic results for one exact `MessagePayload`, followed by deterministic remapping of relative diagnostics to every independently retained `DefinitionLocation`.

Sharing the existing key or stored value would make export reuse depend on a catalog location and parser-owned source identity that are not part of payload equivalence. The export cache may reuse established design principles—revision-complete keys, exact-byte collision confirmation, single-flight production, bounded capacity, explicit invalidation, and concurrency-safe immutable values—but it shares no key type, entry storage, capacity budget, eviction state, or lifecycle with `ParseCache`.
...
- `intlify_export` ownership, observability, metrics, persistence and trust boundaries, without borrowing `ParseCache` capacity or residency; and
...
### JS/TS Typed-Key Completion Projection

M1 exposes language-neutral canonical typed-key models but does not ship editor completion. The first JS/TS application-source completion feature is 009-owned E0. E0 depends on M1 but not on L0/L1, M2, or M3, and follows 009's activation, cursor, publication, and source-edit contract.

#### Recovery-aware cursor-query boundary

`intlify_producer_js` owns the workspace-internal synchronous cursor-query API used by E0. Its conceptual boundary is:

```rust
fn query_completion_context(
    source: &str,
    logical_path: &PortableRelativePath,
    cursor: u32,
    config: &ResolvedJsProducerConfig,
) -> Result<Option<JsCompletionContext>, JsCompletionQueryInvariantError>;
```

The exact Rust names and field layout are internal, but the boundary responsibilities are fixed. `source` is the complete exact editor snapshot, including any leading BOM scalar; `logical_path` is its checked normalized project-relative identity; `cursor` is a UTF-8 byte offset in that exact source; and `config` is the immutable resolved `producers.js` include and recognizer configuration already installed for the root. The query repeats the applicable path/profile, cursor-boundary, source-size, and frontend-work admission checks defensively rather than trusting an editor-constructed parser mode.

One successful context contains only:

- the exact admitted JS/TS source profile;
- the exact configured `kind: "lookup"` recognizer identity and its checked scope, domain, and `keySyntax`;
- string-literal or no-substitution-template container kind and its existing delimiter;
- the complete checked half-open UTF-8 content span between those delimiters; and
- enough opaque frontend identity to prove that every admitted recovery result selected the same call, binding, container, delimiter, and content span.

It exposes no AST, recovery diagnostic, parser-owned node, mutable recognizer, application reference, artifact ordinal, candidate key, JavaScript source edit, or LSP type. The editor owns prefix decoding, model selection, candidate filtering, JavaScript escaping, `TextEdit` construction, request freshness, and protocol results.

The strict artifact producer and this query share one internal source-profile table, suffix selection, BOM/source-coordinate mapping, parser grammar configuration, callee-chain grammar, recognizer normalization, and applicable frontend limits. They deliberately use different acceptance policies over parser output: artifact production accepts only a complete parse with zero syntax errors, while the cursor query may inspect recovery output under 009's bounded local-proof rule. Neither path calls the other, and the recovery query can never populate or read the `MessageReferenceArtifact` cache.

Every input-facing inability to prove one safe context returns `Ok(None)`. This includes a path outside `producers.js.include`, an unsupported suffix, an out-of-bounds or split-scalar cursor, an exceeded applicable source/frontend limit, no matching lookup call, a dynamic or substituted argument, a missing delimiter, a syntax error intersecting the required local structure, and disagreement between recovery results. It emits no partial context and exposes no dependency diagnostic text.

`Err(JsCompletionQueryInvariantError)` is reserved for an impossible state after checked frontend construction, such as a returned range outside the supplied source or disagreement between supposedly identical normalized binding identities. It produces no context, artifact, cache value, or completeness change. The 009 adapter treats it as a request-scoped internal operational failure and publishes no document diagnostic; a concrete LSP product owns any protocol-level error mapping rather than allocating an artifact or linker error code.

The API is reentrant, owns all mutable parser/recovery scratch state within one call, and is safe for concurrent calls over independent or shared immutable source/config inputs. It creates no worker, executor, global mutable parser session, or cross-call cache. No N-API or WASM export is implied; the initial project-backed E0 implementation remains the Rust-host target fixed by 005.

Promotion fixtures in `intlify_producer_js` must prove exact agreement with the strict producer for suffix/profile selection, BOM coordinates, callee grammar, recognizer identity, and source limits while separately covering the recovery-only acceptance matrix. They must also prove `Ok(None)` for every ordinary negative case, deterministic agreement across module/script recovery results, checked content spans for every delimiter, invariant rejection of impossible frontend output, reentrant concurrent equivalence, and absence of artifact/cache/completeness side effects.

That feature does not teach `intlify_linker` about JavaScript spelling. `intlify_producer_js` owns one checked reverse projection from an admitted canonical model key to the runtime string accepted by an exact configured `kind: "lookup"` recognizer. Its input includes the binding's domain and `keySyntax`; its result is either one decoded runtime string or a typed not-representable result. It does not return JavaScript source text, select a quote style, inspect editor state, or widen the model.

The initial reverse projection is fixed as follows:

- `canonical` returns every domain-valid canonical key unchanged.
- `literal` is admitted only for `json-pointer` and only when the canonical pointer contains exactly one structural segment. It JSON-Pointer-decodes that segment and returns it as the complete runtime string. `/` represents one empty segment; the empty root pointer has zero segments and is not representable.
- `dot-path` is admitted only for `json-pointer` and only when the canonical pointer contains one or more non-empty structural segments. It JSON-Pointer-decodes every segment, escapes `\` as `\\` and `.` as `\.`, and joins the encoded segments with `.`.

Every candidate must round-trip through the existing forward conversion for the same `Lookup` binding to the exact original `(domain, CatalogKey)`. An unsupported syntax/domain pair, invalid canonical key, `dot-path` empty segment, unrepresentable root key, or failed round trip is omitted rather than returned under another spelling. In particular, the producer never falls back to the canonical string for a `literal` or `dot-path` binding.

The projection remains independent of `coverageBaseline` selection, model construction, locale/fallback policy, and cursor discovery. The consumer first selects an existing M1 model and exact recognizer binding; absence of a model or binding means completion is unavailable before projection is called. Several recognizers may share one resolved scope because the exact cursor callee selects one binding and therefore one reverse syntax.

Completion is a definition-only linker capability. Cursor parsing selects a binding but does not require project-wide reference inventory or artifacts, so activating completion alone uses the capability-aware definition-only fast path. That request selects no reference participants, derives `references: Closed` over the empty selected set, and derives neither `ProducerOmitted` nor `OpenEditorWorld`.

Promotion requires conformance fixtures for all three syntaxes, the empty JSON-Pointer root versus one empty segment, escaped pointer tokens, dot and backslash escaping, literal `*` lookup segments, unsupported domains, non-representable keys, and exact forward/reverse round trips. 009 separately verifies ECMAScript literal/template encoding and cursor-scoped edit behavior.
...
M0's JS/TS recognizer consumes call expressions only. The deferred Vue SFC frontend above retains that same call-expression boundary. Component props, directives, macros, route metadata, JSX attributes, and other framework declarations do not acquire message semantics from familiar names such as `i18n-t`, `keypath`, or `v-t`.
