...
After that structural validation, project inventory inspects every declared selected file's physical identity through the 005 boundary. File-symlink and hard-link declarations that identify one regular file form one physical-source group. Exact project-relative logical paths are ordered segment-by-segment by UTF-8 bytes with the shorter equal prefix first; the first path is the host primary and the remaining paths are host aliases. A group is one project-global producer participant and is ordered among external-artifact groups by its primary path. Primary and aliases are inventory, error-selection, benchmark-observation, and editor-freshness evidence only; they are not inserted into, compared with, or used to rewrite the decoded artifact's embedded identity, producer, records, or fingerprint. The canonical primary alone supplies the path component of the separate consumer-side decode-cache identity fixed above; aliases never enter that key.
