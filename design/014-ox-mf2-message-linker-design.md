...
`benchmark-phases.mjs` exports one canonically ordered boundary descriptor for every active interval phase/cost pair. The interval metrics are duration, peak live memory, and a future separately supported peak RSS metric. Artifact-size phases have no measured interval, must have no boundary descriptor or `boundaryId`, and remain governed by their product-payload fingerprint and bucket-reconciliation contract below.

Each active registry entry contains one exact owner and one boundary descriptor. The descriptor contains the exact phase, cost, unique stable ASCII `boundaryId`, metric tag, first and final measurement-stage markers, and the ordered included and excluded stage-marker sets. A boundary ID is a semantic identifier and carries no `_vN` suffix. The result schema and benchmark-profile revisions are the only version authorities for its meaning, and one active registry never contains two revisions of the same phase/cost boundary. Duration markers delimit the monotonic-clock interval; memory markers delimit the allocator or process-sampling interval. Messages-owned descriptors are defined in this module. An admitted foreign pair uses the exact descriptor imported from its owning module; only the surrounding registry entry adds its owner. The runner and tests import the registry, but results continue to serialize only the familiar phase/cost table and metric records.
...
| `message_definition_project` / `pre_extraction_admission` | Canonical physical-group paths, resolved host format, scope, locale, domain observations, and selected-source checksum admitted before extraction. |
...
- Benchmark fixtures exercise every milestone-required tuple from the exact active `benchmark-profile.mjs`, including the exact `all_definitions_baseline` / `linked_output` artifact-size variants, through `tools/messages-bench`; build, smoke execution, schema validity, the exact active non-serialized interval-boundary, allowed-overlap, and checksum-observation registries, within-invocation duration-checksum equality, valid product payload fingerprints, same-invocation exporter/comparison observation equality, and bucket reconciliation are gated. They prove each required tuple appears exactly once, every active interval phase/cost has exactly one unique boundary ID and metric-compatible descriptor, every active messages-owned interval pair uses a locally owned descriptor, every admitted foreign interval pair imports and re-exports its exact owner-provided descriptor unchanged, and every artifact-size pair has no interval descriptor or boundary ID. Every active messages-owned duration phase/cost has exactly one unique codec ID and checked observation codec; metric kinds that forbid a generic checksum have none. Additional observational cases cannot satisfy a required tuple, and the validator never loads a checked-in or previous result as an expected checksum, fingerprint, timing, memory, or size baseline; cross-invocation differences and elapsed-time, memory, and size values remain observational. Result fixtures reject any serialized boundary registry, overlap topology, observation registry, or codec descriptor.
  - Boundary-conformance fixtures use an injected monotonic clock, stage-marker recorder, and counting allocator to require the exact prepared input, first/final measurement markers, included/excluded marker order, non-overlapping `message_link_core` sibling intervals, topology-declared E2E/component overlap, and M0 input-I/O separation. They require the overlap topology to contain only active IDs in canonical acyclic order with no self-edge or duplicate edge. They reject a missing or duplicate marker, same-ID reentry, undeclared nesting, overlapping siblings, crossing intervals, a reordered or unmatched stop, leaked setup, prefix success, or wrong-metric observation. The imported 013 extraction boundary is accepted as a child only through its declared 014 workflow edge and remains unchanged. A result-schema/profile revision change is required whenever an accepted boundary or overlap meaning changes without a phase/cost rename.
