...
`tools/messages-bench/benchmark-phases.mjs` owns the closed phase/cost table for every active message-linker milestone, `tools/messages-bench/fixture-selection.json` owns the versioned fixture/profile inputs and declared scales, `tools/messages-bench/benchmark-profile.mjs` owns the active required-case matrix, `tools/messages-bench/checksum-observations.mjs` owns the messages-specific duration observation table, and `tools/messages-bench/result-schema.mjs` owns the tool's versioned result shape. The phase and observation tables emitted in a result must equal the active tables exactly. A new phase, cost, fixture meaning, measurement rule, checksum observation, or required case becomes valid only when this design activates it and the owning files, fixtures, revisions, and schema tests are updated together.
...
Every `message_project_link_e2e` record that plans Tier 1 catalog extraction requires exactly one ordinary result record using the exact 013-owned `resource_extract` phase and `host_parse_and_entry_extraction` cost. The messages result-schema validator pairs the two records through equal fixture, variant, scale, runtime, and execution-model fields. The aggregate record uses operation `complete_workflow`; the extraction record uses operation `host_parse_and_entry_extraction`.

013 exclusively owns the extraction record's physical-group order, interval boundary, counters, successful aggregation, failure behavior, and checksum inputs. The initial record therefore covers the aggregate of the selected case's sequential JSON physical-source-group extraction calls, while the end-to-end record retains the complete workflow wall time. A qualifying measured E2E record with no extraction record, more than one matching extraction record, mismatched pairing fields, or failed extraction is an invalid messages benchmark result and fails its schema/integrity gate.

The harness does not nest the metric, rename its phase to `message_resource_extract`, fold it into `message_definition_project`, or introduce a duplicate 014-owned extraction vocabulary. `tools/messages-bench/benchmark-phases.mjs` admits that one imported phase/cost pair with owner 013 rather than redefining it. A later mixed-format or concurrent case must first extend the 013 measurement contract.
...
A milestone cannot activate a duration phase/cost until its table entry, canonical typed observation, JS/Rust shared vectors where both implementations participate, and result-schema fixtures land together. Changing an observation's included fields, order, normalized tags, framing, numeric routine, or repetition aggregation requires the messages result-schema revision and, when existing comparisons change meaning, the benchmark-profile revision to advance.
