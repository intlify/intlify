...
For project-backed editor diagnostics, 009 applies the open-document override before this read/decode boundary. One mapped current buffer, or several mapped buffers with byte-identical current UTF-8 contents, replaces the exact configured artifact source's disk bytes and, after complete successful bounded decode, is authoritative for that editor invocation. The editor captures every contributing URI/version for freshness. Invalid selected bytes are `ProducerFailed` without disk fallback; unequal claims or unresolved source ownership are `Partial(OpenEditorWorld)`; closing every override selects the then-current disk snapshot. The editor does not label a successfully decoded configured override `ExternalArtifactUnverified` or infer upstream-source freshness.
...
The top-level messages benchmark result follows the existing formatter/resource pattern. It declares `schemaVersion`, tool and package version, `generatedAt`, default iterations and warmup iterations, the exact active phase/cost table, benchmark-profile revision, fixture metadata, and the ordered result records. The emitted fixture metadata and profile revision must equal the active checked files. `generatedAt` is report context only and never participates in result matching or comparison.
...
| `message_project_input_io` / `external_artifact_read` | `project_external_artifact_read` | Canonically ordered checked external-artifact declarations | Start before the first safe artifact open. Include no-follow capability checks attached to that open, metadata validation, and complete exact byte reads in declaration identity order. Stop when all artifact-byte snapshots are retained, before BLAKE3 cache-digest construction, contract decoding, or linking. |
...
| `message_project_input_io` / `external_artifact_read` | Canonical configured artifact path, exact byte length, and the codec-owned domain-separated checksum of the complete artifact-file bytes for every declaration in canonical order. |
