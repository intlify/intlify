...
3. After the complete definition extraction attempt, the host aggregates resource-owned domain observations. It first rejects a catalog scope containing multiple observed domains, then compares each single observed domain with the configured recognizer/root declarations. Either failure is `config_validation_failed`, constructs no `MessageDefinitionArtifact`, and starts no reference-source scan.
4. Only after both domain gates succeed does the post-extraction stage consume each successful extraction and its admitted group, perform checked resource-to-contract conversion, and construct one `MessageDefinitionArtifact`.
...
After every group passes admission, the CLI performs one complete extraction per group and retains each successful resource artifact. It constructs no definition artifact until the complete extraction attempt has supplied the project-global domain observations and both catalog and recognizer/root domain gates have succeeded. Post-extraction projection then emits one definition artifact per successful group whose entries use the primary source identity. Input enumeration order, filesystem enumeration order, symlink traversal order, command operand order, and future worker completion cannot affect the gate or artifact.
...
A later project-level finding addendum may promote individual rules only after measuring scan cost, cache behavior, project-scope operand semantics, completeness UX, and false positives.
...
host-owned definition production (initially an intlify_cli project-inventory module)
  ...
  ├─ project-global domain gate: aggregates successful resource-owned domains,
  │  then validates catalog and recognizer/root compatibility
  └─ post-extraction: only after the gate, checks resource-to-contract conversions
...
- **Artifact production:** produce one complete artifact for every selected physical-source group and retain stable raw-entry ordering.
  ...
  - Construct no definition artifact for any candidate group after a configuration-derived assignment, binding, catalog-domain, or recognizer/root-domain failure.
