...
| Resolver Construction Root Package | Revisioned finite immutable construction root built into a conforming resolver implementation and pinned by identity, revision, complete fixed-role member map, and canonical digest in the Conformance Suite Index. Its digest transitively commits every member role and member digest. It is not a sixth caller-supplied Resolver Construction Input Set component and cannot be selected or overridden by configuration, an invocation, or an adapter |
...
Before reading caller-supplied construction authority, a conforming `intlify_config` implementation fixes exactly one built-in Resolver Construction Root Package. Revision `"0"` pins its identity, revision, complete checked-in fixed-role member map, canonical digest, and every member path and digest in the Conformance Suite Index. The package closes the five authority-envelope shapes, supported JSON Schema dialects and exact meta-schemas used to validate schema artifacts, limits for construction bytes, nesting, members, schemas, registry rows, and validation work, singular construction-failure precedence, and every cross-authority relationship. It is the finite construction-validation root: it is not supplied as a sixth component, does not validate itself at runtime, and cannot be selected or overridden by configuration, an invocation, an adapter, or another authority body. An implementation with a different or unavailable root package cannot claim conformance to the pinned suite revision.

The normative revision-`"0"` root package is machine-readable, not an implementation-private interpretation of this prose. It contains exactly these fixed logical roles in order: `construction-admission-specification`, `construction-admission-schema`, `authority-envelope-schema-package`, `json-schema-meta-schema-package`, and `cross-authority-rules`. Its canonical package digest is SHA-256 over the exact ASCII domain separator `intlify.resolver-construction-root-package.v0` followed by the package identity, package revision, member count, and each fixed role plus its complete 32-byte canonical member digest in that order, using the typed length framing defined below for Resolver Construction Identity. The package digest therefore transitively commits every member role and complete member body without incorporating repository paths. The Suite Index separately pins the repository-relative logical path and canonical SHA-256 digest for every member. An implementation may compile those bodies into native static data, but the resulting logical bodies and digests must remain identical. A missing member, unavailable body, role disagreement, or digest disagreement is a conformance or implementation-startup failure and never a resolver construction result.
...
Only successful construction creates one immutable Resolver Construction Identity. Revision `"0"` hashes the exact ASCII domain separator `intlify.resolver-construction-identity.v0` followed by one typed canonical frame containing, in order: the Resolver Construction Root Package identity/revision/canonical-digest tuple; Profile Specification Schema tuple; Profile Specification identity/revision/semantic-digest tuple; Configuration Schema Authority Set tuple and Profile Specification association; every configuration mapping in ascending unsigned UTF-8 `schemaVersion` order with its key, schema identity, revision, dialect, and digest; Resolver Capability Specification tuple; Normative Resolver Registry Package tuple; and the six Registry member role/identity/revision/digest tuples in fixed role order. Individual Root Package member tuples are not repeated in this frame because the package digest already commits their fixed roles and complete member digests.
...
Every Common or Entry component crosses two distinct checks. Pre-invocation admission asks whether a complete value can safely and losslessly enter its formal typed Resolver Invocation Input slot and performs only the boundary-specific binding checks explicitly assigned by the closed pre-invocation rules below; those checks include Safe Origin Kind compatibility and the verification-input, implementation-capability, Snapshot-digest, and Resolver Construction Identity bindings listed in the closed boundary table. Failure is a typed integration result and the resolver is not invoked. After invocation, the owning phase performs every remaining identity, revision, digest, body-integrity, schema, semantic, resource-policy, and cross-input-compatibility check and represents failure as a resolver Finding. Pre-invocation admission never promotes a boundary binding into general resolver-semantic validation. In particular, a formally admitted canonicalization-data envelope may fail semantic admission in Phase 5, and a formally admitted Submitted Profile Resolution Artifact Collection retains every representable invalid, duplicate, conflicting, and unreferenced member for Phase 4 or 9 rather than filtering it at the adapter boundary.
...
- the built-in Resolver Construction Root Package and every fixed-role member, Profile Specification Schema, complete Profile Specification, Configuration Schema Authority Set and every mapped schema, Resolver Capability Specification, and every fixed-role Normative Resolver Registry Package member are materialized, body-valid, cross-reference-valid where applicable, and pinned by the Suite Index at their exact role or mapping key, identity, revision, dialect where applicable, and canonical digest;
...
- successful construction under the exact built-in Resolver Construction Root Package, including every package member, supported JSON Schema dialect and exact meta-schema body, construction-bound ID and value, authority-envelope schema, cross-authority rule, and failure-precedence rule; one exact Resolver Construction Identity covers the Root Package identity/revision/canonical-digest tuple, the Profile Specification Schema, Profile Specification identity/revision/semantic-digest tuple, complete Configuration Schema Authority Set mapping, Resolver Capability Specification, and six fixed Registry Package role tuples; vectors pin the Root Package digest preimage, complete RCI preimage frame, full 32-byte SHA-256 results, and canonical `rci0_` presentation;
- every closed missing, malformed, unsupported, identity-, revision-, dialect-, cross-reference-, body-schema-, digest-mismatch, and over-bound construction reason producing no resolver invocation, Finding, Evidence, dependency, or cacheable output; a missing or mismatched built-in Root Package or member remains a Conformance harness or implementation-startup failure rather than a construction `failureReason`; construction cases with several invalid submitted authorities select exactly one singular `failureReason` under Case Schema precedence, including Configuration Schema Authority Set member-key order and Registry Package logical-role order, independently of discovery, object-member, and worker order;
...
| 015-116 | Fix one revisioned, digest-pinned, non-overridable Resolver Construction Root Package as the built-in construction root and derive Resolver Construction Identity from its identity/revision/canonical-digest tuple plus the five submitted construction-authority components | Accepted | Caller-supplied authority cannot safely define its own envelope grammar, schema dialects, input limits, failure precedence, or cross-authority checks; a finite built-in root package closes construction without becoming a sixth submitted component and ensures every root-member change changes the construction identity | Purpose; Goals; Ownership and Dependencies; Terminology; Active Project Profile Specification and Profile Resolution Specification Input; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
