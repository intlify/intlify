...
| 015 performance measurement | Owns the profile-resolver-specific benchmark phase/cost vocabulary, exact component boundaries, workload dimensions, semantic checksum observations, and local regression evidence; resource-limit accounting remains normative resolver behavior and is never treated as a timing or memory measurement |
...
| 026 conformance and measurement | Owns common measurement metadata and units, cross-target comparison and reporting rules, target budget evidence, and cross-target logical-render equivalence; it consumes but does not redefine 015-owned profile-resolver benchmark boundaries |
...
015 owns the exact profile-resolver benchmark vocabulary and boundaries because only this design can identify where one resolver-owned operation begins and ends. 026 owns common measurement metadata and units, cross-target comparison and reporting rules, target budget evidence, and any Intlify-wide numeric regression policy. The absence of an admitted 026 revision does not block local 015 benchmark construction or implementation profiling; it blocks only claims that independently produced measurements are cross-target comparable or satisfy an Intlify-wide budget. When 026 becomes active, it maps the 015-owned records into its common model without renaming, merging, or reinterpreting the boundaries below.
...
The benchmark profile is not a sixth Resolver Conformance Suite manifest kind and does not enter the Suite Index authority closure. The generated 015 traceability report maps Decision 015-131 and every active benchmark phase/cost to the implementation-owned checked benchmark registry, required-case matrix, boundary tests, result-schema validation, checksum tests, and smoke command. This mapping is implementation-validation evidence, not a synthetic Resolver Case or Handoff Vector.
...
- common measurement metadata and units, cross-target result comparison, target budget evidence, any Intlify-wide numeric regression threshold, cross-target logical-render equivalence, and conformance evidence: 026; 015 retains the exact profile-resolver benchmark phase/cost vocabulary, boundaries, workload vectors, and semantic checksum observations;
...
| [026 — Conformance and measurement](./026-intlify-conformance-and-measurement-design.md) | Owns common measurement metadata and units, cross-target comparison and reporting rules, target budget evidence, any Intlify-wide numeric regression policy, cross-target logical-render equivalence, and capability/conformance evidence required by explicit hydration relations. It consumes but does not redefine 015-owned profile-resolver benchmark phase/cost boundaries, workload vectors, or semantic checksum observations. |
