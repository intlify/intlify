...
The shared Rust crate `intlify_config` owns the `IntlifyConfig` authoring model, configuration-schema version admission, JSON Schema definition and generation, named-profile selection, semantic resolution, configuration Findings, and the `LocalizationProjectProfile` settings IR. Each resolver instance is constructed from one complete Resolver Construction Input Set. That construction authority independently admits the Localization Project Profile Specification Schema, the Active Project Profile Specification, the Configuration Schema Authority Set, the Project Profile Resolver Capability Specification, and the complete Normative Resolver Registry Package before yielding one Resolver Construction Identity. It defines provider-driven locale-canonicalization, typed Profile Resolution Specification Input, explicit Intent Surface-Class Vocabulary Artifact, and submitted-artifact-collection boundaries but does not embed raw or generated CLDR data or acquire artifacts. `intlify_cli` is a product adapter: it discovers and reads repository configuration, normalizes the user-facing selector, supplies an admitted vocabulary artifact, canonicalization data artifact, Admitted Implementation Capability, and one finite already acquired Submitted Profile Resolution Artifact Collection through the Resolver Invocation Input Set, calls `intlify_config`, and renders the outcome. Optional programmatic frontends call the same core rather than depending on CLI internals.
...
| Optional programmatic frontend | Safely inspects a live host value into one complete inert Programmatic Entry Snapshot plus its exact programmatic-source identity/revision and optional canonical locator inputs, safely normalizes the selector into the fixed External Profile Selector Input, maps configuration and selector entry to the fixed programmatic origin kinds, and enters the shared materialization and `IntlifyConfig` admission path; exact API naming, packaging, and language bindings remain 029-owned |
...
| Configuration Schema Authority Set | Supplies the closed mapping from each admitted root `schemaVersion` to one exact configuration-schema identity, revision, JSON Schema dialect, complete schema body, and canonical digest; it is construction authority, not profile semantics |
...
| 029 product workflow | Owns file discovery, workspace selection, user-facing selector input, adapter-specific exact logical source identity/revision and stable versioned identity-domain construction, adapter-side Portable Source Locator canonicalization including any IDNA-to-A-label conversion, commands, schema packaging, optional helper API UX, Intent Surface-Class Vocabulary, canonicalization-data, and Profile Resolution Artifact acquisition and caching, and product packaging without defining alternate configuration or profile-selection semantics |
...
| Configuration Schema Authority Set | Finite immutable closed mapping from each admitted configuration `schemaVersion` string to one configuration-schema identity, exact revision, JSON Schema dialect, complete schema body, and canonical digest; root `schemaVersion` can select only one member of this constructed set |
...
| Provisional Selector Observation | Bounded internal bootstrap state derived from the External Profile Selector Input and the applicable Admitted Implementation Capability `bootstrapOnly` bound; it supports provisional selection but is never retained as Evidence, tokenized, assigned a Finding Occurrence Key, or exposed as resolver output |
...
| Admitted Implementation Capability | Complete finite immutable invocation capability value normalized by a binding or adapter against the constructed Project Profile Resolver Capability Specification before resolver invocation. Its `policyComparable` bounds are exactly the Resource Limit Policy-comparable namespaces, while `bootstrapOnly` bounds cover raw entry, snapshot, artifact-bootstrap, Evidence, and mandatory-reserve work and are never compared with semantic policy |
...
| Resolver Construction Identity | Compact canonical identity of one successfully constructed revision-specific resolver, derived from the canonically framed identity/revision/digest tuples and closed mapping keys of every Resolver Construction Input Set component. It includes the Profile Specification semantic digest but identifies rather than replaces the complete construction authority |
...
| Resolver Invocation Input Set | Finite immutable values submitted for one invocation: one complete applicable file or programmatic Entry Admission Input Set, the complete common Profile Resolution Specification, vocabulary, canonicalization, artifact-collection, and capability inputs with their Safe Origin Kinds, and a reference to the exact Resolver Construction Identity. It contains every exact source, parser or frontend, selector, and digest value required by the applicable entry path; no harness or resolver default completes it |
...
| Entry Admission Input Set | Invocation inputs required to reproduce resolver-bootstrap, configuration-entry, selector-origin, and occurrence evidence and, together with the matching Resolver Construction Input Set, by itself before materialization or with the Materialized Resolution Input Set afterward, the complete Entry Resolution Evidence and Finding Occurrence Keys. Common inputs are a Resolver Construction Identity reference; the complete Profile Resolution Specification Input and fixed origin; the External Profile Selector Input and fixed per-entry selector origin; the Intent Surface-Class Vocabulary Artifact, Locale Canonicalization Data Artifact, Submitted Profile Resolution Artifact Collection, and exact Safe Origin Kinds; and Admitted Implementation Capability. File entry additionally includes raw configuration bytes plus digest, parser specification, exact logical source identity, Source-content revision, fixed `configuration-file` origin, optional locator candidate, and selector origin `profile-selector-cli`. Programmatic entry additionally includes the complete Programmatic Entry Snapshot plus canonical snapshot-content digest, frontend identity/version, programmatic-entry rule revision, exact logical source identity, Source-content revision, fixed `configuration-programmatic` origin, optional locator candidate, optional exact immutable UTF-8 call-site source snapshot, and selector origin `profile-selector-programmatic`; the live host value is never part of this set |
| Materialized Resolution Input Set | Finite immutable replay snapshot created only after entry materialization. It contains the matching Resolver Construction Identity reference, complete JSON-compatible configuration value, normalized External Profile Selector Input, selected configuration-schema identity/revision/digest, complete fixed-slot Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact, Locale Canonicalization Data Artifact, complete Submitted Profile Resolution Artifact Collection including multiplicity and unreferenced artifacts, and exact Admitted Implementation Capability. Together with the matching Resolver Construction Input Set it reproduces shared structural and semantic resolution; schema bodies remain construction authority and are not duplicated here. It contains no confirmed selector, admitted Resource Limit Policy, Profile Resolution Specification Set, Profile Resolution Artifact Set, Structural Analysis Result, Finding, or evaluation status and is not a second caller input |
...
Configuration `schemaVersion` selects authoring admission only through the constructed Configuration Schema Authority Set and belongs to the Materialized Resolution Input Set together with the selected schema identity, revision, and digest. It is not a field of the canonical profile semantic projection. A future configuration version that resolves to the same Profile Specification revision and the same canonical semantic fields therefore produces an equal profile, although changing the configuration version always requires re-resolution.

The Configuration Schema Authority Set is finite, closed, and independent from the Normative Resolver Registry Package. Each mapping has the logical form:
...
- one successful Resolver Construction Case containing the exact Profile Specification Schema, Profile Specification, Configuration Schema Authority Set with its `"0"` schema, Resolver Capability Specification, and six-role Registry Package used by the fixture;
...
| 8 | `implementation-capability` | `implementation-capability` | Explicit Admitted Implementation Capability input |
...
2. the Admitted Implementation Capability's `bootstrapOnly` selector bound when the Policy cannot be selected, admitted, or proved capability-compatible.
...
- `implementation-capability` permits the empty root or a sequence of `fixed-role` segments beginning with zero-ranked `policyComparable` or one-ranked `bootstrapOnly`. Under `policyComparable`, group and bound roles are exactly the Resource Limit Policy-comparable namespaces declared by the constructed Capability Specification. Under `bootstrapOnly`, group and bound roles are the closed raw-input, snapshot, artifact-bootstrap, Evidence-metadata, and mandatory-reserve namespaces declared by that specification. Each group and bound rank is its canonical Capability Specification order; an implementation cannot derive rank from map iteration or reuse a policy path for a bootstrap-only bound.
...
Before an invocation can begin, `intlify_config` constructs a revision-specific resolver from exactly one finite immutable Resolver Construction Input Set. Its five independent construction-authority components are:
...
3. one finite closed Configuration Schema Authority Set, including the complete `schemaVersion` mapping and every mapped configuration-schema identity, exact revision, JSON Schema dialect, complete body, and canonical digest;
...
These are sibling construction authorities; the Profile Specification Schema and Configuration Schema Authority Set are not Registry Package members. Resolver construction first admits each authority's own envelope and digest, validates the complete Profile Specification body against the independently admitted Profile Specification Schema, validates every configuration schema against its declared dialect, validates the Capability Specification's closed vocabulary and minimum vector, validates each Registry Package member against its role schema, and finally validates every cross-authority reference. The Profile Specification digest is computed from the canonical semantic representation of every behavior-affecting field; transport formatting and member order with no specified meaning are excluded. Identity and digest references identify authority members but cannot reproduce their contents.

Only successful construction creates one immutable Resolver Construction Identity from the canonically framed authority tuples: the Profile Specification Schema tuple; Profile Specification identity/revision/semantic-digest tuple; Configuration Schema Authority Set mapping plus every selected-key/schema identity/revision/dialect/digest tuple; Resolver Capability Specification tuple; and the six Registry Package logical-role/identity/revision/digest tuples in fixed role order. Two byte-different bodies with the same applicable canonical semantic projection may retain a digest, but any behavior-affecting difference changes the applicable digest and therefore the construction identity even when a publisher incorrectly reuses a revision. The identity names the authority used by invocation, dependency, and cache records but never substitutes for the complete construction inputs. Construction failure is an implementation or toolchain-admission error with no resolver invocation, `project-profile-*` Finding, Resolver Construction Identity, or partial Resolution Evidence.

Every invocation supplies one Resolver Invocation Input Set carrying the exact construction-identity reference plus its file bytes or Programmatic Entry Snapshot, External Profile Selector Input, Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact, Locale Canonicalization Data Artifact, Submitted Profile Resolution Artifact Collection, and Admitted Implementation Capability. The binding or adapter admits the construction-identity reference, programmatic snapshot envelope, Safe Origin Kind, selector, source identity/revision, vocabulary-artifact envelope, and concrete capability value under their applicable closed pre-invocation rules before the resolver starts. Capability admission asserts the constructed Capability Specification identity/revision, admits exactly its closed groups and bound IDs, and rejects a value below the specification's mandatory bootstrap-minimum vector. A missing, malformed, unbounded, below-minimum, or construction-identity-mismatched value at those boundaries is an integration failure with no resolver outcome, Finding, partial Evidence, dependency, or cache entry. Every subsequent phase and pre-policy Finding uses only the already constructed authority and admitted invocation values. A `project-profile` assertion can match or fail against that authority but can never reconstruct or replace it.
...
- materialized re-resolution dependency records scoped by the outcome's mandatory Resolver Construction Identity and carrying configuration value content, normalized selector input, selected configuration-schema identity/revision/digest, Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact, canonicalization data, Submitted Profile Resolution Artifact Collection, and Admitted Implementation Capability inputs whose dependency identity was established; derived admission results are recomputed and are not replay inputs.
...
The resolver keeps three inventories distinct. The Resolver Construction Input Set supplies schema, semantic, capability, and registry authority and is consumed only to construct the immutable resolver. The Resolver Invocation Input Set contains only boundary-admitted values for one run: the complete applicable file or programmatic Entry Admission Input Set; the selector, Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact, canonicalization data, Submitted Profile Resolution Artifact Collection, and Admitted Implementation Capability with their exact Safe Origin Kinds; and the matching Resolver Construction Identity reference. Derived Admission State contains resolver-created values such as the JSON-compatible configuration value, selected configuration-schema tuple, Structural Analysis Result, `IntlifyConfig`, confirmed selector, admitted Profile Resolution Specification Set, admitted Resource Limit Policy, and admitted Profile Resolution Artifact Set; none of those derived values is a second caller input.
...
1. Admitted Implementation Capability supplies its admitted `bootstrapOnly` limits for entry work, materialization, structural analysis, provisional Profile ID selection, independent Entry/Shared diagnostic retention, and bounded inspection of the submitted artifact collection;
2. the resolver provisionally selects exactly one declaration under those limits, scans the complete collection's envelope metadata, classifies candidates against the selected `resourceLimits` reference under the predicates and precedence below, and admits exactly one Resource Limit Policy candidate without creating a Profile Resolution Artifact Set;
...
The resolver scans the complete submitted envelope-metadata collection under bootstrap occurrence and byte bounds; inability to complete that scan prevents policy admission. It then applies this closed decision order without using submission order:
...
4. reject body-integrity failure for the sole exact candidate;
5. admit the sole body-valid exact candidate;
6. otherwise select `digest-mismatch` when the conflict group contains one valid non-exact digest;
7. select `specification-revision-mismatch` when a candidate matches `K + I + V` but differs in `S`;
8. select `revision-mismatch` when a candidate matches `K + I` but differs in `V`;
9. select `kind-mismatch` when a candidate matches `I` but differs in `K`;
10. select `identity-mismatch` when a candidate matches `K` but differs in `I`; or
11. select `not-found` when no related candidate exists.
...
| Sole exact candidate fails body integrity | `project-profile-resource-policy-not-admitted` | `body-integrity-invalid` |
| Sole exact candidate succeeds | none | none |
...
- `policyComparable` contains exactly the capability bounds whose namespaces and units correspond to the Resource Limit Policy's `projectProfileResolution` bounds. Only these values participate in Phase 4 policy compatibility.
- `bootstrapOnly` contains the capability bounds needed before semantic policy can exist, including safe raw-input inspection, Programmatic Entry Snapshot construction and envelope admission, artifact-metadata bootstrap, Evidence metadata, token and collision handling, and fixed mandatory-output reserves. These values are never compared with or copied into Resource Limit Policy semantics.
...
Raw file bytes and parser work are protected by the 018-owned bootstrap envelope rather than this semantic group. The same logical value and normalized selector input receive the same `configurationInput` accounting through file and programmatic entry paths. Successful policy admission and capability compatibility make the Policy bound final selector-projection authority before recheck; successful recheck confirms the provisional selector result without rerunning selection under a different declaration set. A selector recheck failure leaves the outcome blocked, replaces any provisionally safe observation with the final `over-limit` projection, and keeps the provisional result unavailable to consumers. When policy admission or compatibility fails, Admitted Implementation Capability remains final projection authority for the bounded blocked outcome.
...
2. `structural-admission` selects root `schemaVersion` from the constructed Configuration Schema Authority Set, retains the selected schema identity/revision/digest, performs schema-guided analysis of the closed input under that exact body and dialect, records per-fragment admission status and structural Findings, and produces `IntlifyConfig` only when the complete root succeeds;
3. `profile-selection` derives a Provisional Selector Observation and provisionally applies the normalized External Profile Selector Input under Admitted Implementation Capability when the `profiles` container, applicable Profile IDs, selected declaration boundary, and `resourceLimits` reference fragment are independently admitted, and otherwise records dependent work as not evaluated with its causal Finding Keys;
4. `resource-policy-admission` performs Bootstrap Resource Policy Candidate Admission for only the provisional declaration's exact resource-limit reference, scans the complete envelope-metadata collection under Admitted Implementation Capability, applies the exact/conflict predicates, Finding mapping, and closed mismatch precedence, admits exactly one Resource Limit Policy candidate without creating an Artifact Set, compares that Policy only with the already admitted capability's `policyComparable` bounds, fixes the final selector-projection authority, reconstructs selector-dependent records, reapplies `configurationInput` and `artifactAdmission` bounds to complete pre-policy inputs, recalculates each pre-policy diagnostic prefix from zero under the Policy, and confirms the provisional selection when policy/capability compatibility and every non-diagnostic protected-input recheck succeeds even if a diagnostic domain has already fixed an incomplete blocked result;
5. `specification-admission` admits the remaining seven fields of the Profile Resolution Specification Input, creates the complete Profile Resolution Specification Set, admits one Intent Surface-Class Vocabulary Artifact matching the complete identity/revision/digest assertion, and admits one Locale Canonicalization Data Artifact matching its `locale-canonicalization` member under the confirmed resource-policy envelope;
...
| `safe-origin-kind-registry` | Exactly the nine revision-`"0"` IDs, unique ranks, and permitted evidence-kind sets fixed above; the prose boundary descriptions are generated documentation rather than registry payload fields |
...
Resolution Evidence records these relationships without changing them. A semantic dependency record links one established canonical semantic fact or checked absence state to the exact admitted inputs used to derive it. Every outcome carries the Resolver Construction Identity as a mandatory fixed envelope field; materialized and entry re-resolution dependency records are scoped by that identity and never substitute for the complete Resolver Construction Input Set. A materialized re-resolution dependency additionally identifies configuration value content, normalized selector input, the selected configuration-schema identity/revision/digest, Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact, canonicalization data, Submitted Profile Resolution Artifact Collection, or Admitted Implementation Capability needed to decide whether shared resolution is stale. Admitted Resource Limit Policy, Profile Resolution Specification Set, Profile Resolution Artifact Set, confirmed selector, Structural Analysis Result, Findings, and evaluation status are recomputed Derived Admission State and are not materialized replay dependencies. An entry re-resolution dependency additionally identifies specification-input origin, the admitted exact logical source identity/Source-content revision pair, complete canonical Programmatic Entry Snapshot content plus its canonical snapshot-content digest when applicable, or selector origin needed to rerun resolver bootstrap or configuration entry and update presentation. The digest accelerates staleness comparison but never substitutes for the complete Snapshot during replay. Semantic dependencies support invalidation slicing; construction-identity changes schedule resolver reconstruction, while both re-resolution classes schedule their applicable invocation boundary. Missing exact identities or revisions are never replaced by paths, timestamps, object addresses, random invocation values, or adapter guesses, and an unevaluated dependency remains represented only through evaluation status until its identity can be established.
...
Staleness is tracked separately from equality and is evaluated at the construction boundary before either invocation boundary. A changed Resolver Construction Identity requires reconstruction from the complete new Resolver Construction Input Set and makes every prior resolver outcome stale. An equal identity permits reuse only when the matching complete construction set is still available and admitted; its digest tuples identify that set but cannot reproduce an omitted Profile Specification Schema, Profile Specification, Configuration Schema Authority Set member, Capability Specification, registry schema, or registry body. Any body or closed mapping that differs under the same identity/revision/digest tuple is construction-authority corruption and forbids reuse. Repackaging the same admitted bodies and mappings without changing their canonically framed keys and identity/revision/digest tuples is not a construction change.

Within one Resolver Construction Identity, the initial implementation treats the complete Profile Resolution Specification Input and origin, exact configuration logical source identity/Source-content revision pair, normalized selector origin, Admitted Implementation Capability, and complete canonical Programmatic Entry Snapshot content plus canonical snapshot-content digest when applicable as Entry re-resolution dependencies. A source revision change always schedules Entry re-resolution, while an equal exact pair permits source-level reuse only when the adapter has satisfied the byte-snapshot invariant; observing different bytes under an equal pair is an adapter conformance failure and forbids reuse. It treats the complete JSON-compatible configuration value content, selected configuration-schema identity/revision/digest, normalized External Profile Selector Input including absence, every exact field of the Profile Resolution Specification Input, the Intent Surface-Class Vocabulary Artifact, Locale Canonicalization Data Artifact, complete Submitted Profile Resolution Artifact Collection including occurrence multiplicity and every admitted/rejected envelope state, and exact Admitted Implementation Capability as materialized re-resolution dependencies. Resource Limit Policy admission and every other Derived Admission State are recomputed from those values. A change to either invocation class schedules its applicable entry or shared resolution boundary. Replacing only a typed-input or collection representation while preserving every exact field or occurrence value is not a distinct dependency value, while changing multiplicity is.
...
Resolver Construction Input Set
  ...
  + complete Configuration Schema Authority Set mapping and every
    schema identity/revision/dialect, complete body, and canonical digest
  + Project Profile Resolver Capability Specification
    identity/revision, complete body, and canonical digest
  + every fixed-role Normative Resolver Registry Package member's
    identity/revision, complete body, and canonical digest
  -> revision-specific resolver + Resolver Construction Identity
...
Entry Admission Input Set
  common:
    ...
    + Admitted Implementation Capability
  file:
    raw configuration bytes + byte digest + parser specification
    + exact logical source identity + Source-content revision
    + fixed configuration-file origin + locator candidate
    + External Profile Selector Input + profile-selector-cli origin
  programmatic:
    complete canonical Programmatic Entry Snapshot
    + canonical snapshot-content digest + frontend identity/version
    + exact programmatic-entry rule revision
    + exact logical source identity + Source-content revision
    + fixed configuration-programmatic origin + locator candidate
...
Materialized Resolution Input Set
  ...
  + selected configuration schema identity/revision/digest
...
The Resolver Construction Input Set reproduces the complete schema, semantic, capability, and registry authority used to construct the resolver; the Resolver Construction Identity alone only identifies that authority. Together with that matching construction set, the Entry Admission Input Set reproduces Profile Resolution Specification Input assertion admission, entry-specific Safe Origin Kinds, syntax, duplicate-member and JSON-compatibility behavior, selector origin, source-map and Logical input path projection, file-span admission, retained programmatic Portable Source Span evidence, resolver-bootstrap and configuration-entry Evidence, and entry-dependency behavior. The file bytes are also the source-span validation snapshot. A missing or rejected path candidate has already normalized to Logical input path absence at this boundary, while a present empty path remains the distinct input-root location. A programmatic call-site snapshot is present only when an admitted span is retained and is excluded from Resolution Evidence after bounded validation; a rejected or unprovable span candidate has already normalized to span absence at this boundary. The complete canonical Programmatic Entry Snapshot, its canonical snapshot-content digest, frontend identity/version, and closed entry rules reproduce every safely classified invalid host condition without retaining the originating live host value. The digest alone is insufficient for replay. If configuration entry is blocked before a complete JSON-compatible value exists, the construction and Entry sets reproduce the complete blocked outcome, including its mandatory Resolver Construction Identity, complete Entry projection, `shared: null`, and shared-unavailability cause set; no Materialized Resolution Input Set exists for that invocation. After entry success, all three sets together reproduce the complete Entry projection because occurrences and source evidence for Shared Findings depend on both the shared check result and the entry path. The combined inputs reproduce the final Profile Selector Evidence Projection under its policy-or-bootstrap authority and every Resolver Input Component Path from the exact Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact, canonicalization-data input, Submitted Profile Resolution Artifact Collection, and Admitted Implementation Capability plus the pinned Finding Registry path grammar; acquisition and physical collection order are not additional inputs. The Materialized Resolution Input Set exists only after entry success and, together with the matching construction set, reproduces shared structural admission under the selected construction-owned configuration schema, semantic resolution, and Shared Resolution Evidence by recomputing the confirmed selector, Resource Limit Policy, specification and artifact Sets, Structural Analysis Result, Findings, and evaluation status rather than storing those Derived Admission State values as replay inputs.
...
| Project graph and query service | Canonical profile identity and semantic dependency facts | Complete binding set only for dependency storage and authorized downstream projection | Mandatory Resolver Construction Identity, corresponding complete Resolver Construction Input Set, Entry and Materialized Resolution Input Sets, Resolution Evidence, source revisions, cache state, and query request |
...
The project graph may index, invalidate, and locate construction authority by Resolver Construction Identity, but replay is possible only while the complete matching Resolver Construction Input Set is retained or can be supplied explicitly. The compact identity and its component digests never stand in for omitted Profile Specification, schema, or registry bodies. An ordinary dependency record omitted by reporting exhaustion cannot erase the mandatory outcome-level construction identity.
...
Revision `"0"` requires four machine-readable artifact families:

1. one Suite Index;
2. the Profile Specification Schema and Profile Specification, Configuration Schema Authority Set and all mapped schemas, and Resolver Capability Specification construction authorities;
3. closed Resolver Construction Case, Resolver Input Admission Case, Resolver Case, Handoff Vector, and Conditional Obligation manifests; and
4. the complete Normative Resolver Registry Package defined above.
...
Every listed ID occurs exactly once within and across all five manifest arrays, every referenced manifest passes its applicable closed schema, and every construction-authority tuple matches its canonical checked-in artifact and the corresponding Resolver Construction Input Set. The Configuration Schema Authority Set's `members` keys are admitted `schemaVersion` values in canonical string order; each value pins the full schema tuple including dialect. `registryPackageMembers` is a closed object with exactly the six fixed logical roles in the shown order. A role key is not an artifact identity, no unknown or duplicate logical role is permitted, and each role value must explicitly carry identity, revision, and digest. Array order is canonical ID order and duplicate IDs are invalid. A Suite Index that omits or mismatches the Profile Specification Schema, complete Profile Specification semantic digest, Configuration Schema Authority Set or mapped schema tuple, Resolver Capability Specification, Case Schema, or any registry-package role is not a revision-`"0"` index.
...
No `unknown`, `other`, implementation-defined, or free-form construction reason is admitted. Case Schema revision `"0"` pins one construction `failurePrecedence`: Profile Specification Schema; Profile Specification envelope and digest; Profile Specification body validation; Configuration Schema Authority Set envelope followed by `schemaVersion` key order and member checks; Resolver Capability Specification; Registry Package fixed logical-role order; then cross-authority checks. Within one member, envelope presence, closed shape, identity, revision, dialect when applicable, digest, body-schema validation, and cross-reference checks follow their Case-Schema order. If several failures are present, the case and implementation expose only the first reason in this order. Discovery order, object-member order, and worker completion cannot choose it.
...
| `configuration-source-identity-revision` | `missing-source-identity`, `missing-source-revision`, `over-bound-source-identity`, `over-bound-source-revision`, `unstable-source-identity`, `unbound-source-revision`, `source-snapshot-mismatch` |
| `programmatic-entry-snapshot` | `missing-snapshot`, `malformed-snapshot`, `snapshot-envelope-over-bound`, `unsafe-host-inspection`, `frontend-identity-missing`, `frontend-rule-revision-unsupported` |
| `implementation-capability` | `missing-capability`, `malformed-capability`, `capability-specification-identity-mismatch`, `capability-specification-revision-mismatch`, `incomplete-capability`, `non-positive-bound`, `unrepresentable-bound`, `below-bootstrap-minimum` |
...
Raw-envelope admission is distinct from resolver-semantic admission: for example, a structurally admitted vocabulary artifact with an assertion or semantic-digest mismatch is a Resolver Case, not an Input Admission Case. Likewise, a valid Programmatic Entry Snapshot containing several ordinary rejected-node or canonical `bootstrap-limit-exceeded` markers is admitted and produces fail-complete Resolver Findings; `snapshot-envelope-over-bound` applies only to the invalid formal representations defined above. This case type never emits a resolver outcome, Finding, Evidence, dependency record, token, or cache entry. A boundary failure that occurs after resolver invocation starts is instead represented by a Resolver Case.
...
```json
{
  ...
  "commonInvocationInputs": {
    "profileResolutionSpecificationInput": {
      "value": {},
      "safeOriginKind": "profile-resolution-specification-input"
    },
    "intentSurfaceClassVocabularyArtifact": {
      "value": {},
      "safeOriginKind": "intent-surface-class-vocabulary"
    },
    "canonicalizationData": {
      "value": {},
      "safeOriginKind": "locale-canonicalization-data"
    },
    "submittedProfileResolutionArtifactCollection": {
      "value": {},
      "safeOriginKind": "profile-resolution-artifact-collection"
    },
    "admittedImplementationCapability": {
      "value": {},
      "safeOriginKind": "implementation-capability"
    }
  },
  "entries": {
    "file": {
      "entryAdmissionInputs": {
        "rawConfigurationFixture": "fixtures/locale-default-not-in-project.json",
        "rawConfigurationDigest": "<sha256>",
        "parserSpecification": { "identity": "<identity>", "revision": "0" },
        "exactLogicalSourceIdentity": "<identity>",
        "sourceContentRevision": "<revision>",
        "configurationOrigin": "configuration-file",
        "locatorCandidate": { "relative-path": "intlify.config.json" },
        "externalProfileSelectorInput": {},
        "selectorOrigin": "profile-selector-cli"
      },
      "expected": {
        "resolverConstructionIdentity": "<construction-identity>",
        "outcome": "blocked",
        "profile": null,
    ...
    "programmatic": {
      "entryAdmissionInputs": {
        "programmaticEntrySnapshot": {},
        "snapshotContentDigest": "<sha256>",
        "frontendIdentity": "<identity>",
        "frontendVersion": "<version>",
        "programmaticEntryRuleRevision": "0",
        "exactLogicalSourceIdentity": "<identity>",
        "sourceContentRevision": "<revision>",
        "configurationOrigin": "configuration-programmatic",
        "locatorCandidate": { "safe-label": "project configuration" },
        "callSiteSourceSnapshot": null,
        "externalProfileSelectorInput": {},
        "selectorOrigin": "profile-selector-programmatic"
      },
      "expected": {
        "resolverConstructionIdentity": "<construction-identity>",
        "outcome": "blocked",
...
`commonInvocationInputs` is complete and explicit. It contains the full Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact, Locale Canonicalization Data Artifact, Submitted Profile Resolution Artifact Collection, and Admitted Implementation Capability; every item carries its required Safe Origin Kind. `entries.file.entryAdmissionInputs` contains raw bytes through a content-addressed fixture plus required digest, parser specification, exact logical source identity, Source-content revision, fixed `configuration-file` origin, optional locator candidate, External Profile Selector Input, and fixed `profile-selector-cli` origin. `entries.programmatic.entryAdmissionInputs` contains the complete Snapshot and digest, frontend identity/version, programmatic-entry rule revision, exact logical source identity and revision, fixed `configuration-programmatic` origin, optional locator candidate, optional exact call-site source snapshot, selector input, and fixed `profile-selector-programmatic` origin. The selector is intentionally per-entry because its Safe Origin Kind differs.

A large value may use a checked-in content-addressed fixture reference only when the manifest carries the required canonical digest and the harness verifies it before use. Materialized Resolution Input Set is resolver-derived and never a parallel caller-input object in this manifest. The Case Schema forbids harness defaults for parser, frontend, rule revision, source identity or revision, Safe Origin Kind, selector, locator state, Snapshot digest, or any common invocation input. Every executable entry is self-contained apart from its explicit construction and content-addressed fixture references.
...
- the Profile Specification Schema, complete Profile Specification, Configuration Schema Authority Set and every mapped schema, Resolver Capability Specification, and every fixed-role Normative Resolver Registry Package member are materialized, body-valid, cross-reference-valid, and pinned by the Suite Index at their exact role or mapping key, identity, revision, dialect where applicable, and canonical digest;
...
- successful construction with one exact Resolver Construction Identity covering the Profile Specification Schema, Profile Specification semantic digest, complete Configuration Schema Authority Set mapping, Resolver Capability Specification, and six fixed Registry Package role tuples; plus every closed missing, malformed, unsupported, identity-, revision-, dialect-, cross-reference-, body-schema-, and digest-mismatch reason producing no resolver invocation, Finding, Evidence, dependency, or cacheable output;
- construction cases with several invalid authorities selecting exactly one singular `failureReason` under Case Schema precedence, including Configuration Schema Authority Set member-key order and Registry Package logical-role order, independently of discovery, object-member, and worker order;
- accepted and rejected cases for every closed pre-invocation boundary kind—Profile Resolution Specification Input, Intent Surface-Class Vocabulary Artifact envelope, Safe Origin Kind, External Profile Selector binding, configuration source identity/revision, Programmatic Entry Snapshot, Implementation Capability, and Resolver Construction Identity reference—and every boundary-specific closed reason, with normalized accepted values and no resolver-shaped output on rejection;
- a capability value asserting the constructed Capability Specification identity/revision, complete `policyComparable` and `bootstrapOnly` namespaces, canonical bound-order admission, exact mandatory-minimum vectors, and first-insufficient-bound selection; plus several independently invalid input components remaining separate typed admission results with no combined resolver outcome;
...
- Bootstrap Resource Policy Candidate Admission across permuted collection order, including the exact five-field and four-field conflict predicates, one exact candidate, body-integrity failure, duplicate exact candidates with equal canonical submitted bodies whether valid or invalid, conflicting digests or unequal canonical submitted bodies in the selected conflict group, every fixed mismatch-precedence tier and its exact code/reason mapping, several candidates in one tier ordered only as related Evidence, bootstrap envelope-scan exhaustion, no provisional Set or unrelated-artifact admission, distinct bootstrap-versus-Phase-9 check IDs, and an unrelated duplicate or conflict that becomes blocking during later complete collection admission;
...
- the exact nine Safe Origin Kind Registry IDs, ranks, and permitted evidence-kind sets, including `configuration-file` shared by strict JSON and any compatible file parser, `configuration-programmatic` shared across conforming language bindings, explicit `intent-surface-class-vocabulary`, and resolver-input origins unchanged by acquisition path, URL, registry, cache, or provider implementation;
...
- Materialized Resolution Input Sets containing only the exact replay inventory—materialized value, normalized selector, selected schema identity/revision/digest, fixed-slot specification input, vocabulary artifact, canonicalization artifact, complete submitted artifact collection, and Admitted Implementation Capability—while schema bodies remain construction authority and confirmed selector, admitted Resource Limit Policy, specification/artifact Sets, Structural Analysis Result, Findings, and evaluation status are absent and deterministically recomputed;
- malformed, incomplete, specification-identity/revision-mismatched, non-positive, unrepresentable, or below-minimum implementation capability failing its closed pre-invocation boundary without a Resolver outcome, contrasted with one admitted capability whose selected Resource Limit Policy requirement exceeds a `policyComparable` bound and produces `project-profile-resource-policy-capability-incompatible` during Phase 4 while no `bootstrapOnly` bound is compared;
...
5. materialize the Profile Specification Schema, Profile Specification, Configuration Schema Authority Set and mapped schemas, Resolver Capability Specification, and complete Normative Resolver Registry Package; establish full Resolver Construction Input Set admission and identity, closed canonical Programmatic Entry Snapshot and Admitted Implementation Capability boundaries, exact Entry and Materialized replay inventories, Safe Origin Kind admission, resolver-owned Logical input path projection, Profile Selector Evidence Projection and Resolver Input Component Path construction, mandatory outcome-level construction identity, evidence-free Finding and occurrence identity plus Finding-Key-to-payload functional dependency, dependency-aware Finding collection, atomic evaluation status and reporting limits, exact resource accounting, profile equality, construction staleness, and invocation staleness;
6. Conformance Suite revision-`"0"` Suite Index with every construction-authority and Case Schema tuple, closed singular construction and boundary failure vocabularies plus precedence, Resolver Construction Case, Resolver Input Admission Case, and complete-input Resolver Case manifests, paired-entry harness, fixture-only profile view, golden expectations, Handoff Vector manifests, conditional cross-version obligations, and zero-gap lifecycle traceability report; and
7. file loader, optional Programmatic Entry Snapshot frontend, vocabulary- and canonicalization-data product integration, host Delivery Unit Graph handoff, complete Resolver Construction Input Set retention for replay, and downstream-consumer evidence.
...
| 015-084 | Define Safe Origin Kind as a required closed registry identity with nine revision-`"0"` IDs including the explicit Intent Surface-Class Vocabulary Artifact origin, fixed ranks, exact evidence-kind compatibility, token-domain separation, and input-boundary rejection of missing or unknown kinds | Accepted | Evidence kind alone does not identify whether equal evidence entered through file, programmatic, selector, specification, vocabulary, artifact, or capability boundaries; one pinned vocabulary makes tokens, occurrence identity, ordering, and disclosure reproducible without retaining arbitrary adapter or transport labels | Ownership and Dependencies; Terminology; Canonical Configuration Input and Resolution; Findings and Failure Model; Dependency, Invalidation, and Reproducibility; Security and Credential Handling; Conformance and Fixtures; Implementation Phasing |
...
| 015-100 | Include the canonical semantic digest of the complete Localization Project Profile Specification body in Resolver Construction Identity and pin the same digest in the Conformance Suite Index | Accepted | A reused revision must not let behavior-changing specification bodies share resolver, cache, or reproduction identity, while canonical semantic framing avoids treating irrelevant transport formatting as authority changes | Terminology; Active Project Profile Specification and Profile Resolution Specification Input; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
...
| 015-102 | Define Materialized Resolution Input Set as an exact replay snapshot of the materialized value and invocation inputs, excluding confirmed selector, admitted policies and Sets, Structural Analysis Result, Findings, and evaluation status | Accepted | Shared resolution must be reproducible by recomputing Derived Admission State rather than treating prior validation results as caller authority or ambiguous parallel inputs | Terminology; Canonical Configuration Input and Resolution; Provider, Governance, and Glossary References; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
| 015-103 | Define Bootstrap Resource Policy exact-reference grouping by the full five-field tuple, conflict grouping by kind/identity/revision/specification-revision, and one closed duplicate/conflict/body-integrity/mismatch precedence independent of submission order | Accepted | Bootstrap policy authority must be unique and deterministic before the complete artifact Set exists, and mismatch diagnostics cannot depend on whichever near candidate happens to be encountered first | Terminology; Provider, Governance, and Glossary References; Deterministic Resolution Algorithm; Findings and Failure Model; Conformance and Fixtures |
...
| 015-105 | Carry the exact Resolver Construction Identity as a mandatory fixed field of every checked or blocked outcome outside ordinary reporting limits, and require the project graph to retain the corresponding complete Resolver Construction Input Set for replay | Accepted | Dependency records can be omitted by reporting exhaustion and a compact identity cannot reproduce omitted authority bodies, so every outcome needs an unconditional construction scope without pretending that identity alone is replay data | Terminology; Active Project Profile Specification and Profile Resolution Specification Input; Dependency, Invalidation, and Reproducibility; Consumer Input Boundaries; Conformance and Fixtures |
| 015-106 | Define the Conformance Suite over the complete 015 lifecycle, close construction and boundary-specific failure vocabularies in the digest-pinned Case Schema, and route every normative rule to its exact case family or handoff owner | Accepted | Construction and pre-invocation failures are neither resolver Findings nor downstream behavior; explicit lifecycle classification and zero-gap traceability prevent untestable free-form failure reasons and resolver-only coverage claims | Terminology; Conformance and Fixtures; Implementation Phasing |
...
| 015-108 | Construct one finite Configuration Schema Authority Set mapping every admitted `schemaVersion` to an exact schema identity, revision, dialect, complete body, and digest; retain only the selected schema tuple in materialized replay and exclude it from profile semantics | Accepted | Authoring schemas may evolve independently while resolving to one semantic Profile Specification, but schema selection and body changes must remain reproducible construction authority rather than implementation-embedded behavior | Goals; Ownership and Dependencies; Terminology; Canonical Configuration Input and Resolution; Deterministic Resolution Algorithm; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
| 015-109 | Represent the Normative Resolver Registry Package in the Suite Index as one closed `registryPackageMembers` object with six fixed logical roles whose values each carry explicit identity, revision, and digest | Accepted | Logical construction roles and artifact identities are different concepts; a closed role map prevents omitted, duplicated, inferred, or order-dependent registry authority | Findings and Failure Model; Conformance and Fixtures |
| 015-110 | Separate the construction-owned Project Profile Resolver Capability Specification from each invocation's Admitted Implementation Capability, divide concrete bounds into `policyComparable` and `bootstrapOnly`, and pin a canonical mandatory bootstrap-minimum vector | Accepted | A concrete capacity value needs one reproducible vocabulary, unit system, comparison order, and minimum reserve before safe host inspection or resolver invocation, while bootstrap-only safety capacity must never become semantic Resource Limit Policy | Terminology; Canonical Configuration Input and Resolution; Resolver capability authority and invocation value; Project-profile resolution resource limits; Deterministic Resolution Algorithm; Findings and Failure Model; Conformance and Fixtures |
...
| 015-113 | Make every Resolver Case explicitly contain its construction reference, complete common invocation inputs, complete per-entry file or programmatic admission inputs, exact Safe Origin Kinds, and per-entry expected outcome with no harness defaults | Accepted | Self-contained manifests are required to reproduce resolver behavior across entry paths; per-entry selector origin, parser or frontend identity, source revision, and Snapshot content cannot be inferred safely by the harness | Terminology; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
