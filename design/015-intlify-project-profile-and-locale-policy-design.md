...
The shared Rust crate `intlify_config` owns the `IntlifyConfig` authoring model, configuration-schema version admission, JSON Schema definition and generation, named-profile selection, semantic resolution, configuration Findings, and the `LocalizationProjectProfile` settings IR. Each resolver instance is constructed with one already admitted Active Project Profile Specification and its complete Normative Resolver Registry Package. It defines provider-driven locale-canonicalization, typed Profile Resolution Specification Input, and submitted-artifact-collection boundaries but does not embed raw or generated CLDR data or acquire artifacts. `intlify_cli` is a product adapter: it discovers and reads repository configuration, normalizes the user-facing selector, supplies an admitted canonicalization data artifact and one finite already acquired Submitted Profile Resolution Artifact Collection through those boundaries, calls `intlify_config`, and renders the outcome. Optional programmatic frontends call the same core rather than depending on CLI internals.
...
- Separate the unordered Submitted Profile Resolution Artifact Collection, which retains multiplicity for admission and resource accounting, from the duplicate-free Profile Resolution Artifact Set used for exact reference resolution.
...
| Active Project Profile Specification | Supplies the exact Profile Specification identity/revision and validated Safe Origin Kind, Check, and Finding Registries used from the first resolver phase; resolver construction admits it before any invocation can produce a 015 outcome |
...
| Entry Admission Input Set | Inputs required to reproduce resolver-bootstrap, configuration-entry, selector-origin, and occurrence evidence and, by itself before materialization or together with the Materialized Resolution Input Set afterward, the complete Entry Resolution Evidence and Finding Occurrence Keys. Common inputs are the Active Project Profile Specification identity/revision and Registry Package digests, the complete Profile Resolution Specification Input and fixed origin, the External Profile Selector Input and fixed selector origin, and bootstrap capability. File entry additionally includes raw configuration bytes, exact logical source identity, Source-content revision, Entry Source Evidence, and parser specification. Programmatic entry additionally includes the submitted host value, frontend identity/version, revision-`"0"` entry rules, exact logical source identity, Source-content revision, Entry Source Evidence, and an exact immutable UTF-8 call-site source snapshot when an admitted span is retained |
| Materialized Resolution Input Set | Finite immutable set of the Active Project Profile Specification identity/revision and Registry Package digests, JSON-compatible configuration value, normalized External Profile Selector Input, configuration schema revision, fixed-slot Profile Resolution Specification Input, Locale Canonicalization Data Artifact, complete Submitted Profile Resolution Artifact Collection including multiplicity and unreferenced artifacts, and resource and implementation-capability admission needed to reproduce shared structural and semantic resolution |
...
| Profile Resolution Artifact Set | Closed finite immutable duplicate-free admitted value created from a successfully bounded and validated Submitted Profile Resolution Artifact Collection; only this set is used for exact reference resolution and checked Resolved Profile Artifact Bindings |
...
Structural Analysis Result
  ...
  + Active Project Profile Specification
  + Profile Resolution Specification Input
...
Before an invocation can begin, `intlify_config` constructs a revision-specific resolver with exactly one Active Project Profile Specification. Resolver construction admits its exact Localization Project Profile Specification identity/revision and the complete Normative Resolver Registry Package, verifies the pinned schemas and digests, and rejects every registry inconsistency. Construction failure is an implementation or toolchain-admission error with no resolver invocation, `project-profile-*` Finding, or partial Resolution Evidence. Every subsequent phase, including input-boundary Safe Origin Kind validation and pre-policy Findings, uses only this active authority.
...
- materialized re-resolution dependency records for configuration value content, normalized selector input, configuration schema, Active Project Profile Specification and Registry Package, Profile Resolution Specification Input, canonicalization data, Submitted Profile Resolution Artifact Collection, admitted artifact set, Resource Limit Policy, and implementation-capability inputs whose dependency identity or rejected admission state was established.

Its Entry Resolution Evidence projection contains:

- ordered resolver-bootstrap and configuration-entry Findings;
...
| `maxRecords` | All retained ordinary Entry-owned records | One Entry Finding, status, Finding Occurrence, Coverage Decision Evidence record, source-evidence-index record, entry dependency, or shared-unavailability cause reference | After kind-specific deduplication and canonical owner selection, before Entry projection commit | Finding Occurrences remain distinct through their ordinals; other equal records count once only where their kind defines deduplication |
...
15. `reporting-finalization` verifies already selected Evidence record owners, assigns zero-based Finding Occurrence ordinals within equal Finding-Key/evidence-identity candidate groups, deduplicates and canonically orders the retained Findings, evidence, evaluation status, Coverage Decision Basis, Coverage Decision Evidence, and dependency records within their owning reporting domains, verifies exact-cover Basis-to-Evidence correspondence when both domains are complete, fixes the bundle-level Resolution Evidence completeness and canonical domain-terminal-state set plus either the present Shared projection or its canonical unavailability causes, and returns exactly one checked or blocked outcome; it cannot create an ordinary semantic candidate, change an owner, repair a missing or orphaned mapping, or move a record between domain slices.
...
| `project-profile-specification-set-invalid` | `missing-slot` for a required field absent from the typed Profile Resolution Specification Input; duplicate or unknown raw members fail decoding before resolver invocation |
...
Source identity, Source-content revision, Portable Value Locator evidence, transient exact pointer, Portable Source Span, primary evidence identity, human message text, emitted-list index, raw or unprojected subject content, an unordered collection's authoring index, and related-evidence order are excluded. A semantic ordinal selected by `semantic-position` remains inside the projected Logical subject key because changing it changes the ordered policy. Every Finding candidate contributes one Finding Occurrence candidate. After final selector projection and canonical candidate grouping, all candidates sharing one Finding Key and one admitted disclosure-safe primary-evidence identity receive the complete ordinal set `0..n-1`, including ordinal `0` when `n` is one. Each Finding Occurrence Key is composed of that Finding Key, evidence identity, and ordinal. Because candidates in one such group are otherwise indistinguishable, the ordinal set depends only on multiplicity and never associates a candidate with physical input or worker order. For Materialized Value Evidence, the evidence identity contains only the retained Portable Value Locator, optional admitted Portable Source Span, and other admitted evidence fields, never Source-content revision, the transient exact pointer, or a rejected span candidate; the source-evidence-index identity follows the same rule. Excluded raw identity, locator, selector, member-name, value, revision, or rejected span coordinates never enter either key. Findings with the same Finding Key are deduplicated: the lowest registry-permitted candidate primary evidence in canonical evidence order becomes the emitted primary evidence, and every other distinct primary or related evidence record is merged as related evidence in canonical order. Equal evidence records are not repeated merely to express multiplicity; their separate Finding Occurrences remain visible through ordinal-distinct keys. Identical invalid members of an unordered collection, and distinct values intentionally grouped by one redacted-structure projection, therefore produce one Shared Finding and one Entry Finding Occurrence per retained candidate whether their admitted occurrence evidence differs or is equal. Suppression causes refer only to Finding Keys and cannot recover unprojected subject content.
...
Resolution Evidence records these relationships without changing them. A semantic dependency record links one established canonical semantic fact or checked absence state to the exact admitted inputs used to derive it. A materialized re-resolution dependency identifies configuration value content, normalized selector input, configuration schema, Active Project Profile Specification and Registry Package, Profile Resolution Specification Input, canonicalization data, Submitted Profile Resolution Artifact Collection, admitted artifact set, Resource Limit Policy, or implementation-capability input needed to decide whether shared resolution is stale. An entry re-resolution dependency identifies the active specification/registry authority, specification-input origin, admitted exact logical source identity/Source-content revision pair, or selector origin needed to rerun resolver bootstrap or configuration entry and update presentation. Semantic dependencies support invalidation slicing; both re-resolution classes schedule their applicable resolver boundary. Missing exact identities or revisions are never replaced by paths, timestamps, object addresses, random invocation values, or adapter guesses, and an unevaluated dependency remains represented only through evaluation status until its identity can be established.
...
Entry Admission Input Set
  common:
    ...
    + Normative Resolver Registry Package digests
...
- complete Submitted Profile Resolution Artifact Collection admission, including multiplicity-preserving referenced and unreferenced Policy and Target Profile envelopes, duplicate/conflict rejection, construction of the duplicate-free admitted Set, exact-reference matching, closed artifact kinds, and absence of resolver I/O;
...
- every Safe Origin Kind Registry row and Check/Finding Registry row, closed origin/evidence compatibility, subject-key strategy and scope, Subject content projection, evidence-disclosure policy and non-secret classification, allowed code/reason combination, semantic Finding Key, entry-specific Finding Occurrence Key including its zero-based ordinal, canonical semantic/evidence order, Finding deduplication, occurrence multiplicity, and atomic domain-specific terminal cursor;
...
3. define profile scope, named-profile selection, identity, inline locale-policy inputs, typed Policy and Target Profile references, typed Profile Resolution Specification Input admission, Submitted Profile Resolution Artifact Collection admission and duplicate-free Set construction, delivery-specification and placement-policy resolution, and deterministic semantic resolution;
...
| 015-088 | Separate the unordered multiplicity-preserving Submitted Profile Resolution Artifact Collection from the duplicate-free admitted Profile Resolution Artifact Set, reject duplicate or conflicting envelopes before any exact lookup, and preserve indistinguishable diagnostic multiplicity with zero-based Finding Occurrence ordinals | Accepted | Exact reference resolution must never choose among duplicates, while resource accounting, staleness, and diagnostics must still retain every submitted occurrence without depending on transport order or fabricating evidence paths | Goals; Ownership and Dependencies; Terminology; Provider, Governance, and Glossary References; Deterministic Resolution Algorithm; Findings and Failure Model; Conformance and Fixtures |
