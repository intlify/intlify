...
- Fix one revisioned, non-overridable Resolver Construction Root Package that closes construction envelopes, schema dialects and meta-schemas, construction-input safety limits, failure precedence, and cross-authority admission before any caller-supplied construction authority is trusted.
...
- Defining Intlify-wide measurement metadata, cross-target performance comparison, or target-specific size and performance budgets owned by [026](./026-intlify-conformance-and-measurement-design.md) and the applicable target designs.
...
| Optional programmatic frontend | Safely inspects a live host value into one complete inert Programmatic Entry Snapshot plus its exact Programmatic Entry identity/revision and optional canonical Programmatic Entry locator, optionally supplies one bounded Programmatic Call-Site Evidence Input with its own optional canonical Call-Site Source locator, safely normalizes the selector into the fixed External Profile Selector Input, maps configuration and selector entry to the fixed programmatic origin kinds, and enters the shared materialization and `IntlifyConfig` admission path; exact API naming, packaging, and language bindings remain 029-owned |
...
| 015 performance measurement | Owns the profile-resolver-specific benchmark phase/cost vocabulary, exact component boundaries, workload dimensions, semantic checksum observations, storage/reuse invariants, and the initial 026 measurement projection and local regression evidence; resource-limit accounting remains normative resolver behavior and is never treated as a timing or memory measurement |
...
| Resolver construction and invocation boundaries | One built-in Resolver Construction Root Package governs admission of one complete five-component Resolver Construction Input Set and produces one immutable revision-specific resolver plus Resolver Construction Identity; one invocation then accepts only the Resolver Invocation Input Set and creates Derived Admission State without replacing its construction authority |
| Resolver Construction Root Package | Supplies the exact non-overridable construction root, including the Resolver Construction Admission Specification and schema, authority-envelope schemas, supported schema meta-schemas, and cross-authority rules used before any of the five submitted construction components can become authority |
| Resolver Construction Admission Specification | Supplies the Root Package member that closes construction-input limits, validation precedence, fixed component roles, and rule references under the package-pinned schemas and cross-authority rules |
...
| 026 conformance and measurement | Owns shared performance implementation constraints, diagnostic profiling and isolation, common measurement metadata and units, evidence admission, cross-target comparison and reporting rules, target budget evidence, and cross-target logical-render equivalence; it consumes but does not redefine 015-owned profile-resolver benchmark boundaries |
...
| Resolver construction staleness | State in which the Resolver Construction Identity changed; the resolver must be rebuilt under the exact new Resolver Construction Root Package from the new complete Resolver Construction Input Set, and every outcome from the prior construction is stale |
...
| Resolver Construction Root Package | Revisioned finite immutable construction root built into a conforming resolver implementation and pinned in the Conformance Suite Index by identity, revision, complete fixed-role member map, artifact file-integrity digest, and a separate canonical package digest. The canonical package digest transitively commits every member role and member digest and is the digest used by Resolver Construction Identity. The package is not a sixth caller-supplied Resolver Construction Input Set component and cannot be selected or overridden by configuration, an invocation, or an adapter |
| Resolver Construction Admission Specification | Required Root Package member that closes construction-input byte/depth/count/work bounds, construction-failure precedence, fixed component roles, and cross-authority rule references under the package-pinned schemas and rule bodies |
...
| Resolver Construction Identity | Compact canonical identity of one successfully constructed revision-specific resolver. Revision `"0"` is the full SHA-256 digest of the 015-owned domain-separated canonical construction record with exact record-local field tags, field lengths, and collection framing, and is presented as `rci0_` plus unpadded base64url; it identifies rather than replaces the complete construction authority |
...
Resolver Construction Root Package (built in)
...
live host value
  ...
  + optional Programmatic Entry locator
  + optional Programmatic Call-Site Evidence Input
    with its own Call-Site Source identity/revision and optional locator
...
- one successful Resolver Construction Case executed under the Suite-pinned Resolver Construction Root Package and containing the exact Profile Specification Schema, Profile Specification, Configuration Schema Authority Set with its `"0"` schema, Resolver Capability Specification, and six-role Registry Package used by the fixture;
...
A programmatic frontend instead admits one exact Programmatic Entry identity and Programmatic Entry revision bound to the complete canonical Programmatic Entry Snapshot and matching digest, plus an optional normalized Programmatic Entry locator associated only with that identity. It may additionally admit one Programmatic Call-Site Evidence Input whose independent call-site source identity/revision pair is bound to exact immutable UTF-8 source bytes and whose optional normalized Call-Site Source locator is associated only with that call-site identity. The Programmatic Entry pair and locator never claim to identify those source bytes, and the call-site pair and locator never substitute for Snapshot identity or staleness.
...
- an optional `safe-label` or `safe-uri` Portable Source Locator selected from the same source as that identity: the Programmatic Entry locator when no mapped Portable Source Span applies, or the Call-Site Source locator when one does;
...
Before reading caller-supplied construction authority, a conforming `intlify_config` implementation fixes exactly one built-in Resolver Construction Root Package. Revision `"0"` pins its identity, revision, checked-in artifact path and file-integrity digest, complete fixed-role member map, canonical package digest, and every member path and digest in the Conformance Suite Index. The package closes the five authority-envelope shapes, supported JSON Schema dialects and exact meta-schemas used to validate schema artifacts, limits for construction bytes, nesting, members, schemas, registry rows, and validation work, singular construction-failure precedence, and every cross-authority relationship. It is the finite construction-validation root: it is not supplied as a sixth component, does not validate itself at runtime, and cannot be selected or overridden by configuration, an invocation, an adapter, or another authority body. An implementation with a different or unavailable root package cannot claim conformance to the pinned suite revision.

The normative revision-`"0"` root package is machine-readable, not an implementation-private interpretation of this prose. It contains exactly these fixed logical roles in order: `construction-admission-specification`, `construction-admission-schema`, `authority-envelope-schema-package`, `json-schema-meta-schema-package`, and `cross-authority-rules`. The Suite Index `digest` verifies the Root Package artifact at `path`; it is package-file integrity metadata and never enters Resolver Construction Identity. The separate `canonicalDigest` is SHA-256 over the exact ASCII domain separator `intlify.resolver-construction-root-package.v0` followed by a three-field record containing package identity, package revision, and the fixed-order member collection, using the typed framing defined below. Its top-level tags are respectively `0x01`, `0x02`, and `0x03`; each member item is a two-field record with fixed role at `0x01` and its complete 32-byte member digest at `0x02`. The Root Package artifact does not contain its own `canonicalDigest`: after file-integrity and schema validation, the harness or implementation recomputes it and compares it with the Suite Index value. The canonical package digest therefore transitively commits every member role and complete member body without incorporating the package path or member paths. The Suite Index separately pins the repository-relative logical path and digest for every member. An implementation may compile those bodies into native static data, but the resulting logical bodies and both package digests must remain verifiable. A missing member, unavailable body, role disagreement, file-integrity digest mismatch, or canonical package digest mismatch is a conformance or implementation-startup failure and never a resolver construction result.
...
Only successful construction creates one immutable Resolver Construction Identity. Revision `"0"` hashes the exact ASCII domain separator `intlify.resolver-construction-identity.v0` followed by one typed canonical top-level record with exactly these fields and tags:

```text
0x01 rootPackage { identity, revision, canonicalDigest }
0x02 profileSpecificationSchema { identity, revision, digest }
0x03 profileSpecification { identity, revision, semanticDigest }
0x04 configurationSchemaAuthoritySet {
       identity, revision, digest,
       profileSpecification { identity, revision, semanticDigest }
     }
0x05 configurationMappings[] {
       schemaVersion, schemaIdentity, revision, dialect, digest
     }
0x06 resolverCapabilitySpecification { identity, revision, digest }
0x07 registryPackage { identity, revision, digest }
0x08 registryMembers[] { role, identity, revision, digest }
```

Fields inside each braced nested record use record-local tags `0x01`, `0x02`, and so on in the exact displayed order. Configuration mappings are ordered by ascending unsigned UTF-8 `schemaVersion` bytes, and the six Registry member records use fixed Registry role order. Individual Root Package member records are not repeated in this frame because `rootPackage.canonicalDigest` already commits their fixed roles and complete member digests.

A field frame is exactly `tag:u8 || payload_len:u64be || payload`. Tag `0x00` is invalid. Within each revision-`"0"` record, the first normative field has tag `0x01`, each following field increments that tag by one, every field appears exactly once in ascending tag order, and tags are local to that enclosing record rather than globally allocated. A nested record payload is the concatenation of its field frames. A collection payload is exactly `count:u64be` followed by `count` occurrences of `item_len:u64be || item_record_bytes`; each item record restarts its local field tags at `0x01`. No unknown, repeated, omitted, reordered, or out-of-range tag is admitted. Admitted strings contribute exact UTF-8 bytes without Unicode normalization, and SHA-256 digests contribute their complete 32 binary bytes. The fixed domain separator precedes the top-level record bytes without a terminator or normalization. 015 owns the domain separators, record shapes, fields, orders, tag assignment, collection framing, and SHA-256 algorithm. 017 may provide the shared framing and presentation primitives but cannot change those identity inputs. The logical result retains all 32 digest bytes and its canonical text presentation is `rci0_` followed by unpadded base64url; truncation is prohibited. Any future field, order, tag, item-framing, algorithm, or other frame change requires a new Resolver Construction Identity revision. Construction vectors pin the complete Root Package and RCI preimage frames and expected digests.
...
Every Common or Entry component first crosses pre-invocation admission. That admission asks whether a complete value can safely and losslessly enter its formal typed Resolver Invocation Input slot and performs only the boundary-specific binding checks explicitly assigned by the closed pre-invocation rules below; those checks include Safe Origin Kind compatibility and the verification-input, implementation-capability, Snapshot-digest, and Resolver Construction Identity bindings listed in the closed boundary table. Failure is a typed integration result and the resolver is not invoked. A component with resolver-owned validation then enters its owning resolver phase, where every remaining identity, revision, digest, body-integrity, schema, semantic, resource-policy, and cross-input-compatibility check produces a resolver Finding on failure. An evidence-only or invocation-routing component does not acquire an artificial second semantic check. Pre-invocation admission never promotes a boundary binding into general resolver-semantic validation. In particular, a formally admitted canonicalization-data envelope may fail semantic admission in Phase 5, and a formally admitted Submitted Profile Resolution Artifact Collection retains every representable invalid, duplicate, conflicting, and unreferenced member for Phase 4 or 9 rather than filtering it at the adapter boundary.
...
Construction context
  Resolver Construction Root Package
    identity/revision, complete fixed-role member map, and canonical digest
    + Resolver Construction Admission Specification and schema
    + authority-envelope schemas
    + exact JSON Schema meta-schema closure
...
Resolver Invocation Input Set
  ...
  + exactly one Entry Admission Input Set:
    ...
    programmatic:
      ...
      + fixed configuration-programmatic origin
      + normalized Programmatic Entry locator state
      + External Profile Selector Input + profile-selector-programmatic origin
      + optional Programmatic Call-Site Evidence Input:
        call-site source identity/revision
        + independently associated normalized Call-Site Source locator state
        + exact UTF-8 source snapshot
...
Entry Resolution Evidence applies the same rule to origin and location metadata. Every origin-bearing source-evidence or occurrence component retains one required registry-admitted Safe Origin Kind. Configuration Entry Source and Materialized Value Evidence additionally retain one required `safe-opaque` or `redacted` Portable Source Identity, only an optional admitted `relative-path`, `safe-label`, or `safe-uri` Portable Source Locator associated with that same identity, an optional admitted typed Logical input path for Entry Source Evidence or a required Portable Value Locator for Materialized Value Evidence, and an optional admitted Portable Source Span. Profile-selector evidence instead retains exactly one `absent`, `safe-profile-id`, or `redacted-value` Profile Selector Evidence Projection and never a source locator. Specification, artifact, Resource Limit Policy verification, and implementation-capability evidence instead retain exactly one Resolver Input Component Path and never pretend to have a configuration source identity.
...
The project graph may index, invalidate, and locate construction authority by Resolver Construction Identity, but replay is possible only while the exact Resolver Construction Root Package and complete matching Resolver Construction Input Set are retained or can be supplied explicitly. The compact identity and its component digests never stand in for an omitted Root Package member, Profile Specification, schema, capability, or registry body. An ordinary dependency record omitted by reporting exhaustion cannot erase the mandatory outcome-level construction identity.

## Performance and Measurement

Profile resolution is a build- and tooling-facing operation whose physical cost must remain visible by semantic stage. Revision `"0"` therefore adopts measurement-driven, phase-separated, local-first benchmarks. The benchmark design follows the product-owned phase and result principle in [001](./001-ox-mf2-toolchain-foundation.md) and the checked boundary, workload, and checksum pattern in [014](./014-ox-mf2-message-linker-design.md), but it does not import either product's phase registry or result schema.

Resource protection and performance measurement are separate concerns:

- `ResourceBoundValue`, Admitted Implementation Capability, and the admitted Resource Limit Policy define normative logical admission and fail-closed behavior. Exact-bound and first-over conformance proves those semantics.
- elapsed duration, peak live memory, and optional allocation observations describe one physical implementation under one recorded environment. They do not alter admission, Findings, profile identity, Resolver Construction Identity, cache identity, or conformance.
- a fast implementation cannot exceed or skip a logical resource bound, and a slow implementation does not become semantically non-conforming merely because one observed performance value is larger.

015 owns the exact profile-resolver benchmark vocabulary and boundaries because only this design can identify where one resolver-owned operation begins and ends. It adopts the revision-`"0"` observational measurement scope in [026's Initial 015 Adoption](./026-intlify-conformance-and-measurement-design.md#initial-015-adoption), together with its applicable [Adoption and Verification Requirements](./026-intlify-conformance-and-measurement-design.md#adoption-and-verification-requirements). 026 owns the shared performance implementation constraints, profiling specification, measurement metadata and units, evidence admission, comparison, reporting, and budget rules. Applying them does not rename, merge, or reinterpret the 015-owned boundaries below.

The first active benchmark boundary retains projection-ready owner results and verifies the applicable common projection and report integrity. The benchmark profile pins its adopted 026 specification revision outside resolver semantics and identity. Required measurement helpers and validators are implemented with the adopting harness or supplied by a verified shared implementation; they do not become dependencies of ordinary profile resolution merely to collect evidence. Unavailable support remains explicit rather than producing a claim of common-evidence conformance. Numeric comparison, qualified-runner budgets, cross-target campaigns, and Runtime integrations are not prerequisites for the initial observational scope. 026 has no independent implementation phase that 015 must wait for.

### Memory ownership and reuse

Apply 026's [memory lifetime classes](./026-intlify-conformance-and-measurement-design.md#memory-lifetime-classes), [common-case rules](./026-intlify-conformance-and-measurement-design.md#common-case-fast-paths-and-optional-work), and [lookup constraints](./026-intlify-conformance-and-measurement-design.md#data-layout-lookup-and-identity) to the resolver as follows:

- Construction authority retained by an immutable resolver is separate from invocation scratch. Returned Profiles, Resolved Profile Artifact Bindings, Findings, Evidence, and retained replay inputs own or safely share their storage; none may refer to resettable scratch.
- Temporary traversal queues, canonicalization lookups, duplicate-detection indexes, and construction buffers have an explicit invocation or worker lifetime. A Scratch Workspace is preferred where repeated calls benefit from retained collection capacity; it remains invocation- or worker-owned rather than mutable state shared by unrelated callers of an immutable resolver.
- Reset invocation-local semantic state before the next invocation, including after a blocked outcome. A workspace reset may retain capacity but not previous input associations, Findings, or resource-counter state. No result reference survives into that resettable storage.
- Capacity hints are bounded by admitted input and implementation capacity, not allocated from an untrusted declared maximum. Shrinking and pathological high-water capacity release follow an explicit policy outside the latency-sensitive path. Moving an owned output buffer into a result is not capacity reuse for the next call.
- Reuse canonicalized values and derived indexes only within their valid checked input and specification scope. Reuse does not omit prescribed admission or policy rechecks, collapse counted occurrences, change logical work accounting, or let internal hash iteration determine canonical output or Finding order.
- Required Findings, Evidence, dependencies, and replay information remain required. Deferring optional diagnostics or avoiding empty allocations must not turn required output into opt-in output or expose a partial checked profile.

Core resolution remains synchronous and scheduler-neutral; this adoption does not introduce a thread pool or the 019-owned project cache. Concrete collection types, compact IDs, arena allocation, and SIMD are implementation choices justified under 026, not requirements for the minimum resolver. Safe optimized primitives are preferred; any unsafe or architecture-specialized path still needs the separate owner decision and verification required by [026](./026-intlify-conformance-and-measurement-design.md#low-level-specialization-and-unsafe-code).

### Resolver benchmark vocabulary

Benchmark phase names are distinct from the resolver's runtime resolution phases and the seven Implementation Phases. The initial 015 benchmark profile admits the following closed phase/cost pairs. A pair becomes required only when its owning production boundary is implemented.

| Benchmark phase | Costs | Measured operation |
| --- | --- | --- |
| `profile_resolver_construct` | `construction_authority_admission`, `construction_identity` | Admit the complete construction authority and construct the immutable resolver, then derive its Resolver Construction Identity separately |
| `profile_entry_materialize` | `file_decode_and_source_map`, `programmatic_snapshot_admission` | Materialize already acquired file bytes or admit an already constructed inert Snapshot; exclude repository I/O and live-host inspection |
| `profile_structural_admit` | `structural_analysis`, `authoring_model_construction` | Analyze the complete materialized value and, only on complete success, construct `IntlifyConfig` |
| `profile_select` | `named_profile_selection` | Resolve the normalized selector to exactly one profile declaration without performing later semantic resolution |
| `profile_locale_resolve` | `locale_canonicalization`, `locale_policy_resolution` | Canonicalize locale occurrences and resolve source, requested, default, negotiation, fallback, and coverage-policy inputs |
| `profile_artifact_admit` | `resource_policy_bootstrap`, `complete_artifact_collection_admission`, `resolved_binding_construction` | Admit the selected Resource Limit Policy, recheck the complete submitted collection, construct the duplicate-free Set, and project exact resolved bindings |
| `profile_target_group_resolve` | `target_and_group_resolution`, `hydration_relation_validation` | Resolve Target locale facts, the exact group partition, canonical order, and the fixed revision-`"0"` hydration checks |
| `profile_evidence_materialize` | `entry_evidence`, `shared_evidence`, `outcome_finalization` | Materialize the applicable bounded Evidence projection and atomically finalize the complete checked or blocked outcome |
| `profile_resolve_e2e` | `file_entry_complete_workflow`, `programmatic_entry_complete_workflow`, `materialized_reresolution` | Execute the declared complete in-memory workflow from an already acquired entry or a replay input through final outcome construction |
| `profile_resolve_peak_memory` | `construction_peak_live_memory`, `invocation_peak_live_memory` | Observe peak live memory for fresh resolver construction and complete invocation as separate cases |

An implementation-owned checked registry fixes the active phase/cost table, exact boundary IDs, permitted nesting or overlap, workload-profile revision, required-case matrix, and checksum observation codecs. Adding or changing a phase, cost, semantic workload dimension, interval boundary, overlap rule, or checksum observation advances that benchmark profile and updates this design before the changed meaning is used as release evidence. File paths, tool package names, and one physical timer or allocator implementation remain implementation choices.

### Workloads and observations

The benchmark matrix must not collapse profile resolution into one aggregate number. It contains at least:

- a small representative valid project profile;
- independently scaled raw bytes, materialized nodes and depth, profile declarations, locale occurrences and canonical locale count, coverage rules and cells, artifact occurrences and canonical bytes, targets, groups, memberships, hydration relations, and Finding/Evidence records and bytes;
- valid, structurally blocked, semantic blocked, duplicate-heavy, artifact-conflict, and diagnostic-dense outcomes;
- exact-bound and first-over cases for applicable 015 resource counters, with a blocked resolver outcome treated as the expected semantic result rather than a harness failure;
- equivalent JSON-member, submitted-collection, and set-member permutations;
- paired file and programmatic cases when both entry paths can represent the same materialized value;
- fresh resolver construction, repeated invocation of one immutable resolver, and materialized re-resolution as distinct variants; and
- canonicalization-provider cases that pin one exact provider/specification/data identity rather than consulting ambient locale data.

Every measured record declares its fixture, variant, scale, entry kind when applicable, phase, cost, iteration count, relevant logical work-vector observations, and one semantic checksum. Duration is required for every ordinary component and end-to-end pair. Peak live memory is required for the two `profile_resolve_peak_memory` costs. Allocation count and allocated bytes may be emitted when one documented allocator setup can measure them reliably, but their absence cannot be represented as zero.

The semantic checksum observes the complete deterministic output projection applicable to that boundary and prevents unused work from disappearing. Shared semantic output and entry-specific Evidence are framed as separate checksum domains so a paired file/programmatic case can prove equal shared resolution without pretending that its source evidence is equal. Timing, allocator state, pointer identity, worker identity, filesystem path, and environment metadata never enter the semantic checksum. The checksum is benchmark integrity evidence, not an artifact digest, Resolver Construction Identity, authenticity proof, or checked-in golden performance value.

Logical work-vector observations use the same units and counting stages as the applicable capability or Resource Limit Policy counters where such counters exist. They explain workload scale but do not substitute for duration or memory measurement. Revision `"0"` defines no universal Big-O or wall-clock service-level objective; instead, single-dimension scale cases make repeated scans, unexpected allocation growth, and phase-cost migration observable. Any later normative complexity ceiling or numeric budget requires an explicit 015 or 026 decision with a reproducible measurement environment.

### Measurement records and common projection

The product-owned checked result schema preserves the data required by [026's initial projection](./026-intlify-conformance-and-measurement-design.md#initial-015-adoption) from the first active boundary. It does not emit only a console average or replace its own schema with the common evidence schema.

- Retain the exact owner schema and benchmark-profile revision, fixture and workload identity, phase/cost and interval identity, ordered raw samples, positive repetition counts, and the applicable semantic checksum for each sample. Warmup observations remain outside measured samples.
- Record duration as losslessly retained canonical nanoseconds under a declared clock/conversion Measurement Method Descriptor. Use the applicable common units, checked arithmetic, and Sample Aggregation Kind; peak memory is not divided by the repetition count.
- Record complete Build Identity, Environment Observation including Runner Context, metric-provider identity, and the applicable Execution State. State the fixture, process, resolver, workspace, cache, and output-buffer reset/reuse policy rather than inferring it from a warm or cold label.
- Define separate versioned Memory Observation Domains for resolver construction and invocation, including the observer, covered storage, retained inputs and locale data, allocator scope, and reuse state. Use the applicable descriptors for memory/allocation cases. Total allocated bytes remains an owner-specific diagnostic in 026 revision `"0"`; unsupported or unavailable observations are not zero.
- Use 026's minimum mapping for category, operation class, and Performance Surface while preserving every owner pair. Bind active required and optional cases through Measurement Run Plans and typed Run/Case Evaluations so missing, skipped, unsupported, failed, projection-ineligible, stale, and invalid attempts are not omitted or presented as successful measurements.

Projection and report validation cover the adopted capability's positive, negative, exact-boundary, and first-over fixtures. Partial or overflowed observations never supply successful numeric samples. The initial smoke profile may use one measured sample and remains observational; it does not establish numeric-budget eligibility.

### Optional profiling

Diagnostic profiling is optional for the minimum implementation. If introduced, it follows [026's Profiling Specification](./026-intlify-conformance-and-measurement-design.md#profiling-specification):

- Use an explicit non-default compile-time feature and a finite versioned span registry aligned with resolver operations. Keep span IDs or labels static and recording bounded.
- Keep allocation profiling separately enabled; enabling timing spans does not replace or wrap the global allocator implicitly.
- Preserve ordinary resolver semantics, required output, resource accounting, identity, error handling, and cleanup with profiling enabled or disabled.
- Collect projection-ready benchmark samples with diagnostic profiling disabled. Benchmark-owned timers and allocator observers still follow their declared Measurement Method Descriptors; Profiler Observations are separate diagnostic records, not Measurement Evidence.
- Verify enabled/disabled semantic equivalence and the applicable execution-binding, bounded-recording, and failure fixtures. A zero-required-runtime-work claim additionally needs passing Instrumentation Isolation Evidence, including call-site erasure and profiler-dependency exclusion from ordinary builds.

Not providing a profiler does not block 015 phase completion. Providing one activates its applicable verification requirements in the owning phase; absent or failed isolation evidence must not be replaced by a zero-cost claim.

### Measurement boundaries and regression policy

Fixture generation, authority and input acquisition, repository discovery, file reads, live-host Snapshot construction, timer and allocator setup, checksum observation, result validation, reporting, and teardown remain outside component intervals. A component interval begins only after its declared checked input exists and ends after its ordinary complete typed result exists. A declared end-to-end interval intentionally includes its component work and may overlap separately reported component intervals; measured results are never arithmetically subtracted to invent another phase.

An expected checked or blocked resolver outcome is a successful benchmark operation. Panic, unsafe inspection, malformed benchmark input, missing required case, boundary mismatch, checksum mismatch, non-deterministic logical observation, or inability to emit and validate the result is a benchmark failure. A failed measured operation never emits a successful prefix record.

Revision `"0"` uses these acceptance rules:

- local benchmark and smoke commands must build and execute without network or ambient configuration;
- checked result shape, active phase/cost table, required cases, boundary topology, semantic checksum, and deterministic logical observations are CI gates;
- machine-specific raw timing and memory baselines are not committed as normative fixtures;
- duration, peak-memory, and allocation values are initially observational and do not by themselves fail normal CI;
- a performance change is reviewed against the affected phase, work vector, memory behavior, and end-to-end result rather than one aggregate time; and
- a numeric regression threshold becomes a release gate only after an explicit 026-compatible decision defines the environment, statistic, sample policy, baseline lifecycle, tolerance, and failure disposition.

The Resolver Conformance Suite may verify that benchmark inputs reproduce the expected semantic outcomes, but observed duration, memory, and allocation values are not Resolver Case expectations. Benchmark registry/schema/boundary tests and the 026 measurement suite remain distinct from the semantic pass/fail status of one profile resolution.

The benchmark profile is not a sixth Resolver Conformance Suite manifest kind and does not enter the Suite Index authority closure. The generated 015 traceability report maps Decisions 015-131 and 015-132 and every active benchmark phase/cost to the implementation-owned checked benchmark registry, required-case matrix, boundary tests, result-schema and common-projection validation, checksum and storage/reuse tests, optional profiling verification where applicable, and smoke command. This mapping is implementation-validation evidence, not a synthetic Resolver Case or Handoff Vector.
...
```json
{
  ...
  "resolverConstructionRootPackage": {
    ...
    "digest": "<file-integrity-sha256>",
    "canonicalDigest": "<transitive-package-sha256>",
    "members": {
...
Every listed ID occurs exactly once within and across all five manifest arrays, every array reference carries exactly `id`, `path`, and `digest`, every referenced manifest passes the complete Case Schema body, and every construction-root or construction-authority tuple matches its canonical checked-in artifact. Every referenced body carries its suite-root-relative `path` and file-integrity `digest`; the Profile Specification additionally carries its representation-independent `semanticDigest`. The Resolver Construction Root Package additionally carries `canonicalDigest`: `digest` verifies only the artifact at `path`, while `canonicalDigest` is recomputed from the admitted package identity, revision, fixed member roles, and member digests and is the only Root Package digest that enters Resolver Construction Identity. The two values are independent and neither substitutes for the other. Each manifest contains one required `manifestKind`: `resolver-construction`, `resolver-input-admission`, `resolver`, `handoff-vector`, or `conditional-obligation`. The Resolver Construction Root Package identifies the exact built-in root against which the suite runs and is not copied into the five-component Resolver Construction Input Set. The Configuration Schema Authority Set's `profileSpecification` tuple must exactly match the indexed Profile Specification, and its `members` keys are admitted `schemaVersion` values in canonical string order; each value pins the full schema tuple including dialect and path. `registryPackage.members` is a closed object with exactly the six fixed logical roles in the shown order. A role key is not an artifact identity, no unknown or duplicate logical role is permitted, and each role value must explicitly carry identity, revision, path, and digest. Array order is canonical ID order and duplicate IDs are invalid. A Suite Index that omits or mismatches the Suite Index Schema, Construction Root Package file-integrity digest or canonical digest, Root Package member, Profile Specification Schema, complete Profile Specification file-integrity or semantic digest, Configuration Schema Authority Set association or mapped schema tuple, Resolver Capability Specification, complete Case Schema body, manifest reference, fixture closure, Registry Package tuple, or registry member is not a revision-`"0"` index.

### Resolver Construction Case manifest

Every Resolver Construction Case references one content-addressed fixture that materializes a complete five-component Resolver Construction Input Set and asserts whether that authority constructs a usable resolver under the Suite Index's exact built-in Resolver Construction Root Package:
...
- the built-in Resolver Construction Root Package and every fixed-role member, Profile Specification Schema, complete Profile Specification, Configuration Schema Authority Set and every mapped schema, Resolver Capability Specification, and every fixed-role Normative Resolver Registry Package member are materialized, body-valid, and cross-reference-valid where applicable; the Suite Index pins every artifact at its exact role or mapping key, identity, revision, dialect where applicable, path, and file-integrity digest, and additionally pins the Root Package `canonicalDigest` and Profile Specification `semanticDigest` where required;
...
- every testable resolver-semantic Accepted Decision and normative 015 invariant maps to at least one positive case and, where rejection or suppression is meaningful, at least one negative case; an implementation-measurement decision maps instead to the checked benchmark validation evidence explicitly allowed by this design;
...
- every non-resolver ownership, packaging, static-generation, integration, downstream-consumer, or implementation-measurement decision maps to an explicit verification owner and a check, Handoff Vector, or checked benchmark-validation record explicitly permitted by its owning section instead of being silently marked covered;
...
- no Accepted Decision, normative rule, construction reason, boundary kind, boundary failure reason, check, Resolver Construction Case, Resolver Input Admission Case, Resolver Case, Handoff Vector, conditional obligation, active benchmark phase/cost, required benchmark case, or declared relation remains unexplained or unmapped.
...
- successful construction under the exact built-in Resolver Construction Root Package, including every package member, supported JSON Schema dialect and exact meta-schema body, construction-bound ID and value, authority-envelope schema, cross-authority rule, and failure-precedence rule; one exact Resolver Construction Identity covers the Root Package identity/revision/`canonicalDigest` tuple, the Profile Specification Schema, Profile Specification identity/revision/semantic-digest tuple, complete Configuration Schema Authority Set mapping, Resolver Capability Specification, and six fixed Registry Package role tuples; vectors pin the complete Root Package canonical-digest preimage, complete RCI preimage, every record-local `u8` field tag, unsigned 64-bit big-endian field length and collection count/item length, full 32-byte SHA-256 results, and canonical `rci0_` presentation, while framing rejection vectors prove that tag `0x00`, unknown, repeated, omitted, reordered, or out-of-range tags and inconsistent collection counts or item lengths cannot define a revision-`"0"` digest;
- every closed missing, malformed, unsupported, identity-, revision-, dialect-, cross-reference-, body-schema-, digest-mismatch, and over-bound construction reason producing no resolver invocation, Finding, Evidence, dependency, or cacheable output; a missing Root Package or member, Root Package file-integrity `digest` mismatch, recomputed `canonicalDigest` mismatch, or malformed built-in package remains a Conformance harness or implementation-startup failure rather than a construction `failureReason`; construction cases with several invalid submitted authorities select exactly one singular `failureReason` under Case Schema precedence, including Configuration Schema Authority Set member-key order and Registry Package logical-role order, independently of discovery, object-member, and worker order;
...
- Portable Source Locator absence followed by fixed present-variant order `relative-path`, `safe-label`, `safe-uri`, then exact unsigned payload-byte order, independent from locale collation, filesystem order, and adapter discovery order, with each locator admitted only for its associated file, Programmatic Entry, or Call-Site Source identity;
...
- Programmatic Entry and Call-Site Source locator states admitted independently, with evidence using the Programmatic Entry identity and locator together when no call-site span applies and the Call-Site Source identity and locator together when one does, never borrowing either locator across identities;
...
- programmatic Entry Source Evidence with absent, empty-root, complete, and longest-safely-established-prefix Logical input paths, plus Materialized Value Evidence containing a Portable Source Identity, its same-source optional admitted `safe-label` or `safe-uri` Portable Source Locator, and required Portable Value Locator in both exact and redacted variants, with and without an admitted call-site Portable Source Span;
...
- equal Resolver Construction Root Packages, complete Resolver Construction Input Sets, and Materialized Resolution Input Sets under the same Resolver Construction Identity, for invocations that reach materialization, producing the same entry-independent structural/semantic resolution and Shared Resolution Evidence across conforming implementations, plus byte-identical canonical profile projection and binding domain whenever both complete bundles expose checked outcomes;
...
The accepted semantic decisions define the following seven named Implementation Phases in dependency order. These phases are implementation-readiness gates, not the runtime resolution phases defined above, release milestones, commits, or PR boundaries. One Implementation Phase may span several independently reviewable PRs. Experimental work may exercise a later semantic slice through test-owned inputs before every preceding production artifact exists, but it cannot claim completion of that later phase or revision-`"0"` conformance.

1. **Implementation Phase 1 — Configuration Foundation.** Establish `intlify_config`; extract reusable existing duplicate-aware input, deterministic validation, source-evidence, and schema-generation behavior; define the version-`"0"` `IntlifyConfig` project-profile schema; and establish strict input materialization, dependency-aware structural analysis, and complete structural admission.
2. **Implementation Phase 2 — Locale Canonicalization Boundary.** Define the Locale Canonicalization Specification, provider boundary, data-artifact admission requirements, and a data-free `intlify_config` integration, then evaluate the ICU4X reference adapter against conformance fixtures.
3. **Implementation Phase 3 — Semantic Profile Resolver.** Define profile scope, named-profile selection, identity, inline locale-policy inputs, explicit Intent Surface-Class Vocabulary Artifact admission, typed Policy and Target Profile references, typed Profile Resolution Specification Input admission, exact/conflict-group Bootstrap Resource Policy Candidate Admission, Submitted Profile Resolution Artifact Collection admission and duplicate-free Set construction, Resolved Profile Artifact Bindings, delivery-specification and placement-policy resolution, and deterministic semantic resolution.
4. **Implementation Phase 4 — Target and Compatibility Groups.** Establish exact Target ID partitioning into Deployment Compatibility Groups, canonical Target/Group/member/relation ordering, hydration-relation validation, and Group ID selection-vector handoff to 020.
5. **Implementation Phase 5 — Resolver Construction, Invocation, and Evidence.** Materialize the complete machine-readable Resolver Construction Root Package, Profile Specification Schema, Profile Specification, profile-associated Configuration Schema Authority Set and mapped schemas, Resolver Capability Specification, and complete Normative Resolver Registry Package; establish full Resolver Construction Input Set admission and exact framed Resolver Construction Identity, closed canonical Programmatic Entry Snapshot, optional Programmatic Call-Site Evidence Input, Resource Limit Policy Verification Input, and Admitted Implementation Capability boundaries, exact Entry and Materialized replay inventories, Safe Origin Kind admission, resolver-owned Logical input path projection, Profile Selector Evidence Projection and Resolver Input Component Path construction, mandatory outcome-level construction identity, evidence-free Finding and occurrence identity plus Finding-Key-to-payload functional dependency, dependency-aware Finding collection, atomic evaluation status and reporting limits, exact ResourceBoundValue accounting, profile equality, construction staleness, and complete-invocation staleness.
6. **Implementation Phase 6 — Conformance Suite Revision `"0"`.** Materialize the Conformance Suite revision-`"0"` Suite Index Schema and Suite Index, complete construction package and authorities, one complete Case Schema and all five manifest kinds, complete Registry Package, and every referenced fixture; require repository-relative content-addressed closure with no scanning or harness defaults, closed singular construction and boundary failure vocabularies plus precedence, paired-entry harness, fixture-only profile view, golden expectations, conditional cross-version obligations, and a zero-gap lifecycle traceability report.
7. **Implementation Phase 7 — Product Integration.** Integrate the file loader, optional Programmatic Entry Snapshot frontend, vocabulary and canonicalization-data product inputs, host Delivery Unit Graph handoff, exact Resolver Construction Root Package and complete Resolver Construction Input Set retention for replay, and downstream-consumer evidence.

Apart from the accepted internal Rust crate name `intlify_config`, these Implementation Phases do not reserve package names, commands, or public APIs.

Performance measurement is activated with its owning production boundary rather than postponed until the resolver is complete. Phase 1 establishes the local benchmark harness and entry-materialization, structural-admission, and selection pairs. Phase 2 activates canonicalization measurement. Phases 3 and 4 activate locale-policy, artifact, binding, target, group, and hydration pairs. Phase 5 activates construction, Evidence, complete invocation, replay, and peak-memory pairs. Phase 6 closes the revision-`"0"` benchmark profile, required-case matrix, boundary and checksum validation, and deterministic smoke gate. Phase 7 compares product-adapter overhead with the already measured core boundary without moving repository I/O or product orchestration into a core component pair; 029 owns broader product-workflow measurements.

The following verification requirements apply alongside that activation schedule and become part of the owning phase's completion criteria. They refine the existing seven phases rather than create another phase or a separate 026 delivery milestone.

| Implementation Phase | Applicable 026 adoption and verification |
| --- | --- |
| Phase 1 | Establish the checked observational result path, exact raw samples and descriptors, required/optional case accounting, common projection and report validation, and result/scratch ownership tests for the active entry, structural, and selection boundaries. |
| Phase 2 | Extend the same measurement path to canonicalization with exact provider/specification/data binding and explicit reuse state; retain fresh/reused semantic equivalence where reuse is implemented. |
| Phases 3 and 4 | Extend projection and fixtures with each active locale-policy, artifact, target, and group boundary; verify that reused indexes and scratch preserve resource accounting, required diagnostics, canonical ordering, and checked/blocked behavior. |
| Phase 5 | Verify immutable-resolver and complete-outcome storage ownership, reset after checked and blocked invocations, fresh/reused/replay equivalence, and separate construction/invocation memory domains and observations. |
| Phase 6 | Close the adopted benchmark registry, case inventory, common projection/report fixtures, and deterministic integrity gates. Cover optional profiling requirements if profiling is provided and isolation evidence if zero required runtime work is claimed; no numeric threshold is added by phase completion. |
| Phase 7 | Preserve core records while measuring adapter-inclusive work separately, including actual host transfer/materialization where applicable; record the relevant surface, environment, memory domain, and reuse state without absorbing 029-owned workflow semantics. |

The implementation plan assigns any necessary shared measurement helpers and their fixture coverage to these adopting phases. Checks for an active capability begin with its owning boundary rather than all being postponed to Phase 6. A later numeric comparison, budget, or cross-target capability requires its applicable 026 dependencies and verification before use, but does not retroactively change the initial observational resolver scope.

An implementation cannot claim Profile Specification revision `"0"` support until Implementation Phases 1 through 6 are complete. In particular, Phases 5 and 6 must produce every construction-authority artifact and the complete registry package, a Suite Index that pins all authority tuples and mappings, schema and cross-reference validation, reachable registry coverage, and a zero-gap traceability report. Earlier or partial phases may be implemented experimentally, but they are not a complete revision-`"0"` resolver. Phase 7 completes the product integration defined by this design but is not required for an alternate conforming embedding to claim core resolver support.
...
| 015-116 | Fix one revisioned, non-overridable Resolver Construction Root Package as the built-in construction root, pin its artifact file-integrity `digest` separately from its transitive `canonicalDigest`, and derive Resolver Construction Identity from its identity/revision/`canonicalDigest` tuple plus the five submitted construction-authority components | Accepted | Caller-supplied authority cannot safely define its own envelope grammar, schema dialects, input limits, failure precedence, or cross-authority checks; separating artifact integrity from the transitive package identity avoids self-reference while ensuring every root-member change changes the construction identity | Purpose; Goals; Ownership and Dependencies; Terminology; Active Project Profile Specification and Profile Resolution Specification Input; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
...
| 015-123 | Separate Programmatic Entry identity/revision and optional locator over the canonical Snapshot from an optional one-source Programmatic Call-Site Evidence Input with its own source identity/revision, optional locator, immutable UTF-8 bytes, and Snapshot-path-plus-role span mappings; select identity and locator from the same source; reject a malformed sidecar before invocation but treat an invalid individual span as absent | Accepted | Configuration-value staleness and editor source locations identify different content domains; separating them preserves cache correctness while allowing precise optional navigation without borrowing presentation metadata across identities or letting one bad coordinate invalidate valid configuration semantics | Terminology; Canonical Configuration Input and Resolution; Findings and Failure Model; Dependency, Invalidation, and Reproducibility; Security and Credential Handling; Conformance and Fixtures |
| 015-124 | Materialize revision `"0"` as one machine-readable Resolver Construction Root Package containing the complete construction body and schema, authority-envelope schemas, exact Draft 7 and Draft 2020-12 meta-schema closure, cross-authority rules, failure precedence, and exact closed bound inventory; pin the package artifact with file-integrity `digest`, derive a separate `canonicalDigest` from package identity, revision, fixed member roles, and complete member digests without storing that digest in the package body; and define numeric values only in the checked body | Accepted | Resolver construction cannot be implemented reproducibly from prose, ambient schema registries, or hidden implementation limits; independently checking artifact bytes and the transitively committed package identity avoids a self-referential artifact while making every root rule executable and reviewable | Active Project Profile Specification and Profile Resolution Specification Input; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures; Implementation Phasing |
| 015-125 | Define Resolver Construction Identity revision `"0"` as full SHA-256 over domain `intlify.resolver-construction-identity.v0`, the Resolver Construction Root Package identity/revision/`canonicalDigest` tuple, and the five construction-authority components in one canonical record whose fields use exact record-local sequential `u8` tags and unsigned 64-bit big-endian field-length and collection count/item-length framing, presented as `rci0_` plus unpadded base64url | Accepted | Exact tags, framing, fixed order, a root digest that transitively commits every root member, and an untruncated algorithm remove cross-implementation ambiguity and collision risk while keeping a recognizable portable textual identity | Terminology; Active Project Profile Specification and Profile Resolution Specification Input; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
...
| 015-127 | Make Conformance Suite revision `"0"` a self-contained content-addressed closure of five artifact families—Suite Index schema/index, construction root and authorities, Case Schema and five manifest kinds, Registry Package, and fixtures—with explicit `{id, path, digest}` ordinary artifact references, an additional Root Package `canonicalDigest`, and no directory scan, default, or network lookup | Accepted | Independent implementations can reproduce construction, boundary, resolver, handoff, and conditional cases only when every executable body is explicitly named and integrity-checked from one finite root while the Root Package's transitive construction identity remains independently verifiable | Conformance and Fixtures; Implementation Phasing |
...
| 015-130 | Define seven named Implementation Phases in dependency order, distinguish them from runtime resolution phases and PR boundaries, require Phases 1 through 6 for revision-`"0"` core resolver support, and reserve Phase 7 for product integration | Accepted | Stable phase identities let implementation plans, parallel work, readiness claims, and review scopes refer to the same design-owned progression without treating an experimental slice or one PR as completion of a conforming resolver | Implementation Phasing |
| 015-131 | Define phase-separated local-first profile-resolver benchmarks with 015-owned phase/cost boundaries, workload vectors, semantic checksums, duration and peak-memory observations; keep resource admission separate from physical performance; gate benchmark integrity and determinism rather than revision-`"0"` numeric values; and delegate common comparison and numeric regression policy to 026 | Accepted | Resolver performance must be attributable and observable from the first implementation slice without making machine-dependent measurements semantic, confusing logical safety limits with speed, or waiting for the cross-target measurement specification | Ownership and Dependencies; Performance and Measurement; Conformance and Fixtures; Implementation Phasing; Relationship to Other Documents |
| 015-132 | Apply 026 revision-`"0"` observational measurement, storage/reuse constraints, projection-ready records, conditional profiling verification, and owner-phase verification requirements without changing resolver semantics or the seven-phase structure | Accepted | Initial implementations retain reusable measurement data and safe ownership while shared evaluators, optional diagnostics, and later numeric policy do not become unrelated prerequisites | Ownership and Dependencies; Performance and Measurement; Implementation Phasing; Relationship to Other Documents |
...
- shared performance implementation, profiling, measurement/evidence model definitions, cross-target result comparison, target budget evidence, any Intlify-wide numeric regression threshold, cross-target logical-render equivalence, and conformance evidence: 026; 015 adopts the applicable initial measurement and verification requirements while retaining its profile-resolver benchmark phase/cost vocabulary, boundaries, workload vectors, semantic checksum observations, and physical storage/reuse choices;
...
| [026 — Conformance and measurement](./026-intlify-conformance-and-measurement-design.md) | Supplies shared performance implementation constraints, profiling and isolation requirements, and the initial observational measurement projection adopted by 015. Owns common metadata and units, evidence admission, cross-target comparison and reporting, target budgets, Intlify-wide numeric regression policy, logical-render equivalence, and capability/conformance evidence. It does not redefine 015-owned benchmark boundaries, workload vectors, or semantic checksums and has no independent implementation phase that blocks 015. |
