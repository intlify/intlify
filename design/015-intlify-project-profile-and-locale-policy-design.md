...
- Define one closed Profile Resolution Specification Set so admission, profile semantics, reproducibility, and staleness use the same exact toolchain-owned specification members.
...
| `intlify_config` crate | Owns the authoring model, configuration-schema version, JSON Schema generation, dependency-aware structural analysis, complete structural admission, Safe Origin Kind/Check/Finding Registries and origin-kind admission, configuration-source, Profile Selector, and Resolver Input evidence projection and retained-identity validation, profile selection, semantic resolver, closed Profile Resolution Specification Set admission, canonicalization-provider and Profile Resolution Artifact Set input boundaries, configuration Findings, checked profile IR, Resolver Conformance Suite harness, producer-side Handoff Vector validation, and traceability check without embedding CLDR-derived data or acquiring artifacts |
...
| Profile Resolution Specification Set input | Supplies the exact closed toolchain-owned specification identities/revisions that govern one resolver invocation; configuration and product adapters cannot add, remove, or override members |
...
| Profile Selector Evidence Projection | Closed non-semantic retained representation of the external selector slot; it is exactly `absent`, `safe-profile-id`, or `redacted-value` and never exposes an arbitrary rejected selector value |
| String Length Fact | Selector-evidence-only disclosure fact that is absent for a non-string and is exactly `exact(n)` for one completely counted in-bound UTF-8 string or `over-limit` after bounded inspection crosses the applicable limit without retaining the observed final length |
...
| Entry Resolution Evidence | Entry-specific projection containing entry-admission Findings and evaluation status, Finding Occurrences, Coverage Decision Evidence, the source-evidence index needed to resolve them, entry-source and selector-origin dependency records, and no profile semantics |
...
| Diagnostic reporting domain | One of two independently bounded reporting and record-admission domains: `entry` owns Entry Findings and evaluation status, every retained Finding Occurrence, Coverage Decision Evidence, source-evidence-index records, entry-source and selector-origin dependency records, and shared-unavailability cause references; `shared` accounts for pre-deduplication Shared Finding candidates and owns Shared Findings and evaluation status, Coverage Decision Basis, semantic dependency records, and materialized re-resolution dependency records; each domain owns its reserved terminal reporting records, and exhaustion in one domain cannot truncate records in the other |
...
| Bootstrap admission | Pre-policy, implementation-capability-bounded processing that may materialize and structurally analyze input, provisionally select one Profile ID, and admit only the selected resource-limit reference and artifact needed to establish the semantic Resource Limit Policy; it creates no checked profile fact |
...
| Entry Admission Input Set | Inputs required to reproduce entry-specific admission and selector-origin evidence and, by itself before materialization or together with the Materialized Resolution Input Set afterward, the complete Entry Resolution Evidence and Finding Occurrence Keys: raw configuration bytes submitted for strict UTF-8 admission, exact logical source identity, Source-content revision, Entry Source Evidence, selector evidence, parser specification, and bootstrap capability for file input; or submitted host value, frontend identity/version, revision-`"0"` programmatic-entry rules, exact logical source identity, Source-content revision, Entry Source Evidence, an exact immutable UTF-8 call-site source snapshot when an admitted span is retained, selector evidence, and bootstrap capability for programmatic input |
| Materialized Resolution Input Set | Finite immutable set of the JSON-compatible configuration value, selector, configuration schema revision, complete Profile Resolution Specification Set, Locale Canonicalization Data Artifact, complete submitted Profile Resolution Artifact Set including unreferenced artifacts, and resource and implementation-capability admission needed to reproduce shared structural and semantic resolution |
| Canonical resolution traversal | Revision-`"0"` logical order used for deterministic accounting, subject enumeration, and check scheduling after materialization; it is independent of JSON object-member order, filesystem enumeration, submitted set order, and worker completion |
| Profile Resolution Specification Set | Closed finite immutable toolchain-supplied resolver input containing exact identity/revision slots for the Localization Project Profile, Locale Canonicalization, Locale Negotiation, Message Locale Fallback, Coverage, Intent Surface-Class Vocabulary, Delivery Graph, and Delivery Placement Policy specifications; configuration and host APIs cannot select or override its members |
...
| Finding Key | Entry-location-independent semantic identity of one Finding derived from the owning specification and revision, phase ID, check ID, code, Redacted subject token, and reason; source evidence, message text, raw subject content, and an unordered collection's authoring position are excluded, while a specification-defined ordered sequence position may be semantic |
| Finding Occurrence Key | Identity of one diagnostic occurrence derived from a Finding Key plus the admitted, disclosure-safe primary evidence identity; it supports editor and incremental tracking without becoming a suppression cause or shared semantic identity and cannot recover an excluded source value |
...
Structural Analysis Result
  + IntlifyConfig when structurally complete
  + Profile ID selector
  + admitted semantic artifacts
  -> checked | blocked
```

Before shared structural admission, the input is a JSON-compatible configuration value, not `IntlifyConfig`. File entry rejects malformed UTF-8, invalid JSON, duplicate object members, strings or keys outside the Unicode-scalar domain, and numbers outside the Portable JSON Number domain before ordinary schema validation. A programmatic frontend cannot represent raw-token errors, but it must admit host input into the same string, number, array, object, and finiteness domain without implicit omission or coercion. After both entry paths succeed, configuration-version admission, schema-guided structural analysis, named-profile selection, semantic resolution, and Finding semantics are shared.
...
- the eight required Profile Resolution Specification Set slots, each at admitted identity and revision `"0"`;
...
- exactly the five referenced required Policy artifacts and two referenced Target Profile artifacts above in the Profile Resolution Artifact Set;
...
| 4 | `profile-resolution-specification-set` | `specification-input` | Complete explicit Profile Resolution Specification Set input |
| 5 | `locale-canonicalization-data` | `artifact-input` | Explicit Locale Canonicalization Data Artifact provider input |
| 6 | `profile-resolution-artifact-set` | `artifact-input` | Complete explicit Profile Resolution Artifact Set input and its member artifacts |
...
After file materialization, Materialized Value Evidence adds the required Portable Value Locator and retains a Portable Source Span when a corresponding token exists. A missing-member Finding uses the nearest safely locatable owning object followed by the schema-defined missing-field segment. An invalid member or value points to the applicable key or value token; an unsafe dynamic key uses a `redacted-member` segment while its admitted span remains actionable. A cross-field Finding may have one primary evidence item and related evidence for the other relevant locations.
...
- `safe-profile-id` contains the exact ASCII bytes of a selector that passed Profile ID syntax and applicable byte-bound admission. It is permitted whether or not that ID names a declaration, but only when the applicable Finding Registry disclosure policy classifies the complete Profile ID as non-secret.
- `redacted-value` represents every syntactically admitted selector whose complete Profile ID is not classified non-secret and every type-invalid, syntax-invalid, over-limit, or otherwise arbitrary selector value. It contains one canonical JSON type tag, an applicable String Length Fact, and one Redacted evidence token over only those safe facts. The raw value, container contents, scalar payload, and their direct digest are excluded.

The selector JSON type rank is `null`, `boolean`, `number`, `string`, `array`, then `object`. A non-string `redacted-value` has no String Length Fact. A string has exactly one of `exact(n)`, when its complete UTF-8 byte length `n` was established within the applicable admitted bound, or `over-limit`, when bounded inspection established only that the bound was crossed; `over-limit` never retains the observed final length. String Length Fact order is absence, `exact(n)` in ascending numeric order, then `over-limit`. A binding rejects a non-JSON host selector before resolver invocation rather than executing user behavior or inventing a JSON type.
...
- `profile-resolution-specification-set` permits the empty root or one `fixed-role` segment for `project-profile`, `locale-canonicalization`, `locale-negotiation`, `message-locale-fallback`, `coverage`, `intent-surface-class-vocabulary`, `delivery-graph`, or `delivery-placement`, with respective zero-based ranks `0` through `7`;
...
- `profile-resolution-artifact-set` permits the empty root or zero-ranked `fixed-role("artifact-member")` followed by exactly one `safe-key` or `redacted-key`; an admitted artifact uses its owning specification's canonical exact-reference identity as the source key, while an envelope lacking that tuple uses the 017-supplied stable content-derived admission key before disclosure projection; and
...
### Profile Resolution Specification Set

Every resolver invocation receives exactly one finite immutable Profile Resolution Specification Set from the admitted toolchain. Revision `"0"` defines these required named slots:
...
The slot set is closed. Every slot occurs exactly once; a missing, duplicate, unknown, unsupported, or mutually incompatible member is blocking before member-dependent semantic resolution. Slot order and physical container representation are non-semantic. Configuration fields, environment variables, host APIs, product adapters, and Target Profiles cannot select, replace, append, or downgrade a member.

Configuration schema revision is not a member because root `schemaVersion` selects that independent admission domain. The Locale Canonicalization Data Artifact remains a separate input admitted against the set's canonicalization member. Policy and Target Profile specification revisions remain inside their exact references and artifacts. Check and Finding Registries are bound by the project-profile member. Implementation capability, package version, physical engine version, provider schema, and artifact representation are admission or implementation facts rather than specification-set members.

Every set member belongs to the Materialized Resolution Input Set and to resolution-staleness tracking. The members retained by the closed canonical semantic inventory participate individually in profile equality; the set container does not add a second aggregate semantic identity. Replacing only a physical container while preserving every exact slot value is non-semantic, while changing any member identity or revision requires re-resolution and changes the applicable canonical profile field.
...
- ordered source-independent structural and semantic Findings;
...
- materialized re-resolution dependency records for configuration value content, selector value, schema, specification, canonicalization-data, artifact-set, Resource Limit Policy, and implementation-capability inputs whose dependency identity or rejected admission state was established.

Its Entry Resolution Evidence projection contains:

- ordered entry-admission Findings;
- deterministic entry-admission evaluation status, including causal suppression before materialization;
- retained Finding Occurrences and their Finding Occurrence Keys;
...
- entry-source identity/content-revision and selector-origin dependency records required to reproduce presentation, watching, and entry-specific invalidation.
...
The complete checked-path semantic resolver input is `IntlifyConfig`, the Profile ID selector, one complete Profile Resolution Specification Set, one matching Locale Canonicalization Data Artifact, this Profile Resolution Artifact Set, and implementation-capability admission. A structurally blocked path may evaluate only checks supported by its Structural Analysis Result and can never reach profile projection. Every artifact is finite, immutable, and already acquired before resolution. Exact reference matching uses artifact kind, identity, revision, specification revision, and digest. The canonicalization data artifact remains a separate provider input and is not inserted into either set. Revision `"0"` enumerates the admitted specification slots and artifact kinds; an unknown member is not a generic extension.
...
2. the resolver provisionally selects exactly one declaration under those limits and resolves only its required `resourceLimits` reference sufficiently to admit the referenced Resource Limit Policy artifact;
3. the resolver checks that the admitted policy does not require capacity beyond the declared implementation capability;
4. the resolver reapplies `configurationInput` and `artifactAdmission` bounds to the complete already submitted inputs and independently reaccounts every already produced Entry-owned and Shared-owned record under `diagnostics.entry` and `diagnostics.shared`, including the selector, all profile declarations, every artifact and reference occurrence, and the resource-limit reference and artifact themselves; and
...
`configurationInput` bounds the complete shared materialized root after either entry path. The external Profile ID selector is the sole non-root value rechecked by this group. Implementation capability first protects the same work during bootstrap; after the selected Resource Limit Policy is admitted, every row is reapplied to the already materialized input before any post-policy semantic phase:
...
| `maxProfileIdBytes` | Every declared Profile ID and a present external selector | UTF-8 byte length of one raw ID | Bootstrap before ID syntax admission and provisional selection; policy recheck immediately after policy admission | Each declaration key and the selector are checked independently |

Raw file bytes and parser work are protected by the 018-owned bootstrap envelope rather than this semantic group. The same logical value and selector receive the same `configurationInput` accounting through file and programmatic entry paths. Successful policy recheck confirms the provisional selector result without rerunning selection under a different declaration set; failure leaves the outcome blocked and the provisional result unavailable to consumers.
...
`artifactAdmission` bounds the complete submitted Profile Resolution Artifact Set before referenced/unreferenced filtering. Bootstrap capability first bounds collection inspection and admission of the selected resource-limit reference and artifact; the admitted policy then rechecks the complete collection and every selected-declaration reference occurrence, including those two bootstrap inputs, before any remaining exact-reference resolution:

| Bound | Scope | Counting unit | Stage | Duplicate handling |
| --- | --- | --- | --- | --- |
| `maxArtifacts` | Complete submitted set | One submitted artifact envelope | Bootstrap before resource-policy lookup; policy recheck before remaining kind, identity, or reference filtering | Every submitted envelope counts |
| `maxPolicyArtifacts` | Complete submitted set | One artifact declaring an admitted Policy kind | Bootstrap before resource-policy lookup; policy recheck before remaining reference filtering | Every submitted Policy envelope counts |
| `maxTargetProfileArtifacts` | Complete submitted set | One artifact declaring Target Profile kind | Bootstrap before resource-policy lookup; policy recheck before remaining reference filtering | Every submitted Target Profile envelope counts |
...
| `maxTotalArtifactCanonicalBytes` | Complete submitted set | One canonical byte across submitted artifacts | Bootstrap during bounded resource-policy lookup; policy recheck in canonical artifact identity order | Duplicate/conflicting envelopes and the resource-policy artifact still contribute before rejection |
...
| Bound | Scope | Counting unit | Stage | Duplicate handling |
| --- | --- | --- | --- | --- |
| `maxFindingOccurrences` | All ordinary Entry projection Finding-occurrence candidates, including occurrences of Shared Findings | One candidate occurrence | Before Finding Occurrence Key deduplication | Every candidate counts |
| `maxFindings` | Canonical entry-admission Finding set | One distinct Entry Finding Key | After Finding Key deduplication, before Entry projection commit | Equal keys merge and count once |
| `maxRelatedEvidencePerFinding` | One Entry projection Finding-occurrence group | One distinct related-evidence record | After evidence merge and deduplication | Equal evidence records count once |
...
| `maxRecords` | All retained ordinary Entry-owned records | One Entry Finding, status, Finding Occurrence, Coverage Decision Evidence record, source-evidence-index record, entry dependency, or shared-unavailability cause reference | After kind-specific deduplication and canonical owner selection, before Entry projection commit | Equal records count once only where their kind defines deduplication; repeated references remain distinct records |
| `maxTotalCanonicalBytes` | All retained ordinary Entry-owned records | One byte in the 019-defined canonical resource-accounting representation | After kind-specific deduplication and canonical owner selection, before Entry projection commit | Every retained record contributes its complete framed bytes; repeated content in distinct records contributes again |
...
- the Profile Resolution Specification Set uses `project-profile`, `locale-canonicalization`, `locale-negotiation`, `message-locale-fallback`, `coverage`, `intent-surface-class-vocabulary`, `delivery-graph`, and `delivery-placement` in that exact order;
- the Locale Canonicalization Data Artifact and external Profile ID selector are singleton subjects in their applicable algorithm phases;
- admitted artifact envelopes are ordered by artifact-kind bytes, opaque identity bytes, exact revision bytes, specification-revision bytes, and semantic-content-digest bytes;
...
The content-derived admission key is an ordering and occurrence-admission fact, not artifact semantic identity. It cannot make an invalid envelope referenceable or enter profile equality. Physical collection order, filesystem path, registry response order, cache location, and acquisition timestamp are never ordering keys. Byte-identical repeated envelopes may share the content-derived key because their contributions commute; distinct occurrence evidence is used only for deterministic occurrence reporting.
...
3. `profile-selection` provisionally applies the external Profile ID selector under bootstrap capability when the `profiles` container, applicable Profile IDs, selector, selected declaration boundary, and `resourceLimits` reference fragment are independently admitted, and otherwise records dependent work as not evaluated with its causal Finding Keys;
4. `resource-policy-admission` resolves only that provisional declaration's exact resource-limit reference under bootstrap capability, admits the Resource Limit Policy, checks implementation capability, reapplies `configurationInput` and `artifactAdmission` bounds to complete pre-policy inputs, independently reaccounts pre-policy records under `diagnostics.entry` and `diagnostics.shared`, and confirms the provisional selection when policy/capability admission and every non-diagnostic protected-input recheck succeeds even if a diagnostic domain has already fixed an incomplete blocked result;
5. `specification-admission` admits every required slot of the closed Profile Resolution Specification Set and one Locale Canonicalization Data Artifact matching its `locale-canonicalization` member under the confirmed resource-policy envelope;
...
15. `reporting-finalization` verifies already selected Evidence record owners, deduplicates and canonically orders the retained Findings, Occurrences, evidence, evaluation status, Coverage Decision Basis, Coverage Decision Evidence, and dependency records within their owning reporting domains, verifies exact-cover Basis-to-Evidence correspondence when both domains are complete, fixes the bundle-level Resolution Evidence completeness and canonical domain-terminal-state set plus either the present Shared projection or its canonical unavailability causes, and returns exactly one checked or blocked outcome; it cannot create an ordinary record, change an owner, repair a missing or orphaned mapping, or move a record between domain slices.
...
| `project-profile-specification-set-invalid` | `missing-slot`, `duplicate-slot`, or `unknown-slot` for the closed Profile Resolution Specification Set |
| `project-profile-specification-not-admitted` | The present `project-profile` member is `unsupported` or `incompatible`; a missing slot uses `project-profile-specification-set-invalid` |

Revision `"0"` owns these locale/default codes:

| Code | Blocking | Stable reasons or notes |
| --- | --- | --- |
| `project-profile-locale-specification-not-admitted` | yes | Present canonicalization member unsupported or incompatible; a missing slot is a specification-set Finding |
...
| Code | Stable reasons or notes |
| --- | --- |
| `project-profile-negotiation-specification-not-admitted` | Present portable-lookup member unsupported or incompatible; a missing slot is a specification-set Finding |
...
| `project-profile-message-fallback-specification-not-admitted` | Present fallback member unsupported or incompatible; a missing slot is a specification-set Finding |
...
| `project-profile-coverage-specification-not-admitted` | Present Coverage Specification or Intent Surface-Class Vocabulary member unsupported or incompatible; `details.component` distinguishes them, while a missing slot is a specification-set Finding |
...
| `project-profile-delivery-specification-not-admitted` | Present graph or placement member unsupported or incompatible; `details.component` distinguishes them, while a missing slot is a specification-set Finding |
...
Source identity, Source-content revision, Portable Value Locator evidence, transient exact pointer, Portable Source Span, primary evidence identity, human message text, emitted-list index, raw or unprojected subject content, an unordered collection's authoring index, and related-evidence order are excluded. A semantic ordinal selected by `semantic-position` remains inside the projected Logical subject key because changing it changes the ordered policy. Every Finding candidate has a Finding Occurrence Key composed of its Finding Key plus the admitted disclosure-safe primary evidence identity. For Materialized Value Evidence, that identity contains only the retained Portable Value Locator, optional admitted Portable Source Span, and other admitted evidence fields, never Source-content revision, the transient exact pointer, or a rejected span candidate; the source-evidence-index identity follows the same rule. Excluded raw identity, locator, selector, member-name, value, revision, or rejected span coordinates never enter either key. Findings with the same Finding Key are deduplicated: the lowest registry-permitted candidate primary evidence in canonical evidence order becomes the emitted primary evidence, and every other distinct candidate or related evidence record is merged as related evidence in canonical order. Identical invalid members of an unordered collection, and distinct values intentionally grouped by one redacted-structure projection, therefore produce one Shared Finding and distinct Entry Finding Occurrences whenever their admitted occurrence evidence differs. Suppression causes refer only to Finding Keys and cannot recover unprojected subject content.

Equivalent file and programmatic semantic failures therefore have the same Finding Key and suppression dependency but may have different Finding Occurrence Keys. A presentation-evidence-only edit, including reordering an unordered collection, can change occurrence identity, Portable Value Locator, or admitted Portable Source Span without changing its Logical subject key, semantic diagnostic identity, or profile semantics. 019 owns the common envelope, the retained occurrence identities, and incremental occurrence tracking, while 017 owns canonical encoding and digest framing for these records.
...
Resolution Evidence records these relationships without changing them. A semantic dependency record links one established canonical semantic fact or checked absence state to the exact admitted inputs used to derive it. A materialized re-resolution dependency identifies configuration value content, selector value, schema, specification, canonicalization-data, artifact-set, Resource Limit Policy, or implementation-capability input needed to decide whether shared resolution is stale. An entry re-resolution dependency identifies the admitted exact logical source identity/Source-content revision pair or selector origin needed to rerun entry admission or update presentation. Semantic dependencies support invalidation slicing; both re-resolution classes schedule their applicable resolver boundary. Missing exact identities or revisions are never replaced by paths, timestamps, object addresses, random invocation values, or adapter guesses, and an unevaluated dependency remains represented only through evaluation status until its identity can be established.
...
Resolution staleness is tracked separately from equality. The initial implementation treats the exact configuration logical source identity/Source-content revision pair and selector origin as Entry re-resolution dependencies. A revision change always schedules Entry re-resolution, while an equal exact pair permits source-level reuse only when the adapter has satisfied the byte-snapshot invariant; observing different bytes under an equal pair is an adapter conformance failure and forbids reuse. It treats the complete JSON-compatible configuration value content, configuration `schemaVersion`, Profile ID selector value including omission, every exact member of the complete Profile Resolution Specification Set, the Locale Canonicalization Data Artifact, every referenced semantic artifact, complete submitted Profile Resolution Artifact Set admission envelope, Resource Limit Policy admission, and applicable implementation-capability admission as materialized re-resolution dependencies. A change to either class schedules its applicable entry or shared resolution boundary. Replacing only a specification-set container while preserving every member identity/revision does not create a distinct materialized dependency value.
...
Entry Admission Input Set
  file:
    ...
    + selector evidence + bootstrap implementation capability
  programmatic:
...
```json
{
  ...
  "inputs": {
    "profileSelector": {},
    "profileResolutionSpecificationSet": {},
...
- every Safe Origin Kind Registry row and Check/Finding Registry row, closed origin/evidence compatibility, subject-key strategy and scope, Subject content projection, evidence-disclosure policy and non-secret classification, allowed code/reason combination, semantic Finding Key, entry-specific Finding Occurrence Key, canonical semantic/evidence order, deduplication, and atomic domain-specific terminal cursor;
...
- pinned `intlify.project-profile.evidence` token vectors for every Safe Origin Kind, evidence kind, and disclosure policy, using exact origin ID bytes, complete 256-bit non-truncation, origin separation, deterministic injected collision ordinals, and Finding Occurrence Keys derived only from admitted evidence identities;
- Profile Selector Evidence Projection vectors for `absent`, non-secret syntax-and-bound-admitted `safe-profile-id`, secret syntax-and-bound-admitted `redacted-value`, and type-invalid, syntax-invalid, over-limit, and unknown selectors, with variant order `absent`, `safe-profile-id`, `redacted-value`, exact ASCII safe-ID ordering, and no literal invalid selector;
...
- every allowed Resolver Input Component Path shape: root and eight fixed specification slots, canonicalization-data root, artifact-set root and safe/redacted artifact-member keys, and implementation-capability root and fixed group/bound paths, with no additional role, key, or path shape;
...
- non-string redacted selectors retaining no String Length Fact, in-bound strings retaining only `exact(n)`, over-limit strings retaining only the `over-limit` fact, and non-JSON host selectors failing binding admission before resolver invocation without executing user behavior or fabricating resolver Evidence;
...
- one complete Profile Resolution Specification Set containing exactly the eight required slots, with missing, duplicate, unknown, unsupported, and incompatible member cases blocked before member-dependent resolution;
...
- a missing `project-profile` slot producing the closed-set Finding, and a present unsupported or incompatible Profile Specification identity/revision producing its member-specific blocking Finding;
...
- equal Materialized Resolution Input Sets, for invocations that reach materialization, producing the same entry-independent structural/semantic resolution and Shared Resolution Evidence across conforming implementations, plus byte-identical canonical profile projection and binding domain whenever both complete bundles expose checked outcomes;
...
- malformed JSON, duplicate members, invalid programmatic host values, ancestor cycles, and bootstrap snapshot-limit failures producing a complete blocked Resolution Evidence bundle with entry-admission Findings and evaluation status, `shared: null`, canonical shared-unavailability cause keys, and no Materialized Resolution Input Set;
...
- a missing `locale-canonicalization` slot rejected as an invalid specification set and present unsupported or incompatible Locale Canonicalization Specification identities rejected without host fallback;
...
5. materialize the complete Normative Resolver Registry Package; establish Entry and Materialized evidence stages, Safe Origin Kind admission, Profile Selector Evidence Projection and Resolver Input Component Path construction, semantic Finding and occurrence identity, dependency-aware Finding collection, atomic evaluation status and reporting limits, exact resource accounting, profile equality, and re-resolution staleness;
...
| 015-048 | Separate the entry-independent semantic Finding Key from the evidence-bearing Finding Occurrence Key; define complete machine-readable Check and Finding Registries; and order evaluation units, Evidence record owners, atomic per-domain reporting slices, and deduplicated Findings by fixed phase, subject, check, code, reason, and key ranks | Accepted | Stable suppression and paired-entry conformance require evidence-independent causes, while editors still need occurrence identity; complete registries, canonical ownership, and total per-domain commit order remove implementation-defined diagnostics and partial-slice reporting | Terminology; Deterministic Resolution Algorithm; Findings and Failure Model; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures; Implementation Phasing |
...
| 015-054 | Resolve the Resource Limit Policy dependency cycle through implementation-bounded provisional profile selection, bootstrap admission of only the selected `resourceLimits` reference and artifact, mandatory policy recheck of the complete root and artifact/reference inputs before confirming selection, and independent reaccounting of retained Entry/Shared reporting records | Accepted | The selected profile must supply the semantic limits without allowing an untrusted policy to bound its own admission, while every protected occurrence—including the policy itself—must still be counted, reporting exhaustion must not change Shared input selection, and no alternate profile or implicit default may authorize resolution | Terminology; Provider, Governance, and Glossary References; Deterministic Resolution Algorithm; Findings and Failure Model; Security and Credential Handling; Conformance and Fixtures |
...
| 015-057 | Require one closed Profile Resolution Specification Set with exact project-profile, canonicalization, negotiation, fallback, coverage, Intent surface-class vocabulary, delivery-graph, and delivery-placement identity/revision slots as an explicit Materialized Resolution Input Set dependency | Accepted | A single closed input removes omitted or adapter-selected semantics from specification admission, makes reproduction and staleness enumerate the same dependencies, and preserves independent configuration-schema, data-artifact, referenced-artifact, and implementation-capability domains | Terminology; Ownership and Dependencies; Canonical Configuration Input and Resolution; LocalizationProjectProfile Semantic Model; Deterministic Resolution Algorithm; Dependency, Invalidation, and Reproducibility; Conformance and Fixtures |
