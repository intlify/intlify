...
- `input_read_failed`:
  ...
  The top-level `path` is the input file path. Phase 3B reason values are `not_found`, `permission_denied`, `not_file`, `not_directory`, `invalid_utf8`, and `unknown`. The shared CLI physical-grouping boundary adds `metadata_failed` when physical-file identity inspection fails before reading a selected target; its required `ioKind` and optional `rawOsError` fields are owned by [005-ox-mf2-phase-3-tooling-transport-design.md](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-file-execution-boundary). The 005 project-inventory boundary separately adds `source_changed` when it cannot establish one authoritative current filesystem-participant snapshot because bytes, physical identity, or the checked read changed during acquisition. It applies to definition, selected built-in producer-source, and configured external reference-artifact groups. That synthetic reason carries neither `ioKind` nor `rawOsError`, is not retried automatically by the acquisition boundary, and is not emitted by Phase 3B operand-driven file mode. Tier 3 XLIFF adds `unsupported_encoding` for its explicitly recognized UTF-16 BOM cases when that adapter ships; the value is not emitted before then. `invalid_utf8` reports input bytes that are not valid UTF-8 and requires `details.validUpTo` as a non-negative JSON integer equal to the byte length of the longest valid UTF-8 prefix. This shape is shared by file and stdin inputs; the source is not parsed and the target is reported as a file-specific operational error.
...
- `output_write_failed`:
  ...
  The top-level `path` is the output file path. Stable reason values are `not_found`, `permission_denied`, `not_file`, `not_directory`, and `unknown`. Input-only reasons such as `invalid_utf8`, `metadata_failed`, `source_changed`, and `unsupported_encoding` never apply to this code.

  For `ignore_file_read_failed`, `input_read_failed`, and `output_write_failed`, an underlying operating-system I/O error adds `details.rawOsError` only when the OS exposes a raw error code; it is serialized unchanged as a signed JSON integer. Synthetic decoding, classification, or freshness reasons such as `invalid_utf8`, `source_changed`, and `unsupported_encoding` do not carry `rawOsError`. Portable behavior must branch on `reason`, never on the platform-specific number.
