...
The initial resource catalog work is an unnumbered layered milestone that starts only after the Phase 3C linter product sequence is complete and finishes before Phase 3D begins its opted-in catalog editor path. Phase 3B and Phase 3C continue to ship and validate their standalone message-level products without resource implementation in their product PR sequences. Phase 3D then consumes the completed shared layer rather than defining temporary editor-local host adapters. The Tier 1 milestone is required tooling v0.1 scope; a tooling v0.1 release is not complete merely because the standalone Phase 3B and Phase 3C commands have shipped.

Implementation lands in this dependency order:

1. Add `crates/intlify_resource` with the consumer-neutral registry and artifact APIs, fixed resource limits and errors, Tier 1 JSON extraction, identity and offset mapping, candidate admission, validated write-back, thread-safety assertions, and crate-level fixtures. This crate remains independent of parser, formatter, and linter cores.
2. Extend the shared project config model and committed unified schema with `resources`; add the resource-owned validator, resolved catalog membership/format resolver, and CLI supported-input classification without yet duplicating message-level processing.
3. Integrate catalog formatting into `intlify fmt` through the Phase 3B bounded formatter path, shared physical-file scheduler, candidate validation, catalog result specialization, write/check behavior, and formatter end-to-end fixtures.
4. Integrate catalog linting into `intlify lint` through the Phase 3C message pipeline, the same classification and scheduler path, mapped catalog diagnostics, result specialization, and linter end-to-end fixtures.
5. Complete the resource benchmarks, worker-width memory gate, cross-command determinism tests, committed schema verification, and release evidence. Only after this gate is accepted may Phase 3D add project-configured or editor-overlay catalog consumption.

Each step may use more than one reviewable PR, but a later step must not introduce a second registry, config resolver, extraction path, or error conversion while waiting for an earlier owner. Standalone `.mf2` behavior remains usable throughout; partially landed resource internals do not make catalog files a supported public target before the corresponding CLI integration and complete milestone gate are present.
...
The first resource-catalog release also applies the shared worker-width memory gate from [005](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary). `MAX_CLI_WORKERS = 4` remains the provisional implementation candidate while width-one and width-four runs measure near-limit message-dense and structure-dense extraction, lint, and changed-format/write-back workloads under one documented peak-live-allocation or peak-RSS setup. The release records the measured envelope and explicitly accepts width four; otherwise, including when reliable measurements are unavailable, the internal ceiling is reduced to two or one and the same gate is rerun. No dynamic size-weighted scheduler, command-wide memory option, or new resource error is introduced by this gate. The selected constant remains an internal tuning value, while the fixed per-target resource limits and every observable result stay unchanged.
...
- the pre-release worker-width memory gate at widths one and four, using near-limit message-dense and structure-dense extraction, lint, and changed-format/write-back target sets under one documented peak live allocation or peak RSS setup; if width four is not explicitly accepted, the selected lower ceiling is rerun and recorded before release
...
- CLI scheduler tests at worker width one and multiple: runnable group counts below, at, and above the active production ceiling selected by the pre-release memory gate; bounded active physical groups; forced out-of-order group completion; stable normalized-path reporting; width-independent summaries and exit status; serial aliases with rereads; write-failure containment; continued unrelated groups; and injected initialization/dispatch/execute/join failures that produce the command-fatal empty-results envelope without serial fallback. Benchmark validation separately records the width-one/width-four near-limit memory envelope and, when four is not accepted, the rerun at the selected lower ceiling. Formatter tests also confirm that already completed writes are not rolled back. Resource-level determinism tests additionally cover stable entry ordering and lowest-raw-order operational-error selection when future chunked execution is enabled.
