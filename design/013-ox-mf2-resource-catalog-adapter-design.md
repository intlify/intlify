...
The resource layer resolves scope and locale identity as consumer-neutral entry metadata. Message-linker M0 projects them into mandatory `MessageDefinition.scope` and `MessageDefinition.locale` while using a locale-agnostic key union; M2 applies fallback semantics. The future catalog linter groups the same metadata with entry identity, then owns rule definitions, severity, and reporting. Scope and locale binding do not select a runtime locale, define locale fallback, change message text, or enable rules by themselves.
...
One locale-aware scope may contain entries from exactly one `CatalogKeyDomain`. Initial entry-level formatting and linting may still process a mixed-format catalog definition or a Vue artifact containing several inner domains because they do not compare `CatalogKey` values across entries or files. The first locale-aware consumer resolves domains from the successfully extracted entries before constructing linker definitions or running catalog rules. Zero participating entries contribute no domain and do not make an otherwise valid scope fail. A scope with entries from two domains is `config_validation_failed` with `details.reason: "catalog_scope_key_domain_conflict"`; the workflow does not silently partition it.
...
- Overlay-resolution fixtures cover resource-owned normalized-object validation, root and `/catalogs` pointer evidence, grammar equivalence with project catalogs, equivalent missing and empty overlay catalogs, private fields and infallible compilation, compile-time `Send + Sync` for `ResolvedCatalogOverlay`, one combined lookup per document, project-only and overlay-only matches with exact provenance, overlay fallback after project `PolicyAbsent` and `Unmatched`, fallback blocking after project `PolicyEmpty` and `Excluded`, authoritative project `Matched` resolution without path-level overlay evaluation, overlay-internal same-format de-duplication and different-format conflicts only in the fallback branch, deterministic same-layer source-qualified decisive references, canonical editor conflict fingerprints, config-path presence only for project-layer conflicts, and transition from overlay to project ownership without assignment reconstruction or duplicate extraction.
