...
When a 014 `message_project_link_e2e` case plans Tier 1 catalog extraction, the harness also emits exactly one independent companion record using the exact 013-owned `resource_extract` phase and `host_parse_and_entry_extraction` cost. It uses the [shared benchmark record envelope](./001-ox-mf2-toolchain-foundation.md#shared-benchmark-record-envelope), retains the parent's complete `BenchmarkCaseIdentity`, and names the one aggregate `message_project_link_e2e` / `complete_workflow` record as its `BenchmarkRecordRelation::Companion` parent. A different case identity, missing parent, duplicate companion, nested metric, or second 014-owned extraction phase makes the result document invalid.

The initial M0 companion covers only that case's sequential JSON physical-source-group extraction calls. Successful aggregation visits groups in the same canonical primary-source order used by project inventory and reports checked `u64` sums for:
...
- `elapsedNanoseconds`: the sum of only the non-overlapping measured `HostFormatRegistry::extract` intervals.

Discovery, metadata lookup, physical grouping, pre-extraction admission, definition projection, linker work, surrounding loop overhead, and file I/O outside the exact selected-snapshot extraction call are excluded. The parent end-to-end record retains the complete workflow wall time, so the companion is not subtracted from or folded out of that measurement.

Each successful group produces a BLAKE3-256 semantic checksum over a domain-separated, length-prefixed canonical observation containing its canonical physical-group identity, resolved host format, exact selected-source BLAKE3-256 fingerprint, and every public entry semantic value in raw entry order. It includes structural and catalog identity, optional display key, decoded message text, raw span, offset map, and read-only state; artifact-local handles, pointer identity, allocation layout, timing, cache state, and worker identity are excluded.

The companion semantic checksum is BLAKE3-256 over its domain tag, the complete case identity, the four aggregate counters above, and each `(physical-group identity, group semantic checksum)` pair in canonical group order. A successful companion requires all planned groups to succeed and forbids a prefix checksum. An extraction failure produces the shared failed-record shape with the canonical failing group evidence and no successful aggregate payload or semantic checksum; the related E2E record also cannot report successful completion.

The companion does not rename the phase to `message_resource_extract`, fold it into `message_definition_project`, or create a duplicate 014-owned extraction vocabulary. A later mixed-format or concurrent case must extend this 013-owned payload, interval, aggregation-order, and checksum contract before changing those semantics.

Extraction and re-escaping costs must be reported separately from parser, semantic, rule, formatter, and file I/O costs.

## Validation

- Cross-product benchmark fixtures require exactly one envelope-correlated `resource_extract` companion for each qualifying 014 parent; reject a different case identity, missing or ambiguous parent, duplicate companion, nested extraction metric, renamed phase, and successful parent after companion failure. Sequential JSON aggregation fixtures permute discovery and worker-ready order while preserving canonical physical-group order, exact checked group/host-byte/entry/time sums, group and aggregate BLAKE3-256 checksums, and exclusion of discovery, I/O, admission, projection, loop overhead, and linker time.
