...
The messages benchmark checks equality of the per-iteration checksum before aggregation and validates the serialized aggregate against related records in the current invocation. It never treats a checked-in or previous resource/messages result as a golden checksum; cross-invocation changes remain reportable observations under 014's benchmark policy.
...
- Messages benchmark fixtures cover every qualifying 014 project-workflow E2E phase/cost pair, require exactly one ordinary `resource_extract` / `host_parse_and_entry_extraction` record for each complete profile-declared E2E tuple, and reject a profile mapping to a missing, duplicate, or already-associated extraction tuple. They import and re-export the exact resource-owned `resource_host_parse_and_entry_extraction` descriptor with owner `013`, reject a missing, replaced, mutated, or locally declared descriptor for that pair, and require the same first/final markers and included/excluded marker order as the resource harness. They require equal fixture, variant, scale, runtime, and execution-model fields; reject mismatched pairing fields, a nested extraction metric, a renamed phase/cost, or a measured E2E record after extraction failure; and prove that neither the descriptor nor the checked one-to-one relation is serialized. Sequential JSON aggregation fixtures permute discovery and worker-ready order while preserving canonical physical-group order, exact checked group/host-byte/entry/time values, and group, per-iteration, and repetition-aggregate `u32` checksums; require equal per-iteration structural counts and checksums; validate the serialized wrapping aggregate against the measured iteration count; use shared JS/Rust vectors for source, group, entry, offset-map, record, and wrapping-overflow cases; prove that elapsed time, warmups, and benchmark metadata do not change either checksum input; and exclude discovery, I/O, admission, projection, checksum observation, loop overhead, and linker time from the extraction interval.
