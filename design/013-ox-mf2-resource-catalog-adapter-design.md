...
The 014 catalog-definition path is a separate read-only consumer. The `intlify_cli` project inventory forms a physical-source group, the host-owned pre-extraction stage admits the complete group, `intlify_resource` extracts that source exactly once, and the host-owned post-extraction stage performs checked conversion into one `MessageDefinitionArtifact`. This path neither invokes a message-level core nor requests re-escaping, and it does not move grouping, contract conversion, or linker analysis into `intlify_resource`.
...
These stages intentionally reuse the shared `config_validation_failed` code but are not one validation pass. Every structural reason above is selected before ignore setup or discovery and returns only the first error under the fixed shared and section-local order. `catalog_format_conflict` and `locale_capture_mismatch` are assignment-time failures because they require concrete selected paths. The coordinated M0 `catalog_binding_conflict` is selected after linker-participating logical assignments and physical-source groups are known but before extraction. The coordinated M0 `catalog_scope_key_domain_conflict` is later still: it requires successfully extracted entries from the complete definition inventory. A consumer must not move structural failures later, infer a domain from host-format spelling, or attempt static glob-intersection analysis merely to collapse these stages.
...
The host-owned pre-extraction stage then receives that already formed group. It requires equal namespace, canonical host-format id, scope, and locale bindings and admits the complete primary-plus-alias set under 014's path and alias limits. A configuration-derived binding conflict rejects the complete group through `config_validation_failed` before extraction and never weakens linker completeness to `SourceFailed`. A path or alias admission failure instead retains its owning operational classification. The CLI invokes `intlify_resource` once only after this admission succeeds; the host-owned post-extraction projection consumes that one successful extraction, performs the checked resource-to-contract conversion, and emits one complete definition artifact. Remaining aliases are ordered evidence rather than duplicate definitions. Physical identity helps only with CLI-local grouping and never enters the public artifact. Formatter and linter CLI workflows continue to retain per-alias results and the serialized reread/write behavior defined by 005/007 and this document.
