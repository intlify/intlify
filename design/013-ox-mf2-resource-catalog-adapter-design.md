...
This document tracks the detailed design of the shared resource catalog adapter layer for ox-mf2: extraction of MF2 message entries from multi-entry host files such as JSON catalogs, mapping between message text and raw host documents, write-back re-escaping, the CLI resource workflows that formatting and linting build on that layer, and the read-only extraction consumed by host-owned linker definition production.

The Phase 3 tooling boundary is defined in [005-ox-mf2-phase-3-tooling-transport-design.md](./005-ox-mf2-phase-3-tooling-transport-design.md). That document treats resource files, framework-specific i18n files, and multi-locale catalogs as layered consumers of the message-level formatter and linter. The formatter-side expectations are recorded in [007-ox-mf2-phase-3b-formatter-design.md](./007-ox-mf2-phase-3b-formatter-design.md#resource-and-catalog-formatting), the linter-side expectations in [008-ox-mf2-phase-3c-linter-design.md](./008-ox-mf2-phase-3c-linter-design.md#resource-and-catalog-linting), the editor-side consumption in [009-ox-mf2-phase-3d-lsp-editor-design.md](./009-ox-mf2-phase-3d-lsp-editor-design.md), and the host-owned definition-production boundary in [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md#catalog-definition-path). This document is the dedicated resource adapter design those documents defer to.
...
- **message text**: the exact Unicode-scalar MF2 string obtained by applying the selected host format's unescaping contract to one extracted host value; the input to message-level core APIs. The term describes that value conversion and does not claim that a particular runtime selects, shadows, or otherwise evaluates the entry.
...
- Define one consumer-neutral message entry model and host format adapter contract shared by CLI formatting, CLI linting, editor adapters, and host-owned linker definition production.
- Expose complete read-only extraction artifacts to the 014 definition-production boundary without introducing an `intlify_resource` dependency on `intlify_contract` or `intlify_linker`.
...
- Providing a general-purpose host-format validator, schema validator, language-service diagnostic surface, or host-format styling. Built-in adapters nevertheless own the complete span-preserving host parse and binary syntax-acceptance decision required for fail-complete extraction and candidate write-back validation; rejection is exposed only as the resource operational error `resource_parse_failed`. Detailed JSON, YAML, Vue, or XML syntax diagnostics, schema diagnostics, fixes, and document layout remain host-format-tooling responsibilities.
- Catalog-native structural rules such as duplicate concrete host keys or structural inconsistencies between entries that are already present in the initial milestone. The entry model must support them later, but their rule design is future work. Absence-based key parity, missing or orphaned translations, and unused-message analysis belong to the 014 linker track rather than this rule track.
...
The resource layer sits beside the message-level cores, not on top of them. It depends only on host-format parsing and produces message entries; it does not call the parser, formatter, linter, contract, or linker itself. The CLI fmt workflow, CLI lint workflow, and editor adapters compose extraction with per-entry message-level core calls and own their surface-specific presentation.

The 014 catalog-definition path is a separate read-only consumer. The `intlify_cli` project inventory forms physical-source groups, the host-owned pre-extraction stage admits every complete group, and `intlify_resource` extracts each admitted source exactly once. The host then aggregates resource-owned domain observations from the complete successful extraction set and runs one project-global domain admission stage before constructing any contract artifact. Its catalog-domain gate runs before its recognizer/root-domain gate. Only after both ordered gates succeed does the host-owned post-extraction stage perform checked conversion into one `MessageDefinitionArtifact` per successful group. This path neither invokes a message-level core nor requests re-escaping, and it does not move grouping, project-domain aggregation, contract conversion, or linker analysis into `intlify_resource`.

That ownership also applies to concurrency. Resource APIs are synchronous and thread-safe, while the initial CLI owns sequential physical-file-group processing and deterministic aggregation. A later CLI-owned worker runtime may invoke the same APIs concurrently under the deferred 005 contract. Editor adapters retain their separate document-lifecycle scheduling model.

The architecture overview intentionally collapses host adapter work into `extract` and re-escaping. Their complete fail-safe flows are expanded under [Extraction](#extraction) and [Write-Back Re-Escaping](#write-back-re-escaping). Re-escaping is not a consumer-callable operation by itself; it runs only inside the integrated validated write-back boundary. The diagram separately shows the 014 read-only path so that one resource extraction is not mistaken for a resource-to-contract dependency.
...
- Message text must be the exact unescaped string represented by that specific candidate host value. Adapters must not trim, normalize, or append to message text; per the [Phase 3B file framing contract](./007-ox-mf2-phase-3b-formatter-design.md#file-framing), message-level core APIs never receive an injected final newline or lose message-leading content. Entry inclusion and occurrence identity follow the adapter's syntactic extraction contract rather than runtime key-selection or shadowing behavior, so every representable duplicate occurrence retains its own message text even when a particular runtime would select only one occurrence.
...
- One `CatalogKey` contains at most `256` structural tokens. A token is one decoded domain path segment after the selected domain's separator, escaping, and typed-segment grammar have been validated; a domain-valid root key contains zero tokens. JSON and YAML produce one token per container edge from the root to the candidate, so the parser-local nesting limit proves this bound. A composed Vue entry copies the already bounded inner key. An XLIFF catalog key contains only the identity-bearing file/group/unit/segment ancestor tokens and omits its concrete final source/target side token, so the outer XML nesting limit also proves the bound. A future adapter must prove the same relation or enforce the bound independently before publishing an entry; adapter composition never adds a token outside the selected inner key.
- The shared artifact builder checks the incremented token count before retaining each decoded catalog-key token. The first otherwise representable candidate that would require token `257` fails the complete extraction with `resource_limit_exceeded`, `resource: "catalog_key_tokens"`, `limit: 256`, and `actual: 257`; it is never truncated, hashed, flattened, or admitted under a display spelling. This fixed source-side ceiling is coordinated with 014's separately owned `CatalogKeyTokens` artifact counter. The 014 host projection rechecks the contract value and may apply a caller-selected lower linker budget without changing this resource-owned maximum.

These resource-owned key, path, scope, and resolved-locale values are not Rust aliases or re-exports of the similarly named `intlify_contract` types. Neither crate depends on the other. The host-owned post-extraction projection reconstructs checked contract values from read-only resource payloads, exhaustively maps every comparable built-in `CatalogKeyDomain`, and converts `EntryKey` into the contract-owned `EntryReference`. It rejects `StandaloneMf2` for linker projection. No blanket infallible `From` / `Into` bridge may hide a newly added variant or a contract-limit failure; coordinated conformance fixtures require every resource-domain addition to update or explicitly reject the mapping.
...
A host format adapter is the format-specific component that receives an already resolved host format, parses and interprets that host syntax, maps it into message entries, and re-escapes formatted message text back into host raw text. Its parse owns the complete accept/reject gate needed to prove that extraction and generated write-back operate on valid syntax, including deterministic source location for `resource_parse_failed`. It does not expose a host AST, host diagnostic collection, recovery tree, schema result, fix, or styling API and is not a replacement for a JSON, YAML, Vue, or XML language service. Catalog matching, assignment, extension classification, and top-level `HostFormat` resolution occur before adapter dispatch and are not adapter operations. A composed outer adapter may resolve a nested declaration such as Vue `lang` only to select an inner adapter for an already selected outer host; that embedded declaration resolution does not reclassify the top-level catalog target.
...
HostFormatRegistry::resolve_direct_extension(
    &self,
    extension: &str,
) -> Option<ResolvedHostFormat>

HostFormatRegistry::supported_direct_extensions(
    &self,
) -> &'static [&'static str]
...
`CandidateMessageAdmission` is a resource-owned, target-local preflight for consumers processing formatter results in raw entry order. It retains only the next raw-order entry index, the running effective-message byte total, and the first admission state; it never owns message text. The consumer observes every artifact entry exactly once in raw entry order. `admit_original` selects the artifact's original message for an unselected, diagnostic-bearing, or read-only entry; a read-only handle is valid on this operation. `admit_formatted_bytes` selects a successful writable formatter output, rejects a read-only handle, and checks that entry's `message_bytes` before adding and checking `total_message_bytes`. `finish` succeeds only after every entry was observed without a limit failure. A foreign, repeated, skipped, or out-of-order handle, or a read-only handle supplied as formatted, is `internal_error` with `details.reason: "resource_invalid_entry_handle"` and `details.phase: "validate_write_back"`; these are consumer invariant failures rather than a second candidate ordering convention.

The admission methods construct the same typed `resource_limit_exceeded` value, original-entry site, and `phase: "validate_write_back"` details as integrated write-back. The initial consumer supplies the complete materialized formatter output length as the observed byte count, so the resource layer — not the formatter crate or CLI/editor adapter — owns the public error conversion and reports that exact observed length. Once admission returns its first error, that admission object is terminal and may be dropped; the consumer may continue ordinary formatter calls only to preserve formatter-operational-error precedence, without retaining their successful output for candidate construction. Successful admission is candidate-budget preflight, not write-back authorization: `build_and_validate_write_back` still validates the actual supplied strings and complete candidate independently. The deferred bounded formatter API may later replace an over-limit exact length with its first-byte-over signal without changing error ownership or admission ordering.
...
`resolve_direct_extension` is the separate config-free classification boundary for a consumer that has already established direct-file opt-in. Its input is the exact final extension spelling including the leading `.`, not a path. Lookup is ASCII case-insensitive, retains the supplied spelling in the returned `ResolvedHostFormat`, and succeeds only for a top-level adapter shipped in that release. An empty, unknown, or known-but-unshipped extension returns `None`; the method performs no content sniffing and does not produce `resource_format_unsupported`. `supported_direct_extensions` returns the canonical lowercase spellings accepted by that boundary in ASCII ascending order. The initial list is `[".json"]`. The CLI combines that list with its reserved standalone `[".mf2"]`, de-duplicates, and ASCII-sorts it for `unsupported_input_file.details.supportedExtensions` rather than duplicating registry knowledge.
...
`FormattedEntry` is the complete, successfully admitted formatter output for one selected writable entry, whether or not its text changed. A consumer omits an entry whose formatter result contains parser diagnostics, an unselected entry in an editor range request, and every read-only entry after retaining any parser diagnostics for reporting. After all selected formatter calls establish that no formatter operational error exists and candidate-message admission finishes successfully, the consumer passes all retained outputs together in one call to `build_and_validate_write_back`. An empty input is valid.

`build_and_validate_write_back` is the only consumer-callable re-escaping and candidate-construction boundary. It validates supplied handles in two deterministic passes rather than selecting an error from caller slice order. The first pass examines the complete slice for any handle whose artifact identity is foreign or whose entry index does not exist in this artifact. If at least one exists, the method returns `internal_error` with `details.reason: "resource_invalid_entry_handle"`, `details.phase: "validate_write_back"`, and no `entryKey`; which invalid numeric handle was encountered is intentionally unobservable. Only when every handle addresses an existing entry in this artifact does the second pass index them by artifact entry and walk entries in raw order. At each entry, duplicate supply is checked before read-only supply, and the first violation returns the same internal reason with that original entry's key. Valid inputs are then processed by artifact entry rather than caller slice order. Adapter-specific re-escaping and replacement application remain private implementation steps, so CLI and editor consumers cannot bypass candidate limit checks or full validation by calling a lower-level method.
...
Candidate validation first compares the candidate and original entry counts. A count mismatch is the document-wide `resource_write_back_failed` result and has no `entryKey`. Equal-length sequences are then compared in raw entry order. Within each pair, comparison order is `EntryKey`, `CatalogKeyDomain`, `CatalogKey`, optional `displayKey`, `readOnly`, then effective message value. The first mismatch returns `resource_write_back_failed` with the corresponding original entry's key, including when its candidate `EntryKey` differs. While the `FormattedEntry` borrows are still valid inside `build_and_validate_write_back`, the effective expected message is the caller-supplied formatted message when present, including a byte-identical one, otherwise the original artifact message. The borrowed lookup is discarded before return and is never copied into `RawReplacement`. Future consumer-visible entry metadata, such as intrinsic locale when it becomes part of an artifact, is equality-checked by default; the design that introduces a field must explicitly place it in this fixed candidate-comparison order and justify any candidate-time change rather than inheriting either accidentally.
...
4. **Entry construction.** For each candidate, the shared artifact builder derives structural and catalog identity, validates the catalog key's decoded structural tokens against the fixed `256`-token ceiling, assigns occurrence in raw order, unescapes the candidate's exact per-entry message text, constructs the raw value span and offset map, records read-only state, and updates the cumulative budgets and interner. A composed outer adapter supplies absolute source bases to the same builder rather than publishing an inner artifact.
...
- `CatalogKey` represents the runtime-style logical message independently of a concrete host role. JSON and YAML set it to the same serialized value as `StructuralPathKey`; formats whose host identity contains locale-role syntax may derive a different value under an explicit format contract. Future cross-locale logical grouping uses the explicit scope, `CatalogKeyDomain`, and `CatalogKey`; locale identifies one definition within that group rather than becoming part of the group identity. After linker scope mapping, the resolved scope replaces the declared scope in that same tuple. Consequently, equal JSON Pointer tokens compare as one logical JSON key even when one catalog reaches the final numeric token through an object and another through an array. A host duplicate-key rule instead groups by `StructuralPathKey` without occurrence, so distinct XLIFF source and target roles are never duplicates merely because they share a catalog key. Occurrence and concrete evidence always use `EntryKey`.
...
1. **Validate inputs.** Every supplied `EntryHandle` must belong to the artifact, exist once, and identify a writable entry. One complete pass first rejects any foreign or nonexistent handle without an entry key; a second raw-entry-order pass checks duplicate supply before read-only supply and reports the first corresponding original entry. The method then indexes valid inputs by artifact entry, so caller slice order cannot affect behavior or error selection.
...
7. **Verify candidate equivalence.** Candidate entry count must first equal the original count. Equal-length entries are compared in raw order using the fixed identity, catalog-key domain, catalog key, display identity, read-only state, and effective-message order above. Only complete equivalence returns `WriteBackOutcome::Changed` with raw-ordered replacements and the validated candidate artifact.
...
When the host document itself cannot be parsed, extraction fails for the whole document. The built-in adapter owns that acceptance decision and its normalized `resource_parse_failed` site, but the resource layer does not translate the rejection into an MF2 diagnostic or publish a detailed host-language syntax diagnostic. Host-format tooling owns that diagnostic presentation. The CLI reports `resource_parse_failed` as a target-local operational error. An editor publishes no resource diagnostic for that code and leaves the current document generation with an empty ox-mf2 diagnostic set; it never retains MF2 diagnostics whose host ranges came from an earlier document version or mapping.
...
- Catalog host formats require an explicit selection signal. Project-wide and bulk CLI selection comes from configuration, editor selection comes from project policy or a validated overlay, and the CLI treats an individual file operand or stdin filepath as direct user opt-in only when project catalog policy is absent and its extension maps to a shipped adapter. Arbitrary JSON, YAML, or XML found through a directory, a CLI-native glob, or editor auto-detection must not be assumed to contain MF2 messages; a false positive reports wrong diagnostics on unrelated files and, worse, exposes them to formatting write-back.
- Project-configured catalog opt-in comes from the `resources` section of the unified project config defined below. Editor adapters may additionally use the explicitly editor-only normalized ad-hoc catalog settings defined by [009-ox-mf2-phase-3d-lsp-editor-design.md](./009-ox-mf2-phase-3d-lsp-editor-design.md#configuration-sources) under the fixed fallback precedence below. Filename conventions such as `*.mf2.json` are only recommended `include` patterns; outside the narrow direct CLI exception, they never imply catalog membership in any tier.
- CLI resource workflows and editor adapters use the same project-policy states and host-format assignment algorithm. CLI direct-file and stdin modes additionally have the narrow config-free classification exception defined under [Input Selection](#input-selection); editor routing never applies that exception automatically.
- The editor ad-hoc layer is fallback-only. It may opt in a document only when project `resources.catalogs` is absent or a non-empty project policy has no matching include. An explicitly empty project policy and a path excluded by every including project definition both block overlay opt-in. A project match is authoritative, so the resolver does not evaluate the overlay for that path and an overlapping overlay definition cannot de-duplicate, reclassify, or conflict with it.
- When fallback is permitted, overlapping definitions within the overlay use the normal same-format de-duplication and different-format conflict rule. An invalid project or overlay structure is still reported before document routing; fallback-only path resolution does not bypass validation of either source.
...
Within an opted-in catalog document, leaf-oriented formats default to every host-typed string leaf value, including string elements of arrays. JSON, JSONC, JSON5, and YAML use that default. A semantic container format may instead define an explicit candidate set in its format-specific section; that definition takes precedence over the leaf default. In particular, Vue considers only entries lifted from selected inline `<i18n>` regions, and XLIFF considers only the documented primary `<source>` and `<target>` elements. A format added later must either adopt the leaf default explicitly or define its complete candidate-set boundary; it must not inherit one accidentally from a parser dependency. After scope selection, the adapter validates each candidate's path and unescaped value against the message-entry representability invariants above. If any candidate fails, extraction fails for the complete host document with `resource_entry_unsupported`; no partial artifact or entry list is returned, and linting, formatting, and write-back do not process that file. When multiple candidates fail, the error for the lowest raw source offset is selected deterministically.
...
Locale identity is not required by per-entry formatting and linting, because parsing, formatting, and linting one MF2 message do not depend on locale. The scope and locale bindings defined in [Scope and Locale Binding for Catalog Consumers](#scope-and-locale-binding-for-catalog-consumers) associate stable logical namespace and locale metadata with entries for the message linker and later catalog-native structural lint rules without changing message extraction or message-level core APIs.
...
Only Tier 1 is implemented in the initial resource milestone, and that complete Tier 1 milestone is part of the tooling v0.1 feature scope. Here, tooling v0.1 is independent of the Binary AST snapshot format version and the existing `@intlify/cli` npm package version. Tier 2 and Tier 3 formats are outside tooling v0.1, deferred, and tracked in [Deferred Follow-Up Notes](#deferred-follow-up-notes). Their subsections below record contract-level design notes so the shared contracts stay tier-proof; they are not initial implementation commitments.
...
The Tier 1 JSON adapter uses a crate-private iterative lexer/parser rather than deserializing into `serde_json::Value` or another decoded object map. Its first phase validates the complete host syntax and builds a compact `JsonSyntaxTape` whose events retain raw token spans, container boundaries, member order, duplicate members, and string-versus-non-string value roles. String tokens remain raw source slices in this phase: the parser validates JSON escape spelling but does not require every member name or value to become a Rust `String`. An explicit container stack enforces the public nesting limit when each opening object or array is confirmed and avoids recursive descent whose call-stack limit could differ from the resource contract.

Only after that complete syntax phase succeeds does the adapter traverse the tape in raw source order. It decodes string leaves selected as message candidates, decodes only the member names or array indices needed for each candidate's JSON Pointer path, and builds the message offset map while consuming each raw string escape. An unpaired surrogate escape therefore becomes the documented representability error only when it belongs to a candidate message or a member name on that candidate's path; the same raw member spelling in a subtree with no string candidate never has to become a Rust string and does not fail extraction. Because the tape preserves duplicate members instead of inserting them into a map, occurrence assignment and error selection remain source ordered.

`JsonSyntaxTape` is construction-local parser state, not a public artifact, cache identity, or adapter extension interface. The Tier 1 JSON artifact retains only the exact host source, published entries and mappings, resolved format context, and the minimal private write-back state required by the common artifact contract; it does not retain a decoded JSON DOM. Candidate validation invokes the same iterative parser and two-phase traversal, so original extraction and candidate re-extraction have identical syntax, depth, ordering, and representability precedence.
...
The initial Vue adapter extracts inline `<i18n>` blocks only. A block with a `src` attribute is an external-resource reference and contributes no entries to the `.vue` extraction artifact; the resource layer does not resolve its path, read it transitively, validate that it exists, write through the SFC, or opt the referenced file in. That file is processed only when a consumer independently selects it as its own catalog target: through project `resources.catalogs`, through an eligible editor overlay, or through the CLI's config-free direct-file classification under `PolicyAbsent`. Directory and CLI-native glob discovery still require a project `Matched` result. This explicit external-resource boundary prevents duplicate diagnostics and gives reads, errors, cache identity, and edits to the file that actually owns the message bytes. A `src` block is therefore intentionally outside the inline candidate set and is not `resource_entry_unsupported`.
...
`jsonc` is intentionally not a Vue embedded-language declaration, even after the top-level JSONC adapter ships. An inline `lang="jsonc"` block therefore produces embedded `resource_format_unsupported`; a `.jsonc` file referenced through `src` may still be selected and processed independently under the external-resource rules above once the top-level JSONC adapter ships. The Vue adapter never treats `lang="json"` as permissive JSONC.
...
A syntactically valid reserved directive whose name is neither `YAML` nor `TAG`, such as `%FOO bar`, is accepted and ignored without a resource warning. The initial resource workflow has no host-syntax warning channel, and a reserved directive that does not alter the representation graph must not become an MF2 diagnostic or operational error merely because the host specification recommends warning about it. Its complete source bytes remain outside entry spans and are preserved byte-for-byte by write-back. Malformed directive syntax remains `resource_parse_failed`; a `%TAG` directive that resolves a subsequently used non-Core tag remains subject to the `custom_tags` rule above.
...
Project-wide catalog policy is a new optional `resources` section of the unified project config, alongside the existing `fmt` and `lint` sections, because bulk CLI discovery and editor membership are consumed across product surfaces. The section follows the Phase 3A configuration contract in [006-ox-mf2-phase-3a-tooling-foundation-design.md](./006-ox-mf2-phase-3a-tooling-foundation-design.md): root-only discovery, strict unknown-field validation, Rust config model as the source of truth, and schema generation into the committed `config.schema.json` artifact. The CLI's config-free direct-file and stdin exception is an input-selection rule, not an implicit `resources.catalogs` definition and not a policy available to editor auto-detection.

The unified config contract in [006-ox-mf2-phase-3a-tooling-foundation-design.md](./006-ox-mf2-phase-3a-tooling-foundation-design.md) is the sole owner of root admission and the command-independent cross-section validation order. The `resources` validator runs after `lint` and, once 014 M0 adds the `messages` section, before `messages`. This document owns only the deterministic validation order and stable violation evidence within `resources`; adding a later root section does not require another complete global-order copy here.
...
7. When the coordinated 014 M0 fields are admitted, the `scope` / `locale` both-or-neither invariant is checked after `format` and before either present value is interpreted.
8. When both are present, `scope` is checked next: scalar string shape, non-empty Unicode-scalar content, then the inclusive 255-byte decoded UTF-8 ceiling.
9. `locale` is then checked as an object, followed by unknown locale-object fields in ASCII ascending order.
10. The required `from` discriminator is checked for scalar string shape and then for the exact `path` or `fixed` value.
11. Finally, the selected variant is checked: it must contain its one required `pattern` or `value`, must not contain the other variant's field, and that payload must satisfy its variant-specific shape and value contract.

An earlier definition's pattern, format, scope, or locale failure therefore precedes every failure in a later definition; validation does not perform a shapes-across-all-definitions pass before semantic checks. Appending the coordinated fields after `format` preserves the relative first-error precedence of every pre-M0 field. An omitted `resources` object and an omitted `resources.catalogs` field preserve policy absence rather than defaulting to an empty array. A present empty `resources.catalogs` array is valid and preserves an explicit disable-all policy. JSON `null` is still a present value: `resources: null` is `invalid_section_shape`, and `catalogs: null` is `invalid_catalogs_shape`, never policy absence. Only omitted `exclude` and `format` values reach their documented field defaults after the applicable enclosing shape and unknown-field checks succeed; omission of both `scope` and `locale` instead preserves entry-level-only participation and creates no default binding.
...
The coordinated 014 M0 extension adds the following resource-owned structural reasons without changing the earlier reasons or their precedence:

- `invalid_catalog_scope_locale_pair` with `pointer: "/resources/catalogs/<index>"` when exactly one of `scope` and `locale` is present. Relation-wide evidence points to the definition rather than inventing a pointer for the missing member.
- `invalid_catalog_scope` with `pointer: "/resources/catalogs/<index>/scope"` when a paired value is not a string, is empty, or exceeds 255 decoded UTF-8 bytes.
- `invalid_catalog_locale_binding` at the narrowest applicable `/resources/catalogs/<index>/locale...` pointer when `locale` is not an object, `from` is missing, non-string, or unsupported, a required variant payload is missing or non-string, or a field valid only for the other variant is present. A locale-object member outside the closed `from`, `pattern`, and `value` vocabulary continues to use `unknown_field` at that exact member before discriminator validation.
- `invalid_locale_capture_pattern` with `pointer: "/resources/catalogs/<index>/locale/pattern"` when the `path` variant's string payload violates the capture-pattern grammar defined below.

The `fixed` variant's `value` is admitted through the resource-owned checked `ResolvedLocale` contract defined below. An empty or over-255-byte value uses `invalid_catalog_locale_binding` at `/resources/catalogs/<index>/locale/value`; exact spelling is otherwise preserved. Assignment-time `locale_capture_mismatch` remains a separate post-discovery configuration failure rather than a structural reason.
...
These stages intentionally reuse the shared `config_validation_failed` code but are not one validation pass. Every structural reason above and every statically known fixed-locale production-membership failure is selected before ignore setup or discovery and returns only the first error under the fixed shared and section-local order.

After those path-independent checks succeed, the coordinated project-link workflow uses one total late-gate order. Concrete targets are visited by exact slash-normalized project-relative path. For each target, format assignment and `catalog_format_conflict` are resolved first, then path capture and `locale_capture_mismatch`, then captured `catalog_locale_not_production`; the first failure on the earliest target wins. Only after every target assignment succeeds are physical-source groups visited by canonical primary-path order and checked for `catalog_binding_conflict` under each group's existing primary-plus-alias and catalog-definition order. Only after every group passes that admission does the workflow attempt the complete definition extraction inventory and select `catalog_scope_key_domain_conflict` from the successful domain observations under the order defined below. The final configuration gate is 014's `scope_key_domain_mismatch`, using its recognizer-then-root order.

Exactly one configuration failure is returned. The workflow does not aggregate several reasons, rescan every target once per reason class, or let filesystem enumeration, hash-map order, physical alias discovery, cache state, or future worker completion select it. A consumer must not move structural failures later, infer a domain from host-format spelling, or attempt static glob-intersection analysis merely to collapse these stages.
...
The presence boundary is exact: both `{}` and `{ "resources": {} }` mean `PolicyAbsent`, including when either object came from an explicitly supplied `--config` file. `{ "resources": { "catalogs": [] } }` means `PolicyEmpty`. The empty policy is therefore an intentional project-level disable switch, not another spelling of the default.

The validated and resolved Rust configuration types are conceptually:
...
enum ResolvedCatalogPolicy {
    Absent,
    Empty,
    // This vector is guaranteed to contain at least one compiled definition.
    Configured(Vec<ResolvedCatalogDefinition>),
}
...
The resource crate owns section-local normalization and resolution rather than exposing config data for consumers to reinterpret. `ResourcesConfig` and `CatalogConfig` fields are private; consumers may inspect documented read-only values when needed but cannot construct an unvalidated normalized value or mutate it after validation. `ResourcesConfig::default()` represents `catalogs: None`, and both an absent `resources` object and an object without `catalogs` produce that value. A present `catalogs: []` produces `Some(Vec::new())`; validation must not use a deserialization default that collapses those states. The `Option` models property presence only and must not make explicit JSON `null` deserialize as `None`. The resource validator or a presence-aware custom deserializer rejects `null` before constructing `ResourcesConfig`, and the generated schema marks `catalogs` optional while permitting only an array when present. After the unified config loader reaches the `resources` step in the shared validation order, it delegates this section's exact shape, field, pattern, and format checks to the resource-owned validator, which is the only non-default construction path and returns `ResourcesConfig` only on success.

The unified loader first parses JSON or normalized JSONC into its presence-preserving raw value and completes root, `fmt`, and `lint` validation. It then calls the resource validator with either an absent marker or the exact present `resources` value, including `null`; it does not deserialize that property through `Option<ResourcesConfig>` first. The resource crate returns path-independent structured evidence containing the stable reason, JSON Pointer, and optional scalar `value`, but it does not construct the CLI `ConfigError`, mention a config file, or choose an exit status. `crates/intlify_cli` adds the loaded config path and converts that evidence to the ordinary `config_validation_failed` envelope.

The resource-facing portion of the runtime integration is conceptually:

The following is not an exhaustive definition of either aggregate type. The unified configuration contract in 006 owns the complete `ProjectConfig` shape; `fmt`, `lint`, and additive product sections such as the 014-owned `messages` section remain defined by their owning documents. This excerpt shows only the resource-owned normalized and resolved values that this design adds.

```rust
pub struct ProjectConfig {
    // Other product-owned configuration fields are omitted.
    pub resources: ResourcesConfig,
}

pub struct LoadedProjectConfig {
    // Existing project root, config path, source, normalized config,
    // and other product-owned resolved values are omitted.
    pub config: ProjectConfig,
    pub resolved_resources: ResolvedResources,
}
```

The loader constructs `ResolvedResources` exactly once after successful section validation and all commands borrow that compiled value for classification; formatter and linter paths do not compile catalog patterns independently. `ResourcesConfig::default()` supplies the normalized absent state when no config file exists as well as when the field is omitted. `ResolvedResources` is immutable and `Send + Sync`, so a future CLI scheduler may share the one value without changing resource ownership.

The resource-owned normalized config types also own the `schemars` description of the resource schema fragment used by the unified project schema. The root schema embeds that fragment but does not maintain a CLI-local duplicate of catalog fields or format values. Every presence-only option is optional but non-null in the generated schema: this applies to the root `resources` property and to present `catalogs`, `exclude`, and `format` properties. Schema customization must override `Option`'s default nullable projection where necessary, and committed-schema fixtures verify both omission acceptance and explicit-`null` rejection for each property.

`ResourcesConfig::resolve(self) -> ResolvedResources` is infallible. It preserves `None` as `ResolvedCatalogPolicy::Absent`, preserves `Some([])` as `ResolvedCatalogPolicy::Empty`, and compiles a non-empty vector into `Configured`. Pattern compilation uses the same resource-owned grammar implementation and retains definition indices needed for deterministic evidence. The accepted validated representation must make compilation total; it does not invoke a dependency-specific parser, perform a second semantic validation pass, or introduce `ResourceConfigError`. Every user-correctable failure is therefore selected by the earlier deterministic `config_validation_failed` contract, while consumers cannot bypass that contract with direct struct construction.

`CatalogOverlayConfig` is the consumer-neutral normalized form of an additive catalog overlay. Its private catalog definitions have the same `include`, `exclude`, and shipped-only `format` contract as project definitions, but the value has no `resources` wrapper and cannot remove, override, or mutate a project definition. It deliberately has no `scope` or `locale` binding: those and every other project-only catalog field are rejected as unknown overlay fields. An overlay assignment can classify and extract an editor-local catalog for per-document resource and message processing, but it is never a linker-participating definition source and cannot produce a `MessageDefinitionArtifact` or change linker completeness. Linker participation begins only after a project `resources.catalogs` definition supplies the coordinated exact scope and locale binding.

The resource-owned overlay validator accepts only an object with optional `catalogs`, applies the same definition-field validation order and resource glob implementation, and returns `CatalogOverlayConfig` only after complete success. The root object uses RFC 6901 pointer `""`; its catalogs and definition pointers begin at `/catalogs`. Thus product-specific editor setting names do not leak into this shared value or its validation evidence. `CatalogOverlayConfig::default()` is empty, and `CatalogOverlayConfig::resolve(self) -> ResolvedCatalogOverlay` is infallible for the same accepted-representation reason as project resolution.
...
ResolvedResources::policy_state(&self) -> CatalogPolicyState
...
`policy_state` exposes only whether project catalogs are absent, explicitly empty, or non-empty and configured. `Configured` guarantees at least one validated definition but exposes no definition, pattern, or assignment. A CLI consumer uses this accessor when a direct file or stdin virtual path lies outside `projectRoot` and therefore cannot construct `ProjectRelativeResourcePath`: `Absent` permits direct-extension classification, `Empty` skips, and `Configured` treats the outside path as unmatched. The accessor may also avoid pointless resource lookups for bulk discovery under absent or empty policy, but it never substitutes for `resolve_path` when a configured project-relative path may match or be excluded.

For project-only `resolve_path`, `PolicyAbsent` and `PolicyEmpty` are path-independent results for those two preserved policy states. Under a non-empty policy, the resolver first records definitions whose `include` matches before applying each definition's own `exclude`. No include match produces `Unmatched`. One or more include matches for which every including definition excludes the path produce `Excluded`. If at least one including definition survives its own excludes, the result is `Matched` even when another including definition excludes the same path; only the surviving definitions participate in format assignment and conflict selection.

`Matched` exposes read-only access to the final `HostFormatClassification`, the original extension spelling used for classification, every surviving project definition, and the subset that contributed the one resolved format assignment. A matched result with no contributing assignment carries `UnrecognizedExtension`; a unique known assignment carries `Shipped` or `KnownButUnshipped` as appropriate. A conflict exposes its source-qualified decisive definition references under project definition-index order; the CLI combines those project indices with the already supplied path and loaded config path to form its existing surface-specific pointer details. Unrecognized matches never appear in a same-layer assigning subset.

`resolve_path_with_overlay` validates neither source; it receives two already validated and resolved layers and applies fallback-only path resolution without exposing a constructible assignment. A project `Matched` result is returned immediately with `Project` origin, and the overlay is not path-resolved even if it contains a same-path or different-format definition. Project `PolicyEmpty` and `Excluded` results return layered `Unmatched` and likewise block overlay path resolution. Only project `PolicyAbsent` or `Unmatched` permits overlay resolution. In that branch, an overlay match returns `Overlay` origin, no overlay match returns layered `Unmatched`, and a conflict among overlay definitions returns their normal same-layer conflict. A project-layer conflict is returned before any fallback decision. There is no same-path cross-layer de-duplication or cross-layer conflict because the overlay is not an overriding peer.

A layered `Unmatched` result is normal editor non-membership, not `unsupported_input_file`. If a configuration transition changes a previously matched open document to layered `Unmatched`, the editor integration removes its resource artifact and resource-specific caches and publishes an empty current-generation diagnostic set under the 009 lifecycle contract. The resource resolver returns classification only; it does not retain editor state or publish that clearing notification itself.

`LayeredCatalogMatch` exposes only read-only `origin()` and `assignment()` accessors. `CatalogDefinitionRef` likewise exposes its origin and zero-based definition index without exposing compiled patterns. In the editor configuration channel, `catalog_format_conflict` canonical details use `targetPath` plus `assignment` and `conflictingAssignment` objects, each containing `source: "project" | "overlay"` and `definitionIndex`. Both references always come from the same resolving layer; `assignment` is the later decisive reference and `conflictingAssignment` is the earlier one. The optional top-level config path is present only for a project-layer conflict; an overlay-layer conflict has no truthful filesystem config path and omits it. These source-qualified details are for editor configuration state and fingerprints; they do not replace the CLI project-only pointer shape defined below.

CLI consumers call `resolve_path` for project catalog policy, membership, same-format de-duplication, extension classification, and conflict selection. Editor consumers call `resolve_path_with_overlay` after both source structures validate; they do not resolve either layer independently and recombine its evidence. For every project or layered `Matched` result, the consumer calls `HostFormatRegistry::resolve_format` and moves its `ResolvedHostFormat` result into extraction. Consumers do not branch on `KnownButUnshipped` or `UnrecognizedExtension`, reconstruct extension evidence, construct assignments, compile resource globs, repeat either overlap algorithm, or duplicate unsupported-format detail construction. The editor uses `LayeredCatalogMatch::origin()` only to label project coverage versus editor-local coverage.

`HostFormat` is the shipped-only enum defined under [Format IDs and Extension Classification](#format-ids-and-extension-classification). The root `resources` section and its `catalogs` field are optional, but their absence remains semantically distinct from a present empty array. Each catalog definition requires a non-empty `include` array. `exclude` is optional and defaults to an empty array; `format` is optional and defaults to extension-based classification. The initial `HostFormat` schema accepts only `"json"` and expands as deferred adapters ship. The schema permits zero catalog definitions so users can express the explicit disable-all policy.
...
Mixed host formats remain valid for the initial entry-level formatting and linting workflows. Locale-aware catalog consumers use `CatalogKeyDomain`, not outer host format, to decide whether keys are comparable inside one scope. The single-domain scope rule is defined under [Scope and Locale Binding for Catalog Consumers](#scope-and-locale-binding-for-catalog-consumers).
...
- Key selectors are not accepted as reserved, nullable, or placeholder fields in the initial schema; their future names and semantics are designed with their workflow. The future `scope` and locale-binding shapes are fixed below, but both fields remain invalid until the coordinated 014 M0 configuration change introduces their first consumer.
...
An absent `resources.catalogs` policy permits only the CLI's config-free direct-file and stdin classification; directory and CLI-native glob discovery do not infer catalog membership, and editor routing still requires a project match or eligible overlay match. An explicit `resources.catalogs: []` disables every catalog-adapter input through project policy, producing the surface-specific normal skip behavior defined below. Neither state affects standalone `.mf2` workflows.

### Resource Membership Glob Grammar

Resource `include` and `exclude` patterns decide catalog membership over concrete normalized document paths. `intlify_resource` performs no filesystem discovery itself. The consuming surface supplies paths and owns traversal; the patterns then provide the one resource-owned membership and assignment decision.

For operand-driven `fmt` and `lint` file mode, the supplied path comes from the extension-neutral filesystem-enumeration stage of the shared discovery contract, not the final selected-target set. Supported-input classification then applies standalone MF2 precedence followed by the membership and format rules in this document. The ordinary file-mode hidden-file, directory, ignore, symlink, and explicit-operand rules remain unchanged.

For the 014 project-inventory workflow, `crates/intlify_cli` instead enumerates the configured project-root-relative include space so every matching logical file can be accounted for without positional operands. In that surface, the validated resource `include` and `exclude` sets are the definition-inventory authority: ordinary formatter/linter ignore sources and default hidden, dependency, or output exclusions do not remove a configured match. This does not move I/O into `intlify_resource`; the CLI enumerator supplies each concrete path to the same `ResolvedResources::resolve_path` boundary.
...
## Implementation Placement and Landing Order

The initial resource catalog work is an unnumbered layered milestone whose consumer-neutral foundation may start before the Phase 3C linter product sequence is complete and whose complete integration finishes before Phase 3D begins its opted-in catalog editor path. `crates/intlify_resource`, the Tier 1 JSON adapter, and the resource configuration and resolution foundation do not depend on `intlify_lint`; they may land in separate resource PRs while Phase 3C is still in progress. Catalog formatting depends on the completed Phase 3B message-level formatter, while catalog linting separately depends on the completed Phase 3C message-level pipeline. Neither initial integration waits for the deferred bounded formatter API or deferred common CLI worker scheduler. Phase 3B and Phase 3C continue to ship and validate their standalone message-level products without absorbing resource implementation into their product PR sequences. Phase 3D then consumes the completed shared layer rather than defining temporary editor-local host adapters. The Tier 1 milestone is required tooling v0.1 scope; a tooling v0.1 release is not complete merely because the standalone Phase 3B and Phase 3C commands have shipped.

Implementation follows this dependency graph:

1. Add the `crates/intlify_resource` foundation: fixed resource errors and limits, the built-in registry, entry and catalog identity, spans and offset maps, artifact construction, candidate admission, common validated write-back contracts, compile-time thread-safety assertions, and their focused fixtures. This slice exposes no public CLI catalog target and remains independent of parser, formatter, and linter cores.
2. Add the Tier 1 JSON adapter on that foundation: the iterative lexer/parser and construction-local `JsonSyntaxTape`, extraction, JSON Pointer and occurrence identity, escape-aware mappings, re-escaping, complete candidate validation, write-back, resource-limit coverage, and JSON adapter fixtures. This slice still exposes no public CLI catalog target.
3. Add resource configuration: the resource-owned validator, `ResourcesConfig`, `ResolvedResources`, catalog membership and format resolution, the generated resource schema fragment, unified CLI config-loader integration, and committed schema fixtures. This slice preserves absent versus explicit-empty policy and does not yet duplicate message-level processing.
4. Refactor the shared CLI input pipeline to the extension-neutral discovery model: retain and merge operand origins, classify standalone and catalog targets, apply the catalog-assignment conflict gate, apply ordinary ignore rules after classification, and group selected aliases by physical identity. Existing standalone `.mf2` behavior and fixtures must remain unchanged; catalog selection may remain integration-gated until a command consumer is ready.
5. Implement or verify the ordinary Rust-only `intlify_format::format_parsed` and `check_parsed` entry points, including original `SourceStore` / `ParseResult` pairing checks, parser-diagnostic gating, and direct reuse of the shared formatter pipeline without reparsing or snapshot conversion. Then integrate catalog formatting into `intlify fmt` through that parsed-artifact path, sequential physical-file processing, candidate validation, catalog result specialization, write/check behavior, and formatter end-to-end fixtures. This step does not wait for Phase 3C, the bounded formatter API, or the common worker scheduler.
6. After the relevant Phase 3C message-level linter prerequisites are complete, integrate catalog linting into `intlify lint` through the same classification and sequential physical-file path, mapped catalog diagnostics, result specialization, and linter end-to-end fixtures. Steps 1 through 4 and formatter integration do not wait for this slice.

After the applicable command integrations land, complete the resource benchmarks, cross-command determinism tests, committed schema verification, and release evidence as the Tier 1 milestone acceptance gate. Only after this gate is accepted may Phase 3D add project-configured or editor-overlay catalog consumption.

Each step may use more than one reviewable PR. The numbering expresses reviewable dependency slices, not a requirement that formatter integration wait for linter integration or Phase 3C completion. A dependent step must not introduce a second registry, config resolver, extraction path, or error conversion while waiting for an earlier owner. Standalone `.mf2` behavior remains usable throughout; partially landed resource internals do not make catalog files a supported public target before the corresponding CLI integration and complete milestone gate are present.
...
`crates/intlify_cli` initially owns sequential physical-file grouping, serial logical-alias execution, and deterministic aggregation. The formatter owns the exact `output_write_failed` and `alias_processing_blocked` result contract under [Physical Alias Write Failure](./007-ox-mf2-phase-3b-formatter-design.md#physical-alias-write-failure). The future common worker scheduler and parallel group execution remain deferred under the shared [CLI Parallel Execution Boundary](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary).

The read-only linker-definition production in [014](./014-ox-mf2-message-linker-design.md#logical-alias-grouping-and-primary-identity) intentionally specializes this alias boundary without changing formatter or linter results. After resolving each selected alias independently, the `intlify_cli` project inventory inspects physical identity and forms one physical-source group. It orders the group's portable logical paths segment-by-segment by UTF-8 bytes with the shorter equal prefix first; the first path is primary and the remaining paths are ordered aliases.

The host-owned pre-extraction stage then receives every already formed group. It requires equal namespace, canonical host-format id, scope, and locale bindings and admits each complete primary-plus-alias set under 014's path and alias limits. A configuration-derived binding conflict rejects the complete project input through `config_validation_failed` before extraction and never weakens linker completeness to `SourceFailed`. A path or alias admission failure instead retains its owning operational classification.

After every group passes configuration admission, the CLI invokes `intlify_resource` once for each group and retains the successful extraction artifacts without projecting them into `intlify_contract`. Once the complete definition extraction inventory has been attempted, the host runs one project-global domain admission stage. Its catalog-domain gate aggregates resource-owned domain observations and selects `catalog_scope_key_domain_conflict` when applicable; its following recognizer/root-domain gate applies 014's recognizer/root domain comparison. Neither ordered gate reparses a host source or constructs a provisional contract artifact.

Only after both ordered gates in that project-global stage succeed does the host-owned post-extraction projection consume each successful extraction, perform the checked resource-to-contract conversion, and emit one complete definition artifact. A source-attributable extraction or projection failure contributes the applicable `SourceFailed` evidence; it does not manufacture a missing domain observation. Remaining aliases are ordered evidence rather than duplicate definitions. Physical identity helps only with CLI-local grouping and never enters the public artifact. Formatter and linter CLI workflows continue to retain per-alias results and the serialized reread/write behavior defined by 005/007 and this document.

For a catalog logical target, one caller-thread group workflow performs the complete read, resource extraction, per-entry message-core processing, candidate validation, result construction, and optional write sequence. `crates/intlify_resource` remains synchronous and scheduler-neutral: it creates no worker pool or runtime, and concurrent calls through shared registry or artifact state obey the `Send + Sync` contract above so a later consumer-owned scheduler requires no resource API redesign. Catalog-specific classification conflicts and result payload specializations remain owned below.

### Linker Project Inventory Selection

The initial `intlify messages emit` and `intlify messages prune` workflows do not call the operand-driven input selection below. They accept no file, directory, glob, or stdin operand and have no config-free direct-extension exception. Their definition inventory is the complete set of concrete logical files selected by linker-participating `resources.catalogs` definitions under the discovered 006 `projectRoot`.

Only catalog definitions promoted with the coordinated 014 `scope` and locale binding participate. Entry-level-only catalog definitions remain available to `fmt` and `lint` but neither enter the linker inventory nor become omitted linker sources. `PolicyAbsent` and `PolicyEmpty` provide no implicit definition scan; the 014 configuration validator decides whether the requested linker command has the required linkable scopes and policy.

`crates/intlify_cli` enumerates candidate entries without an extension filter, converts every representable path lexically inside `projectRoot` to `ProjectRelativeResourcePath`, and calls `ResolvedResources::resolve_path`. It retains only linker-participating `Matched` assignments, reports their existing assignment or shipped-format failures, and never content-sniffs or reconstructs compiled resource patterns. A syntactically valid catalog definition with no current match contributes a complete empty source set. It does not fall back to directory `.`, standalone `.mf2`, or the registry's direct-extension list.

The configured include/exclude policy is authoritative for this surface. Root `.gitignore`, `--ignore-path`, `fmt.ignorePatterns`, `lint.ignorePatterns`, hidden-name filtering, and default VCS/dependency/output-directory filtering do not silently subtract matches. Enumeration may prune a subtree only when the compiled include set proves that no descendant can match. It follows file symlinks and does not follow directory symlinks, matching the common traversal safety rule; selected file symlinks and hard-link aliases then use the physical grouping and definition-projection contract above.

A logical file may independently belong to the definition inventory and to a language producer's reference inventory. Once 014's deferred Vue SFC reference frontend is promoted, a Vue SFC containing both an `<i18n>` block and message calls is one example; the initial M0 reference producer does not yet admit `.vue`. Selection on one side never de-duplicates or suppresses its participation on the other. Reusing one immutable read snapshot is an orchestration optimization only when both artifacts are proven to use the same selected bytes.

The CLI records successful or failed execution against the complete resolved inventory before constructing 014's `ScopeCompletenessTable`. After every configuration-derived assignment, locale, and physical-group binding has succeeded, and after the project-global domain admission stage has passed both ordered gates, an enumeration, metadata, read, extraction, freshness, or definition-projection failure prevents the affected definition side from becoming `Closed`; it is never converted to a normal skip. For original extraction this includes `resource_format_unsupported`, `resource_parse_failed`, `resource_entry_unsupported`, `resource_document_unsupported`, and `resource_limit_exceeded` with `phase: "extract"`. Each contributes `SourceFailed` only for an authoritative current-generation selected group after candidate configuration installation when applicable. A `validate_write_back` failure concerns only a rejected formatting candidate and never changes linker completeness.

The linker may still return partial-world findings, but `emit` produces no artifacts and `prune` performs no mutation under the 014 gates. An isolated candidate execution failure remains uninstalled evidence. A configuration-derived contradiction instead retains its owning `config_validation_failed`, constructs no `LinkRequest`, and never becomes `Partial(SourceFailed)`.

### Input Selection

A file enters resource processing through either a project-policy match or the CLI's narrow config-free direct-input exception. Native `.mf2` classification remains a separate reserved workflow and always takes precedence over catalog resolution. `resources.catalogs`, including an explicit empty array, controls only host resource adapters and never disables a direct or discovered standalone `.mf2` file.

Operand kind is determined from the tokens received by the CLI. A concrete individual file token is a direct file operand even when a shell produced it by expanding an unquoted shell glob; multiple such tokens remain multiple direct operands. A token that still contains CLI-recognized glob syntax is a CLI-native glob, and a token whose metadata identifies a directory is a directory operand. The CLI neither receives nor attempts to reconstruct shell-expansion provenance. Consequently, `intlify fmt locales/*.json` may reach the CLI as config-free direct files when the shell expands it, while `intlify fmt "locales/*.json"` remains bulk glob discovery and requires project catalog matches.

Filesystem enumeration de-duplicates candidates by slash-normalized absolute path while retaining every operand origin. If the same path is reached through both an individual direct file operand and a directory or CLI-native glob, the merged candidate is classified once using direct-file semantics. Direct origin therefore grants the config-free exception under `PolicyAbsent`; a bulk origin cannot downgrade it to a skip, and the candidate still produces at most one target or one classification error. Multiple bulk origins without a direct origin remain bulk discovery.

The implementation carries this information through the crate-private `DiscoveredCandidate`, `CandidateOrigin`, and `SelectedTarget` staging model owned by [005's CLI file execution boundary](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-file-execution-boundary). Resource resolution consumes the candidate's logical path and complete origin set during supported-input classification; `intlify_resource` does not enumerate files, apply ordinary ignore rules, inspect physical identity, or own these CLI-only types.

For a direct non-`.mf2` file operand, project policy is authoritative when present:

- `PolicyAbsent` asks `HostFormatRegistry::resolve_direct_extension` to classify the file's exact final extension. A shipped direct extension selects the corresponding catalog workflow; no extension, an unknown extension, or a known-but-unshipped extension produces `unsupported_input_file`. Classification is ASCII case-insensitive and never uses content sniffing.
- `PolicyEmpty` is an ignore-like normal skip.
- `Unmatched` produces `unsupported_input_file`; a non-empty project policy does not fall back to config-free extension classification.
- `Excluded` is an ignore-like normal skip.
- `Matched` resolves the project assignment through `HostFormatRegistry::resolve_format`; an unrecognized or known-but-unshipped assignment therefore produces `resource_format_unsupported` rather than `unsupported_input_file`.

Directory and CLI-native glob discovery select standalone `.mf2` files plus only project `Matched` catalogs. `PolicyAbsent`, `PolicyEmpty`, `Unmatched`, and `Excluded` candidates are skipped without direct-extension fallback. JSON and other host files discovered through those bulk operands never become catalogs merely from their extension. A directory or glob that matches concrete filesystem entries but yields no standalone or project-matched targets is the shared zero-target success, including when all concrete matches are otherwise directly supported JSON files. This avoids treating unrelated JSON under a broad directory operand as catalog input.

The CLI's no-operand default is the directory operand `.`, not a set of inferred direct file operands. Under `PolicyAbsent`, bare `intlify fmt` or `intlify lint` therefore discovers standalone `.mf2` files but skips JSON catalogs; naming one concrete JSON file directly or supplying it through `--stdin-filepath` invokes the separate direct-input rule.

For `unsupported_input_file`, `details.supportedExtensions` lists the canonical lowercase config-free direct extensions: the registry's shipped direct catalog extensions plus `.mf2`, de-duplicated and ASCII-sorted. The Tier 1 value is `[".json", ".mf2"]`. Case variants such as `.JSON` are accepted but are not duplicated in that list. An extensionless or arbitrary filename remains processable through an explicit project `format` assignment even though its spelling cannot appear in `supportedExtensions`.

A direct non-`.mf2` file outside `projectRoot` cannot match project-relative catalog definitions. Under `PolicyAbsent`, it may nevertheless select a shipped config-free direct extension and uses absolute machine-readable path reporting. Under `PolicyEmpty` it is skipped, and under a non-empty configured policy it is effectively unmatched and produces `unsupported_input_file`. Directory and CLI-native glob discovery outside the root may still select standalone `.mf2` files, but non-`.mf2` entries there are not catalog targets.

Project-root discovery and exact Unicode representability run before config discovery under the Phase 3A contract. A non-Unicode project root therefore produces the shared `projectRoot: null` error envelope and never reaches resource configuration or membership. After a representable root is established, an explicit `--config` path is checked for exact representability before config I/O; structural configuration loading and validation then run before ignore setup or discovery. Merely supplying `--config`, an empty `resources` object, or another resource field does not change `PolicyAbsent` when `resources.catalogs` itself is omitted. A non-Unicode config path or structural config error aborts before any resource membership work. After config succeeds, all OS-native `--ignore-path` values are resolved and checked for exact representability in CLI order before any ignore source is read or discovery begins. Their first representability failure likewise aborts with empty results before resource membership. Otherwise ignore sources are read and validated, extension-neutral filesystem enumeration produces the concrete candidate path set, and every top-level discovery/input error is retained in its existing deterministic order.

Supported-input classification processes each representable candidate in this order: reserved standalone MF2 classification; project policy and membership resolution when a project-relative identity exists; config-free direct-extension classification only for a direct file under `PolicyAbsent`; then operand-kind-specific unsupported or skip handling. Project catalog assignment resolution runs for every `Matched` concrete candidate before the CLI reads or processes any target. If any path resolves to conflicting host formats, the command appends one top-level `config_validation_failed` after those already discovered or classified input errors, aborts the complete target set with an empty `results` array and no command-specific target counters, and does not perform ignore matching, physical grouping, reads, or formatting. Conflict selection is deterministic: choose the first conflicting slash-normalized target path in ascending order, then the lowest-index later surviving catalog definition that conflicts for that path. The earlier conflicting definition is the lowest-index surviving definition assigning a different format to that same path.
...
`PolicyEmpty` and `Excluded` direct files are removed before reads, physical grouping, command-specific counters, and `results[]` construction. They do not receive a synthetic `status: "skipped"` result. If no other target remains, file mode returns the normal zero-target success. After supported-input membership and catalog format classification, remaining file-mode targets pass through the invoking command's existing ordered ignore contract: root `.gitignore`, then `--ignore-path` files in CLI argument order, then `fmt.ignorePatterns` or `lint.ignorePatterns`. Ignore rules apply to explicit file operands as well as discovered targets. An ignored catalog is not extracted, and an empty target set after ignore filtering retains the existing zero-target success behavior. Formatter and linter ignore lists are intentionally independent, so the same catalog may be skipped by `fmt` and processed by `lint`, or vice versa.

Because policy classification precedes ordinary ignore matching, an explicit project `Unmatched` file produces `unsupported_input_file` even when a later `.gitignore`, `--ignore-path`, or command-specific ignore pattern would match it. This preserves the shared rule that ignore does not hide an unsupported explicit operand. A project catalog `exclude` or `PolicyEmpty`, rather than the later ignore stack, expresses policy-level normal skip for a directly named host resource.

After ignore filtering, standalone and catalog targets follow the shared physical-identity inspection, grouping, metadata-failure, alias ordering, fresh-read, and fail-stop contract in [005](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-file-execution-boundary). Physical identity never replaces a catalog target's normalized logical path in results, diagnostics, config matching, or cache namespaces. `matchedFiles`, `results[]`, and human path output continue to treat each logical alias as a separate target; for example, the first alias may format the physical file and a later alias may observe the resulting source as unchanged.
...
The CLI does not reinterpret an absolute path as a project-relative suffix or permit `..` matching. An outside virtual path cannot match project-relative catalog definitions, but under `PolicyAbsent` it remains eligible for the same config-free direct-extension classification as a direct file operand. Its machine path remains absolute.
...
2. `PolicyAbsent` applies `HostFormatRegistry::resolve_direct_extension` to the exact final extension. Success selects catalog stdin; failure is `unsupported_input_file` with the same direct-extension details as file mode.
3. `PolicyEmpty` skips the virtual catalog input normally.
4. Under a non-empty project policy, `Unmatched` is `unsupported_input_file`, `Excluded` is a normal skip, and `Matched` resolves exactly one host format under the normal override, extension, and overlap rules.
5. Supported-input classification happens before ordinary ignore matching, preserving the existing rule that an ignored unsupported virtual path is still an input error. After classification, normal formatter or linter ignore sources may skip a selected target.

Policy-skipped stdin still undergoes the shared UTF-8 validation but no resource host-byte preflight or extraction. For valid UTF-8, normal formatter mode passes the original bytes through to stdout exactly, formatter check and lint produce no human output, JSON uses the existing zero-target stdin summary, and the command exits successfully. This is the same observable passthrough contract as ignored stdin and prevents a skipped pipeline input from disappearing.
...
| One catalog key's structural tokens         |                       `256` |
...
The read-only 014 definition-production boundary inherits the same inclusive `host_bytes`, `entries`, `catalog_key_tokens`, `message_bytes`, `total_message_bytes`, and `identity_bytes` ceilings and counter semantics for its source-local admission. Under protocol defaults, a catalog accepted here is not rejected merely because post-extraction projection chose a stricter duplicate source budget. The integration reuses this extraction's accepted counters and messages without a second host parse; caller-selected lower linker-artifact budgets may still intentionally stop projection before emitting an artifact. Source-only facts that are unavailable to a published artifact consumer are not duplicated as trusted claims in that artifact; 014 owns its separate observable wire, decoded, aggregate, graph, and output ceilings.
...
3. resource limits, checked as entry count, one catalog key's structural-token count, one message's byte length, running total message byte length, running distinct identity byte length, and running total offset-map segment count
...
The initial contract does not add a public `host_syntax_nodes` or token/event-count limit. Non-message host structure remains bounded by `host_bytes`, while parser and retained-state memory amplification is controlled by the linear-memory implementation contract above and initial sequential target execution. Allocation failure is not converted into a guessed `resource_limit_exceeded` value when no documented resource counter crossed its limit. Peak-memory benchmarks and adversarial structure tests must demonstrate near-linear scaling; a dependency or representation that shows superlinear growth is replaced or wrapped rather than hidden behind a dependency-specific limit or error.

The first resource-catalog release does not include a worker-width memory gate because CLI target processing is sequential. Before the deferred common worker scheduler is enabled, [005](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary) requires its own measured memory gate without changing the fixed per-target resource limits or public resource errors.

The same limits apply to every write-back candidate in write and check modes. The initial catalog formatter calls the ordinary Rust parsed-artifact formatter path and reports the complete materialized output byte length to `CandidateMessageAdmission`. Admission checks `message_bytes` first and only an output at or below that limit proceeds to the running `total_message_bytes` check. The deferred bounded Rust path in [007](./007-ox-mf2-phase-3b-formatter-design.md#bounded-rust-rendering-for-resource-consumers) may later stop at the first byte over `MAX_MESSAGE_BYTES`; that optimization must preserve this precedence and resource-owned error conversion.

The CLI or editor integration opens one `CandidateMessageAdmission` and aggregates formatter results in artifact raw entry order. It admits an original message for an unselected, diagnostic-bearing, or read-only entry and the observed complete byte length for a successful writable output. A successful writable string, including a byte-identical one, is added to retained candidate storage only after that admission call succeeds. Under the initial sequential intra-file execution, admitted formatted strings therefore total at most 64 MiB for one target, plus at most one currently returned ordinary formatter output awaiting admission. That transient output has no formatter-enforced byte cap in the initial implementation; avoiding its complete materialization is the explicit bounded-renderer follow-up in 007. Initial sequential CLI target processing prevents several target-local candidate sets from being active concurrently. Any future file-level or intra-file parallelism must add measured memory bounds and backpressure before replacing this execution model.

If admission finds the first raw-order candidate limit failure, the integration drops the current output, retains the typed resource error rather than later successful strings, and continues any still-required ordinary formatter calls only to establish formatter operational-error precedence. A formatter operational error still wins and is selected at the lowest raw-order entry. If no formatter operational error exists, the retained admission error is returned and `build_and_validate_write_back` is not called. A formatted output from a read-only entry is discarded as a clean read-only skip because that output is not an effective candidate message; its admitted value remains the already bounded original message. Parser-diagnostic and unselected entries likewise admit their originals.
...
Candidate re-extraction rebuilds the same one-builder, one-budget composition, creates a new artifact-local interner, and re-applies `host_bytes`, parser-local `nesting_depth`, per-entry `catalog_key_tokens` and `message_bytes`, and artifact-wide `entries`, `total_message_bytes`, `identity_bytes`, and `offset_map_segments`; it does not assume that candidate identity-equality checks make those limits redundant. A candidate overrun returns `resource_limit_exceeded` with `details.phase: "validate_write_back"` and is never converted to `resource_write_back_failed`. Initial extraction uses `details.phase: "extract"`. This applies equally when check mode will discard the otherwise valid candidate.
...
- Initial `resource` values are `"host_bytes"`, `"nesting_depth"`, `"entries"`, `"catalog_key_tokens"`, `"message_bytes"`, `"total_message_bytes"`, `"identity_bytes"`, and `"offset_map_segments"`.
...
- `entries`, `catalog_key_tokens`, `message_bytes`, and `total_message_bytes` point to the offending entry's complete original `raw_value_span` and carry its key. Entry representability has already succeeded under the same-offset precedence above, so this key is available even when the entry is not added to the returned artifact. `catalog_key_tokens` always reports `actual: 257`; it never exposes the complete submitted token count.
...
### Determinism and Deferred Parallelism

File results are reported in stable normalized path order; entries within a file are reported in raw span order. The initial shared CLI path processes physical groups sequentially and serializes logical aliases. The formatter contract in [007](./007-ox-mf2-phase-3b-formatter-design.md#physical-alias-write-failure) inserts `alias_processing_blocked` results for later aliases after a group write failure and contains that failure to its group. Target processing returns keyed structured results, while the resource crate remains scheduler-neutral and creates no thread pool or runtime. The common CLI worker scheduler that may later parallelize physical groups is deferred in [005](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary).

The initial implementation has no file-level or intra-file parallelism. Intra-file execution may be reconsidered only after the deferred file-level scheduler is implemented and benchmarks show that a single very large catalog leaves its pool materially underutilized. The future unit is a contiguous chunk of entries in raw span order, not one task per entry and not a host-format syntax subtree. Chunking amortizes scheduling overhead for small messages, stays host-format-neutral, and lets a single catalog share the same CLI-owned pool without creating nested crate-owned parallelism or oversubscription.
...
Initial benchmarks report extraction, re-escaping, core calls, sequential aggregation, and file I/O as separate phases. Future parallelism may land only when representative benchmarks demonstrate a repeatable improvement after scheduling, memory, and ordered aggregation costs are included.
...
On successful extraction and complete per-entry processing, `entries` contains every extracted message entry in raw span order, including clean, unchanged, diagnostic-bearing, and read-only entries. In both catalog entry variants, `displayKey` is an optional JSON string placed immediately after `key`. It contains the adapter-provided display identity exactly and is omitted, rather than serialized as `null`, when the adapter has no display identity. The remaining linter field order is `status`, then `diagnostics`; the remaining formatter field order is `status`, `changed`, `readOnly`, then `diagnostics`. The Tier 1 JSON adapter never emits `displayKey`, so its entries retain the shorter examples below. The linter variant is:
...
- Every extracted entry's message text is represented by one original `SourceStore` / `ParseResult` owner pair and goes through the Rust-only `format_parsed(sources, result, options)` path with the same resolved fmt configuration as `.mf2` files, regardless of writeability. A consumer that already has a message parse cache borrows the pair from `CachedParse`; otherwise it constructs the pair with exactly one parse for this entry. The parse options enable trivia for preserve mode; standard mode may reuse a result with `trivia_collected: false`, and those result-affecting options remain part of the cache key. `intlify_format` depends only on the parser types, builds its private validated parsed-input view, and neither knows about `CachedParse` nor reparses or snapshot-encodes/decodes the message. The consumer retains the result's parser diagnostics in message-local coordinates. It observes effective original or complete formatted byte lengths through one raw-order `CandidateMessageAdmission` and adds a successful writable output to candidate storage only after admission. After all formatter calls finish, any formatter operational error wins over a prospective admission failure. Otherwise successful admission passes every retained writable output, including byte-identical output, to one `build_and_validate_write_back` call; failed admission returns its resource error without that call. The deferred bounded formatter API may later avoid materializing an oversized output before admission without changing this workflow.
...
- Read-only entries are excluded from write-back and difference reporting, not from ordinary formatter evaluation. A read-only entry with parser diagnostics uses `status: "diagnostic"`; otherwise write mode and check modes discard its complete formatted output, admit the original message as the effective candidate value, and record `status: "skipped"`, `changed: false`, and `readOnly: true`. The discarded formatted length is not `resource_limit_exceeded` because no formatted value can enter the candidate. The entry is not supplied as a `FormattedEntry`, and no `RawReplacement` is constructed for it. Human output, summary counts, file status, and the process exit code do not report or change because of a clean read-only skip.
...
For project-configured and editor-overlay catalog documents, editor adapters consume the resource-owned `ResolvedResources`, `ResolvedCatalogOverlay`, layered path resolver, registry, extraction, offset maps, and write-back re-escaping through this shared layer, so project membership, catalog assignment, top-level host-format resolution, conflict evidence, and mapping are identical in CI and in the editor. Project resolution runs first and is authoritative; editor-only ad-hoc opt-in is fallback-only under the project-policy states above. An overlay-owned target still uses the same host adapter contract, but its membership is intentionally editor-local until persisted in project configuration. Because the overlay carries no scope or locale, its extracted entries remain in per-document workflows and are not projected into the 014 definition-artifact contract. An overlay-only change therefore invalidates only affected document-local classification, extraction, mapping, diagnostics, and formatting state under 009; it does not change project inventory, completeness, linker artifacts, typed-key models, or `LinkOutcome`. Standalone `.mf2` documents continue through the editor-owned adapter defined by [009-ox-mf2-phase-3d-lsp-editor-design.md](./009-ox-mf2-phase-3d-lsp-editor-design.md), not catalog extraction. Surface behavior stays intentionally different: editors convert mapped spans to editor position encodings, clear mapped diagnostics when a document version or mapping changes, leave the current generation empty after a host parse failure, and no-op on stale document versions, while the CLI reports host parse failures as target-local operational errors and writes files directly.

## Catalog-Level Checks

Catalog-native structural linting remains a deferred follow-up in both design completion and implementation. Nothing in this section schedules a rule or adds a reporter/result variant to the current resource milestone. The entry identity, key-domain, scope, and locale-binding constraints already fixed below are inputs to a future dedicated linter addendum; that addendum must complete the product and output contracts before the first catalog-native or L0/L1 linker-backed rule lands. Separately, 014 M0 is the first scheduled consumer of explicit scope plus `path` and `fixed` locale binding and promotes those configuration and metadata contracts together.

Catalog-native checks operate only on affirmative structural evidence from entries that are present in successful extraction artifacts. A within-file duplicate-key rule groups complete concrete structural paths without occurrence and therefore operates without locale metadata. A future comparison between two present entries may use their domain-qualified catalog keys, scope, and locale metadata, but it must diagnose a concrete structural inconsistency supported by those entries rather than infer that another key, locale, reference, or catalog should exist.

Key parity inferred from absence, missing or orphaned translations, unused messages, ambiguous definitions, unresolved references, fallback coverage, and every other application-level cross-file or cross-locale analysis are outside the catalog-native rule track. The language-neutral artifacts, completeness evidence, reachability analysis, locale/fallback resolution, finding identities, and bundle-plan architecture for those cases are owned by [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md). The linker consumes this document's extraction, catalog-key, explicit-scope, and locale-binding contracts; it does not introduce another catalog parser. Its M0 milestone brings `scope` plus the `path` and `fixed` locale-binding fields forward through a coordinated 013/014 change rather than accepting undocumented placeholders. M0 produces linker-owned findings without depending on `intlify_lint`; only their later L0/L1 presentation as lint rules waits for the dedicated linter presentation addendum and the Phase 3C contracts.
...
The logical subject of a catalog-native finding is a typed variant under one comparison scope. A concrete-path subject carries `StructuralPathKey` for a host duplicate rule; a catalog-message subject carries the pair of `CatalogKeyDomain` and `CatalogKey` only for a structural comparison between present entries. Occurrence is absent from both subject variants. Any concrete evidence references the normalized host path and complete `{ path, occurrence }` `EntryKey`. A duplicate-key finding can therefore reference every duplicate occurrence of one concrete role without conflating XLIFF source and target.

A catalog-native finding has no owning host file merely because one related entry happened to be discovered first. The linter must not attach a multi-entry structural finding to an arbitrary entry, synthesize a nonexistent entry or zero-length host span, or duplicate one logical finding across all related entries. The current `results[]` file variants and `entries[].diagnostics` remain the contract for diagnostics that belong to one concrete message entry only. This paragraph does not reshape an 014 `LinkFinding`; the L0/L1 adapter preserves linker-owned identity, cardinality, and evidence under its separate presentation contract.

Before the first catalog-native or L0/L1 linker-backed rule ships, its linter addendum adds one dedicated project-level finding collection beside the file results. The collection owns cross-file and unanchored findings; existing `results[]` and `entries[].diagnostics` remain limited to one concrete file or message entry. The addendum must define the serialized comparison-scope identity, typed subject variants, normalized project-relative path and UTF-8 span for each concrete location, applicable complete `EntryKey`, optional primary location, canonical related-location order, deterministic finding order, human and editor presentation, `--quiet` and `--max-warnings` behavior, summary and problem-file counting, and operational-error precedence. One logical finding is counted once regardless of its number of locations, and an absent primary never becomes an arbitrary owner file or synthetic span. No placeholder JSON fields, Rust reporter variants, or output schemas are added before the first using rule and its complete output contract land together. L0/L1 linker-backed variants preserve the one-to-one 014 `LinkFinding` identity and evidence in this collection; they do not become catalog-native analysis.

## Scope and Locale Binding for Catalog Consumers

When message-linker M0 first consumes catalog identity, `CatalogConfig` gains an optional project-local `scope` name together with an optional, discriminated `locale` object. They are a pair: both may be absent for entry-level-only formatting and linting, while a locale-aware definition specifies both; specifying exactly one is invalid configuration. This is a coordinated prerequisite of 014 M0, and later catalog-level linting reuses the same resolved metadata rather than introducing another namespace or locale binding.

`scope` is the stable public name used by project configuration and reference producers. It contains one or more Unicode scalar values, compares exactly and case-sensitively, and is never trimmed or Unicode-normalized. It is not derived from catalog definition order, `include`, a matched path, or the config file location. Definitions that declare the same exact name join one logical scope across files and locales; no implicit comparison scope remains once locale-aware configuration lands.

The `locale` field does not accept a string shorthand, and locale capture is not embedded into `include` glob syntax. The two initial binding variants are a path capture:
...
The coordinated M0 extension adds `scope: Option<CatalogScopeName>` and `locale: Option<LocaleBindingConfig>` to the existing `CatalogConfig`. `CatalogScopeName` is a validated non-empty Unicode-scalar string newtype whose decoded UTF-8 length is 1 through 255 bytes inclusive, matching 014's `catalog_scope_name_bytes` protocol ceiling. Schema validation rejects empty values, and resolved configuration counts decoded bytes before constructing the newtype; exactly 255 bytes is valid and the first byte above is rejected without truncation or normalization. The resolved configuration also enforces the both-or-neither invariant before any consumer receives a catalog definition.

The resource layer separately owns an opaque `ResolvedLocale` newtype. Its checked constructor accepts one non-empty Unicode-scalar string whose decoded UTF-8 length is 1 through 255 bytes inclusive, retains the exact spelling, and exposes read-only string access. `Fixed` values construct it while resolved configuration is compiled; `Path` captures construct it after concrete catalog assignment; and the future `Host` binding constructs it from each adapter-provided entry locale. The type follows the same value grammar and fixed protocol ceiling as the 014 contract-owned `Locale`, but it is not an alias, re-export, or wrapper around that type and introduces no `intlify_resource` dependency on `intlify_contract`.

`scope` and `locale` remain absent from the initial resource-only schema and Rust `CatalogConfig`; both fields and the enum land in the same release and implementation sequence as 014 M0. Before that coordinated change, both fields remain invalid. Once present, strict validation rejects an empty scope, a definition containing only one member of the pair, unknown binding variants, a string `locale` value, and variant-inappropriate fields. The pair remains optional for entry-level resource workflows, but a linker definition input requires every participating catalog entry to resolve both one exact scope and one exact locale and fails before linking if a selected catalog lacks them. This promotion covers `Path` and `Fixed` only. The previously deferred `group` field is replaced before implementation by `scope`; it is never accepted as a schema alias or deprecated spelling.
...
The `Host` variant is a consumer-neutral resource capability. This document owns its configuration shape, adapter-provider capability, per-entry locale resolution, checked `ResolvedLocale` construction, and typed failure semantics. The milestone containing its first catalog-level consumer owns coordinated activation and maps the resource-owned failure onto that consumer's integration surface; neither the linker nor the linter defines a second provider or locale-resolution contract.

`from: "host"` resolves locale per entry rather than once per file, using the selected adapter's documented provider: the static inline-block `locale` for Vue entries or the XLIFF source/target rules above. It is not accepted as a placeholder before at least one intrinsic provider and its first catalog-level consumer ship. A catalog definition using it may match only adapters with a documented intrinsic-locale provider; matching another format is `config_validation_failed` at that definition rather than a fallback to path inference. If any selected entry has no non-empty locale under its provider's rules, the future catalog workflow returns an explicit resource-owned typed target-local operational failure and does not silently exclude that entry. Before `Host` ships, a coordinated 013 addendum must fix that failure's exact variant, stable CLI mapping, evidence, precedence, and consumer-surface projection; the first consumer reuses that contract rather than defining a linter- or linker-specific substitute.
...
Path captures, `fixed.value`, and host-provided entry locales must resolve to the resource-owned `ResolvedLocale` newtype described above. Resolution counts the exact captured, fixed, or host-provided bytes before retaining the value or projecting a linker definition. Exactly 255 bytes is valid; empty and 256-byte values are rejected without truncation, hashing, replacement, or fallback to another binding.

The host-owned post-extraction projection reconstructs the distinct contract-owned [`Locale`](./014-ox-mf2-message-linker-design.md#locale-identity) through its checked constructor from `ResolvedLocale`'s exact string value. That conversion reapplies the common protocol grammar and ceiling and checks any caller-selected lower `locale_bytes` budget for the linker invocation. A lower invocation budget can reject projection, but it never changes the resource value, resolved configuration identity, or resource crate's fixed admission contract.

When a `messages` section activates linker definition participation, every resolved catalog locale must be an exact member of the finite `messages.locales` production set. A `fixed.value` is checked during pre-discovery cross-section validation even when its include patterns later match no file. A `path` value is checked after a concrete selected path matches the capture pattern and before extraction. A valid path pattern with no selected file produces no locale and remains a complete empty inventory contribution rather than an error.

An out-of-set value is `config_validation_failed` with `details.reason: "catalog_locale_not_production"`, exact `scope` and `locale` fields, and no copy of the potentially large allowed set. A fixed binding uses `pointer: "/resources/catalogs/<index>/locale/value"` and omits `targetPath`; a path binding uses `pointer: "/resources/catalogs/<index>/locale/pattern"` and includes the slash-normalized project-relative `targetPath`. Capture mismatch is checked first because no locale exists to test in that case. Among concrete path failures, normalized target-path order and then catalog-definition index select the first error. The future `Host` variant must define equivalent entry-level evidence before it can participate in this cross-section contract.

Catalog definitions without the coordinated `scope` / `locale` pair remain entry-level-only inputs and are not checked against `messages.locales`. Projects that format or lint non-production locales keep those files outside linker-participating definitions rather than relying on silent linker filtering. No out-of-set definition enters the M0 locale-agnostic union, M2 fallback/coverage analysis, bundle planning, or prune reachability.

The resource layer otherwise preserves the string byte-for-byte: it does not trim whitespace, change case, replace `_` with `-`, validate BCP 47 or Unicode locale syntax, canonicalize deprecated subtags, or reject an otherwise valid non-empty Unicode-scalar sequence merely because it contains whitespace or control scalars. Locale identity comparison and canonical ordering use exact decoded UTF-8 bytes and are case-sensitive. Standards conformance, suspicious whitespace/control spelling, and canonical-spelling advice belong to future configurable linter rules, so project- or runtime-specific locale identifiers remain representable without changing resource extraction. Reporters and downstream integrations escape the opaque value for their output context and never concatenate it directly into a host or output path.

The resource layer resolves scope and locale identity as consumer-neutral entry metadata. Message-linker M0 projects production-set members into mandatory `MessageDefinition.scope` and `MessageDefinition.locale` while using a locale-agnostic key union; M2 applies fallback semantics. The future catalog-native linter may group the same metadata with entry identity for structural rules over present entries, then owns those rule definitions, severity, and reporting. It does not own or reconstruct 014 absence, fallback, reachability, completeness, or finding semantics. Scope and locale binding alone do not select a runtime locale, define locale fallback, change message text, or enable rules; production-set admission arises only from the coordinated cross-section linker workflow above.

All files matched by definitions with the same `scope` are parts of the same logical message collection. Before linker scope mapping, cross-locale logical grouping uses the tuple of the exact scope name, `CatalogKeyDomain`, and `CatalogKey`. The exact resolved locale identifies one locale-bearing definition within that group and is not part of the cross-locale group identity. The linker applies any scope mapping and uses the corresponding resolved-scope, domain, and key tuple; exact definition-collision identity additionally includes locale. The complete `EntryKey` continues to identify each reported occurrence inside that scope. Projects with independent message namespaces use different scope names so identical catalog keys do not create false cross-file relationships.

One locale-aware scope may contain successfully extracted entries from exactly one `CatalogKeyDomain`. Initial entry-level formatting and linting may still process a mixed-format catalog definition or a Vue artifact containing several inner domains because they do not compare `CatalogKey` values across entries or files. The coordinated 014 M0 orchestration resolves observed domains only after attempting the complete definition inventory and before constructing linker definitions or scanning configured reference sources.

Zero successfully extracted participating entries contribute no observed domain and do not make an otherwise valid scope fail. A recognizer or configured root may retain its explicit supported domain for such a scope; the workflow does not reject it merely because an empty catalog cannot confirm the declaration. A failed or deliberately omitted definition source likewise contributes no observation. Its definition side becomes `Partial` through 014 completeness evidence, and the workflow does not turn missing evidence into a domain mismatch.

If successful entries establish two domains for one scope, the orchestration reports `config_validation_failed` with `details.reason: "catalog_scope_key_domain_conflict"`, constructs no `MessageDefinitionArtifact` for any candidate group, and does not start reference-source scanning. It never silently partitions the scope. Conflict selection processes successful observations by slash-normalized project-relative logical path, then resolved catalog-definition index, then extraction entry order. Per scope, the first observation establishes the earlier domain; the first later observation with another domain is a conflict. The first such conflict in that complete order is reported.

The stable details are `pointer` and `conflictingPointer` to the later and earlier catalog definitions' `/scope` fields, `scope`, `domain` and `conflictingDomain`, plus `targetPath` and `conflictingTargetPath`. `domain` and `targetPath` describe the later conflicting observation; the `conflicting...` fields describe the earlier one. Both pointers and both paths may be equal when one composed host file or one catalog definition produces both domains. The top-level error path remains the loaded config file.

Outer host-format equality is neither necessary nor sufficient for this check. JSON, JSONC, JSON5, and Vue entries lifted from those inner adapters may share one `JsonPointer` scope despite different outer host formats. Conversely, one Vue target whose participating blocks use JSON and YAML, or one scope containing both XLIFF 1.2 and XLIFF 2.x entries, conflicts because their domains differ. This M0 resource/integration contract owns the error and its evidence; a later locale-aware linter reuses successfully bound domains and does not redefine the failure. A valid scope with no concrete assigned file remains non-erroneous.

Multiple catalog definitions form one scope by declaring the same exact `scope` name. Separate fixed-locale definitions therefore use:
...
The config resolver interns equal project-local scope names as one consumer-facing scope identity. Reordering definitions or changing a definition's glob without changing `scope` therefore preserves artifact identity. A definition with neither `scope` nor `locale` remains entry-level-only and is ineligible to contribute a linker definition or cross-locale comparison scope; a linker invocation that explicitly selects one of its catalogs fails instead of silently omitting it.

If one file is matched by multiple locale-aware definitions, de-duplication is valid only when they resolve to the same host format, comparison scope, and exact locale assignment for every extracted entry. For path and fixed bindings that assignment is one repeated file locale; for host binding it is the ordered per-entry locale sequence. The extracted catalog-key domain sequence is intrinsic to the one de-duplicated host artifact rather than independently assigned by either definition.

A canonical host-format disagreement remains the earlier `catalog_format_conflict`. After format resolution succeeds, a different scope or exact locale across assignments for one logical target, or across logical aliases in one physical-source group, is `config_validation_failed` with `details.reason: "catalog_binding_conflict"`. For the initial `Path` and `Fixed` variants, the error is selected before extraction. When deferred `Host` binding is promoted, equal-scope aliases may require the one already admitted extraction to expose their ordered per-entry locale sequences; a disagreement still fails before definition projection or reference-source scanning. Either stage emits no definition artifact, does not construct a `LinkRequest`, and never becomes `Partial(SourceFailed)`.

Conflict selection visits the physical group's logical paths in canonical primary-plus-alias order and then their contributing catalog definitions in array-index order. The first assignment establishes the expected binding; the first later unequal assignment is the conflict. Scope is compared before locale when both differ. Stable details contain `field` with exact value `"scope"` or `"locale"`, `pointer` and `conflictingPointer` to that field in the later and earlier catalog definitions, and `targetPath` and `conflictingTargetPath` for their slash-normalized project-relative logical paths. The two paths may be equal for overlapping definitions on one logical target. Values are not copied into the evidence.

M0 uses only the fixed `Project` artifact namespace, so namespace disagreement is not a reachable configuration case. A future namespace extension must add its own admission and error contract before it can make that tuple component configurable.
...
- whether overrides apply to entry-level rules only or also to future catalog-native structural checks
...
`tools/resource-bench/benchmark-phases.mjs` owns the closed phase/cost table and the resource-owned non-serialized extraction interval descriptor defined below, and `tools/resource-bench/result-schema.mjs` owns that tool's versioned result shape. A later phase, cost, or shared interval boundary becomes valid only when this design activates it and those resource-owned files are updated together; 001 does not maintain a duplicate cross-product registry.
...
- `fmt_catalog_output_admission_peak_memory`: ordinary per-entry formatter rendering plus raw-order candidate-message admission across expanding and byte-identical outputs, including the transient pre-admission output allocation
...
- `lint_catalog_e2e`
- sequential multi-file physical-group execution and stable ordered aggregation
...
When an 014 project-workflow E2E case actually invokes Tier 1 catalog extraction, `tools/messages-bench` also emits exactly one ordinary result record using the exact 013-owned `resource_extract` phase and `host_parse_and_entry_extraction` cost. The qualifying 014 phase/cost pairs are `message_project_link_e2e` / `complete_workflow`, `messages_emit_e2e` / `complete_workflow`, `messages_emit_check_e2e` / `complete_workflow`, and the M5 `messages_prune_e2e` / `dry_run_planning` or `structural_mutation` pair when that concrete case performs extraction.

The 014-owned `benchmark-profile.mjs` declares one checked companion mapping from each complete required E2E tuple to one complete imported extraction tuple. The two ordinary result records retain equal declared fixture, variant, scale, runtime, and execution model and retain their own exact phase, cost, and operation. Those five values are the complete cross-record equality set; phase, cost, operation, status, count payload, and checksum are validated independently under each record's owning contract and are never equality fields between the pair. No companion extraction tuple may be shared by two E2E tuples. The result document serializes no relation field; the messages result-schema validator applies the checked profile mapping. A mismatch in one of the five pairing fields, a record-local contract mismatch, missing, duplicate, or shared extraction record, nested metric, or second 014-owned extraction phase makes the messages result document invalid.

Every `resource_extract` / `host_parse_and_entry_extraction` result record uses exact metric token `duration`. That token is owned by this resource benchmark contract. The standalone resource profile, the exported interval descriptor, and an importing messages profile and result must all use that exact lowercase spelling; no alias, case variant, or importer-local remapping is valid.

The initial M0 extraction record and later qualifying records cover only their related case's sequential JSON physical-source-group extraction calls. Successful aggregation visits groups in the same canonical primary-source order used by project inventory and reports three checked `u64` structural counts plus one duration:

- `physicalGroupCount`: one for every invoked physical-source group;
- `hostInputBytes`: the exact selected host snapshot bytes supplied to `extract`, charging equal bytes in different groups independently;
- `extractedEntryCount`: every entry in each successful checked `ExtractedCatalog`;
- `elapsedMs`: the sum of only the non-overlapping measured `HostFormatRegistry::extract` intervals, converted to milliseconds once after the complete measured iteration set.

The three structural counts, ordered group list, and semantic result describe one workflow iteration. Warmup iterations do not contribute. Every measured iteration must reproduce those values and its per-iteration checksum exactly; a mismatch fails the benchmark rather than being averaged. `iterations` records the measured repetition count, while `elapsedMs` and the serialized checksum aggregate across those repetitions.

Discovery, metadata lookup, physical grouping, pre-extraction admission, definition projection, linker work, surrounding loop overhead, and file I/O outside the exact selected-snapshot extraction call are excluded. The related end-to-end record retains the complete workflow wall time, so the extraction record is not subtracted from or folded out of that measurement.

### Resource Extraction Measurement Boundary

`tools/resource-bench/benchmark-phases.mjs` owns and exports the one duration-boundary descriptor for `resource_extract` / `host_parse_and_entry_extraction`. Its stable boundary ID is `resource_host_parse_and_entry_extraction`, and its exact metric tag is `duration`. The ID carries no version suffix; the resource result-schema revision, and the coordinated messages revisions while imported, version its exact meaning. The descriptor owns that metric tag, its occurrence policy, first and final measurement markers, and ordered included and excluded marker sets for each non-overlapping `HostFormatRegistry::extract` call covered by the aggregate record. Its occurrence policy is one complete sequential interval per planned physical-source group in canonical group order; the successful occurrence count equals `physicalGroupCount`. A second occurrence may start only after the previous one stops.

The prepared input is one already admitted physical-source group, its exact selected host snapshot, resolved host format, and effective extraction limits. The measured interval begins immediately before invoking `HostFormatRegistry::extract` and ends only when that call returns one complete checked `ExtractedCatalog`. Discovery, metadata, source reads, physical grouping, pre-extraction admission, definition projection, checksum observation, aggregation, and every caller stage remain outside.

`tools/messages-bench/benchmark-phases.mjs` imports and re-exports that exact descriptor unchanged when a qualifying 014 E2E tuple activates the imported extraction record. It may attach owner `013` in its checked registry view, but it cannot copy, restate, replace, or extend the descriptor's ID, metric, or marker sets. The 014-owned workflow topology may separately reference this boundary ID as an allowed child of an E2E interval; that relationship is not part of the resource descriptor and grants 014 no authority over its interval. Neither the imported descriptor nor the topology is serialized. A boundary change is made first in this resource-owned module and advances the resource result-schema revision; when the pair is active in the messages benchmark, the coordinated 014 result-schema and benchmark-profile revisions advance as well.

### Resource Benchmark Checksum Contract

The resource benchmark owns one non-cryptographic `u32` checksum routine for this result-schema revision. It begins at FNV-1a offset basis `2,166,136,261`. For every input byte it XORs the current state with that byte and then multiplies by `16,777,619` with wrapping modulo `2^32`. JavaScript uses exact unsigned 32-bit bitwise operations, and Rust uses `u32::wrapping_mul`; both serialize the result as the same non-negative JSON safe integer.

Typed checksum input uses fixed binary framing rather than JSON spelling. Every field is `tag:u8 || payload_len:u64be || payload`. A sequence payload is `item_count:u64be` followed by `item_len:u64be || item_payload` for each item in semantic order. Validated strings contribute exact UTF-8 bytes, unsigned values use their fixed big-endian width, booleans use exact `0x00` or `0x01`, and an option uses `0x00` for absent or `0x01 || value` for present. Unknown, duplicate, reordered, or omitted fields are invalid checksum input rather than alternate encodings.

Before measured extraction begins, the harness computes `selectedSourceChecksum` by applying the same routine to exact ASCII domain bytes `intlify-resource-selected-source-v1`, one zero byte, and the complete exact selected host-source bytes. This value is benchmark-local source evidence. It is not 014 `InputFingerprint`, artifact identity, cache identity, authenticity evidence, or a field of `ExtractedCatalog`.

Each successful group then produces one per-iteration checksum over the following framed fields:

1. tag `0x01`, the canonical physical-group path sequence, with the primary project-relative path first and every logical alias following in the already established canonical order; each path is its exact UTF-8 segment sequence;
2. tag `0x02`, the resolved host format's canonical lowercase ID;
3. tag `0x03`, `selectedSourceChecksum` as `u32be`;
4. tag `0x04`, every entry in raw entry order.

Each entry record uses these ascending tags:

1. `0x01`: exact `StructuralPathKey` UTF-8 bytes;
2. `0x02`: `occurrence` as `u32be`;
3. `0x03`: the active schema revision's exact lowercase `CatalogKeyDomain` token;
4. `0x04`: exact `CatalogKey` UTF-8 bytes;
5. `0x05`: optional exact `displayKey`;
6. `0x06`: exact decoded `message_text` UTF-8 bytes;
7. `0x07`: absolute `raw_value_span.start` followed by `end`, each `u32be`;
8. `0x08`: the complete public `MessageOffsetMap` behavior described below;
9. `0x09`: `read_only` as one boolean byte.

Tier 1 admits exact domain token `json-pointer`. Adding a format or domain to this benchmark requires its owner to define the exact canonical token and update the resource benchmark result-schema revision before that value enters checksum input.

Offset-map input observes public behavior rather than private segment storage. For every message byte position from zero through the complete message length, it records `map_span(p..p)` as one pair of absolute `u32be` raw offsets. It then records `map_span(p..p + 1)` for every byte position below that length in ascending order. These valid public queries must all succeed. Together they retain empty-position behavior plus the start/end behavior across identity, unescape, raw-only, and empty-message boundaries without making segment coalescing or allocation layout part of the checksum. This observation runs outside every measured extraction interval.

Artifact-local handles, pointer identity, allocation layout, timing, cache state, worker identity, and private offset-map segment representation are excluded.

For one measured iteration, the extraction record applies the framed routine to exact domain `intlify-resource-extraction-record-v1`, the three deterministic structural counts as checked `u64be` values, and each `(physical-group path sequence, group checksum:u32be)` pair in canonical group order. Fixture, variant, scale, runtime, execution model, operation, iterations, elapsed time, allocator state, cache state, and worker identity are excluded from that per-iteration value.

After requiring every measured iteration to produce the same value, repetition aggregation begins at zero and wrapping-adds each measured per-iteration `u32` checksum in measurement order. The resulting `u32` is the record's serialized checksum. Warmups never enter it. The checksum routine, framing, and repetition aggregation are part of the resource benchmark result-schema revision rather than a product fingerprint or authenticity contract; changing any of them requires that schema revision to change.

A measured record requires all planned groups to succeed and forbids a prefix checksum. An extraction failure fails the benchmark case and cannot emit a schema-valid measured extraction/E2E pair; no successful aggregate payload, prefix checksum, or measured E2E record survives.

The messages benchmark retains the measured per-iteration resource checksums as invocation-local conformance state. It requires them all to be equal, recomputes the wrapping repetition aggregate from that complete measured sequence, and requires the recomputed value to equal the checksum serialized in the imported resource record. It never compares that checksum for equality with the related E2E record's independently defined checksum. The companion relation validates only the fields and one-to-one association defined above. Neither benchmark treats a checked-in or previous resource/messages result as a golden checksum; cross-invocation changes remain reportable observations under 014's benchmark policy.

The record does not rename the phase to `message_resource_extract`, fold it into `message_definition_project`, or create a duplicate 014-owned extraction vocabulary. A later mixed-format or concurrent case must extend this 013-owned payload, interval, aggregation-order, and checksum contract before changing those semantics.

Extraction and re-escaping costs must be reported separately from parser, semantic, rule, formatter, and file I/O costs.

## Validation

- Messages benchmark fixtures cover every qualifying 014 project-workflow E2E phase/cost pair, require exactly one ordinary `resource_extract` / `host_parse_and_entry_extraction` record for each complete profile-declared E2E tuple, and reject a profile mapping to a missing, duplicate, or already-associated extraction tuple. They import and re-export the exact resource-owned `resource_host_parse_and_entry_extraction` descriptor with owner `013`, require its exact `duration` metric in the descriptor, profile, and imported result, reject a missing, replaced, mutated, or locally declared descriptor for that pair, and require the same occurrence policy, first/final markers, and included/excluded marker order as the resource harness. They require equal fixture, variant, scale, runtime, and execution-model fields; reject mismatched pairing fields, a nested extraction metric, a renamed phase/cost, or a measured E2E record after extraction failure; and prove that neither the descriptor nor the checked one-to-one relation is serialized. Sequential JSON aggregation fixtures permute discovery and worker-ready order while preserving canonical physical-group order, exact checked group/host-byte/entry/time values, and group, per-iteration, and repetition-aggregate `u32` checksums; require equal per-iteration structural counts and checksums; recompute and validate the serialized wrapping aggregate from the current invocation's complete measured per-iteration resource sequence without comparing it to the distinct E2E checksum; use shared JS/Rust vectors for source, group, entry, offset-map, record, and wrapping-overflow cases; prove that elapsed time, warmups, and benchmark metadata do not change either checksum input; and exclude discovery, I/O, admission, projection, checksum observation, loop overhead, and linker time from the extraction interval.
...
- Tier 1 JSON frontend tests cover complete syntax validation before candidate traversal, exact nesting-limit acceptance and first-opening-token rejection beyond the limit, exact 256-token catalog-key acceptance and first attempted token `257` rejection, raw-order tape events across duplicate members and mixed containers, lazy decoding of only candidate values and the path segments needed to identify them, and identical syntax, depth, catalog-key-token, raw-order, and representability precedence during candidate re-extraction. Private unit tests also verify that `JsonSyntaxTape` is discarded after artifact construction and that the retained artifact contains neither a decoded JSON DOM nor decoded strings for irrelevant subtrees.
...
- Tier 3 YAML fixtures cover fixed 1.2 Core resolution and explicit tags, accepted and unsupported version directives, accepted and byte-preserved reserved directives without resource warnings, malformed directives, optional single-BOM lifting and preservation, empty and single-document streams, multi-document rejection, custom tags including a `%TAG` handle that resolves to a used non-Core tag, aliases and merge keys, typed-pointer escaping and numeric canonicalization, duplicate paths and ancestors, every scalar style, `LF`/`CRLF`/`CR` folding and continuation maps, block indentation and chomping maps, read-only block reporting, original-style changed-value attempts, every canonical double-quoted escape branch, and full candidate reparse/value verification.
...
- Candidate-message admission fixtures cover exact formatter output at `message_bytes`, complete observed lengths above that limit, per-message precedence over running-total failure at one entry, exact cumulative acceptance at `total_message_bytes`, the first raw-order byte over, original-value admission for unselected and diagnostic-bearing entries, clean read-only output discard, byte-identical writable output admission, and foreign, repeated, skipped, or out-of-order handle rejection. Integration fixtures verify that outputs are added to retained candidate storage only after admission, admitted output stays within the total budget while one ordinary result may exist transiently before admission, later successful outputs are dropped after the first prospective admission error, a later formatter operational error still wins with lowest-raw-order selection, admission failure prevents integrated write-back, and successful admission is defensively rechecked against the actual borrowed strings.
...
- CLI end-to-end fixtures: lint and fmt write/check over catalog fixtures, including broken-entry and broken-host-file cases, every resource-specific public error code and internal reason, shared I/O/config code reuse, non-Unicode project-root abort before config with `projectRoot: null`, non-Unicode explicit config rejection before config I/O, non-Unicode repeated ignore-path rejection in CLI order after config validation but before ignore reads or discovery with exact `ignorePathIndex`, empty setup-error results without lossy path output, non-Unicode input rejection before resource membership without lossy path output, config-free direct JSON and ASCII-case variants with no content sniffing, unknown and known-but-unshipped direct extensions, canonical Tier 1 `[".json", ".mf2"]` supported extensions, multiple individual operands, shell-expanded individual-path equivalence, merged direct-plus-bulk origin de-duplication with direct classification and one result, CLI-native glob, explicit directory, and no-operand implicit-`.` policy requirements with zero-target success, root-external file and stdin behavior under absent, empty, and non-empty project policies, root-internal logical symlinks whose physical targets are outside, policy classification before command-specific ignore filtering, explicit empty and excluded file skips without results or counters, policy-skipped and ignored stdin byte-exact passthrough, intentional fmt/lint ignore asymmetry, zero-entry catalog file and stdin formatter `Unchanged` and linter `clean` results, exact-limit and first-byte-over-limit bounded reads for file and stdin targets, oversized valid and oversized invalid-UTF-8 precedence, under-limit invalid-UTF-8 handling, no resource preflight for policy-skipped or ignored targets, symlink and hard-link physical grouping, same-classification aliases with serialized rereads after success and continued processing after pre-write failure, first- and middle-alias injected `output_write_failed`, empty predecessor entries and exact predecessor counts after write failure, no subsequent filesystem access in the failed group, exact `alias_processing_blocked` details and counts for every remaining alias, continued unrelated physical groups, no fail-stop in check or lint mode, conflicting-classification alias rejection and continued unrelated groups, metadata-failure reporting without path fallback, one integrated write-back call and identical validation across write/check/list-different/stdin modes, formatter operational errors preventing candidate construction, lowest-raw formatter error precedence, raw-entry candidate error precedence independent of formatter task completion and input slice order, mixed changed-plus-diagnostic formatter results and overlapping summary counts, original-versus-candidate diagnostic coordinate selection by output mode, diagnostic mapping and successful-result construction before file write or stdin host emission, raw-entry/diagnostic/primary-then-label mapping-failure precedence, mapping failures producing no write, no host-source emission, and no alias fail-stop, read-only parser diagnostics and discarded clean formatter differences, per-entry operational failures with no partial file write, deterministic ordering, complete nested entry arrays, exact optional-string `displayKey` emission immediately after `key` in both entry variants and omission without `null`, Tier 1 JSON omission, host-coordinate-only diagnostic JSON shapes, and JSON-only read-only skip reporting.
- Configuration fixtures: `resources` section validation; preserved `PolicyAbsent` for missing `resources`, missing `catalogs`, and explicit config files without that field; rejection of explicit `resources: null` and `catalogs: null` without `Option` collapse or nullable schema output; valid `PolicyEmpty` for `catalogs: []`; exact public `CatalogPolicyState` values without definition leakage; non-empty `Unmatched`, all-including-definitions `Excluded`, and at-least-one-survivor `Matched` resolution; every accepted `*`, `?`, standalone `**`, escape, and literal-`!` behavior in the resource glob grammar; rejection of empty, absolute, drive-prefixed, UNC, dot-segment, empty-segment, embedded-`**`, trailing-escape, character-class, brace, and extglob patterns; OS-independent case-sensitive membership and exact Unicode matching; valid no-match definitions; exact canonical format ids; rejection of alias, uppercase, whitespace-padded, list-like, and known-but-unshipped explicit `format` values; platform-independent final-extension extraction for `messages.json`, `.json`, `messages.`, `.config.json`, and `messages..json`; OS-independent ASCII-case-folded extension mapping and alias pairs; direct-extension lookup and canonical shipped-extension enumeration; all three `HostFormatClassification` variants; preserved qualifying extension spelling and empty no-extension details in errors; known-but-unshipped and unrecognized extension `resource_format_unsupported` targets; same-format overlap de-duplication; an unrecognized broad match resolved by one specific format assignment; all-unrecognized overlap remaining unsupported; concrete-path different-known-format conflicts that exclude unrecognized and excluded definitions from exact pointer selection; target path, deterministic selection, empty results, and omitted counters. The coordinated 014 M0 configuration fixtures additionally cover exact non-empty scope names at the 1-byte and 255-byte decoded UTF-8 boundaries, first-over rejection after JSON decoding, no trimming or normalization, `scope` / `locale` pair validation, rejection of the removed `group` spelling, `Path` and `Fixed` tagged bindings, capture-pattern validation, path matches, exact locale identity at the 1-byte and 255-byte decoded UTF-8 boundaries, empty and 256-byte locale rejection for path and fixed bindings, exact preservation of case, whitespace, control scalars, and canonically equivalent Unicode without trimming, normalization, BCP 47 validation, or rewriting, rejection of string shorthand, same-scope joining across definitions, single-domain scopes, zero-entry scopes, `catalog_scope_key_domain_conflict`, and overlap conflicts across scope or locale assignments. When the consumer-neutral `Host` binding lands with its first catalog-level consumer, its fixtures cover provider-capability admission, Vue inline-block and XLIFF source/target locale resolution, missing-locale typed failure, stable surface mapping, and preservation of resource-owned semantics across consumers. Later locale-aware catalog-native structural-lint fixtures reuse those successful locale results for concrete-path and present-entry domain-qualified catalog-key subjects without introducing absence-based parity or usage findings.
- Coordinated linker-binding fixtures require `catalog_format_conflict` to win before binding comparison, then exercise scope-only, locale-only, and simultaneous scope-plus-locale disagreement both across overlapping definitions for one logical path and across different aliases in one physical-source group. They require canonical path-then-definition selection, scope-before-locale field precedence, exact `field`, narrow `pointer` / `conflictingPointer`, exact path evidence including the allowed equal-path case, no copied binding values, no initial `Path` / `Fixed` extraction, no reference-source scan, no `LinkRequest`, and no `SourceFailed` completeness state. The future `Host` promotion adds equivalent fixtures in which one successful admitted extraction exposes unequal per-entry locale sequences but still produces only the same fail-complete configuration result before projection.
- Coordinated late-gate ordering fixtures combine failures across different canonical target paths, physical groups, scopes, and recognizer/root declarations. They require path-independent fixed-locale validation first; target path before per-target format, capture, and captured production-locale order; complete target assignment before canonical physical-group binding; complete admitted-group extraction before the project-global domain admission stage; and its catalog-domain gate before its recognizer/root-domain gate. They prove that exactly one error is selected independently of config member order, filesystem traversal, alias discovery, cache state, and future worker completion. Domain-admission fixtures retain every successful extraction privately until both ordered gates pass, construct no provisional contract artifact, and map ordinary extraction or projection failures to `SourceFailed` only when no configuration contradiction wins.
- Overlay-resolution fixtures cover resource-owned normalized-object validation, root and `/catalogs` pointer evidence, grammar equivalence with project catalogs, equivalent missing and empty overlay catalogs, rejection of `scope`, `locale`, and every other project-only field, private fields and infallible compilation, compile-time `Send + Sync` for `ResolvedCatalogOverlay`, one combined lookup per document, project-only and overlay-only matches with exact provenance, overlay fallback after project `PolicyAbsent` and `Unmatched`, fallback blocking after project `PolicyEmpty` and `Excluded`, authoritative project `Matched` resolution without path-level overlay evaluation, overlay-internal same-format de-duplication and different-format conflicts only in the fallback branch, deterministic same-layer source-qualified decisive references, canonical editor conflict fingerprints, config-path presence only for project-layer conflicts, no definition-artifact projection or linker-completeness effect from an overlay-owned match, and transition from overlay to project ownership without assignment reconstruction or duplicate extraction.
- Editor classification, range, and failure fixtures: standalone `.mf2` or exact case-sensitive `messageformat2` language-id precedence over catalog matches with no `mf2` alias, no config-free JSON auto-detection, normalized ad-hoc overlay shape validation, project-root-relative glob and exact-path matches, extension-derived and explicit-format overlay classification, non-file URI rejection, project-authoritative matches, ad-hoc opt-in only for absent or unmatched project policy, blocked ad-hoc opt-in for empty, excluded, and matched project states, no same-path cross-layer de-duplication or conflict, ad-hoc/ad-hoc errors when fallback is eligible, invalid project configuration preventing overlay fallback, editor-local coverage labeling, overlay-only changes invalidating only affected document-local state while retaining every project inventory/linker/model value and starting no linker work, transition to project ownership without duplicate extraction, matched-to-unmatched configuration transitions that discard resource state and publish empty diagnostics without `unsupported_input_file`, URI-and-version document identity with no stat or canonicalization, independent state for two URIs that name one symlinked or hard-linked physical file, no cross-URI invalidation or CLI alias fail-stop, exact host-byte-limit acceptance and one-byte-over rejection before host parsing, defensive extraction recheck, integrated single-entry and multi-entry write-back, `Unchanged` no-edit behavior, validated `Changed` replacements only, selected-entry input with unselected originals retained, formatter operational failure before resource invocation, strict incoming UTF-8/UTF-16/UTF-32 range conversion, out-of-bounds, reversed, and split-scalar invalid-params rejection without core calls, half-open non-empty range intersection, zero-length entry point and caret selection, document-change diagnostic clearing and an empty current-generation publication after parse failure, actionable original-extraction resource diagnostic replacement, candidate `resource_limit_exceeded` returning no edits without diagnostic publication or retained-state mutation, transactional config/internal failure retention, and recovery without formatting edits from a failed state.
- CLI execution tests cover stable sequential physical-group order, serial aliases with rereads, write-failure containment, continued unrelated groups, stable normalized-path reporting, and deterministic summaries and exit status. Deferred scheduler tests and memory gates are owned by the 005 follow-up. Resource-level determinism tests additionally cover stable entry ordering and lowest-raw-order operational-error selection when future chunked execution is enabled.

## Relationship to Other Documents

| Document | Owns |
| --- | --- |
| [005-ox-mf2-phase-3-tooling-transport-design.md](./005-ox-mf2-phase-3-tooling-transport-design.md) | Phase 3 boundary; sequential CLI execution and deterministic aggregation, plus the deferred common worker-runtime contract |
...
| [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md) | application-reference artifacts; CLI-owned physical-source grouping; host-owned pre-extraction group admission; project-global aggregation and admission of successful resource-owned domains before any contract artifact construction; and checked post-extraction linker-definition projection into one artifact per successful physical-source group, without a resource-to-contract dependency or resource-owned file aggregation; selector/domain contracts and linker-owned matching; language-neutral linking, reachability, locale/fallback resolution, and bundle-plan generation; the future M5 consumer of a separate resource-owned structural-deletion boundary |
...
- Bounded formatter rendering is deferred to the [007 follow-up contract](./007-ox-mf2-phase-3b-formatter-design.md#bounded-rust-rendering-for-resource-consumers). The initial formatter integration materializes ordinary `format_parsed` output before resource-owned candidate admission; the follow-up may reduce transient peak memory without changing resource errors, ordering, or validated write-back.
- The common CLI worker scheduler and file-level parallel execution are deferred to the [005 follow-up contract](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary). Initial CLI catalog processing is sequential, while `intlify_resource` remains synchronous, scheduler-neutral, and safe for concurrent consumer calls.
- The 014 M5 `messages prune` milestone requires a separate structural-mutation boundary in `intlify_resource`. It must not widen `build_and_validate_write_back`, whose formatter-facing contract continues to replace message value spans while preserving entry count, identity, order, and metadata. Linker and CLI orchestration own the semantic decision and map selected definition evidence back to artifact-local entry handles; the resource layer owns format-specific structural edit planning, complete candidate reparse and re-extraction, and fail-complete validation that exactly the requested entries disappeared while every surviving definition and message value remained unchanged under the format's deletion-aware identity rules. The initial structural operation is deletion only. No partial edits are returned for a foreign, duplicate, read-only, structurally unsupported, or otherwise invalid target. Before M5 implementation, a dedicated addendum must fix the concrete Rust API and outcome types, deterministic error precedence and public mapping, JSON member and array-element deletion behavior, duplicate-occurrence renumbering, empty-ancestor handling, raw edit ordering, candidate equality rules, and the deletion capabilities of each shipped adapter. Until that contract lands, current validated write-back does not support pruning.
...
- Third-party host-format adapter providers are a future extension milestone. The planned direction is a separately versioned, out-of-process provider protocol rather than publishing the crate-private Rust dispatch trait or loading Rust dynamic libraries with an unstable ABI. A provider executable is enabled only by explicit project configuration; there is no extension-based auto-discovery, content-triggered execution, or implicit override of a built-in format. Its versioned manifest advertises namespaced format ids, extension mappings, protocol versions, extraction/write-back capabilities, and provider-local catalog-key domain names. Conceptual protocol operations describe the provider, extract one complete host source into ordered declarative entries and offset maps, and plan or materialize re-escaping for selected entries. The host retains ownership of fixed resource budgets and validates every returned UTF-8 span, ordering, edit-disjointness, identity, offset map, read-only marker, output size, and deterministic error before constructing an artifact. For changed output, the host applies only validated bounded replacements, asks the same provider to re-extract the complete candidate, and enforces the same identity, metadata, effective-message, and resource-limit comparisons as built-in adapters.
  ...
  Linker selector matching does not extend the provider execution boundary. Built-in selector-domain specifications ship through the versioned `intlify_contract` contract, and `intlify_linker` alone enumerates definitions and executes those semantics. Definitions in a future provider-owned domain may participate in domain-qualified `Exact`, `AllInScope`, and `UnboundedDynamic` behavior after the domain identity is validated because those cases require no provider-defined matcher. A reference artifact may use `Prefix` or `Pattern` for that domain only after the follow-up protocol extends `intlify_contract` with a bounded, declarative, versioned selector-domain descriptor schema and shared conformance fixtures, and the host validates the provider's declared descriptor under that contract. Provider processes never inject executable matcher callbacks into `intlify_linker`; until that descriptor contract exists, `Prefix` and `Pattern` for provider-owned domains are rejected during artifact compatibility validation rather than widened.
...
- Application-reference selectors and linker analysis are overviewed in [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md). Its M0 milestone promotes explicit `scope` and the fixed `Path` / `Fixed` locale-binding object through a coordinated 013/014 change; before M0, both fields remain invalid in the resource schema. M0's linker-native finding contract is independent of the linter. Intrinsic-host binding remains deferred consumer-neutral 013 work and may be promoted with whichever catalog-level consumer needs it first; it does not wait for catalog-linter implementation. The L0/L1 lint presentation adapters and concrete catalog-native structural checks remain separate deferred work tracked in the sections above. Before the first rule from either track ships, a dedicated linter addendum must choose its rule id, default severity and preset membership, complete inputs, exact project-level finding collection and machine codec, serialized scope and typed subject identities, primary and related location evidence, deterministic ordering, text/editor presentation, `--quiet`, `--max-warnings`, summary counting, and every new operational-error detail and precedence rule. L0/L1 preserve the existing 014 finding identity, cardinality, and evidence through their one-to-one presentation adapter. Project-level finding fields and reporter variants are not reserved or accepted in the initial schema.
...
- Intra-file parallelism remains disabled until the deferred file-group scheduler is implemented and representative benchmarks justify contiguous-entry chunking on the same CLI-owned pool under the determinism rules above.
