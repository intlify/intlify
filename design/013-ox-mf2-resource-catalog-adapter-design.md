...
`build_and_validate_write_back` is the only consumer-callable re-escaping and candidate-construction boundary. It first validates every supplied handle against the artifact and rejects foreign, duplicate, nonexistent, or read-only inputs as `internal_error` with `details.reason: "resource_write_back_failed"` and `details.phase: "validate_write_back"`; valid inputs are indexed by their artifact entry rather than processed in caller slice order. Adapter-specific re-escaping and replacement application remain private implementation steps, so CLI and editor consumers cannot bypass candidate limit checks or full validation by calling a lower-level method.
...
An `output_write_failed` result in formatter write mode is the one fail-stop case. Phase 3B direct writes are not atomic and provide no rollback guarantee, so the physical file may be unchanged, partially written, or otherwise indeterminate after the failure. The CLI retains the failing alias's normal `output_write_failed` result, does not stat, open, read, parse, format, or write any later alias in that physical group, and gives each remaining alias a target-local `alias_processing_blocked` result. Unrelated physical groups continue. The implementation applies this rule conservatively even when a particular operating-system error appears to have happened before the first output byte, because the shared write boundary does not certify an unchanged file state.
...
`pointer` identifies the later conflicting catalog definition and `conflictingPointer` the earliest definition that assigned a different format. Both point to the complete definition because either format may be extension-derived rather than present at a `/format` field. `targetPath` is the concrete project-relative slash-normalized path. The error is selected under the deterministic path and definition-index order above and follows any discovery/input errors already retained in the top-level array. Its JSON envelope has `summary.status: "error"`, `results: []`, and no command-specific target counters because a complete unambiguous target set was not established.

Top-level resource-selection error ordering therefore follows pipeline order: retained discovery/input errors first, then the one selected catalog assignment conflict when present; otherwise retained discovery/input errors followed by every physical-group conflict in group-path order. Structural config errors abort before this aggregation. Once target processing begins, metadata, read, extraction, formatter/linter, validation, and write failures belong only to their logical target's `results[].errors` and are never duplicated in the top-level array.
...
- When that one file write returns `output_write_failed`, the current logical target reports the write failure and the CLI scheduler applies the physical-group fail-stop rule: every later alias receives `alias_processing_blocked` without another filesystem access. A failure before the write boundary and every no-write mode continue to the next alias normally.
