...
After every group passes configuration admission, the CLI invokes `intlify_resource` once for each group and retains the successful extraction artifacts without projecting them into `intlify_contract`. Once the complete definition extraction inventory has been attempted, the host aggregates resource-owned domain observations, selects `catalog_scope_key_domain_conflict` when applicable, and then applies 014's recognizer/root domain comparison. Neither domain gate reparses a host source or constructs a provisional contract artifact.
