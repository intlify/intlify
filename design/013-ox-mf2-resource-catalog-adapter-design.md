...
- Coordinated late-gate ordering fixtures combine failures across different canonical target paths, physical groups, scopes, and recognizer/root declarations. They require path-independent fixed-locale validation first; target path before per-target format, capture, and captured production-locale order; complete target assignment before canonical physical-group binding; complete admitted-group extraction before catalog domain conflict; and catalog domain conflict before recognizer/root domain mismatch. They prove that exactly one error is selected independently of config member order, filesystem traversal, alias discovery, cache state, and future worker completion. Domain-gate fixtures retain every successful extraction privately until both project-global gates pass, construct no provisional contract artifact, and map ordinary extraction or projection failures to `SourceFailed` only when no configuration contradiction wins.
