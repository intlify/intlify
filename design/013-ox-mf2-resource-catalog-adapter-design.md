...
- The map's crate-private builder uses an immutable segment sequence with `Identity`, `Unescape`, and `RawOnly` segment kinds. It validates ordered, gap-free, non-overlapping coverage of every raw byte in `raw_value_span`; complete ordered coverage of message bytes; segment-kind length rules; and UTF-8 boundaries. No raw gap is implicit: syntax that consumes no message bytes must be represented by `RawOnly`. Invalid adapter-produced maps are internal invariant failures.
- Consumers map core ranges through `MessageOffsetMap::map_span(Utf8ByteSpan) -> Result<Utf8ByteSpan, OffsetMapError>` rather than inspecting segments. Validation first returns `Reversed` when `start > end`, then `OutOfBounds` when `end` exceeds the map's complete message byte length. No UTF-8 scalar-boundary condition is imposed on an otherwise ordered in-bounds byte span. Non-empty ranges apply the escape-boundary and raw-only-gap rules above. An empty span maps to the host position before the next message byte, or before trailing raw-only syntax when it is at message end; an empty position inside the output of one `Unescape` unit therefore maps to that unit's raw escape start. A returned `OffsetMapError` indicates internal consumer misuse of a published map and converts to `internal_error` with `details.reason: "resource_offset_map_failed"` and `details.phase: "map"`; the consumer includes the containing entry key when it invoked the map for a known entry.
...
`FormattedEntry` is the complete successful formatter output for one selected writable entry, whether or not its text changed. A consumer omits an entry whose formatter result contains parser diagnostics, an unselected entry in an editor range request, and every read-only entry after retaining any parser diagnostics for reporting. After all selected formatter calls complete without an operational error, the consumer passes all remaining outputs together in one call to `build_and_validate_write_back`. An empty input is valid.
...
Candidate extraction necessarily mints new `EntryHandle` values under a newly allocated artifact identity. Raw value spans, offset maps, raw spelling, the complete candidate source, and adapter-private parse state may also change as required by the validated replacements. Those artifact-local and source-coordinate changes never relax the equality requirements for consumer-visible identity and capability metadata above. A candidate resource-limit overrun remains `resource_limit_exceeded` with `details.phase: "validate_write_back"`; it is expected input-size handling rather than an adapter invariant failure. Artifact-identity exhaustion retains its dedicated internal reason above. Any other parse, extraction, identity, metadata, or value mismatch fails the whole operation as `internal_error` with `details.reason: "resource_write_back_failed"`; no `WriteBackOutcome`, candidate artifact, or partial replacement set is returned. A successful changed result contains the complete re-extracted `ExtractedCatalog` for the validated candidate, including its exact candidate source, entries, and offset maps.
...
1. **Preflight and dispatch.** A byte-owning consumer checks `host_bytes` before UTF-8 decoding, and `extract` repeats the check against the supplied `Arc<str>`. The already resolved shipped `HostFormat` selects one built-in adapter; registry dispatch does not repeat catalog membership or configuration resolution.
2. **Span-preserving host parse.** The selected adapter parses the complete host syntax while retaining raw token and value spans and enforcing the parser-local nesting limit. Value-only deserialization in the plain `serde` style is insufficient. A syntax or depth failure returns before entry traversal.
...
Any failure returns no `WriteBackOutcome`, candidate artifact, partial replacement set, or consumer-visible edit. This validation is identical for CLI write, check, stdin, and editor formatting; those consumers differ only in how they use the successful outcome.
...
The resource crate owns section-local normalization and resolution rather than exposing config data for consumers to reinterpret. After the unified config loader reaches the `resources` step in the shared validation order, it delegates this section's exact shape, field, pattern, and format checks to the resource-owned validator, which returns `ResourcesConfig` only on success. `ResourcesConfig::resolve(self) -> Result<ResolvedResources, ResourceConfigError>` compiles the already validated glob strings and retains definition indices needed for deterministic evidence; compilation must not introduce a dependency-specific second pattern grammar or validation order.
...
`Unmatched` means no definition includes the path after its own excludes. `Matched` exposes read-only access to the final `HostFormatClassification`, the original extension spelling used for classification, every matching definition index, and the subset that contributed the one resolved format assignment. A matched result with no contributing assignment carries `UnrecognizedExtension`; a unique known assignment carries `Shipped` or `KnownButUnshipped` as appropriate. A conflict exposes the earliest and later assigning definition indices selected by the fixed resolution order; the caller combines those indices with the already supplied path and loaded config path to form surface-specific error details. Unrecognized matches never appear in the assigning subset or conflict evidence.

CLI and editor consumers must call this resolver for project catalog membership, same-format de-duplication, extension classification, and conflict selection. They may add surface-specific selection, reporting, or editor ad-hoc overlay behavior around its result, but they do not compile resource globs or repeat the project-definition overlap algorithm independently.
...
`crates/intlify_cli` owns parallel execution for `intlify fmt` and `intlify lint`. After setup, discovery, ignore filtering, supported-input classification, and physical identity inspection complete, each file-mode command creates at most one bounded, command-scoped worker-thread pool. No pool is created when no runnable physical group remains. Otherwise, its default width is the smaller of the runnable physical-group count and the operating system's available parallelism, with a minimum of one. Failure to obtain that parallelism or to initialize, dispatch, execute, or join the worker runtime is the command-fatal `cli_worker_runtime_failed` internal error defined by the shared CLI boundary; resource workflows do not fall back to serial execution or retain partial target results. All parser, formatter, linter, and resource calls for that command execute under this one scheduling boundary; none of the composed crates creates a nested pool. Stdin mode remains one caller-thread workflow. Public concurrency controls such as `--threads` are deferred and do not postpone automatic file-level parallelism.

The scheduler work item is one physical file group. Different groups may execute concurrently, while logical aliases within one group execute the complete read, extract, message-core processing, candidate validation, result construction, and optional write sequence serially in normalized logical-path order. The scheduler applies backpressure and passes path/config work descriptions rather than eagerly retaining every selected file's source and parse artifacts in the queue.

Workers do not render stdout or stderr and do not append directly to a shared result array. Each returns structured results carrying stable logical identities. The coordinating thread orders them under the contracts below, computes summaries and exit status, and then renders text or JSON. Worker completion order, operating-system thread scheduling, and the width selected for a run therefore cannot change public result order, selected errors, output bytes, or exit behavior. A formatter write may occur concurrently only with writes to different physical identities; the fail-stop effect of a write failure remains confined to later aliases in its own group.
...
After ignore filtering, file-mode targets are grouped by physical file identity while retaining their distinct logical path identities from the shared symlink contract. Physical identity is a scheduling and safety key only; it never replaces the normalized logical path in results, diagnostics, config matching, or cache namespaces. Targets in one group are ordered by normalized logical path and execute the complete read, extract, core processing, candidate validation, and optional write sequence serially. Each later alias therefore opens and reads the source after every earlier alias in that group has completed, except for the failed-write stop rule below. `matchedFiles`, `results[]`, and human path output continue to treat each logical path as a separate target; for example, the first alias may be formatted and a later alias may observe the resulting source as unchanged.

A target-local operational failure before a filesystem write attempt does not stop its physical group. This includes a read failure through one logical path, extraction failure, formatter or linter operational failure, candidate validation failure, diagnostic mapping failure, and successful-result construction failure. The next alias is opened through its own logical path and may succeed; check modes and lint therefore always continue to later aliases after such a failure. A successful write also continues normally so the next alias rereads the updated source.

An `output_write_failed` result in formatter write mode is the one fail-stop case. Phase 3B direct writes are not atomic and provide no rollback guarantee, so the physical file may be unchanged, partially written, or otherwise indeterminate after the failure. A catalog predecessor therefore reports `status: "error"`, `changed: false`, an empty `entries` array, and only the write error in `results[].errors`; completed pre-write entry results and diagnostics are discarded because neither their original-host nor candidate-host coordinates truthfully describe an indeterminate file. It contributes to `matchedFiles` and `errorCount`, but not to formatted, different, unchanged, or diagnostic counts. The CLI then does not stat, open, read, parse, format, or write any later alias in that physical group, and gives each remaining alias a target-local `alias_processing_blocked` result. Unrelated physical groups continue. The implementation applies this rule conservatively even when a particular operating-system error appears to have happened before the first output byte, because the shared write boundary does not certify an unchanged file state.

Physical identity follows the target file and groups both symbolic-link and hard-link aliases. POSIX implementations use the device ID plus inode; Windows implementations use the volume ID plus file ID. Another supported native platform must provide an equivalent stable file identifier before enabling file-mode resource processing. Canonical or normalized path text is never a fallback identity because it cannot detect hard links reliably. Identity is captured once after ignore filtering and before target processing; it prevents this CLI process from scheduling aliases concurrently but does not add external mutation detection. Rename, symlink retargeting, or file replacement after that snapshot follows the existing Phase 3B direct-write and no-stale-check contract shared with standalone `.mf2` files.

Failure to inspect a selected target's physical identity is a target-local `input_read_failed` with `details.reason: "metadata_failed"`. `details.ioKind` is required and is normalized to exactly `"not_found"`, `"permission_denied"`, `"not_file"`, `"not_directory"`, or `"unknown"`; dependency debug names and newly added Rust `ErrorKind` names never become JSON values implicitly. `details.rawOsError` is included only when the underlying I/O error exposes a raw operating-system code and is serialized unchanged as a signed JSON integer. A broken symbolic link therefore reports its concrete metadata failure rather than receiving a guessed identity. That logical target is not grouped or processed; other successfully identified targets continue under the normal mixed-input contract.

Normalization selects `not_found` and `permission_denied` for their corresponding portable I/O conditions, `not_file` when the inspected target no longer denotes a processable file, and `not_directory` when a path component required to reach it is not a directory. Every other condition, including a future platform or Rust error kind without an explicitly designed mapping, becomes `unknown` while retaining `rawOsError` when available. The human message may add non-stable operating-system prose, but no such prose is copied into `details`.

`metadata_failed` is added to the formatter and linter's shared `input_read_failed` reason vocabulary with the initial resource workflow. `unsupported_encoding` joins that vocabulary only when the Tier 3 XLIFF preflight that emits it ships. Neither condition introduces a separate operational error code or changes target-local result placement.
...
The same limits apply to every write-back candidate in write and check modes. `build_and_validate_write_back` owns their enforcement: during its raw-entry-order measurement pass it checks each effective candidate message against `message_bytes` and computes running `total_message_bytes` from supplied formatted values and original values for omitted, diagnostic-bearing, read-only, or unselected entries. Planning re-escaping for one changed entry occurs only after that entry's two message checks pass and returns its exact raw output length without materializing that output.
...
The CLI scheduler adds one resource-workflow-specific target-local code that is not a `ResourceError`:

| Code | Kind | Exit | When |
| --- | --- | --- | --- |
| `alias_processing_blocked` | `io` | `2` | A previous logical alias in the same physical group returned `output_write_failed`, so the shared file state is indeterminate and this alias is not processed. |
...
The operational error's top-level `path` is the blocked alias. `reason` is required and initially has only `"prior_alias_write_failed"`. `predecessorPath` is required and is the normalized logical path whose `output_write_failed` stopped the group; it follows the shared project-relative-or-absolute machine path rule and is not the physical identity. The predecessor's own result retains the underlying write-error details; they are not copied into blocked results. Each blocked formatter result has `status: "error"`, `changed: false`, an empty catalog `entries` array when applicable, and this one error in `results[].errors`. It remains a selected target, so it contributes to `matchedFiles` and `errorCount`, but not to formatted, different, unchanged, or diagnostic counts. Human output identifies both logical paths without exposing device or inode data.
...
- `resource_invalid_entry_handle`: a foreign, duplicate, nonexistent, read-only, or otherwise invalid artifact-local handle reaches an internal consumer boundary.
...
File results are reported in stable normalized path order; entries within a file are reported in raw span order. This ordering includes synthesized `alias_processing_blocked` results for every unprocessed later alias after a group write failure. The initial CLI integration parallelizes across physical file groups using its one bounded command-scoped worker pool and never between logical aliases of one physical file. A failed write stops only its own group's pending aliases and does not cancel already running or pending unrelated groups. Workers return keyed structured results; the coordinator performs stable aggregation and reporter rendering after group work completes. The resource crate remains scheduler-neutral and does not create a thread pool or runtime.
...
- Every extracted entry's message text goes through `formatMessage(source, options?)` with the same resolved fmt configuration used for `.mf2` files, regardless of writeability. The consumer retains parser diagnostics in message-local coordinates. It discards successful formatted output for a read-only entry and omits diagnostic-bearing entries. After all selected formatter calls finish without an operational error, it passes every remaining successful writable output, including byte-identical output, to one `build_and_validate_write_back` call.
...
- When that one file write returns `output_write_failed`, the current logical target uses `status: "error"`, `changed: false`, `entries: []`, and the write error, with only `matchedFiles` and `errorCount` affected. The CLI scheduler then applies the physical-group fail-stop rule: every later alias receives `alias_processing_blocked` without another filesystem access. A failure before the write boundary and every no-write mode continue to the next alias normally.
...
- Read-only entries are excluded from write-back and difference reporting, not from formatter evaluation. A read-only entry with parser diagnostics uses `status: "diagnostic"`; otherwise write mode and check modes discard its successful formatted output and record `status: "skipped"`, `changed: false`, and `readOnly: true`. It is not supplied as a `FormattedEntry`, and no `RawReplacement` is constructed for it. Human output, summary counts, file status, and the process exit code do not report or change because of a clean read-only skip.
...
- If a per-entry formatter call or `build_and_validate_write_back` returns an operational error, the catalog produces no write and no partial formatting result. Any formatter operational error prevents the integrated method from being called; when several formatter calls fail, the lowest raw-order entry wins. The file result uses `status: "error"`, `changed: false`, an empty `entries` array, and the selected operational error in `results[].errors`, with `details.entryKey` when known. A fixed resource-limit overrun remains `resource_limit_exceeded`; another re-escaping or candidate-validation failure after an entry was classified as writable is an `internal_error` with `details.reason: "resource_write_back_failed"`. Other selected files still continue.
