...
A target-local operational failure before a filesystem write attempt does not stop its physical group. This includes a read failure through one logical path, extraction failure, formatter or linter operational failure, and candidate validation failure. The next alias is opened through its own logical path and may succeed; check modes and lint therefore always continue to later aliases after such a failure. A successful write also continues normally so the next alias rereads the updated source.
...
The typed `ResourceError` also carries an optional primary site expressed against the original host document retained by the caller, never solely against a write-back candidate that the user will not keep. A site contains an original-host `Utf8ByteSpan` and, when the failure belongs to a constructible entry, its `EntryKey`. The initial site rules are:
...
When a site exists, CLI JSON adds `offset`, `line`, and `column` using the same original-host coordinate rules as `resource_parse_failed`; all three fields appear together. It also adds `entryKey` when the site carries one. When no site exists, all four fields are omitted. Editor consumers use the typed original-host span directly and use an empty range at document start only for the no-site case.
