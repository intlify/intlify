...
File results are reported in stable normalized path order; entries within a file are reported in raw span order. This ordering includes synthesized `alias_processing_blocked` results for every unprocessed later alias after a group write failure. The initial resource implementation does not introduce intra-file parallelism and follows the consumer CLI's existing file execution policy. Future concurrency first parallelizes across physical file groups, never between logical aliases of one physical file. A failed write stops only its own group's pending aliases and does not cancel already running or pending unrelated groups. The resource crate itself does not create a thread pool; scheduling belongs to the CLI consumer so all work shares one bounded global pool.
