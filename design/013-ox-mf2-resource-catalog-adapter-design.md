...
When the first catalog-native rule ships, its linter design adds a dedicated catalog-level result and finding collection beside the file results. That design must define the serialized comparison-scope identity, typed subject variant and key, concrete related-entry references, optional real source spans, deterministic ordering, human and editor presentation, `--quiet` and `--max-warnings` behavior, and summary counting. One catalog-native logical finding is counted once regardless of its number of related entries. No placeholder JSON fields, Rust reporter variants, or output schemas are added before those rules and their complete output contract land together. L0/L1 linker-backed rules reuse this broader presentation surface only through their separately defined one-to-one `LinkFinding` adaptation; they do not become catalog-native analysis.
...
The 014-owned `benchmark-profile.mjs` declares one checked companion mapping from each complete required E2E tuple to one complete imported extraction tuple. The two ordinary result records retain equal declared fixture, variant, scale, runtime, and execution model and retain their own exact phase, cost, and operation. No companion extraction tuple may be shared by two E2E tuples. The result document serializes no relation field; the messages result-schema validator applies the checked profile mapping. A mismatched field, missing, duplicate, or shared extraction record, nested metric, or second 014-owned extraction phase makes the messages result document invalid.
