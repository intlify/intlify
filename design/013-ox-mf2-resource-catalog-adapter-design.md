...
4. **Entry construction.** For each candidate, the shared artifact builder derives structural and catalog identity, assigns occurrence in raw order, unescapes the candidate's exact per-entry message text, constructs the raw value span and offset map, records read-only state, and updates the cumulative budgets and interner. A composed outer adapter supplies absolute source bases to the same builder rather than publishing an inner artifact.
...
The `fixed` variant's `value` is admitted through the shared checked `Locale` contract. An empty or over-255-byte value uses `invalid_catalog_locale_binding` at `/resources/catalogs/<index>/locale/value`; exact spelling is otherwise preserved. Assignment-time `locale_capture_mismatch` remains a separate post-discovery configuration failure rather than a structural reason.
...
These stages intentionally reuse the shared `config_validation_failed` code but are not one validation pass. Every structural reason above is selected before ignore setup or discovery and returns only the first error under the fixed shared and section-local order. `catalog_format_conflict` and `locale_capture_mismatch` are assignment-time failures because they require concrete selected paths. The coordinated M0 `catalog_scope_key_domain_conflict` is later still: it requires successfully extracted entries from the complete definition inventory. A consumer must not move structural failures later, infer a domain from host-format spelling, or attempt static glob-intersection analysis merely to collapse these stages.
...
The runtime integration is conceptually:
...
pub struct LoadedProjectConfig {
    // Existing project root, config path, source, and normalized config.
...
The read-only linker-definition projection in [014](./014-ox-mf2-message-linker-design.md#logical-alias-grouping-and-primary-identity) intentionally specializes this alias boundary without changing formatter or linter results. After resolving each selected alias independently, its host integration groups aliases for one physical source, requires equal namespace, canonical host-format id, scope, and locale bindings, and emits one complete definition artifact using 014's UTF-8 segment-array path contract and lexicographically minimum portable logical path as primary. Remaining aliases are ordered evidence rather than duplicate definitions. A binding conflict rejects the complete group before extraction; physical identity helps only with local grouping and never enters the public artifact. Formatter and linter CLI workflows continue to retain per-alias results and the serialized reread/write behavior defined by 005/007 and this document.
...
A logical file may independently belong to the definition inventory and to a language producer's reference inventory, such as a Vue SFC containing both an `<i18n>` block and message calls. Selection on one side never de-duplicates or suppresses its participation on the other. Reusing one immutable read snapshot is an orchestration optimization only when both artifacts are proven to use the same selected bytes.
...
Path captures, `fixed.value`, and host-provided entry locales must resolve to the shared checked [`Locale`](./014-ox-mf2-message-linker-design.md#locale-identity) newtype: a non-empty Unicode-scalar string whose decoded UTF-8 length is 1 through 255 bytes inclusive under the coordinated `locale_bytes` protocol ceiling. Resolution counts the exact captured, fixed, or host-provided bytes before retaining the value or projecting a linker definition. Exactly 255 bytes is valid; empty and 256-byte values are rejected without truncation, hashing, replacement, or fallback to another binding. The protocol ceiling applies when the 014 contract lands; a caller-selected lower linker budget is checked again at that invocation and never changes configuration identity.
...
- Tier 1 JSON frontend tests cover complete syntax validation before candidate traversal, exact nesting-limit acceptance and first-opening-token rejection beyond the limit, raw-order tape events across duplicate members and mixed containers, lazy decoding of only candidate values and the path segments needed to identify them, and identical syntax, depth, raw-order, and representability precedence during candidate re-extraction. Private unit tests also verify that `JsonSyntaxTape` is discarded after artifact construction and that the retained artifact contains neither a decoded JSON DOM nor decoded strings for irrelevant subtrees.
...
| [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md) | application-reference artifacts; one complete linker-definition projection per selected source above resource extraction, without a resource-to-linker dependency or resource-owned file aggregation; selector/domain contracts and linker-owned matching; language-neutral linking, reachability, locale/fallback resolution, and bundle-plan generation; the future M5 consumer of a separate resource-owned structural-deletion boundary |
