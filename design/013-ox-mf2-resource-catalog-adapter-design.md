...
When a 014 `message_project_link_e2e` case plans Tier 1 catalog extraction, `tools/messages-bench` also emits exactly one ordinary result record using the exact 013-owned `resource_extract` phase and `host_parse_and_entry_extraction` cost. The messages result-schema validator pairs it with the one aggregate `message_project_link_e2e` / `complete_workflow` record through the same declared fixture, variant, scale, runtime, and execution model. The two records retain their own exact operations. A mismatched field, missing or duplicate extraction record, nested metric, or second 014-owned extraction phase makes the messages result document invalid.

The initial M0 extraction record covers only that case's sequential JSON physical-source-group extraction calls. Successful aggregation visits groups in the same canonical primary-source order used by project inventory and reports three checked `u64` structural counts plus one duration:
