...
`tools/resource-bench/benchmark-phases.mjs` owns the closed phase/cost table for the active resource milestone, and `tools/resource-bench/result-schema.mjs` owns that tool's versioned result shape. A later phase or cost becomes valid only when this design activates it and those resource-owned files are updated together; 001 does not maintain a duplicate cross-product registry.
