...
- Concrete catalog-level, cross-locale, and linker-backed usage-aware linter rules beyond the message-level core, including the later L0/L1 presentation adapters under the integration boundary in [013-ox-mf2-resource-catalog-adapter-design.md](./013-ox-mf2-resource-catalog-adapter-design.md#catalog-level-checks) and the architecture in [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md). This deferred lint work does not gate linker M0. All L0/L1 linker-backed rules initially remain `off` and outside `recommended`; later promotion is evidence-gated per rule. The adapter resolves enabled-rule input capabilities before work: no linker-backed rule means no linker invocation; definition-only linker rules invoke one shared linker orchestration without loading or producing reference artifacts; and a reference requirement adds the shared producer/cache path to that same single orchestration. Every enabled rule consumes one `LinkOutcome` rather than creating a second analysis path. Severity, preset, reporter, and rule order never enter reference-artifact cache identity. Absent reference production and narrow operands derive typed partial completeness and suppress absence-dependent findings through the linker contract; one adapted linker finding counts once regardless of related-entry count.
