...
- Keep resource/catalog extraction, host-coordinate mapping, and per-entry composition in the integration layer defined by [013-ox-mf2-resource-catalog-adapter-design.md](./013-ox-mf2-resource-catalog-adapter-design.md#catalog-linting), above message-level linting.
- Keep application-reference resolution, cross-file and cross-locale analysis, and linker-native findings in [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md); later L0/L1 adapters present its `LinkOutcome` through the linter rule and reporting contracts without reimplementing linker analysis in `intlify_lint`.
...
The dashed integration paths in the diagram remain outside the message-level `intlify_lint` core. The 013 resource path extracts opted-in catalog entries, calls `lintMessage` for each message, and maps results back to host coordinates. The later 014 linker path consumes one existing `LinkOutcome` and adapts its findings into the L0/L1 rule, severity, result, and reporter contracts. It neither runs entry extraction through the linker nor moves resolution, fallback, reachability, completeness, or finding construction into the linter crate.
...
The Rust concurrency contract is independent of whether the initial CLI schedules calls in parallel. Owned `LintOptions`, `LintResult`, diagnostics, and operational-error data intended to cross a consumer thread boundary must satisfy `Send`. `ResolvedLintConfig` and any immutable rule registry or rule implementation deliberately shared by concurrent calls must satisfy `Send + Sync`; mutable `RuleContext`, report buffers, parser views, `SemanticModel`, and other per-call scratch state remain isolated in one invocation and do not acquire a blanket sharing requirement. Rule registration is complete before concurrent lint execution begins, and the initial built-in registry is not mutated while calls are running.

Compile-time assertions lock those concrete traits. Behavioral tests invoke the same immutable resolved configuration and registry concurrently over independent sources, compare complete diagnostic and error results with serial baselines, and repeat clean calls after an injected returned rule error. A failed invocation must not poison registry/config state, retain partial reports for another call, or affect diagnostic order, severity, counts, or later-call behavior. The deferred CLI scheduler adds its worker-panic containment cases when implemented. N-API isolate affinity and WASM instance threading remain binding-runtime constraints rather than linter-core scheduling responsibilities.
...
1. Register source text in a call-local `SourceStore`, parse it, and retain that original `SourceStore` / `ParseResult` pair plus its `CstView`.
...
3. Call parser-owned `build_semantic_model(&sources, &result)` for the diagnostic-free pair.
...
The shared operational registry and its separation from numeric API errors and JSON-visible diagnostic codes are indexed in [appendix-ox-mf2-error-code.md](./appendix-ox-mf2-error-code.md). This document owns the linter-specific detail variants of reused codes.

Semantic invariant failures use `internal_error` with `details.reason: "semantic_invariant_failed"` and a required `details.stage` of `"semantic_model_construction"` or `"semantic_validation"`. Semantic API misuse uses `internal_error` with only required `details.reason: "semantic_api_misuse"` when a host boundary accidentally asks the parser semantic layer to build a `SemanticModel` from a parse result that still has parser diagnostics; it omits `stage` because that reason currently identifies model-construction misuse uniquely. Built-in rule invariant failures use `internal_error` with `details.reason: "lint_rule_invariant_failed"` and required `details.ruleId`. Semantic invariant failures, semantic API misuse, and rule invariant failures are target-level operational errors during message-level linting: they report an incomplete lint target, while other targets in the same CLI run may still finish normally. Rule invariant failures also discard any diagnostics already emitted by earlier rules for that target because the target's lint result is incomplete. The reason-specific `details.stage` and `details.ruleId` fields are stable debugging aids for implementation failures; the human-readable operational error `message` is not stable. These errors are not user-facing diagnostics and are not configurable through `lint.rules`.
...
- required analysis input kind: `message` or `project-link-outcome`
...
Every Phase 3C message rule uses `message`. The later L0/L1 adapters register their shipped rule ids with `project-link-outcome`; that metadata is implementation-owned and cannot be overridden by configuration, severity, preset membership, reporter choice, or a binding caller.
...
- `lint.rules` keys must be known configurable rule ids admitted by the shipped project-config surface; unknown or not-yet-shipped rule ids are `config_validation_failed` errors
...
Resolution starts from the implicit `recommended` defaults and overlays `lint.rules`. The CLI loads JSON or JSONC config files, validates the shared project config, applies command-specific CLI overrides, and passes only lint-specific unresolved config input into the Rust lint model. `crates/intlify_lint` expands presets from its rule registry, validates configurable rule ids, required input kinds, and severities, and constructs the applicable resolved selection.

Project configuration uses the project-capable resolver. It admits shipped `message` and `project-link-outcome` rules because the CLI/editor integration can compose per-message linting with the one shared 014 project outcome. Rust `lint_message`, N-API, and WASM use the message-only resolver and admit only `message` rules. Both modes share one registry, preset expansion, severity validation, and deterministic ordering; they differ only in whether the caller supplied the required project input capability. Invalid binding options use `invalid_options`.
...
The unified config contract in [006-ox-mf2-phase-3a-tooling-foundation-design.md](./006-ox-mf2-phase-3a-tooling-foundation-design.md) is the sole owner of root admission and the command-independent cross-section validation order. This document owns only the deterministic order within the `lint` section. When the shared validator reaches `lint`, it reports the first `config_validation_failed` failure in this order:

1. `lint` section shape: `lint` must be an object when present, then unknown `lint` fields in ASCII ascending order
2. `lint.rules` shape: `rules` must be an object when present
3. `lint.rules` entries: rule ids are processed in ASCII ascending order; for each entry, parser or semantic diagnostic codes are reported before unknown rule ids, and unknown rule ids are reported before invalid severity values for known configurable rule ids
4. `lint.ignorePatterns` shape: value must be an array, then entries are validated in array order and the first non-string or invalid pattern entry is reported
...
The lint-owned `details.reason` vocabulary is exactly `"invalid_section_shape"`, `"unknown_field"`, `"invalid_rules_shape"`, `"non_configurable_diagnostic"`, `"unknown_rule"`, `"invalid_rule_severity"`, `"invalid_ignore_patterns_shape"`, or `"invalid_ignore_pattern"`. `details.pointer` is a JSON Pointer to the invalid location: `/lint`, `/lint/<field>`, `/lint/rules/<rule-id>`, or `/lint/ignorePatterns/<index>`. Reasons and pointer shapes owned by the root, formatter, resource, and message-linker validators are defined by their respective designs and composed by 006; they are not part of this lint-local closed vocabulary. Binding option validation uses dot paths instead because it points into JavaScript object input rather than a config document. Shared semantic reason names such as `"non_configurable_diagnostic"`, `"unknown_rule"`, and `"invalid_rule_severity"` are intentionally reused across CLI config validation and binding option validation, but the operational error code and path format remain boundary-specific: CLI config errors use `config_validation_failed` with JSON Pointer paths, while bindings use `invalid_options` with dot paths. `details.ruleId` is included for both unknown rules and non-configurable diagnostic codes. `details.field` is included for unknown fields, `details.index` for ignore pattern entry failures, and `details.value` only for scalar JSON values.

This exact lint-local reason vocabulary and section-local ordering are the v0.1 implementation target. They supersede provisional implementation and fixture values such as `expected_object`, `expected_string`, `expected_array`, `invalid_value`, and `model_validation_failed`, as well as validation driven by JSON object insertion order. Because the project is still WIP, implementation code and exact fixtures migrate atomically to this contract without a dual-reason compatibility period.
...
### Linker-Backed Rule Activation and Input Capabilities

This contract becomes active when the L0/L1 linker finding adapters ship. Before then, their rule ids remain unknown rules rather than accepted dormant configuration.

Every shipped L0/L1 rule is registered with required analysis input kind `project-link-outcome`. The project-config resolver admits that kind and the adapter evaluates it only from the one shared checked 014 `LinkOutcome`. It does not inject project data into a message `RuleContext`, call the message-level `LintRule::check` path for a linker finding, or rerun linker analysis.

The source-backed Rust `lint_message` API and its N-API/WASM `lintMessage` bindings remain message-only. If their `LintOptions.rules` explicitly names a shipped `project-link-outcome` rule at any severity, option validation fails before parsing with `invalid_options`, `details.reason: "rule_requires_project_context"`, `details.path: "rules.<rule-id>"`, and the exact `details.ruleId`. The rule is known, so this is not `unknown_rule`; `"off"` is rejected as well rather than accepting a context-inapplicable dormant option. No public `lintLinkOutcome`, resource binding, or linker binding is introduced by L0/L1.

The unified loader first completes the ordinary lint rule-id/severity resolution and every independent `fmt`, `lint`, `resources`, and present `messages` section validation. It then performs the 006-owned post-section activation check over the resolved rule set.

If at least one linker-backed rule resolves to `warn` or `error` while the root `messages` section is absent, configuration fails with exactly one top-level `config_validation_failed`. Its stable details are:

- `reason: "linker_messages_required"`;
- `ruleId`: the ASCII-smallest enabled linker-backed rule id; and
- `pointer`: `/lint/rules/<rule-id>` when that rule has an explicit project-config entry, which is the only initial L0/L1 activation path.

The error carries no `value`, inferred locale, synthetic `/messages` member, or partial policy. It is selected only after an invalid present `messages` section has already had the opportunity to report its earlier 014-owned failure. It stops before ignore setup, discovery, extraction, reference-artifact loading, source scanning, cache lookup, or linking.

If a future preset promotes a linker-backed rule without an explicit rule entry, the same selection first compares all enabled linker-backed ids in ASCII order and uses `/lint` for the selected implicit activation because no narrower config node exists. That promotion must add fixtures for this branch; it does not make `messages` appear implicitly.

After that missing-`messages` activation check succeeds, initial L0/L1 rejects stdin mode whenever at least one linker-backed rule remains enabled. It returns exactly one top-level `invalid_cli_argument` with `kind: "input"`, exit code `2`, `results: []`, and `summary.operation: "stdin"`. Its stable details, in order, are:

- `reason: "linker_rule_requires_filesystem_inventory"`;
- `option: "--stdin-filepath"`; and
- `ruleId`: the ASCII-smallest enabled linker-backed rule id.

The error has no `pointer`, `argument`, `value`, inferred path, or top-level `path`. Complete CLI argument-shape validation and every independent config-section validation occur first; an invalid present `messages` section or the earlier `linker_messages_required` activation failure therefore wins. This combination check then stops before ignore-path representability or reads, ignore discovery, stdin read, resource extraction, project inventory, producer work, cache lookup, or linking. It never disables the selected rule silently and never treats the virtual stdin path as a project overlay.

Every linker-backed rule has static, implementation-owned input capabilities. All use the selected definition inventory and the base checked linker request inputs: scope mapping, per-scope completeness, production locales, and the integration's delivery graph. The following table separately fixes whether application-reference production is additionally required, when the linker can construct the finding, which analysis or policy input it consumes, and when lint/editor presentation becomes available:

| Linker-backed rule | Application references | Linker analysis milestone and input | Lint/editor presentation milestone |
| --- | --- | --- | --- |
| `ambiguous-message-definition` | no | M0 — post-mapping definition collision indexing | L0 |
| `unresolved-message` | yes | M0 — reference resolution against the locale-agnostic production-locale union; M2 refines evidence by requested locale and fallback chain | L0 |
| `missing-translation` | yes | M2 — requested-locale and fallback resolution | L1 |
| `orphaned-translation` | no | M2 — canonical post-mapping coverage-baseline difference | L1 |
| `unused-message` | yes | M0 — configured roots plus reference reachability over the delivery graph | L0 |
| `unbounded-dynamic-reference` | yes | M0 — checked `dynamicReferences` policy and reference evidence | L0 |
| `degraded-analysis` | yes | M0 — reference-side wide-selector evidence plus both completeness sides | L0 |

An analysis milestone makes its findings available in `LinkOutcome`; it does not by itself register a lint rule or publish an editor diagnostic. L0 presents the five M0 finding kinds, while L1 presents both M2 locale-aware finding kinds after M2 analysis exists.

`unresolved-message` retains one rule identity across the milestone transition. In M0 it means that no production locale defines the referenced key. After M2, the same finding records the complete canonical set of requested locales whose fallback chains fail; it does not become an L1-only rule or acquire a second rule id.

`degraded-analysis` requires references because one lint rule represents both its wide-selector and partial-completeness variants. Treating it as definition-only would silently omit the wide-selector branch. A future split may change that cost only by introducing separately configurable rule identities and updating this table.

The adapter unions these static capabilities after preset and severity resolution but before any filesystem or producer work. With no enabled linker-backed rule, it performs no linker orchestration. For a filesystem invocation with at least one enabled linker-backed rule, capability-driven project orchestration begins only after the presentation-scope set below is non-empty; an empty set takes the no-linker path. When that non-empty invocation's union does not require references, it neither loads reference artifacts nor scans application sources nor consults the reference cache. Checked producer declarations remain configuration metadata, but no configured producer or external artifact is selected as a reference participant for that capability-relative request. The reference side is therefore `Closed` over the empty selected participant set; this does not claim that the complete configured producer world was enumerated or scanned and derives neither `ProducerOmitted` nor `OpenEditorWorld`.

For that non-empty presentation-scope set, when references are required, every enabled rule shares exactly one producer/cache/linker orchestration and one `LinkOutcome`. A later capability transition from definition-only to reference-requiring constructs the complete reference inventory and a new full request. It does not reinterpret the earlier request-relative closure as evidence about the configured producer world.

Capability metadata is not a project-config field, rule option, reporter choice, or cache-key component. Rule order and severity cannot change it. Disabled or otherwise unrequested finding kinds, including reference-dependent findings that the definition-only linker result could derive from its empty selected set, are filtered while adapting the shared outcome; individual rules never launch their own scan or link. `messages emit` and `messages prune` are independent consumers that require their ordinary complete reference participation and never reuse the lint-only fast path as closed-world generation evidence.

For a filesystem lint invocation, operands and the ordinary ignore stack select logical message-level lint targets and derive the presentation-scope set below. Once at least one linker-backed rule is enabled and that set is non-empty, its semantic analysis instead receives the complete configuration-authoritative project inventory defined by 005 and 014. File, directory, and glob operands do not narrow either definition or capability-selected reference participation, and root `.gitignore`, `--ignore-path`, `lint.ignorePatterns`, hidden-file filtering, or default directory filtering do not subtract configured linker participants. Narrow or ignored reporting targets therefore do not derive `SourceOmitted`, `ProducerOmitted`, or another `Partial` reason. Only execution evidence for the complete configured participant set may change completeness.

An empty presentation-scope set is a vacuous rule application, not a partial project claim or an implicit rule-disable. After the already required independent config validation and missing-`messages` activation check, it performs no project inventory, producer, external-artifact, cache, or linker work and returns only ordinary message-level lint results.

Initial L0/L1 stdin remains message-level only. The enabled-rule combination follows the exact `invalid_cli_argument` contract above; stdin never becomes an implicit overlay for a configured catalog.
...
- the standalone primary input unit is `1 file = 1 MF2 message`; direct config-free classification supports `.mf2` plus shipped resource extensions for individual file operands and stdin, while directory and CLI-native glob discovery select catalogs only through project policy
...
- non-UTF-8 input reports `input_read_failed` with `details.reason: "invalid_utf8"`; the shared CLI physical-grouping boundary adds the staged `metadata_failed` reason, while Tier 3 XLIFF adds `unsupported_encoding` under the resource-specific raw-prefix contract
- file mode processes physical file groups and their logical aliases sequentially in stable order on the caller thread
...
Resource/catalog input is a layered adapter workflow owned by the resource design. Project policy supports bulk discovery and explicit format assignments for arbitrary or extensionless filenames. When project `resources.catalogs` is absent, the CLI may additionally classify only individual file operands and `--stdin-filepath` through the resource registry's shipped direct extensions; it does not apply that exception to directories or CLI-native globs. The shared `unsupported_input_file.details.supportedExtensions` list therefore includes those config-free resource extensions. Resource adapters own host-file parsing, message extraction, and span mapping; the message-level linter core and the extension-neutral enumeration contract do not change.

These discovery and ignore rules govern the ordinary `results[]` target set. They do not alter the separately constructed configuration-authoritative project inventory used by enabled linker-backed rules. A configured catalog or producer source omitted from ordinary lint targets by operand or ignore selection is still read and analyzed once as linker context; configuration-owned resource and producer include/exclude policy is the only normal way to remove it from that context.

After ordinary logical target selection, every selected target that is also a linker-participating configured definition contributes its checked configured catalog scope. This admission occurs after configuration assignment and scope mapping succeed but before target metadata, physical grouping, reads, extraction, or message-level processing. A later target-local failure does not retract the scope; a configuration-derived assignment or mapping contradiction instead retains its earlier fail-complete behavior and produces no set or `LinkRequest`.

Each selected logical alias contributes independently before canonical scope de-duplication. One selected alias is sufficient even when the physical group's canonical primary or another alias was not selected or was ignored. The already required equal group bindings make every admitted alias resolve to the same scope; unequal bindings fail before presentation rather than choosing an alias.

The adapter maps the contributed values through the same admitted scope mapping used by the one `LinkRequest`, sorts and de-duplicates the resulting `ResolvedCatalogScopeId` values canonically, and retains that internal set only as presentation state. Standalone `.mf2`, config-free direct catalog inputs, and configured resource definitions without the coordinated linker `scope` and locale pair contribute no presentation scope. When the final set is empty, the adapter takes the no-linker path above.

Every linker finding exposes exactly one post-mapping resolved scope through the 014 typed API. The adapter presents the finding if and only if that scope belongs to the presentation set. It never filters by primary source path: a reference-primary finding remains visible when its resolved scope was selected through a catalog operand, even though JS/TS producer sources are not ordinary lint targets. Filtering changes neither the checked `LinkOutcome` nor artifacts, cache identities, completeness, finding construction, severity resolution, or generation behavior.

### Sequential File Execution

After discovery, supported-input classification, ignore filtering, and physical grouping complete, `crates/intlify_cli` processes physical file groups sequentially on the caller thread in stable first-logical-path order. Ignore sources are loaded and validated during setup, but matching occurs only after classification. Each logical alias in a group runs serially through the complete read, framing or extraction, lint, diagnostic mapping, and result-construction pipeline. Stdin remains one caller-thread workflow.

Target processing returns structured results and does not render text or JSON directly. The coordinator preserves stable normalized logical-path order, then calculates diagnostic and operational-error counts, applies `--quiet` and `--max-warnings`, selects the exit status, and renders the reporter. Neither `intlify_lint` nor `intlify_resource` creates worker, entry, or rule tasks.

Parallel execution across physical groups remains deferred under the shared [CLI parallel execution boundary](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary).
...
- `--stdin-filepath <path>`: explicit stdin mode with the same semantics as `intlify fmt`: reads all source text from stdin, uses `<path>` as the virtual input path for supported-input classification, resource-policy skip, ignore rules, and output, and cannot be combined with file, directory, or glob operands. Standalone `.mf2` stdin applies read framing. A selected catalog virtual path instead follows the resource adapter's [stdin selection](./013-ox-mf2-resource-catalog-adapter-design.md#stdin-selection) and receives the exact unframed host document; policy-skipped stdin follows its zero-target behavior. This message-level path is valid only when no linker-backed rule is enabled; initial L0/L1 rejects the enabled-rule combination during setup and never treats the virtual path as a project participant.
...
The current `results[]` variants remain exclusively file- and entry-owned. Before the first catalog-native structural rule or L0/L1 linker-backed rule ships, a dedicated addendum must introduce one project-level finding collection beside `results[]`. That collection, rather than an arbitrary file result, owns a finding whose evidence crosses files or has no truthful primary source. Its typed locations carry their own normalized project-relative path and UTF-8 span plus the applicable concrete entry identity; the primary location is optional, related locations remain canonically ordered, and absence never becomes a synthetic zero-length span.

The same resolved-scope presentation filter applies whether the selected finding has a primary location or not. A selected primary-less finding remains in the project-level collection exactly once with no fabricated file result or span; an unselected-scope finding remains absent even when it has a source location elsewhere in the project. One logical finding appears and is counted exactly once regardless of location count. The addendum lands its Rust DTO, exact machine codec, deterministic ordering, text/editor projection, `--quiet`, `--max-warnings`, file/problem/diagnostic summary treatment, and operational-error precedence atomically with the first rule that uses it. It does not add fields to the message-level `Diagnostic` or its N-API/WASM result.
...
Semantic invariant failures, semantic API misuse, and rule invariant failures use the same target-level `results[].errors` surface during message-level linting. The target reports `status: "error"`, an empty `diagnostics` array, and an `internal_error` entry. Semantic invariant failures include `details.reason: "semantic_invariant_failed"` plus required `details.stage`; semantic API misuse includes only `details.reason: "semantic_api_misuse"` and rejects a `stage` field; rule invariant failures include `details.reason: "lint_rule_invariant_failed"` plus required `details.ruleId`. Partial rule diagnostics emitted before a failing rule are discarded. Such targets contribute to `errorCount`, not `problemFiles`, because they produced an operational error rather than a complete diagnostic result:
...
`LintOptions` is the caller-facing unresolved input model for one MF2 message. It carries rule severity overrides but does not represent the fully resolved rule state. `crates/intlify_lint` resolves it under the message input context by applying the implicit `recommended` defaults, validating rule ids, required input kinds, and severities, and constructing an internal `ResolvedLintConfig` before any rule runs. `RuleContext` always receives `ResolvedLintConfig`, not raw options.

CLI project config, N-API options, and WASM options share the same severity-map shape, registry, preset expansion, and Rust validation implementation, but they do not claim equal available analysis inputs after L0/L1 ships. Project orchestration resolves the config with project context and may select both message and `project-link-outcome` rules. Rust `lint_message` and both bindings resolve `LintOptions` with message context and reject every project-only rule through the exact error below.
...
`LintOptions` carries rule severity overrides; the binding shape is validated like the config `lint.rules` map, with unknown rule ids rejected as `invalid_options`. Parser or semantic diagnostic codes used as rule ids are rejected with `details.reason: "non_configurable_diagnostic"`. A shipped project-only rule id is rejected with `details.reason: "rule_requires_project_context"` rather than being treated as unknown, ignored, or executed without a `LinkOutcome`. Omitted `options` and omitted `rules` use the implicit message-rule `recommended` defaults during config resolution; project-only rules are default-off and absent from that preset. `null` options are invalid, matching the formatter binding contract. Phase 3C does not provide a programmatic `recommended: false`, `preset: "none"`, or equivalent preset-disabling shortcut. Programmatic callers that want parser and semantic diagnostics without configurable message rules must disable each configurable message rule through `rules`.
...
A shipped project-only rule is a known rule but cannot run over one message:

```json
{
  "reason": "rule_requires_project_context",
  "path": "rules.unused-message",
  "ruleId": "unused-message",
  "value": "warn"
}
```

`details.reason` is required and is one of `"non_configurable_diagnostic"`, `"unknown_rule"`, `"rule_requires_project_context"`, `"invalid_rule_severity"`, `"invalid_rules_shape"`, or `"invalid_options_shape"`. `details.path` is always required for Phase 3C `invalid_options`. Root `options` shape failures use `details.path: "options"`, `rules` shape failures use `details.path: "rules"`, and rule-specific failures use `rules.<rule-id>`, such as `rules.no-unused-declaration`. The path is a dot path, not a JSON Pointer. Phase 3C rule ids are kebab-case ASCII and require no escaping; future rule option paths may extend the notation if escaping becomes necessary. `details.ruleId` is required for rule-specific failures. `details.value` is included only for scalar values: string, number, boolean, or `null`. Arrays and objects are omitted even when JSON-safe; `undefined`, functions, and symbols are omitted because they have no JSON representation. CLI project-config validation continues to use `config_validation_failed`; it admits shipped project-only rules and therefore never emits `rule_requires_project_context`. This `invalid_options` detail shape belongs to the message-level programmatic boundary.
...
| `rule_requires_project_context` | `rules.<rule-id>` | required | scalar configured rule value only |
| `invalid_rule_severity` | `rules.<rule-id>` | required | scalar invalid severity value only |

The first validation failure is deterministic. Validation checks `options` shape first, then `rules` shape, then `rules` entries by rule id in ascending order. For each rule entry, parser or semantic diagnostic codes are reported before unknown rule ids; unknown rule ids are reported before a known project-only rule's `rule_requires_project_context`; that context error is reported before inspecting its severity value. A known message rule then validates that the value is `"off"`, `"warn"`, or `"error"`.
...
This document remains authoritative for the message-level lint pipeline, diagnostic and rule contracts, severity, configuration, and standalone `.mf2` result behavior. The resource design is authoritative for catalog membership, host-format parsing, entry mapping, per-file aggregation, resource failures, and the catalog result extension. Future catalog-native structural checks that diagnose affirmative evidence from present entries are tracked by the resource design's [Catalog-Level Checks](./013-ox-mf2-resource-catalog-adapter-design.md#catalog-level-checks) boundary and require a linter design addition when concrete rules are introduced; their integration details are not duplicated here. The [message linker design](./014-ox-mf2-message-linker-design.md) exclusively owns absence-based key parity, missing or orphaned translations, unused messages, application-reference artifacts, completeness, fallback, usage and reachability analysis, linker-native findings, and bundle planning. Linker M0 neither calls nor depends on `intlify_lint`. The later L0/L1 adapters expose those existing findings through `intlify lint` using this document's rule and reporting contracts plus the project-level finding addendum; they preserve each linker finding's identity, cardinality, and evidence and do not define a second analysis or finding model.
...
Minimum coverage includes: every core semantic diagnostic with positive and negative parser-side cases, every core semantic diagnostic flowing through linter results and reporters, every configurable rule in `off` / `warn` / `error` states, preset default behavior, parser-diagnostic short-circuiting, source-order diagnostic reporting, the `duplicate-declaration` / `invalid-declaration-dependency` partition, `--max-warnings` valid and invalid numeric forms, `--quiet` behavior, stdin mode, ignore precedence, JSON reporter output, mixed file operational errors plus diagnostics, stable sequential physical-group and logical-alias processing, and binding parity for `lintMessage`. L0/L1 coverage additionally locks the stdin-plus-linker-rule `invalid_cli_argument` shape and precedence, configuration-authoritative inventory under operand and ignore permutations, canonical resolved-scope presentation-set construction, selected-scope reference-primary and primary-less findings, and rejection of unselected-scope findings without mutating the shared outcome. Linter flow-through smoke tests cover all core semantic diagnostic codes without re-locking parser-owned semantic span, ordering, or cascade behavior. Binding parity fixtures must include `invalid_source_type`, unpaired-surrogate `TypeError`, `invalid_options_shape`, `invalid_rules_shape`, cross-realm `options` and `rules` rejection, `non_configurable_diagnostic`, `unknown_rule`, `rule_requires_project_context` after the first L0 rule ships, and `invalid_rule_severity`. The context fixture covers `"off"`, `"warn"`, `"error"`, and an invalid scalar severity to prove that required-input-kind validation wins for every explicit project-only rule value. N-API release validation must also lock the six-package support matrix, Node.js `>=22.12.0`, wrapper `optionalDependencies`, lazy loader routing, unsupported-target behavior, package contents, native loading, and publish smoke for every advertised target.

Internal implementation failures such as `semantic_invariant_failed`, `lint_rule_invariant_failed`, and `semantic_api_misuse` are not normal `.mf2` fixture cases because user input should not trigger them. They should be covered by unit or synthetic tests across the owning boundaries: parser tests cover the semantic model boundary rejecting diagnostic-bearing parse results and returning `Err` for construction or validation invariant failures; linter tests cover target-level `internal_error` shaping; CLI JSON tests cover `results[].errors`, `status: "error"`, `errorCount`, and exit code `2`; and binding tests cover the equivalent `ok: false` result. These fixtures require `stage` only for `semantic_invariant_failed`, reject it for `semantic_api_misuse`, and require `ruleId` only for `lint_rule_invariant_failed`. A test-only rule that returns `Err(LintRuleInvariantError)` is the preferred way to exercise rule invariant shaping.

## Benchmarks

Linter benchmarks are local-first tooling under `tools/`, following the parser and formatter benchmark patterns. They should separate parse cost, `SemanticModel` construction cost, parser-owned semantic validation cost, rule execution cost (per rule where practical), binding call cost, CLI end-to-end cost, and ordered aggregation, matching the Phase 3 benchmark names `lint_message_core`, `lint_cli_e2e`, `lint_json`, `lint_binding_napi`, and `lint_binding_wasm`. `lint_snapshot_core` becomes relevant when snapshot-backed linting lands. Benchmark commands must be executable and testable, but timings are not CI gates.
...
4. `intlify lint` CLI integration, shared discovery/ignore/framing, deterministic sequential processing, physical file grouping, and JSON reporter
...
Resource catalog implementation is not folded into the Phase 3C product PRs. The shared resource crate, Tier 1 adapter, and configuration/schema foundation may already land in separate resource PRs because they do not depend on `intlify_lint`. Completion of this sequence gates only catalog-aware `intlify lint` composition; catalog-aware formatting depends separately on the Phase 3B formatter. Both command integrations and the complete resource milestone must be accepted before Phase 3D editor integration consumes the shared layer.
...
- Concrete catalog-native structural and linker-backed usage-aware linter rule implementations beyond the message-level core, including the later L0/L1 presentation adapters under the integration boundary in [013-ox-mf2-resource-catalog-adapter-design.md](./013-ox-mf2-resource-catalog-adapter-design.md#catalog-level-checks) and the architecture in [014-ox-mf2-message-linker-design.md](./014-ox-mf2-message-linker-design.md). Catalog-native rules do not infer absent keys, locales, references, or definitions; 014 exclusively owns that analysis. This deferred implementation does not gate linker M0. Its activation, missing-`messages` failure, static input-capability contract, and dedicated project-level finding-collection direction are already fixed above. The first shipping catalog-native or L0/L1 rule must complete that collection's exact addendum rather than extending the file/entry `Diagnostic`. All L0/L1 rules initially remain `off` and outside `recommended`; later promotion is evidence-gated per rule. A producer selected by a request that otherwise requires references but intentionally left unexecuted derives typed partial completeness and suppresses absence-dependent findings through 014. Ordinary lint operands and ignore selection affect only message-level targets and linker-finding presentation; they never narrow the selected reference inventory or derive `Partial`. By contrast, the definition-only fast path selects no reference participant and is `Closed` over that empty request-relative set without claiming a configured-world scan. One adapted linker finding counts once regardless of related-entry count.
- The common CLI worker scheduler, file-group parallelism, and public concurrency controls such as `--threads`; initial CLI execution is sequential and the retained follow-up contract lives in [005](./005-ox-mf2-phase-3-tooling-transport-design.md#cli-parallel-execution-boundary).
