...
Binding source validation always reports `details.path: "source"` for `invalid_input`. For `details.reason: "invalid_source_type"`, `details.value` is included only for JSON-safe scalar values: string, number, boolean, or `null`. Objects, arrays, typed arrays, `undefined`, functions, and symbols omit `details.value`. For `details.reason: "invalid_utf16"`, `details.value` is always omitted. N-API and WASM parity tests compare `code`, `details.reason`, `details.path`, scalar `details.value`, and `details.value` absence for equivalent invalid source inputs.
