...
The adapter unions these static capabilities after preset and severity resolution but before any filesystem or producer work. With no enabled linker-backed rule, it performs no linker orchestration. When the union does not require references, it neither loads reference artifacts nor scans application sources nor consults the reference cache. When references are required, every enabled rule shares exactly one producer/cache/linker orchestration and one `LinkOutcome`.

Capability metadata is not a project-config field, rule option, reporter choice, or cache-key component. Rule order and severity cannot change it. Disabled finding kinds are filtered only while adapting the shared outcome; individual rules never launch their own scan or link.
