...
`reachable_declarations_from_outputs` is conceptual pseudocode: it starts from the parser-owned output-reference and selection-reference helpers, then walks `.local` right-hand-side dependencies to mark declarations that can affect message output, markup, function options, selector annotations, or selection.
