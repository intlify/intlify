...
This core semantic diagnostic reports a selector variable that does not directly or indirectly resolve to a declaration with a function annotation subtree.

External variables are valid in normal message output, but MF2 selectors require an annotated declaration so selection behavior can be determined. This diagnostic is always enabled after successful parsing, is emitted as `error`, and cannot be configured through `lint.rules`. This page uses "function annotation subtree" for the parser-owned structure; reader-facing examples may refer to it as a function annotation.
