...
Duplicate detection is owner-local. Function options are compared only with options on the same function call, and markup options are compared only with options on the same markup placeholder. Each markup placeholder is its own owner; open, close, and standalone placeholders are not compared with each other. Option identifiers are compared by cooked identifier string after the parser's NFC normalization rule, and comparison is case-sensitive.
