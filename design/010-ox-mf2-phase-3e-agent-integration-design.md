...
Profile fixtures cover clean, diagnostic, check-difference, target-local error, global setup error, command-fatal worker-runtime error with empty results, standalone, and catalog envelopes for both fmt and lint. Tests project only the required profile fields, add unknown fields to prove forward-compatible ignoring, remove each required field to prove rejection, and verify that changed message prose does not affect agent branching.
