...
The editor design intentionally projects original-extraction `resource_format_unsupported`, `resource_entry_unsupported`, `resource_document_unsupported`, and `resource_limit_exceeded` into error-severity editor diagnostics with source `ox-mf2-resource`. `resource_parse_failed` remains an operational extraction outcome and does not produce a new ox-mf2 editor diagnostic; it leaves only the current per-document ox-mf2 diagnostic generation empty. These are defined cross-surface presentation differences, not admission into the parser/semantic/lint diagnostic namespace.

For project-link completeness, every user-actionable original-extraction failure — `resource_format_unsupported`, `resource_parse_failed`, `resource_entry_unsupported`, `resource_document_unsupported`, and `resource_limit_exceeded` with `phase: "extract"` — derives `Partial(SourceFailed)` only after the failed document is admitted as the authoritative current-generation snapshot of a known configured definition group, every applicable configuration gate completed without contradiction, and any candidate configuration was installed. A configuration contradiction or stale publication result changes no completeness; after admission, safe project-link findings from other artifacts may still publish. A `validate_write_back` failure is instead a request-scoped formatting failure: it returns no edits, does not change document diagnostics or installed state, and never changes linker completeness. The exact publication and retention behavior is owned by [Diagnostics Publication](./009-ox-mf2-phase-3d-lsp-editor-design.md#diagnostics-publication).
...
Each code requires the closed `details.kind` vocabulary and canonical structured evidence defined by 014. Artifact transport and ordinary configured-source I/O intentionally reuse `input_read_failed`; the 005 project-inventory freshness boundary uses that code with exact `details.reason: "source_changed"` when a current definition, selected built-in producer-source, or configured external reference-artifact snapshot cannot be established during acquisition. The acquisition boundary does not retry automatically. Definition participants derive `SourceFailed`; the two reference-participant classes derive `ProducerFailed` under 014 without changing the operational code. Config admission reuses `config_validation_failed`, and internal invariants reuse `internal_error`.
