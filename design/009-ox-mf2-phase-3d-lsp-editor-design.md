...
- Define the additive project-backed editor session that combines configured disk inventory with current open-buffer contents and projects 014 linker findings without creating editor-specific link semantics.
...
- the successfully resolved `resources` and `messages` configuration plus config generation;
- one resolved definition-source inventory from configured `resources.catalogs`;
- one resolved reference-source inventory for every enabled 014 producer;
...
Project inventory uses the shared discovery and source-identity rules rather than enumerating files inside `intlify_linker`. Each configured logical source participates exactly once. An unopened source is read from disk and projected or produced normally. When one open file document resolves to that same project source, its exact current Unicode buffer and protocol version replace the disk bytes for extraction or producer scanning; the session never combines both versions or lets a stale disk artifact win over an unsaved buffer.

For a `.vue` reference source, disk and open-buffer paths both use 014's same complete M0 SFC profile: inline script blocks plus every standard-template embedded expression, with origins mapped to host-document UTF-8 coordinates. The editor does not scan only the currently selected block or tolerate an invalid unsaved block by retaining references from the preceding version; a producer failure follows the ordinary generation and publication gates below.

A file declared by exact path in `messages.producers.artifacts` follows that same override rule. One unambiguous open document for the configured project source replaces the complete artifact-file bytes; the editor decodes and validates that complete current buffer through the ordinary bounded 014 artifact contract. Successful decode is the authoritative configured snapshot for this editor session and can complete that producer participant. The editor does not regenerate the artifact, inspect its upstream language sources, or demand freshness evidence beyond the same producer semantic claim accepted for the configured disk snapshot.

An invalid current buffer is `ProducerFailed` and never falls back to stale disk bytes. Two live documents that claim the same configured artifact source, an unresolved URI-to-source mapping, or a watcher transition make the reference side `Partial(OpenEditorWorld)` until one source is unambiguous. Closing the sole override returns selection to the current disk snapshot. A successfully decoded configured open-buffer snapshot is not `ExternalArtifactUnverified`; that reason remains for an ad hoc overlay, cache, package artifact, or integration value outside the exact configured participant contract.
...
An editor-overlay-only catalog, standalone `.mf2` document, untitled document, or other document outside every linker-participating project catalog definition has no project-authoritative scope and locale binding. It remains available to the ordinary per-document extraction, parsing, linting, and formatting workflows, but it never produces a `MessageDefinitionArtifact` and does not by itself change linker completeness. Whether an out-of-inventory application source may contribute reference evidence is governed separately by its enabled 014 producer, explicit scope/domain binding, and delivery-unit context; catalog overlay membership never supplies those values.

No first-opened or last-opened winner is selected when two live documents claim the same project source identity, when a logical-to-project-source mapping is unresolved, or while a watcher transition leaves inventory membership unsettled. The affected side becomes `Partial(OpenEditorWorld)` until one unambiguous current source is installed. Existing URI/version isolation for per-document parsing and formatting remains unchanged.

### Completeness derivation

An open buffer does not by itself make a side partial. A scope side is `Closed` when inventory enumeration completed, every configured participant has exactly one current disk-or-buffer source, every required extraction or producer run completed successfully, and no editor-only or unresolved participant affects that scope.

The session derives partial reasons through the 014 precedence contract. A definition read or extraction failure uses `SourceFailed`; an omitted definition uses `SourceOmitted`; a producer failure or omission, including invalid bytes in the selected configured external-artifact buffer, uses `ProducerFailed` or `ProducerOmitted`; unverified non-configured external artifacts use `ExternalArtifactUnverified`; and an otherwise complete but intentionally unsettled editor world uses `OpenEditorWorld`. A unique successfully decoded open buffer for one exact configured artifact participant has the same closure authority as its selected disk snapshot. Raw configuration, an artifact outside that inventory proof, an ambiguous open-buffer count, or a successful result for only the visible files never asserts closure.
...
Every link job captures the config generation, inventory generation, selected-source generation, and exact versions of participating open documents. Completion first constructs one canonical per-URI publication map. It installs that map into the session only if every captured token still matches, then publishes document updates in canonical URI order. A stale job installs and publishes nothing; the event that changed the generation clears affected prior findings and schedules the replacement. Slow project linking never delays independent per-entry parser/lint diagnostics or formatting responses.
...
- definition artifacts keyed by canonical project source identity, exact selected bytes, resolved resource assignment, scope/locale projection, 014 artifact version, and producer revision where applicable;
- reference artifacts keyed by producer identity/revision, exact selected source bytes, resolved recognizer configuration, source identity, delivery-unit context, and artifact version; and
...
- A `resources` membership, host-format, scope, locale-binding, or normalized editor catalog-overlay change reclassifies every potentially affected open document in the root. It discards version-specific extraction artifacts, offset maps, mapped diagnostics, and pending formatting candidates and advances the document mapping generation when a classification or installed mapping changes. It also advances project inventory or definition-projection generation, invalidates affected definition artifacts and `LinkOutcome`, and marks affected link sides partial until replacement inventory and extraction complete. If a document is no longer an MF2 or catalog target, the editor integration publishes an empty diagnostic set. Exact-source message parse or semantic artifacts remain eligible for reuse only through their complete cache keys; no cached host mapping survives.
- A `messages` policy, root, target, or delivery-graph change invalidates `LinkOutcome` while retaining compatible definition and reference artifacts. A producer include, recognizer, producer identity/revision, or source-to-scope/domain binding change also rebuilds the affected reference inventory and reference artifacts. A locale change, an M2-or-later fallback change, or a placement-only change never forces resource extraction or source parsing merely to invalidate semantic link output. The built-in editor has no configurable scope mapping in M0; a future package-composition mapping field adds its own invalidation rule when that configuration contract lands.
...
Cross-locale editor features — missing-translation indicators, catalog key completion, and locale parity lenses — build on M2-or-later 014 linker results and locale-bound catalog configuration, not on editor-local extraction or an editor-owned parity algorithm. Diagnostic presentation of the two M2 locale findings additionally waits for L1; a separately designed completion or navigation capability declares its own activation milestone. Catalog-native structural checks remain a separate 008/013 input where such a feature needs them.
