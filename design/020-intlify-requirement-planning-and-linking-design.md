...
- Library/package composition, cross-owner references, dynamic module admission, or conditional authoring forms still proposed in 016.
...
017 must adopt the exact missing artifact schemas and identity domains before shared serialization or persisted production plans are implemented. 020 defines their logical planning/linking meaning here but does not extend an existing authoring kind, use an arbitrary JSON hash, or promote an in-memory struct layout into a wire format. The necessary 021 Policy/Store and 024 Target-body rules are also separate owner work, not unconditional adapters hidden in this design.
...
- 017 schemas for complete source/localized messages, planning/topology/Store records, portable diagnostics, and downstream handoffs; applicable 018/021 governance and 024 Target-body adoption.
