...
Release preparation is owned by the maintainer's local `bumpp` operation. After package versions are updated, its execute hook updates Cargo metadata and generates the future tag's `CHANGELOG.md` entry before the release commit and tag are created. The release commit therefore contains package metadata, Cargo metadata, and its own changelog entry. The hook must stop the operation before commit, tag, or push when Cargo propagation or changelog generation fails. It requires a clean `main` whose `HEAD` is already available on GitHub and a token with `Contents: write`, normally supplied through `GH_TOKEN="$(gh auth token --hostname github.com)"`.

The tag-push workflow treats the tagged tree as immutable input. It validates version consistency, the exact top-level changelog entry, and tag ancestry before build and publish. It does not create a follow-up `CHANGELOG.md` commit or push to `main`. GitHub Release creation remains the final step after all package publication and published smoke tests succeed. A `workflow_dispatch` `release-notes` retry may create the Release for an existing tag without republishing packages.
...
Every fresh-process product benchmark retains its ordinary aggregate unadjusted timing under its product-owned result schema. When paired startup adjustment is explicitly requested, the paired observation additionally retains the raw product and startup samples used by each ordinal. The adjusted observation is present only when every product sample has one compatible successful startup sample in the same session. A missing sample, command failure, route mismatch, build/environment mismatch, or unequal sample count omits the complete adjusted observation rather than pairing by proximity, reusing a result from another run, or returning a prefix. Without an adjustment request, the product benchmark need not retain a raw sample vector or sample-set identity.
