...
type NativeFormatResult = ReturnType<NativeBinding['formatMessage']>
type NativeFormatCheckResult = ReturnType<NativeBinding['checkFormat']>
type NativeOperationalError = NativeFormatResult['errors'][number]
type NativeDiagnostic = NativeFormatResult['diagnostics'][number]
type NativeDiagnosticLabel = NativeDiagnostic['labels'][number]
type NormalizedFormatOptions = {
  readonly mode: (typeof ALLOWED_MODES)[number]
}

/**
 * Format one complete MF2 message with the native formatter.
 *
...
export function formatMessage(source: string, options?: FormatOptions): FormatResult {
  ...
  return createFormatResult(getNativeBinding().formatMessage(source, normalizedOptions))
}
...
export function checkFormat(source: string, options?: FormatOptions): FormatCheckResult {
  ...
  return createCheckResult(getNativeBinding().checkFormat(source, normalizedOptions))
}
...
export function formatSnapshot(
  ...
  return createFormatResult(
    getNativeBinding().formatSnapshot(snapshot.slice(), source, normalizedOptions)
  )
}
...
export function checkSnapshot(
  ...
  return createCheckResult(
    getNativeBinding().checkSnapshot(snapshot.slice(), source, normalizedOptions)
  )
}
...
function createFormatResult(native: NativeFormatResult): FormatResult {
  if (native.ok) {
    return {
      ok: true,
      code: native.code ?? '',
...
function createCheckResult(native: NativeFormatCheckResult): FormatCheckResult {
  if (native.ok) {
    return {
      ok: true,
      changed: native.changed ?? false
...
function createDiagnostic(diagnostic: NativeDiagnostic): DiagnosticView {
  return {
...
function createDiagnosticLabel(label: NativeDiagnosticLabel): DiagnosticView['labels'][number] {
  return {
    span: label.span,
