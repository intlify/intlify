...
/**
 * Initialize the WASM runtime before formatter APIs are used.
 *
...
export async function init(input?: WasmInitInput): Promise<void> {
  if (initialized) {
    ...
    throw initializationError('format WASM runtime is already initialized')
  }
  ...
  if (inFlight) {
    ...
    throw initializationError('format WASM initialization is already in flight')
  }
...
async function initializeWasm(input?: WasmInitInput): Promise<void> {
  ...
  try {
    module = (await import(
      /* @vite-ignore */ new URL('../dist/intlify_format_wasm.js', import.meta.url).href
    )) as GeneratedWasmModule
  } catch (cause) {
    throw initializationError('format WASM artifact is not built in this workspace yet', cause)
  }
...
async function resolveWasmInput(input?: WasmInitInput): Promise<WasmInitInput> {
  ...
  const defaultUrl = new URL('../dist/intlify_format_wasm_bg.wasm', import.meta.url)
  if (isNodeRuntime()) {
