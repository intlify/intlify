...
Local-first benchmark tooling for project inventory, JavaScript/TypeScript reference production, artifact codecs, definition projection, semantic linking, typed-key model construction, allocator-observed peak live memory, and the complete in-process project-link workflow.
...
The active profile measures coverage-baseline selection and checked typed-key model construction separately at 16, 64, and 256 keys. It intentionally excludes fallback-aware linking, exporters, artifact-size comparison, fresh-process command timing, lint integration, and user-facing message commands.
