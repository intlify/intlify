...
#[test]
fn all_77_native_fixture_collections_are_accounted_for_and_cannot_hide_a_missing_attempt() {
    ...
    let evaluation = plan.evaluate(&submitted, &evidence, &[]);
    assert_eq!(evaluation.outcome(), Outcome::Complete);
    assert_eq!(evaluation.successful_references().len(), 77);
    ...
    let incomplete = plan.evaluate(&submitted, &evidence, &[]);
    assert_eq!(incomplete.outcome(), Outcome::Incomplete);
    assert_eq!(incomplete.successful_references().len(), 76);
