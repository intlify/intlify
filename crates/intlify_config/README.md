...
- Feature-isolated measurement support retains exact quantities, acquires the reported monotonic-clock resolution, measures five actual core operations, captures raw samples, and validates owner-local method/interval/execution descriptors. It does not yet produce complete Owner Results or common evidence.
- Checked-in expectations bind all 94 finite cases to their preparation/input context, complete result observation, and logical work. Ordinary collection requires an immutable admitted fixture rather than caller-chosen expected results.
...
- The locale-core benchmark integration and remaining 026 harness/record/validation path are not implemented yet. These internal operations are not a complete configuration resolver or product entry.
...
The finite case catalog declares 94 ordered cases. The original 77 input/structural/selection cases retain their order and complete pinned expectations; 13 locale spelling/validity cases and four raw/canonical identifier-limit edges are appended. All data generation and limit preparation are outside measured intervals. The repeated locale fixture is structurally valid authoring input, not a claim that semantic duplicate-locale resolution succeeds. Tests compare applicable configuration cases to an independent Draft 7 oracle and locale cases to separately declared canonical outputs or exact invalid-identifier failures. Every prepared case agrees with ordinary calls, including all 24 exact/first-over edges.

Preparation produces an explicitly unadmitted `Candidate`. The separate `cases/registry` gate admits it against `cases/expectations-v1.json`, whose closed revisioned document contains the exact ordered declaration inventory, fixture-input context, shared/entry result checksums, and a checksum over the complete typed logical-work vector. Registry revision `1` extends the inventory without changing the original 77 rows or their observation codec. These are fixed logical expectations, not checked-in timing or memory baselines. They are compiled only with benchmark/test support and require no runtime file access, network, directory scan, or caller-selected registry.
...
| 015 complete semantic observation | `benchmark::operation::tests`: actual five operations, full output mutation, member permutations, separate entry facts, content-free rejected selectors, iterative deep-document encoding |
...
| Connected owner observation | `benchmark::collect::tests`: native-clock capture for all five pairs, serialization and separate-input revalidation, cross-case/run rejection, no successful failure prefix |
...
| Finite case catalog and preparation | `benchmark::cases::tests`, `cases::locale_tests`: 94 unique ordered declarations, independent schema/locale checks, scaled/permuted inputs, 24 exact/first-over edges, exact failure kinds, undeclared-case rejection |
...
| Admitted fixture collection | `benchmark::collect::tests`: all 94 pinned cases through native collection/decode/revalidation; changed input context rejected even when result, logical work, run, and case labels coincide |
