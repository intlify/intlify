...
const REVISION_DOMAIN: &[u8] = b"dev.intlify/resource-definition/build-revision\0";
...
fn main() {
    ...
    let workspace_dir = manifest_dir
        ...
        .expect("the CLI crate is nested below the workspace root");
    let resource_dir = workspace_dir.join("crates/intlify_resource");
    ...
    let mut inputs = vec![
        ...
        resource_dir.join("Cargo.toml"),
        contract_dir.join("Cargo.toml"),
    ];
    for directory in [
        manifest_dir.join("src/messages"),
        resource_dir.join("src"),
        contract_dir.join("src"),
    ] {
    ...
    let mut inputs = inputs
        .into_iter()
        .map(|path| {
            let relative = normalized_relative_path(workspace_dir, &path);
            (relative, path)
        })
        .collect::<Vec<_>>();
    inputs.sort_by(|(left, _), (right, _)| left.as_bytes().cmp(right.as_bytes()));
    ...
    println!("cargo:rustc-env=INTLIFY_RESOURCE_DEFINITION_REVISION={revision}");
}
...
fn collect_files(directory: &Path, output: &mut Vec<PathBuf>) {
    let mut entries = fs::read_dir(directory)
        .expect("revision source directory remains readable")
        .map(|entry| {
            entry
                .expect("revision source entry remains readable")
                .path()
        })
        .collect::<Vec<_>>();
    entries.sort();
